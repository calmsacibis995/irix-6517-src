...
#ident "$Id: pmkstat.c,v 2.44 1999/05/11 00:28:03 kenmcd Exp $"
...
Options:\n\
  ...
  -z            set reporting timezone to local time of metrics source\n",
    ...
    for (i = 0; i < numpmid; i++) {
	sts = pmLookupDesc(pmidlist[i], &desclist[i]);
	if (sts < 0) {
	    fprintf(stderr, "%s: Warning: cannot retrieve description for metric \"%s\" (PMID: %s)\nReason: %s\n",
		pmProgname, _sample[i], pmIDStr(pmidlist[i]), pmErrStr(sts));
...
*/
	...
	if (iter >= 0) {
		...
		if (resp->vset[i]->numval == 1 && prev->vset[i]->numval == 1) {
		    tmp = (int)ulong_diff(firstval(resp, i, lval), firstval(prev, i, lval));
		    if (tmp < 0) {
			sum_time = -1;
			break;
		    }
		...
		if (resp->vset[FREEMEM]->numval == 1)
		    printf(" %8.0lf", (double)firstval(resp, FREEMEM, lval));
		else
		    printf(" %8.8s", "?");
		...
		if (dkread != -1 && lastdkread != -1)
		    printf(" %4.0lf", (double)(dkread - lastdkread) / duration);
		else
		    printf(" %4.4s", "?");

		/* disk writes (per second) */
		if (dkwrite != -1 && lastdkwrite != -1)
		    printf(" %4.0lf", (double)(dkwrite - lastdkwrite) / duration);
		else
		    printf(" %4.4s", "?");
#if HAVE_NETWORK
