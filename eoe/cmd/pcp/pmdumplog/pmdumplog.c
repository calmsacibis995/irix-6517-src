...
#ident "$Id: pmdumplog.c,v 1.23 1999/02/12 04:49:46 kenmcd Exp $"
...
do_size(pmResult *rp)
{
    ...
    int		i;
    int		j;
				    /* hdr, timestamp, numpmid, trailer len */
    nbyte = sizeof(__pmPDUHdr) + sizeof(struct timeval) + sizeof(int) + sizeof(int);
...
rawdump(FILE *f)
{
	...
	printf("Dump ... record len: %d @ offset: %d", len, ftell(f) - sizeof(len));
	len -= 2 * sizeof(len);
	for (i = 0; i < len; i++) {
	    check = fgetc(f);
	    if (check == EOF) {
