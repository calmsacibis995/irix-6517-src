...
#ident "$Id: pmieconf.c,v 1.1 1999/04/28 10:39:51 kenmcd Exp $"
...
static char	usage[] = \
    ...
    "               [for root, default is %s and\n"
    "               for other users, default is $HOME/%s]\n"
    "  -r rulepath  path specifying groups of rule files [%s]\n"
    "  -v           verbose mode\n"
    "\n"
    "Commands:\n%s\n";
...
command_quit(void)
{
  ...
  o  Refer to pmie_check(1) for a convenient mechanism for restarting pmie\n\
     daemons launched under the control of /var/pcp/config/pmie/control;\n\
     e.g.\t/usr/pcp/bin/pmie_check -V\n");
	}
    }
}
...
main(int argc, char **argv)
{
    ...
    if (errflag) {
	fprintf(stderr, usage, pmProgname, DEFAULT_ROOT_PMIE,
		DEFAULT_USER_PMIE, DEFAULT_RULES, help);
	exit(1);
    }
