#ifndef lint
static char sccsid[] = 	"@(#)rpc.ypupdated.c	1.3 88/06/02 4.0NFSSRC Copyr 1988 Sun Micro";
#endif
...
#define	NASTY	";\n\r\t |&"
	...
	/*
	 * Check to make sure that the map name looks reasonable.
	 */
	for (s = mapname; *s; s++) {
		int	i;

		for (i = 0; i < sizeof(NASTY); ++i) {
			if (*s == NASTY[i]) {
