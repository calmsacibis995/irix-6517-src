 ...
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/sun/snoop/RCS/snoop_nfs3.c,v 1.2 1996/07/06 17:51:21 nn Exp $"
...
static void nfscall3();
static void nfsreply3();
static char *perms();
static char *filetype();
static char *sum_access();
static char *sum_readdirres();
static char *sum_readdirplusres();
static char *sum_createhow();
static char *sum_stablehow();
static void detail_sattr3();
static void detail_diropargs3();
static void detail_readdirres();
static void detail_readdirplusres();
static void detail_fattr3();
static void detail_access();
static void detail_mode();
static void detail_wcc_attr();
static void detail_pre_op_attr();
static void detail_wcc_data();
static void skip_postop();
static void skip_wcc_data();
static void skip_sattr3();

char *sum_nfsfh3();
void detail_nfsfh3();
int sum_nfsstat3();
void detail_post_op_attr();
int detail_nfsstat3();
...
#define	NFS3_OK			0	/* no error */
#define	NFS3ERR_PERM		1	/* Not owner */
#define	NFS3ERR_NOENT		2	/* No such file or directory */
#define	NFS3ERR_IO		5	/* I/O error */
#define	NFS3ERR_NXIO		6	/* No such device or address */
#define	NFS3ERR_ACCES		13	/* Permission denied */
#define	NFS3ERR_EXIST		17	/* File exists */
#define	NFS3ERR_XDEV		18	/* Attempted cross-device link */
#define	NFS3ERR_NODEV		19	/* No such device */
#define	NFS3ERR_NOTDIR		20	/* Not a directory */
#define	NFS3ERR_ISDIR		21	/* Is a directory */
#define	NFS3ERR_INVAL		22	/* Invalid argument */
#define	NFS3ERR_FBIG		27	/* File too large */
#define	NFS3ERR_NOSPC		28	/* No space left on device */
#define	NFS3ERR_ROFS		30	/* Read-only file system */
#define	NFS3ERR_MLINK		31	/* Too many links */
#define	NFS3ERR_NAMETOOLONG	63	/* File name too long */
#define	NFS3ERR_NOTEMPTY	66	/* Directory not empty */
#define	NFS3ERR_DQUOT		69	/* Disc quota exceeded */
#define	NFS3ERR_STALE		70	/* Stale NFS file handle */
#define	NFS3ERR_REMOTE		71	/* Too many levels of remote in path */
#define	NFS3ERR_BADHANDLE	10001	/* Illegal NFS file handle */
#define	NFS3ERR_NOT_SYNC	10002	/* update synch mismatch */
#define	NFS3ERR_BAD_COOKIE	10003	/* readdir cookie is stale */
#define	NFS3ERR_NOTSUPP		10004	/* operation not supported */
#define	NFS3ERR_TOOSMALL	10005	/* buffer/request too small */
#define	NFS3ERR_SERVERFAULT	10006	/* server fault */
#define	NFS3ERR_BADTYPE		10007	/* Attempt to CREATE an object of a */
...
#define	FSF3_LINK		0x01	/* (hard) links supported */
#define	FSF3_SYMLINK		0x02	/* Symbolic links supported */
#define	FSF3_HOMOGENEOUS	0x08	/* Pathconf info is for all files */
#define	FSF3_CANSETTIME		0x10	/* Server can set file times */
	...
	if (flags & F_SUM) {
		...
		if (type == CALL) {
			...
			case NFSPROC3_MKDIR:
				fh = sum_nfsfh3();
				(void) sprintf(line, "%s %s",
					fh, getxdr_string(buff, 1024));
				break;
			case NFSPROC3_CREATE:
				fh = sum_nfsfh3();
				name = getxdr_string(buff, 1024);
...
detail_fattr3()
{
	u_int fltype, mode, nlinks, uid, gid;
	u_int major, minor;
...
detail_readdirplusres()
{
	...
	while (getxdr_bool()) {
		...
		(void) showxdr_string(MAXPATHLEN, "Name = %s");
		(void) showxdr_u_longlong("Cookie = %llu");
		detail_post_op_attr("");
		if (getxdr_bool())
			detail_nfsfh3();
