...
#ident	"@(#)rpcgen:rpc_main.c	1.8.4.2"
...
static add_warning(void), do_registers(int argc, char *argv[]);
...
static s_output(int argc, char *argv[],	char *infile, char *define, int extend,
  char *outfile, int nomain, int netflag);
static t_output(char *infile, char *define, int extend, char *outfile);
static open_input(char *infile, char *define);
static open_output(char *infile, char *outfile);
...
#ifdef sgi
int do_prototypes;	/* generate ANSI-C prototypes */
#endif
