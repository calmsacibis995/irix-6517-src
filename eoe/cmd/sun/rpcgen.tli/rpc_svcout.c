...
#ident	"@(#)rpcgen:rpc_svcout.c	1.11.4.2"
...
/*
 * static defined forward referenced procedures
 */
...
static write_inetmost(char *infile);
static print_return(char *space);
static print_pmapunset(char *space);
static print_err_message(char *space);
static write_msg_out(void);
static write_timeout_func(void);
static write_pm_most(char *infile, int netflag);
static write_rpc_svc_fg(char *infile, char *sp);
static open_log_file(char *infile, char *sp);
	...
	char *storage;
{
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
...
#ifdef sgi
		if (do_prototypes) {
			f_print(fout, "(struct svc_req *%s, SVCXPRT *%s)\n", 
				RQSTP, TRANSP);
		} else {
#endif
		f_print(fout, "(%s, %s)\n", RQSTP, TRANSP);
		...
		if (do_prototypes) {
			...
			f_print(fout, "\ttypedef void *(*_svcproc_t)(union _svcargun *, struct svc_req *);\n");
			f_print(fout, "\t_svcproc_t %s;\n", ROUTINE);
		} else {
#endif
		f_print(fout, "\tchar *%s;\n", RESULT);
...
#ifndef sgi	/* obtained from <stdlib.h> */
	f_print(fout, "\t\textern char *getenv();\n");
#endif
	f_print(fout, "\n");
	f_print(fout, "\t\t_rpcpmstart = 1;\n");
#ifndef sgi		/* Boy was this "if" stupid !! */
	if (logflag)
#endif
		open_log_file(infile, "\t\t");
