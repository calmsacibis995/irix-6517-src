#if 0
static char sccsid[] = 	"@(#)rpc_svcout.c	1.3 90/07/25 4.1NFSSRC Copyr 1990 Sun Micro";
#endif
	...
	char *infile;		/* our name */
{
	...
	if (timerflag)
		f_print(fout, "\nstatic void closedown(%s);", 
			do_prototypes ? "void" : "");
	f_print(fout, "\n");
	if (inetdflag) {
		f_print(fout, "\nstatic void _msgout(%s);", 
			do_prototypes ? "char *" : "");
	...
	char *storage;
{
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
		...
		if (do_prototypes) {
			...
			f_print(fout, "\tvoid *%s;\n", RESULT);
			f_print(fout, "\ttypedef void *(*__svcproc_t)(union __svcargun *, struct svc_req *);\n");
			f_print(fout, "\t__svcproc_t %s;\n", ROUTINE);
			f_print(fout, "\n");
		} else {
			f_print(fout, "\tchar *%s;\n", RESULT);
			f_print(fout, 
	...
	char *sp;
{
	...
	f_print(fout, "%sif (_daemonize(0, -1,-1,-1) < 0) exit(1);\n", sp);
	if (!logflag)
