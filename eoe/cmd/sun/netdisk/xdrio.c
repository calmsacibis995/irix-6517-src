#ifndef lint
static  char sccsid[] = "@(#)xdrio.c 1.9 88/02/08 Copyr 1987 Sun Micro";
#endif
/*
 * @(#)xdrio.c	1.1 88/06/08 4.0NFSSRC; from 1.8 88/03/01 D/NFS
 ...
 *		distribution_info *dinfo -> place to put distribution info
 *		volume_info *vinfo -> place to put volume info
 *		toc_entry etab[] -> base of array for entries
...
int max;
{
	...
	while(xdr_toc_entry(xp,tp) && tp < &etab[dinfo->nentries])
	{
		tp++;
...
int nentries;
{
	...
	for(tp = etab; tp < &etab[nentries]; tp++)
	{
		...
		{
			(void) fprintf(stderr,
				"xdr_write_toc: flush rec failed\n");
			(void) xdr_close(xp);
			return(FALSE);
		}
	}
