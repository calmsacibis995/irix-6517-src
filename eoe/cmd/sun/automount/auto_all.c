...
char *optlist[] = {
	...
	MNTOPT_HARD,	MNTOPT_NOSUID,	MNTOPT_NODEV,	MNTOPT_NOINTR,
	MNTOPT_NOAC,	MNTOPT_PORT,	MNTOPT_RETRANS,
	MNTOPT_RSIZE,	MNTOPT_WSIZE,	MNTOPT_TIMEO,	MNTOPT_ACTIMEO,
	MNTOPT_ACREGMIN,MNTOPT_ACREGMAX,MNTOPT_ACDIRMIN,MNTOPT_ACDIRMAX,
	MNTOPT_QUOTA,	MNTOPT_NOQUOTA,	MNTOPT_PRIVATE,	MNTOPT_SYMTTL,
	MNTOPT_SHORTUID,
#ifdef  MNTOPT_POSIX
	MNTOPT_POSIX,
#endif
	"suid",		"intr",		MNTTYPE_NFS3,
	"bds",		"bdsauto",	"bdswindow",	"bdsbuffer",
	NULL
};

char *
type_check(char *opts, int *type)
{
	char buf [256];
	char	*p, *pb;
	...
	for (fs = TAIL(struct filsys, tmpq); fs; fs = nextfs) {
		...
		if (!pathok(tmpq, fs) || umount(fs->fs_mntpnt) < 0) {
			...
			satvwrite(SAT_AE_MOUNT, SAT_SUCCESS,
			    "umount %s: failed: %s", fs->fs_mntpnt,
			    strerror(errno));
			goto inuse;
		}
		satvwrite(SAT_AE_MOUNT, SAT_SUCCESS,
		    "automount: unmounted %s (on %s:%s)", fs->fs_mntpnt,
	...
	struct filsys *tfs;
{
	...
	while (tfs->fs_mntpntdev != tmpdev && tfs->fs_rootfs != tfs) {
		...
		if (fs == NULL) {
			if (pingmount(tfs->fs_addr.sin_addr) != RPC_SUCCESS) {
				if (trace > 1)
