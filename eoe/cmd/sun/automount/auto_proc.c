#ifndef lint
static char sccsid[] = 	"@(#)auto_proc.c	1.3 89/04/28 4.0NFSSRC Copyr 1988 Sun Micro";
#endif
	...
	struct authunix_parms *cred;
{
	...
	/*
	 * Lookup may have taken a while, so use current time rather than
	 * time_now to better track the kernel symlink cache timeout.
	 */
	deadtime = time(0) + max_link_time;
	link->link_death = deadtime;
	if (link->link_fs)
		link->link_fs->fs_rootfs->fs_death = deadtime;
	res.readlinkres_u.data = link->link_path;
	res.status = NFS_OK;
	if (trace > 2) {
		fprintf(stderr, "[READLINK] link_death: %d  ",deadtime);
		if (!link->link_fs)
			fprintf(stderr, "NULL link->link_fs\n");
		else
			fprintf(stderr, "fs_rootfs->fs_death: %d\n", deadtime);
	}
	return(&res);
}
