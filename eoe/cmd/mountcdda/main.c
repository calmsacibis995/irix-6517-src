/*
 * mount_cdda, a Red Book filesystem server.
 *
 * Cannibalized from mount_iso9660
 *
 */
...
#define MNTOPT_NMCONV "nmconv"	/* for ABI compliance; only needed
	when invoked as mount_cdfs, but take for all cases; the MIPS
	ABI Black Book 1.2 specifies system defaults vary, and defines
	3 options: =c: no conversion (==notranslate), =l: convert to lowercase
	(the default for us), and =m: don't show version number (the default
	for us).  */
...
/* static int      print_cache_statistics; */
 ...
 *
 *  Description:
 *      Mount a CDDA file system, and act as an NFS server for that
 *      system.
 *
 ...
 *      mount_cdda, therefore, is invoked by mount(1M) when it
 *      encounters a file system type of cdda.
 *
 *  Usage:
 *      mount_cdda fsname dir type opts
 *          fsname      Must be /dev/scsi/*
 *          dir         The mount point
 *          type        Should be "cdda"
...
/*     int cache_blocks = 128;  */
    ...
    /*
     * Set up Red Book  filesystem and root file handle.
     */
    error = cdda_openfs(mnt.mnt_fsname, rootdir, &rootfh);
    if (error) {
	errno = error;
    ...
    if (!svc_register(transp, SGI_NFS_PROGRAM, SGI_NFS_VERSION, nfs_service,
		      0)) {
	fprintf(stderr,
...
/*     svcudp_enablecache(transp, 64); */
	...
	for (fd = getdtablesize(); --fd >= 0; ) {
	    if (fd == transp->xp_sock || cdda_isfd(fd))
		continue;
    ...
    for (;;) {
	readfds = svc_fdset;
	switch (select(transp->xp_sock+1, &readfds, NULL, NULL, &timeout)) {
	case -1:
	  if (errno == EINTR)
	    continue;
	  ...
	  enum mountstat mstat)
{
    ...
    args.addr = &sin;
    args.fh = fh;
/*     args.flags = NFSMNT_HOSTNAME|NFSMNT_TIMEO|NFSMNT_LOOPBACK|NFSMNT_RSIZE; */
     args.flags = NFSMNT_HOSTNAME|NFSMNT_TIMEO|NFSMNT_RSIZE; 
    args.rsize =  MAX_NFS_BUFFER_SIZE;
    args.timeo = 100;
...
terminate(int status)
{
    ...
    int tmpfd;
    struct mntent *mnt;
    static char tmpname[] = "/etc/mountcddaXXXXXX";
    int lockfd;
