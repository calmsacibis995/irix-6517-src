...
class   Ls120Device:public Device {

public:
	Ls120Device(const DeviceInfo &);
	const   char    *short_name()   const   { return "ls120"; }
	const   char    *full_name()    const   { return "ls120"; }
	const   char    *ftr_name()     const   { return "ls120"; }
...
const   char	Ls120[] = "IODATA  LS-120";

///////////////////////////////////////////////////////////////////////////////

Ls120Device::Ls120Device(const DeviceInfo &info):
	Device(info),
	_brand(GENERIC),
	...
	_dsr(*info.address().as_SCSIAddress())
{
	...
	if (!strncasecmp(vendor, Ls120, strlen(Ls120)))
		_brand = LS120;
}

const   char *
Ls120Device::dev_name(FormatIndex fi, int partno) const
{
...
int
Ls120Device::features()
{
	// Turn off fast poll because the device doesn't eject fast
	// enough to finish ejecting before the 1 second poll comes
	// around.  When the insert check is done before the ejection
	// is finished, it succeeds, which totally hoses everything.
	// (Don't know why fast poll is even the default...)
	return (feature_set(0, FEATURE_FAST_POLL));
}

int
Ls120Device::is_media_present()
{
	dsreq   *dsp = _dsr;
...
bool
Ls120Device::is_write_protected()
{
...
int
Ls120Device::sector_size(FormatIndex /* index */ )
{
	dsreq   *dsp = _dsr;
	char    *block_sz;
	char    *info = (char *)alloca(INFO_SIZE);
...
int
Ls120Device::read_data(char *buffer,
