...
#define JOINM
#undef YIELDM /* we cannot do printf's while running at 60Hz */
...
processD(int cpu)
{
    ...
    /*
     * To avoid overrun on first major frame, we skip it
     */
...
create_frs(int cpu, int master_pid)
{
	...
	frs_fsched_info.intr_source = FRS_INTRSOURCE_VSYNC;
	frs_fsched_info.intr_qualifier = 0; /* graphics pipe = 0 */
