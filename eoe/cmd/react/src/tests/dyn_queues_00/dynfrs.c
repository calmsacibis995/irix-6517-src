...
void
print_queues(int cpu)
{
	int minor;
	printf("FRS Processes on cpu [%d]\n", cpu);
	for (minor = 0; minor < 3; minor++) {
		...
		frs_queue_info.minor_index = minor;
		if ((queuelen = schedctl(MPTS_FRS_GETQUEUELEN, &frs_queue_info))<0) {
			perror("[schedctl getlen]");
			exit(1);
		}
		if (queuelen) {
			if (schedctl(MPTS_FRS_READQUEUE, &frs_queue_info, pids) < 0) {
				perror("[Reading queue members]");
				exit(1);
			}
		}
		printf("CPU<%d> Minor [%d]: ",cpu, frs_queue_info.minor_index);
...
/*
 * Removing last process from minor 1
 */
int
remove_last_process(int cpu, int minor)
{
	frs_queue_info_t frs_queue_info;
	int pids[100];
	int queuelen;
	...
	victim = pids[queuelen - 1];
	printf("Removing Last Process from Minor (%d): [%d]\n", minor, victim);
	frs_queue_info.thread = victim;
	if (schedctl(MPTS_FRS_PREMOVE, &frs_queue_info) < 0) {
		perror("[schedctl premove]");
		exit(1);
	}
	return (victim);
}
...
void
insert_process(int cpu, int minor, int pid)
{
	frs_queue_info_t frs_queue_info;
	int queuelen;
	int basepid;
	...
	if (queuelen == 0) {
		...
		basepid = pids[queuelen - 1];
	}
	
	printf("Inserting Proc [%d] on minor (%d), after pid [%d]\n",
	       pid, minor, basepid);

	if (schedctl(MPTS_FRS_PINSERT, &frs_queue_info, &basepid) < 0) {
		perror("[schedctl pinsert]");
		exit(1);
	}
...
void
modify_queues(int cpu)
{
	int pid1, pid2, pid3;
	print_queues(cpu);
	sleep(4);
	print_queues(cpu);
	pid1 = remove_last_process(cpu, 1);
	print_queues(cpu);
	sleep(4);
	pid2 = remove_last_process(cpu, 1);
	print_queues(cpu);
	sleep(8);
	pid3 = remove_last_process(cpu, 1);
	print_queues(cpu);
	sleep(8);

	insert_process(cpu, 1, pid3);
	print_queues(cpu);
	sleep(8);
	insert_process(cpu, 1, pid2);
	print_queues(cpu);
	sleep(8);
	insert_process(cpu, 1, pid1);
	print_queues(cpu);
	sleep(8);
}
...
create_frs(int cpu, int master_pid)
{
	...
	frs_fsched_info.intr_source = FRS_INTRSOURCE_CCTIMER;
	frs_fsched_info.intr_qualifier = 500000; /* period = 500 [ms] */
   ...
   switch ((pid = fork())) {
    ...
    /*
     * Dynamically modify the queues a few times
     */
