 ...
 * Example using External Interrupts
 */
 ...
 * cpu: 1
 * intr_source: FRS_INTRSOURCE_USER
 * n_minors: 20
 ...
 * cpu: 2
 * intr_source: same as master FRS1
 * n_minors: 20
 * period (intr_qualifier): 50 [ms]
...
int intr_source = FRS_INTRSOURCE_USER;
...
process_for_master_frs(void* arg)
{
	...
	if ((frs = frs_create_master(1, intr_source, 50000, 20, 2)) == NULL) {
	...
	pdesc = pdesc_create(frs, PDESC_MSG_JOIN | PDESC_MSG_YIELD, processK1_loop);
	pid = sproc(process_skeleton, PR_SALL, (void*)pdesc);
	if (pid == -1) {
...
process_for_first_slave_frs(void* mfrs_arg)
{
	...
	pdesc = pdesc_create(frs, PDESC_MSG_JOIN | PDESC_MSG_YIELD, processC_loop);
	pid = sproc(process_skeleton, PR_SALL, (void*)pdesc);
	if (pid == -1) {
	...
	pdesc = pdesc_create(frs, PDESC_MSG_JOIN | PDESC_MSG_YIELD, processK2_loop);
	pid = sproc(process_skeleton, PR_SALL, (void*)pdesc);
	if (pid == -1) {
...
process_for_second_slave_frs(void* mfrs_arg)
{
	...
	pdesc = pdesc_create(frs, PDESC_MSG_JOIN| PDESC_MSG_YIELD, processD_loop);
	pid = sproc(process_skeleton, PR_SALL, (void*)pdesc);
	if (pid == -1) {
	...
	pdesc = pdesc_create(frs, PDESC_MSG_JOIN | PDESC_MSG_YIELD, processE_loop);
	pid = sproc(process_skeleton, PR_SALL, (void*)pdesc);
	if (pid == -1) {
	...
	pdesc = pdesc_create(frs, PDESC_MSG_JOIN | PDESC_MSG_YIELD, processF_loop);
	pid = sproc(process_skeleton, PR_SALL, (void*)pdesc);
	if (pid == -1) {
	...
	pdesc = pdesc_create(frs, PDESC_MSG_JOIN | PDESC_MSG_YIELD, processK3_loop);
	pid = sproc(process_skeleton, PR_SALL, (void*)pdesc);
	if (pid == -1) {
...
void
send_user_intrs(void* arg)
{
        frs_t* frs = (frs_t*)arg;
        extern int errno;

        while (1) {
                if (frs_userintr(frs) < 0) {
                        printf("errno=%d\n", errno);
...
main(int argc, char** argv)
{
	...
	printf("The master has been created\n");
        ...
        /*
         * Start sending user generated intr from this process
         */
        rv = sproc(send_user_intrs, PR_SALL, (void*)master_frs);
        if (rv < 0) {
                perror("[sproc] - intr sender");
                exit(1);
        }
