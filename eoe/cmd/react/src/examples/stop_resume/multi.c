...
/*
 * Barrier to synchronize joins, enqueues and starts.
 * This barrier will have 30 waiters: 9 processes per
 * frs, 3 frs's, plus one start per frs: (9*3 + 3 = 30)
 */
...
print_queues(frs_t* frs)
{
	...
	printf("Processes on cpu [%d]\n", frs->frs_info.cpu);
	for (minor = 0; minor < 20; minor++) {
                if ((queuelen = frs_readqueue(frs, minor, pids)) < 0) {
			perror("[Reading queue members]");
			exit(1);
		}
		printf("CPU<%d> Minor [%d]: ", frs->frs_info.cpu, minor);
