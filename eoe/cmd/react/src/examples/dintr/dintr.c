 ...
 * PARAMETERS:
 * cpu: 1
 * intr_source: FRS_INTRSOURCE_DRIVER (3)
 * n_minors: 1
 *
 * PROCESSES:
 * Process A: 
...
/*
 * Barrier to synchr joins, enqueues, starts
 */
...
processA(frs_t* frs)
{
	 ...
	 * First, we join to the frame scheduler
         * after all processes have been enqueued.
	 */

        barrier(all_procs_enqueued, 2);
        
	printf("Proces A about to join\n");
...
main(int argc, char** argv)
{
	 ...
	 * cpu = cpu_number,
	 * interrupt source = DRIVER (3)
	 * number of minors = 1
	 * slave mask = 0, no slaves
	 */

	if ((frs = frs_create_master(cpu_number, FRS_INTRSOURCE_DRIVER, 3, 1, 0)) == NULL) {
