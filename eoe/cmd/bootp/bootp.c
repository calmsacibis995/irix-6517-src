...
static	cap_value_t cap_network_mgt[] = {CAP_NETWORK_MGT};
...
int bestaddr(struct in_addr *, struct in_addr *);
int handle_diskless(struct bootp *, struct bootp *);
int matchhost(char *);
int reverse_arp(u_char *, struct in_addr *);
int samewire(struct in_addr *, struct in_addr *);
static char *iaddr_ntoa(iaddr_t addr);
void forward(struct bootp *, struct in_addr *, struct in_addr *);
void getfield(char *, int);
void makenamelist(void);
void readtab(void);
void reply(void);
void request(void);
void sendreply(struct bootp *, int, int);
void setarp(struct in_addr *, u_char *, int);
void timeout(int);
...
main(int argc, char *argv[])
{
	...
	if (debug)
		syslog(LOG_DEBUG, "starting at %s", ctime(&tloc));
	...
	for (ifrp = &ifreq[0]; len > 0; len -= sizeof (struct ifreq), ifrp++) {
		if (SINPTR(&ifrp->ifr_addr)->sin_family == AF_INET) {
			...
			if (ioctl(s, SIOCGIFFLAGS, (caddr_t)&ifnetmask) < 0) {
				syslog(LOG_ERR, 
				  "'get interface flags' ioctl failed (%m)");
				exit(1);
			}
			if ((ifnetmask.ifr_flags & (IFF_UP|IFF_LOOPBACK)) != 
			    IFF_UP)
				continue;
			if (ioctl(s, SIOCGIFNETMASK, (caddr_t)&ifnetmask) < 0) {
				syslog(LOG_ERR, 
				  "'get interface netmask' ioctl failed (%m)");
				exit(1);
			}
	...
	/* only log if forwarding state is potentially "unusual" for
	   configuration */
	...
	for (;;) {
		...
		case BOOTREQUEST:
			...
			reg_net = 
			    inet_ntoa(SINPTR(&ifnetmask.ifr_addr)->sin_addr);
...
request()
{
	...
	if (rq->bp_yiaddr.s_addr != 0 && rq->bp_giaddr.s_addr != 0) { 
		...
		if (n == nhosts) {
			...
			if( (*(int *)rp.vd_magic == VM_SGI) &&
			    (rp.vd_flags == VF_GET_IPADDR) ) {
				if ((hostentp = gethostbyname((const char *)rp.vd_clntname))
					== 0) {
					rp.bp_yiaddr.s_addr = 0;
				} else {
					rp.bp_yiaddr.s_addr =
					  *(u_long *)(hostentp->h_addr_list[0]);
			 ...
			 * giving up.
			 */
			else if (!reverse_arp(rq->bp_chaddr, &rp.bp_yiaddr)) {
				/*
	...
	if (fromaddr.s_addr == 0 && *(int *)(rq->vd_magic) == VM_AUTOREG &&
		strlen((const char *)rq->vd_clntname)) {
		if (debug)
 ...
 */
void
sendreply(struct bootp *bp, int forward, int has_sname)
{
	iaddr_t dst;
	struct sockaddr_in to;
	...
	to.sin_addr = dst;
	if (sendto(s, (caddr_t)bp, sizeof *bp, 0, &to, sizeof to) < 0)
		syslog(LOG_ERR, "send to %s failed (%m)", 
			iaddr_ntoa(to.sin_addr));
}
...
readtab()
{
	...
	badhex:		syslog(LOG_ERR, "bad hex address: %s at line %d of bootptab",
		...
		if (++nhosts >= MHOSTS) {
			syslog(LOG_ERR,
				"bootptab exceeds max. number of hosts (%d)",
			 	MHOSTS);
			exit(1);
		}
		hp++;
 ...
 *
 * eap:		Ethernet address to lookup
 * iap:		filled in if successful
 * Returns 1 on success, 0 on failure.
 */
int
reverse_arp(u_char *eap, iaddr_t *iap)
{
	...
	if ((hp = gethostbyname(host)) == 0) {
		syslog(LOG_ERR, "gethostbyname(%s) failed: %s",
 ...
 */
int
samewire(iaddr_t *src, iaddr_t *dest)
{
	register struct netaddr *np;
 ...
 *		initial boot/IPADDR: NOTE: we never actually implemented
 *  any of this in our PROMs. someday we should rip it all out.
 */
int
handle_diskless(struct bootp *rq, struct bootp *rp)
{
	...
	if (debug)
		syslog(LOG_DEBUG, "Autoreg: domain=%s, host=%s",
			domain, t_host);
	...
	if (yp_master(domain, hostmap, &ypmaster)) {
		if (debug)
			syslog(LOG_DEBUG,
				"no \"%s\" YPmaster found for \"%s\" domain",
				hostmap, domain);
		return(-1);
	}
	...
	if (!matchhost(ypmaster)) {
		if (!forwarding) {
			if (debug)
			    syslog(LOG_DEBUG,
				"Autoreg: I am neither YPMASTER nor GATEWAY");
			return(-1);
		}
		if (debug)
