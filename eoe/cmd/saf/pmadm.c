...
# define NOTPRIV	"User not allowed to perform this operation"
...
char *argv[];
{
	...
	uid_t priv_id;          /* privileged id        */
	int sum=0;              /* SUM installed        */
	...
	while ((c = getopt(argc, argv, "acdef:gi:Llm:p:rS:s:t:v:y:z:")) != -1) {
		...
		case 's':
			...
			if (*svctag == '_') {
				Saferrno = E_BADARGS;
				error("service tag must not begin with '_'");
			}
			for (p = svctag; *p; p++) {
				if (!isalnum(*p) && (*p != '_')) {
					Saferrno = E_BADARGS;
 ...
 */
	...
	uid = getuid();
	if ((priv_id = secsys(ES_PRVID,0)) != -1)
		sum=1;

	switch (flag) {
	case ADD:
		/* in SUM check if user is privileged*/
		if (sum && (uid != priv_id)){
	...
	case PLIST:
		...
		break;
	case CONFIG:
		/* if adding script and SUM is installed, check if user	*/
		/* is privileged					*/
		if (script && sum && (uid != priv_id)){
			Saferrno = E_NOPRIV;
			error(NOTPRIV);
		}
...
char *cmdname;
{
	...
	(void) fprintf(stderr, "\t\t-v version [ -f xu ] [ -S scheme ] [ -y comment ] [ -z script ]\n");
	...
	(void) fprintf(stderr, "\t%s -c [ -i id ] [ -S scheme ] -p pmtag -s svctag\n", cmdname);
	Saferrno = E_BADARGS;
	quit();
}
...
*/
		...
		if (script) {
			...
			(void) sprintf(buf, "%s/%s", HOME, fname);
				...
				if (mac_check()) {
					Saferrno = E_SYSERR;
					error("could not get level identifier");                                } else {
					set_level = 0;
				}
			}
			if (set_level) {
				if ( lvlfile(buf, MAC_SET, &level_lid) != 0 ) {
					Saferrno = E_NOPRIV;
					(void) sprintf(buf, "could not set level of %s/%s", HOME, fname);
					error(buf);
				}
			}
			...
			if (chown(buf, (uid_t) SAF_OWNER, (gid_t) SAF_GROUP) != 0) {
				Saferrno = E_SYSERR;
				(void) sprintf(buf, "could not change ownership of %s", HOME, fname);
				error(buf);
			}
		}
 ...
 */
		...
		(void) fprintf(fp, "%s:%s:%s:reserved:reserved:%s:%s#%s\n",
			svctag, (flags ? pflags(flags, FALSE) : ""), id,
...
char *svctag;
{
	...
	if (fp == NULL) {
		Saferrno = E_SYSERR;
		(void) sprintf(buf, "Could not open %s", fullname);
		error(buf);
	}
