...
char *argv[];
{
	...
	case PLIST:
		...
		break;
	case DBREAD:
		if (sum && (uid != priv_id)){ /* in SUM check if user is */
                        Saferrno = E_NOPRIV;  /* privileged		*/ 
                        error(NOTPRIV);
		}
		if (type || script)
			usage(argv[0]);
		...
		break;
	case CONFIG:
		/* if replace script and SUM check if user is privileged*/
 ...
 */
	...
	if ( lvlin("SYS_PRIVATE", &level_pri) != 0 ) {
		if (mac_check()) {
			Saferrno = E_SYSERR;
			error("Could not find level identifiers");
		} else {
			set_level = 0;
		}
	}
	...
	if (set_level) {
		if ( lvlfile(pid_file, MAC_SET, &level_pri) != 0 ||
			...
			Saferrno = E_NOPRIV;
			cleandirs(tag);
			(void) sprintf(buf, "Not able to set levels for %s files & directories", tag);
			error(buf);
		}
	}
	...
	if ( chown(pid_file, (uid_t) SAF_OWNER, (gid_t) SAF_GROUP) != 0 ||
		...
		Saferrno = E_SYSERR;
		(void) sprintf(buf, "Not able to chown() %s files & directories", tag);
		error(buf);
	}
