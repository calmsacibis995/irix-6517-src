	...
	*pp++ = '\r';
	{
		...
		while(c= *last++) switch(c)
		{
			...
			case '\t':
				/* expand tabs */
				while(((pp-editb.e_prompt)%TABSIZE) && pp<ppmax)
					*pp++ = ' ';
				break;
...
ed_fulledit()
{
	...
	/* Do not use p_str() because it also writes the terminating null char */
	/*	p_str((char*)editb.e_inbuf,0); */
		for(i=0;i<=editb.e_eol;++i)
			p_char(editb.e_inbuf[i]);
		st.states |= FIXFLG;
		hist_flush();
	}
