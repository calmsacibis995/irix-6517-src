...
#ifdef sgi
/*
 * initialize io_ftable
 */
void io_ftblinit()
{
	nfile = sysconf(_SC_OPEN_MAX);
	if (nfile < NFILE)
		nfile = NFILE;

	io_ftable = (struct fileblk **)calloc(nfile+1, sizeof(struct fileblk *));

	if (io_ftable == (struct fileblk **)NULL) {
		sh_fail("calloc failed", sys_errlist[errno], ERROR);
	}
	fdmap = (struct filesave *)calloc(MAXFILES+1, sizeof(struct filesave));

	if (fdmap == (struct filesave *)NULL) {
		sh_fail("calloc failed", sys_errlist[errno], ERROR);
	}
	io_ftable[0] = (struct fileblk *)0;
	io_ftable[1] = &io_stdout;
	io_ftable[2] = &io_stdout;
}
#endif /* sgi */
...
#ifdef sgi
	if (fd >= nfile)
		sh_fail(gettxt(_SGI_DMMX_e_flimit,e_flimit),NIL,ERROR);
#endif
	if(!fp)
	{
		if(iofree)
...
register int f2;
{
	...
	if(f1!=f2)
	{
		...
		if(f2 < 0)
			sh_fail(gettxt(_SGI_DMMX_e_file,e_file),NIL,ERROR);
...
register char *fname;
{
	...
	if(fname)
	{
		...
		if(fd<=0)
			sh_fail_why(tmp_name,gettxt(_SGI_DMMX_e_create,e_create),ERROR);
	}
#ifdef sgi
	if (fd >= nfile)
		sh_fail(gettxt(_SGI_DMMX_e_flimit,e_flimit),NIL,ERROR);
...
int			mode;
{
	...
	{
		if ((fd = socket(AF_INET, path[5] == 't' ? SOCK_STREAM : SOCK_DGRAM, 0)) >= 0 && connect(fd, &addr, sizeof(addr)))
		{
			close(fd);
			fd = -1;
		}
	...
	if (fd < 0)
		sh_fail_why(path, (flags & (O_WRONLY|O_RDWR|O_APPEND|O_CREAT)) ? gettxt(_SGI_DMMX_e_create,e_create) : gettxt(_SGI_DMMX_e_open,e_open),ERROR);
	return(fd);
}
...
register int	pv[];
{
	if(pipe(pv)<0 || pv[INPIPE]<0 || pv[OTPIPE]<0)
		sh_fail_why(gettxt(_SGI_DMMX_e_pipe,e_pipe),NIL,ERROR);
...
register struct ionod	*i;
{
	...
	while(i)
	{
		...
		if(maxtry<=0)
			sh_fail_why(io_tmpname,gettxt(_SGI_DMMX_e_create,e_create),ERROR);
		if(i->iolink)
			free(i->iolink);
	...
	for(;iop;iop=iop->ionxt)
	{
		...
		if(*ion)
		{
			...
			else if(iof&IORDW)
			{
				...
				sh_fail(ion,gettxt(_SGI_DMMX_e_restricted,e_restricted),ERROR);
			else
			{
				if(iof&IOAPP)
				{
				...
				else
				{
					...
					if(iof&IOCLOB)
						...
						sh_fail(ion,gettxt(_SGI_DMMX_e_fexists,e_fexists),ERROR);
				}
			openit:
...
fail:
	if(st.states&FUNCTION)		
		st.states &= ~FUNCTION;
	sh_fail(ion,gettxt(_SGI_DMMX_e_file,e_file),ERROR);
	/* NOTREACHED */
}
	...
	if(c==0)
	{
		...
		case F_ISFILE:
			...
			if(sh.readscript)
			{
				...
				sh_fail(cp,gettxt(_SGI_DMMX_e_exec,e_exec),EXEBAD);
			}
		default:
