...
#ifdef POSIX_OLD	/* Renamed from POSIX: precedence now equal - see #else */
...
register int sym;
{
	if(sym&SYMFLG)
	{
		...
		p_str(gettxt(_SGI_DMMX_e_endoffile,e_endoffile),0);
	else
	{
		if(sym&SYMREP)
			p_char(sym);
		if(sym==NL)
			p_str(gettxt(_SGI_DMMX_e_nlorsemi,e_nlorsemi),0);
		else
			p_char(sym);
	}
	p_char('\'');
...
void	sh_syntax()
{
	...
	/* If syntax error in parsing for $(...) command substitution, which
	 * parses upto and including a closing RPAREN, just return if
	 * the error sh.wval is not a closing RPAREN.
	 */
	if(st.states&COMSUB && w != RPAREN){
		st.states &= ~COMSUB;
		return;
	}
	p_setout(ERRIO);
	...
	{
		p_str(gettxt(_SGI_DMMX_e_atline,e_atline),0);
		p_num(sh.olineno,SP);
	}
	p_str(e_colon,'`');
	if(w)
	...
	if(cp == e_unexpected)
		p_str(gettxt(_SGI_DMMX_e_unexpected,e_unexpected),NL);
	else	p_str(gettxt(_SGI_DMMX_e_unmatched,e_unmatched),NL);
	exitset();
	sh_exit(SYNBAD);
}
/*
	...
	argp->argflag = 0;
	{
		...
		while(c= *sp++)
		{
			...
			else if(isexp(c))
			{
				...
				{
					if(c == '{' && *sp == NULL && is_option(WORDEXP))
						sh_exit(CTXBAD);
					/* special case of '[' */
					if(*sp || c!='[')
						flag |= EXP_FILE;
