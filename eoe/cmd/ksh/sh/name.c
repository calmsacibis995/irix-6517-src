...
static int	set_rand();
static long	get_opt_index();
static int	set_opt_index();
...
struct Bfunction opt_indexs = { get_opt_index, set_opt_index};
...
int 	xp;
{
	...
	else if(isalpha(sep) || !expchar(sep))
	{
		do
		{
			sep = *++argscan;
...
#   endif /* MULTIBYTE */
			...
			else if(n==CSWIDTHNOD)
			{
				if(ed_setwidth(argscan) && !initenv)
					sh_fail(n->namid,gettxt(_SGI_DMMX_e_format,e_format),ERROR);
			}
#endif /* MULTIBYTE */
		}
		return(n);
	...
	if(initenv)
		return(0);
failed:
	sh_fail (argi,(root==sh.alias_tree?gettxt(_SGI_DMMX_e_aliname,e_aliname): gettxt(_SGI_DMMX_e_ident,e_ident)),ERROR);
	/* NOTREACHED */
	return(0);
}
 ...
 * Added C_FLAG to indicate continue assigning fields after 
 * encountering readonly vars.
 */

int
env_readline(names,fd,raw)
char **names;
int fd;
{
	...
	while(1)
	{
		...
		if((issep && name) || is_eol)	/* from non-separator to separator */
		{
			...
			if(nam_istype(n, N_RDONLY) && (raw&C_FLAG)) {
				p_setout(ERRIO);
				p_prp(n->namid);
				p_str(e_colon,0);
				p_str(gettxt(_SGI_DMMX_e_readonly,e_readonly),NL);
				ret = 1;
			}
			else nam_putval(n,stakptr(rel));
...
register int	flag;
{
	...
	{
		if(!flag)
			p_str(gettxt(_SGI_DMMX_e_bltfn,e_bltfn),0);
...
void env_scan(file,flag,root,option,pflag)
int file;
struct Amemory *root;
{
	register char **argv;
	...
	while(namec--)
	{
		if((np = nam_search(*argv++,root,N_NULL)) && np!=onp)
		{
			...
			if(nam_istype(np,N_ARRAY))
			{
				...
				do
				{
					/* pflag for 'export -p' and 'readonly -p' */
					if(pflag) p_str((pflag==X_FLAG?"export":"readonly"),' ');
		        ...
		        if(val == N_INTGER && nam_istype(n,N_INTGER))
			{
				if(n->value.namsz != 10)
				{
					...
					if(nam_istype(n, N_DOUBLE))
						p_str(gettxt(_SGI_DMMX_e_precision,e_precision),SP);
					else
#endif /* FLOAT */
						p_str(gettxt(_SGI_DMMX_e_intbase,e_intbase),SP);
					p_num(n->value.namsz,SP);
				}
				break;
			}
...
register int flag;
{
	register const char *arg,*path,*nam;
	register struct namnod *np;
	...
	while(arg = *++com)
	{
		...
		if(sh_lookup(arg,tab_reserved))
		{
			if(flag) {
				p_str(arg,0);
				p_str(gettxt(_SGI_DMMX_is_reserved,is_reserved),NL);
			}
			else 	p_str(arg,NL);
		}
		/* non-tracked aliases */
		else if((np=nam_search(arg,sh.alias_tree,N_NULL))
			...
			&& (nam=nam_strval(np))) 
		{
			if(flag)
			{
				...
				if(nam_istype(np,N_EXPORT))
					p_str(gettxt(_SGI_DMMX_is_xalias,is_xalias),0);
				else
					p_str(gettxt(_SGI_DMMX_is_alias,is_alias),0);
			}
			p_qstr(nam,NL);
		}
		/* built-ins and functions next */
		else if(np=nam_search(arg,sh.fun_tree,N_NULL))
		{
			if(flag)
			{
				p_str(arg,0);
				if(isnull(np))
				{
					if(!nam_istype(np,N_FUNCTION))
						goto search;
					p_str(gettxt(_SGI_DMMX_is_ufunction,is_ufunction),NL);
				}
				else if(is_abuiltin(np))
					p_str(gettxt(_SGI_DMMX_is_builtin,is_builtin),NL);
				else if(nam_istype(np,N_EXPORT))
					p_str(gettxt(_SGI_DMMX_is_xfunction,is_xfunction),NL);
				else
					p_str(gettxt(_SGI_DMMX_is_function,is_function),NL);
			}
			else 	p_str(arg,NL);
		}
		else
		...
		search:
			...
			if(path)
			{
				if(flag)
				{
					...
					if(!notrack && *path == '/') {
						p_str(gettxt(_SGI_DMMX_is_talias,is_talias),0);
						p_qstr(path,NL);
					}
					else {
						p_str(gettxt(_SGI_DMMX_is_,is_),0);
						p_qstr(path,NL);
					}
				}
				else 	p_qstr(path,NL);
...
/*
 * these functions are used to get and set the opt_index variable
 */

static int set_opt_index(n)
long n;
{
	opt_index = (int)n;
	if(opt_index == 1)
