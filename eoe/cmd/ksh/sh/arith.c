...
number n;
{
	...
	case ERRMSG:
		if(lvalue->value == e_function)
			sh_fail(*ptr,gettxt(_SGI_DMMX_e_function,e_function),ERROR);
		else if(lvalue->value == e_number)
			sh_fail(*ptr,gettxt(_SGI_DMMX_e_number,e_number),ERROR);
