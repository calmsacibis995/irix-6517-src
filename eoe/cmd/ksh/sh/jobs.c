...
#   endif /* alliant */
	...
	while((job.mytgid=tcgetpgrp(JOBTTY)) != job.mypgid)
	{
		...
		if(ntry++ > MAXTRY)
		{
			p_str(gettxt(_SGI_DMMX_e_no_start,e_no_start),NL);
			return;
		}
	}
#  endif /* WAIT1ARG */
...
#	endif /* FIOPUSHLD */
		if(lflag==0)
			p_str(gettxt(_SGI_DMMX_e_newtty,e_newtty),NL);
		else
			job.linedisc = -1;
	}
#   else
...
int job_close()
{
	...
	(void)signal(SIGCLD, shold);
	if(beenhere++ == 0 && job.pwlist)
	{
		p_setout(ERRIO);
		if(count)
		{
			p_str(gettxt(_SGI_DMMX_e_terminate,e_terminate),NL);
			return(-1);
		}
		else if(sh.login_sh)
		{
			p_str(gettxt(_SGI_DMMX_e_running,e_running),NL);
			return(-1);
		}
	}
#   ifndef  WAIT1ARG
...
#	else
		if(ioctl(JOBTTY,TIOCSETD,&job.linedisc) !=0)
			return(0);
#	endif /* FIOPUSHLD */
		p_str(gettxt(_SGI_DMMX_e_oldtty,e_oldtty),NL);
	}
#   endif /* NTTYDISC */
#   ifdef CNSUSP
	if(job.suspend==CNSUSP)
...
char *jobs[];
{
	...
	else while(jobid = *jobs++)
	{
		job_string = jobid;
		if(*jobid==0)
			sh_fail(job_string,gettxt(_SGI_DMMX_e_jobusage,e_jobusage),ERROR);
		if(*jobid == '%')
...
register int flag;
{
	...
	do
	{
		...
		{
			msg = gettxt(_SGI_DMMX_e_done,e_done);
			n = px->p_exit;
		}
		else
			msg = gettxt(_SGI_DMMX_e_Running,e_Running);
		p_str(msg,0);
		msize = strlen(msg);
		if(n)
		{
		...
		{
			p_str(gettxt(_SGI_DMMX_e_coredump,e_coredump),0);
			msize += strlen(gettxt(_SGI_DMMX_e_coredump,e_coredump));
		}
...
#	define stopsig	1
...
#if sgi
	if(pid == -1) {
		/* special case to kill all procs owned by user (but 0 + 1) */
		return kill(pid,sig);
	}
#endif
	if(pw->p_flag&P_BYNUM)
	{
		if(pid==0 && job.jobcontrol)
			return(job_walk(job_kill,sig,(char**)0));
#ifdef SIGTSTP
...
#   endif /* _poll_ */
	...
	error:
		p_setout(ERRIO);
		p_str(gettxt(_SGI_DMMX_e_killcolon,e_killcolon),0);
		if(pw && pw->p_flag&P_BYNUM)
			msg = gettxt(_SGI_DMMX_e_no_proc,e_no_proc);
		else
			msg = gettxt(_SGI_DMMX_e_no_job,e_no_job);
		p_str(job_string,0);
		p_str(e_colon,0);
		if(errno == EPERM)
			msg = gettxt(_SGI_DMMX_e_access,e_access);
		p_str(msg,NL);
		r = 2;
	}
	return(r);
...
char *name;
{
	...
	for(;pw;pw=pw->p_nxtjob)
	{
		...
		{
			if(pz)
				sh_fail(name-1,gettxt(_SGI_DMMX_e_ambiguous,e_ambiguous),ERROR);
			pz = pw;
		}
	}
	return(pz);
...
void	job_clear()
{
	...
	register int j = 1+((sh_numjobs-1)/(8));
	SIG_PF shold;
...
#   endif /* apollo */
	...
	while(1)
	{
			...
			if (WIFKILLED(wstat))
			{
...
#endif /* JOBS */
				{
						...
						pw->p_exit!=SIGPIPE)
					{
						...
						p_str(msg,p);
						if(p==0)
							p_str(gettxt(_SGI_DMMX_e_coredump,e_coredump),NL);
					}
					if(!intr)
						pw = job_byjid(jobid);
				}
		...
		else if(pw->p_pgrp==0 && p!=sh.subpid && (p!=job.curpgid || pid==0))
			job_unpost(pw);
#ifdef JOBS
	tryagain:
		if(pid==0)
 ...
 * should always be called with SIGCLD held/blocked.
 */
...
/*
 * check for an unused job number
 * freejobs is a bit vector, 0 is unused
 * Returns 1 for true - job slot available - otherwise 0 for false
 */
...
int job_slot()
{
	...
	/* skip to first word with a free slot */
	while(j < jbytes && job.freejobs[j] == 0xff)
		j++;
	if( j == jbytes )
		return(0);
	freeword = &job.freejobs[j];
