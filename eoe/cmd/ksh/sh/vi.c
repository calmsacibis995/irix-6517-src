...
static int	motion_arg;
static int	save_motion_arg;
static int	repeat_last_cmd = 0;
...
unsigned nchar;				/* number of chars to read */
{
	...
	static int Globals[9];			/* local global variables */
#ifndef FIORDCHK
...
find_b:
		motion_arg = findchar;
		if (repeat_last_cmd) motion_arg = findchar = save_motion_arg;
		tcur_virt = cur_virt;
		count = repeat;
		while( count-- )
		{
...
addin:
	...
	case '_':		/** append last argument of prev command **/
		save_v();
		{
			...
			if(repeat_set==0)
				repeat = -1;
			p = (genchar*)hist_word((char *)tmpbuf,repeat);
#ifndef KSHELL
			if(p==0)
			{
				bell;
