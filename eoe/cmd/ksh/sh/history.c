...
#ident	"@(#)ksh:sh/history.c	1.5.4.1"
...
char *nl;
{
	...
	if(offset<0 || !fp)
	{
		p_str(gettxt(_SGI_DMMX_e_unknown,e_unknown),'\n');
		return;
	}
...
char *replace;
{
	...
	*new++ =  0;
	if((sp=sh_substitute(string,replace,new))==0)
		sh_fail(command,gettxt(_SGI_DMMX_e_subst,e_subst),ERROR);
	*(new-1) =  '=';
...
register char *fname;
{
	...
	if((fd=open(mktemp(fname),O_CREAT|O_RDWR,RW_ALL)) < 0)
		sh_fail(fname,gettxt(_SGI_DMMX_e_create,e_create),ERROR);
	return(fd);
}
