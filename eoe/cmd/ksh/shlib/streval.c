...
#ident	"@(#)ksh:shlib/streval.c	1.2.4.1"
...
number	(*conv)();
{
	...
	convert = conv;
	if(level++ >= MAXLEVEL)
		(void)seterror(gettxt(_SGI_MMX_e_recursive,e_recursive));
	else
	{
		n = expr(0);
		if (peekchr() == ':') (void)seterror(gettxt(_SGI_MMX_e_badcolon,e_badcolon));
	}
	if (cur.errmsg.value)
	{
...
register int	precedence;
{
	...
	case 0:
		if(precedence>5)
			error(gettxt(_SGI_MMX_e_moretokens,e_moretokens));
		return(0);
...
#ifdef FLOAT
		if(cur.isfloat)
			error(gettxt(_SGI_MMX_e_incompatible,e_incompatible));
#endif /* FLOAT */
	...
	while(1)
	{
		...
		if(peekchr()== '=' && !(op->precedence&NOASSIGN))
		{
			if(!noassign && (!lvalue.value || precedence > 2))
				error(gettxt(_SGI_MMX_e_notlvalue,e_notlvalue));
			assignop = lvalue;
			getchr();
			c = 3;
		}
		...
		if(lvalue.value && (op->opcode!=ASSIGNMENT))
		{
			...
			if(cur.nextchr==0)
				error(gettxt(_SGI_MMX_e_number,e_number));
			if(!(op->precedence&SEQPOINT))
				lvalue.value = 0;
			invalid = 0;
		}
		if(invalid && op->opcode>ASSIGNMENT)
			error(gettxt(_SGI_MMX_e_synbad,e_synbad));
		if(precedence >= c)
			goto done;
		...
		{
		case RPAREN:
			error(gettxt(_SGI_MMX_e_paren,e_paren));
		...
		case COLON:
			(void)seterror(gettxt(_SGI_MMX_e_badcolon,e_badcolon));
			break;
#endif
		...
		case MOD:
			if(x!=0)
				...
				error(gettxt(_SGI_MMX_e_divzero,e_divzero));
			break;
