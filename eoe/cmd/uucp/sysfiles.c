...
static void scansys(char *);
static void scancfg(char *, char *);
static void scansys(char *);
static int namematch(char *, char *, char *);
...
extern void dialreset(void);
extern void setdevcfg(char *, char *);
extern void setservice(char *);

/* import these */
extern int eaccess(char *, int);
extern void logent(char *, char*);
extern time_t millitick(void);
...
{	FILE *f;
	...
	if (Systems[0] == NULL) {
		...
		ASSERT(Systems[0], "CAN'T ALLOCATE", "scansys: Systems", 0);
		Systems[1] = NULL;
	}
	if (Devices[0] == NULL) {
		Devices[0] = strsave(DEVICES);
		ASSERT(Devices[0], "CAN'T ALLOCATE", "scansys: Devices", 0);
		Devices[1] = NULL;
	}
	if (Dialers[0] == NULL) {
		Dialers[0] = strsave(DIALERS);
		ASSERT(Dialers[0], "CAN'T ALLOCATE", "scansys: Dialers", 0);
		Dialers[1] = NULL;
	}
	return;
}
...
{	char **tptr, *tok;
	...
	tok = strtok((char *) NULL, ":")) {
		...
		*tptr = strsave(expandpath);
		ASSERT(*tptr, "CAN'T ALLOCATE", "setfile: tptr", 0);
		tptr++;
	}
}
...
{	char **tptr, *tok;
	...
	tok = strtok((char *) NULL, ":")) {
		*tptr = strsave(tok);
		ASSERT(*tptr, "CAN'T ALLOCATE", "setioctl: tptr", 0);
		tptr++;
	}
}
