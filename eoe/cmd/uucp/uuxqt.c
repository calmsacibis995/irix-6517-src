...
static void retosndr(char *,char *, char *, char *, char *, char *);
static void uucpst(char *, char *, char *, char *, char *);
static int chkFile(char *);
static int doFileChk(char *);
static int cuantos(char *, char *);
static void xprocess(char*);
static int gotfiles(char *);
static  char	shchar[] = "`;&|^<>()\t\n ";
...
char	*argv[], *envp[];
{
	...
	if (fp == NULL) {
		...
		if (maxnumb >= 0 && ret >= maxnumb) {
			DEBUG(4, "exiting with maxnumb=%d\n", maxnumb);
			exit(0);
		}
		DEBUG(4, "continuing with maxnum=%d\n", maxnumb);
	}
