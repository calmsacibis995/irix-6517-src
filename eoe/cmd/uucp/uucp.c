...
static void	matchfile(char *);
static void usage(void);
static void uucp_split(const char *, char *, int, char *, int, char *, int);
static void ruux(char *, char *, char *, char *, char *, char *);
static int copy(char *, char *, char *, char *);
...
char	**envp;
{
	...
	*Uopts =  NULLCHAR;
	...
	if (fopt) {
		if (*fopt != '/')
			(void) snprintf(Sfile, sizeof(Sfile), "%s/%s",
					Wrkdir, fopt);
		else
			(void) snprintf(Sfile, sizeof(Sfile), "%s", fopt);
	...
	uucp_split(argv[argc - 1], sys2, sizeof(sys2),
		   fwd2, sizeof(fwd2), file2, sizeof(file2));
	if (*sys2 != NULLCHAR) {
		if (versys(sys2) != 0) {
	...
	for ( ; optind < argc - 1; optind++) {
	    uucp_split(argv[optind], sys1, sizeof(sys1),
		       fwd1, sizeof(fwd1), file1, sizeof(file1));
	    if (*sys1 != NULLCHAR) {
		if (versys(sys1) != 0) {
			(void) fprintf(stderr, "bad system: %s\n", sys1);
			cleanup(-11);
...
/*
 * uucp_split - split the name into parts:
 * sys - system name
...
static void
uucp_split(const char *arg, char *sys, int sys_size,
	   char *fwd, int fwd_size, char *file, int file_size)
{
    register char *cl, *cr;
    const char *n;
    *sys = *fwd = *file = NULLCHAR;
	...
	if ( cl == NULL) {
	    /* no ! in n */
	    (void) strncpy(file, n, --file_size);
	    file[file_size] = NULLCHAR;
	    return;
	}
	...
	cr = strrchr(n, '!');
	(void) strncpy(file, cr+1, --file_size);
	file[file_size] = NULLCHAR;
	if (--sys_size > cl-n)
	    sys_size = cl-n;
	(void) strncpy(sys, n, sys_size);
	sys[sys_size] = NULLCHAR;

	if (cr != cl) {
	    /*  more than one ! */
	    if (--fwd_size > cr-cl-1)
		fwd_size = cr-cl-1;
	    (void) strncpy(fwd, cl+1, fwd_size);
	    fwd[fwd_size] = NULLCHAR;
	}
	return;
    }
    /*NOTREACHED*/
...
static void matchfile(filename)
char	*filename;
{
	glob_t	pglob;

	if ((glob(filename, 0, NULL, &pglob) >= 0) && (pglob.gl_pathc == 1))
		strcpy(filename,*(pglob.gl_pathv));
}
