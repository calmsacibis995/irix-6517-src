...
/* functions */
static char *next_token(char *, struct name_value *);
static char *nextarg(char *, char **);
static void fillFlds(void);
static int fillList(int, char *[]);
static int validateFind(char *);
static int userFind(char *, char *, int);
static int getuline(char *);
static int cmdMatch(char *, char *);
static int nameMatch(char *, char *);
static int parse_tokens(char *[]);
static int listMatch(char *, char *[]);
...
char *name, *rmtname;
{
	...
	if (ret == SUCCESS) { /* found VALIDATE entry */
	    ...
	    if (ret) {
		DEBUG0(5, "machine/login match failed");
		return(FAIL);
	    }
	}
	else
