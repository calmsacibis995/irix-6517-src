 ...
 *	(CLEANUPLOG)--/var/spool/uucp/.Admin/uucleanup
*/
...
extern int _age(char *);		/* find the age of a file */
extern void oprocess(char *), xprocess(char *), cprocess(char *);
extern void dXprocess(char *), dNprocess(char *, char *);
extern void dMprocess(char *, char *), dDprocess(char *);
extern void wprocess(char *, char *); 
extern int toWho(char *, char **, char **);
extern int dType(char *);
extern int sendMail(char *, char *, char *, char *[]);
extern int execRnews(char *);
extern int analFrom(char *, char *, char *);
extern void logit(char *, int);

/* need these dummys to satisy some .o files */
/* ARGSUSED */
void cleanup(int code){}
/* ARGSUSED */
void systat(char *name, int type, char *text, long retry) {}
/* ARGSUSED */
...
char **user;	/* pointer to the user name */
{
	...
	static char fuser[BUFSIZ], fsystem[256+1];  /* from first From */
	static char luser[BUFSIZ], lsystem[256+1];  /* from other From */
#else
