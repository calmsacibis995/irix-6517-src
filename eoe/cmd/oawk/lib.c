...
CELL emptyfld = { OCELL, CFLD, NULL, "", 0.0, FLD | STR };
...
getrec()
{
	...
	while (svargc > 0) {
		...
		for (rr = record; ; ) {
#ifdef	sgi
			for (; (c=getc(inf)) != sep && c != EOF && rr < record + RECSIZE; *rr++ = c)
#else
			for (; (c=getc(inf)) != sep && c != EOF; *rr++ = c)
#endif
				;
#ifdef	sgi
			if (**RS == sep || c == EOF || rr >= record + RECSIZE)
#else
			if (**RS == sep || c == EOF)
#endif
				break;
...
error(f, s, a1, a2, a3, a4, a5, a6, a7) {
	fprintf(stderr, "awk: ");
	fprintf(stderr, (const char *)s, a1, a2, a3, a4, a5, a6, a7);
	fprintf(stderr, "\n");
	if (NR && *NR > 0)
