...
#ident	"@(#)keyserv:keyserv.c	1.13.4.2"
...
static int  getrootkey( des_block *master, int prompt);
static int  root_auth( SVCXPRT *trans, struct svc_req *rqstp);
...
key_gen_1( void)
{
	...
	ecb_crypt(masterkey.c, keygen.c, sizeof (keygen), DES_ENCRYPT | DES_HW);
	key = keygen;
	des_setparity(key.c);
	if (debugging) {
	...
	netnamestr *name)
{
	...
	static int gids[NGROUPS]; /* should be gid_t but USL hdr file is bad */
