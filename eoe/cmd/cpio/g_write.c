 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/cpio/RCS/g_write.c,v 1.3 1995/10/03 03:05:27 ack Exp $"
...
g_write(int devtype, int fdes, char *buf, unsigned nbytes)
{
	...
	if ((rv = rmtwrite(fdes, buf, nbytes)) <= 0) {
		switch (devtype) {
			case G_FILE: /* do not change returns for files */
			case G_NO_DEV:	/* don't change returns here, either */
				break;
			case G_TAPE:
				break;
			default:
