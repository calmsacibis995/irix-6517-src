	...
	while ((c=getc(finput)) != EOF) {
		if (c == '$') {
			if ((c=getc(finput)) != 'A') (void) putc('$', ftable);
				...
				if (faction == NULL) 
					error(badopenid, badopen, ACTNAME,
						strerror(errno));
...
/* VARARGS1 */
void
error(const char *msgid, const char *fmt, ...)
{
	char buf[BUFSIZ];
	char *buf_ptr = buf;
	int len, len1;
	...
	va_start(ap, fmt);
	(void) vpfmt(stderr, MM_NOGET|MM_ERROR, buf_ptr, ap);
	(void) fprintf(stderr, "\n");
	va_end(ap);
