...
#ident "$Header: /proj/irix6.5.7m/isms/eoe/cmd/xfs/dump/inventory/RCS/inventory.h,v 1.16 1997/08/20 23:44:24 prasadb Exp $"
...
#define INV_DIRPATH		"/var/xfsdump/inventory" 
...
typedef struct inv_mediafile {
	...
	ino64_t		 m_startino;	/* file that we started out with */
	off64_t		 m_startino_off;
	ino64_t          m_endino;	/* the dump file we ended this .. */
...
typedef struct inv_stream {
	...
	ino64_t		st_startino;	/* the starting pt */
	off64_t		st_startino_off;
	ino64_t		st_endino;  	/* where we actually ended up. this 
...
/* To convert a invdump found on tape to a inv_session_t without
