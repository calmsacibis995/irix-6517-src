#ident "$Header: /proj/irix6.5.7m/isms/eoe/cmd/xfs/dump/common/RCS/ring.c,v 1.4 1997/08/14 20:39:08 prasadb Exp $"
...
static void ring_slave_entry( void *ringctxp );
	     ...
	     intgen_t *rvalp )
{
	...
	*rvalp = 0;
	...
	for ( mix = 0 ; mix < ringlen ; mix++ ) {
		...
		if ( pinpr ) {
			intgen_t rval;
			rval = mpin( ( void * )msgp->rm_bufp, bufsz );
			if ( rval ) {
				if ( errno == EAGAIN ) {
					*rvalp = E2BIG;
					return 0;
