#ident "$Header: /proj/irix6.5.7m/isms/eoe/cmd/xfs/dump/common/RCS/drive_minrmt.c,v 1.8 1998/05/13 21:52:05 cwf Exp $"
...
ds_match( int argc, char *argv[], drive_t *drivep, bool_t singlethreaded )
{
	...
	/* If it is rmt , check if the min rmt flag and block size have 
	 * been specified.
	 * If so , this is a non-SGI remote drive and this is the right
	 * strategy.
	 */
...
get_tpcaps( drive_t *drivep )
{
	...
	if (contextp->dc_isQICpr == BOOL_TRUE)
		mtcap.mtsubtype = TPQIC24; /* fake a QIC drive */
	else
		mtcap.mtsubtype = -1;
