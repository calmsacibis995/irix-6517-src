#ident	"$Revision: 1.5 $"

#include <sys/param.h>
#include <sys/stat.h>
#include <sys/var.h>
#define _KERNEL 1
#include <sys/buf.h>
#include <sys/cred.h>
#include <sys/vfs.h>
#include <sys/vnode.h>
#include <sys/user.h>
#include <sys/pfdat.h>
#include <sys/grio.h>
#undef _KERNEL
#include <unistd.h>
#include <stdio.h>
#include <fcntl.h>
#include <math.h>
#include <stdlib.h>
#include <bstring.h>
#include <memory.h>
#include <sys/debug.h>
#include "xfs_types.h"
#include "xfs_inum.h"
#include "xfs_sb.h"
#include "xfs_ag.h"
#include "xfs_log.h"
#include "xfs_trans.h"
#include "xfs_mount.h"
#include "xfs_alloc_btree.h"
#include "xfs_bmap_btree.h"
#include "xfs_ialloc_btree.h"
#include "xfs_btree.h"
#include "xfs_ialloc.h"
#include "xfs_dinode.h"
#include "xfs_inode_item.h"
#include "xfs_inode.h"
#include "xfs_bmap.h"
#include "xfs_rw.h"

#include "sim.h"


xfs_ino_t	alloc_inum(xfs_mount_t *mp);

void	print_inum(xfs_mount_t *mp, xfs_ino_t ino);

void	read_file(xfs_mount_t	*mp,
		  xfs_ino_t	ino,
		  int		size,
		  int		iosize,
		  char		c);

void	write_file(xfs_mount_t	*mp,
		   xfs_ino_t	ino,
		   int		size,
		   int		iosize,
		   char		byte);

void	flush_file(xfs_mount_t	*mp,
		   xfs_ino_t	ino);


extern void xfs_write_file(vnode_t	*vp,
			   uio_t	*uiop,
			   int		ioflag,
			   cred_t	*credp);

extern void xfs_read_file(vnode_t	*vp,
			  uio_t		*uiop,
			  int		ioflag,
			  cred_t	*credp);


main(int argc, char **argv)
{
	xfs_mount_t	*mp;
	xfs_sb_t	*sbp;
	xfs_inode_t	*ip;
	char		*file;
	dev_t		dev;
	int		x;
	xfs_ino_t	inum;
	vnode_t		*vp;
	char		*logfile;
	dev_t		logdev;
	extern void	bunlocked(void);
	int		iosize;
	int		ialloc;
	int		size;
	char		c;
	sim_init_t	si;
	char		line[80];

	if (argc < 6) {
		printf("usage: wfile <file> <logfile > <size> <iosize> <char> <inum>\n");
		exit(1);
	}
	file = argv[1];
	logfile = argv[2];
	size = atoi(argv[3]);
	iosize = atoi(argv[4]);
	c = *argv[5];
	if (argc == 7) {
		inum = atoi(argv[6]);
		ialloc = 0;
	} else {
		inum = 0LL;
		ialloc = 1;
	}
	printf("mount %s log %s size %d iosize %d char %c inum %lld\n",
	       file, logfile, size, iosize, c, inum);
	
	si.volname = NULL;
	si.rtname = NULL;
	si.dname = file;
	si.disfile = 1;
	si.dcreat = 0;
	si.logname = logfile;
	si.lisfile = 1;
	si.lcreat = 0;
	xfs_sim_init(&si);
	mp = xfs_mount(si.ddev, si.logdev, NULL);
	sbp = &mp->m_sb;
	if (sbp->sb_magicnum != XFS_SB_MAGIC) {
		fprintf(stderr, "%s: magic number %d is wrong\n", file, sbp->sb_magicnum);
		exit(1);
	}

	if (ialloc) {
		inum = alloc_inum(mp);
	}

	write_file(mp, inum, size, iosize, c);

	flush_file(mp, inum);

	read_file(mp, inum, size, iosize, c);

	print_inum(mp, inum);

	bflush(mp->m_dev);
}
	
xfs_ino_t
alloc_inum(xfs_mount_t	*mp)
{
	xfs_trans_t	*tp;
	xfs_inode_t	*ip;
	xfs_inode_t	*rootip;
	xfs_ino_t	root_ino;
	xfs_ino_t	ino;
	buf_t		*agibp;
	boolean_t	call_again;
	cred_t		cred;

	cred.cr_uid = 0;
	cred.cr_gid = 0;

	call_again = B_FALSE;
	agibp = NULL;
	tp = xfs_trans_alloc(mp, 7);
	if (xfs_trans_reserve(tp, 32, BBTOB(16), 0, 0) != 0) {
		xfs_trans_cancel(tp, 0);
		printf("Out of space, can't allocate inode\n");
		return -1LL;
	}	

	root_ino = mp->m_sb.sb_rootino;
	rootip = xfs_trans_iget(mp, tp, root_ino,
				XFS_ILOCK_EXCL | XFS_IOLOCK_EXCL);
	ip = xfs_ialloc(tp, rootip, IFREG | 0777, 1, NODEV, &cred,
			&agibp, &call_again);
	if (call_again) {
		xfs_trans_bhold(tp, agibp);
		xfs_trans_ihold(tp, rootip);
		xfs_trans_commit(tp, 0);

		tp = xfs_trans_alloc(mp, 7);
		if (xfs_trans_reserve(tp, 0, BBTOB(16), 0, 0) != 0) {
			xfs_trans_cancel(tp, 0);
			printf("Out of space, can't allocate inode\n");
			return -1LL;
		}
		xfs_trans_bjoin(tp, agibp);
		xfs_trans_ijoin(tp, rootip, XFS_ILOCK_EXCL | XFS_IOLOCK_EXCL);
		ip = xfs_ialloc(tp, rootip, IFREG | 0777, 1, NODEV, &cred,
				&agibp, &call_again);
		ASSERT(call_again == 0);
	}
	xfs_trans_ihold(tp, ip);
	xfs_trans_commit(tp, 0);
	ino = ip->i_ino;
	printf("New ino is %lld\n", ino);
	xfs_iflock(ip);
	xfs_iflush(ip, 0);
	xfs_iput(ip, XFS_ILOCK_EXCL);
	bflush(mp->m_dev);
	return ino;
}



void
print_inum(xfs_mount_t	*mp,
	   xfs_ino_t	ino)
{
	xfs_inode_t	*ip;

	ip = xfs_iget(mp, NULL, ino, XFS_ILOCK_EXCL);

	if (ip == NULL) {
		printf("No such inode %lld\n", ino);
		return;
	}
	
	xfs_iprint(ip);

	xfs_iput(ip, XFS_ILOCK_EXCL);
}


void
read_file(xfs_mount_t	*mp,
	  xfs_ino_t	ino,
	  int		size,
	  int		iosize,
	  char		byte)
{
	xfs_inode_t	*ip;
	vnode_t		*vp;
	uio_t		uio;
	iovec_t		iovec;
	char		*buf;
	char		*check_buf;
	int		x;
	int		y;
	int		total_read;
	int		diff;

	ip = xfs_iget(mp, NULL, ino, XFS_IOLOCK_SHARED);
	if (ip == NULL) {
		printf("unknown ino %lld\n", ino);
		return;
	}

	check_buf = (char *)malloc(iosize);
	memset(check_buf, byte, iosize);

	buf = (char *)malloc(iosize);
	bzero(buf, iosize);
	iovec.iov_base = buf;
	iovec.iov_len = iosize;
	uio.uio_iov = &iovec;
	uio.uio_iovcnt = 1;
	uio.uio_offset = 0;
	uio.uio_resid = iosize;
	uio.uio_segflg = UIO_SYSSPACE;
	uio.uio_limit = 0xffffffff;
	uio.uio_fmode = 0;

	vp = XFS_ITOV(ip);

	total_read = 0;
	while (total_read < size) {
/*
		xfs_read_file(vp, &uio, 0, NULL);
*/
		xfs_read(vp, &uio, 0, NULL);
		if (uio.uio_resid != 0) {
			ASSERT(total_read + iosize - uio.uio_resid == size);
		}
		diff = bcmp(buf, check_buf, iosize - uio.uio_resid);
		ASSERT(diff == 0);
		total_read += iosize;
		if (total_read + iosize > size) {
			iosize = size - total_read;
		}
		/* uio_offset is bumped automtically */
		uio.uio_resid = iosize;
		iovec.iov_base = buf;
		iovec.iov_len = iosize;
		bzero(buf, iosize);
	}
			
	xfs_iput(ip, XFS_IOLOCK_SHARED);
}


void
write_file(xfs_mount_t	*mp,
	   xfs_ino_t	ino,
	   int		size,
	   int		iosize,
	   char		byte)
{
	xfs_inode_t	*ip;
	vnode_t		*vp;
	uio_t		uio;
	iovec_t		iovec;
	char		*buf;
	int		total_written;

	ip = xfs_iget(mp, NULL, ino, XFS_IOLOCK_EXCL);
	if (ip == NULL) {
		printf("unknown ino %lld\n", ino);
		return;
	}

	buf = (char *)malloc(iosize);
	memset(buf, byte, iosize);
	iovec.iov_base = buf;
	iovec.iov_len = iosize;
	uio.uio_iov = &iovec;
	uio.uio_iovcnt = 1;
	uio.uio_offset = 0;
	uio.uio_resid = iosize;
	uio.uio_segflg = UIO_SYSSPACE;
	uio.uio_limit = 0xffffffff;
	uio.uio_fmode = 0;

	vp = XFS_ITOV(ip);

	total_written = 0;
	while (total_written < size) {
/*
		xfs_write_file(vp, &uio, 0, NULL);
*/
		xfs_write(vp, &uio, 0, NULL);
		if (uio.uio_resid != 0) {
			ASSERT(total_written + iosize - uio.uio_resid ==
			       size);
		}
		total_written += iosize;
		if (total_written + iosize > size) {
			iosize = size - total_written;
		}
		/* uio_offset is bumped automatically */
		uio.uio_resid = iosize;
		iovec.iov_base = buf;
		iovec.iov_len = iosize;
	}
		

	xfs_iput(ip, XFS_IOLOCK_EXCL);
}


void
flush_file(xfs_mount_t	*mp,
	   xfs_ino_t	ino)
{
	xfs_inode_t	*ip;
	vnode_t		*vp;
	off_t		isize;

	ip = xfs_iget(mp, NULL, ino, XFS_IOLOCK_SHARED);
	if (ip == NULL) {
		printf("unknown ino %lld\n", ino);
		return;
	}

	vp = XFS_ITOV(ip);

	isize = (off_t)ip->i_d.di_size;
	pflushinvalvp(vp, 0, isize);

	xfs_ilock(ip, XFS_ILOCK_SHARED);
	xfs_iflock(ip);
	xfs_iflush(ip, 0);

	xfs_iput(ip, XFS_IOLOCK_SHARED | XFS_ILOCK_SHARED);
}


