...
static void dsm_last_error_ssdb()
{
  fprintf(stderr,"SSDB error code=%d, SSDB error string = %s\n",
	  ssdbGetLastErrorCode(dsmdbError),
	  ssdbGetLastErrorString(dsmdbError));
}
#endif
...
__uint64_t dsm_ssdb_setup_interfaces()
{
  ...
  dsmdbError=ssdbCreateErrorHandle();
  if(!dsmdbError)
    return err;
#if SSDB_CONNECT_ONCE
  DSMdbConnect=
    ssdbOpenConnection(dsmdbError,dbClient,NULL,SSS_SSDB_DBNAME,
...
__uint64_t dsm_check_delete_event_rules_ssdb(int objid)
{
  ...
  while(Pev)
        ...
        for(i=0;(i<sizeof(aryClass)/sizeof(int));i++)
	{
	   ...
	   * Send sql request.
	   */
	  dbReq=ssdbSendRequestTrans(dsmdbError,DSMdbConnect,
				     SSDB_REQTYPE_SELECT,
	  ...
	  if(!dbReq)
	    {				
	      ...
	      if(verbose > VERB_INFO)
		{
		  fprintf(stderr,"SSDB(DSM:1) ssdbSendRequestTrans Error:");
		  dsm_last_error_ssdb();
		}
	      num_errors_in_dsm_ssdb++;
#endif
#if !SSDB_CONNECT_ONCE
	      ssdbCloseConnection(dsmdbError,DSMdbConnect);
#endif
	   ...
	   */
	  while(ssdbGetNextField(dsmdbError,dbReq,&Itemp,sizeof(int)))
	    {
	      /* 
	  ...
	  ssdbFreeRequest(dsmdbError,dbReq);
	}
      }
...
__uint64_t dsm_init_event_ssdb(struct event_private *Pev)
  {
	 ...
	 * Send sql request.
	 */
	dbReq=ssdbSendRequestTrans(dsmdbError,DSMdbConnect,SSDB_REQTYPE_SELECT,
				   SSDB_EVENT_RULE_TABLE,
				   "SELECT action_id from %s WHERE "
				   "class_id=%d and type_id=%d and "
				   "sys_id = '%llX'",
	 ...
	 * Iterate as long as we get the object id.
	 */
	while(ssdbGetNextField(dsmdbError,dbReq,&objid,sizeof(int)))
	  {
	    /*
					    ...
					    ssdb_Connection_Handle dbConnect)
{
    ...
    if(!ssdbIsConnectionValid(dsmdbError,dbConnect))
	return DSM_ERROR(DSM_MAJ_INIT_ERR,DSM_MIN_INVALID_ARG);
    
    /* 
     * Send the sql request.
     */
    dbReq=ssdbSendRequestTrans(dsmdbError,dbConnect,
			       SSDB_REQTYPE_SELECT,
			       SSDB_RULE_TABLE,
	...
	if(verbose > VERB_INFO)
	{
	    fprintf(stderr,"SSDB(DSM:10) ssdbSendRequestTrans Error:");
	    dsm_last_error_ssdb();
	}
	num_errors_in_dsm_ssdb++;
#endif
     ...
     * Get Retry.
     */
    if(!ssdbGetNextField(dsmdbError,dbReq,&rule->Paction->retry,sizeof(int)))
    {
#if EVENTS_DEBUG
	if(verbose > VERB_INFO)
	{
     ...
     * Get Timeout.
     */
    if(!ssdbGetNextField(dsmdbError,dbReq,&rule->Paction->timeout,sizeof(int)))
    {
#if EVENTS_DEBUG
	if(verbose > VERB_INFO)
	{
				     ...
				     ssdb_Connection_Handle dbConnect)
{
   ...
   * here.
   */
  if(!dbConnect || !ssdbIsConnectionValid(dsmdbError,dbConnect))
    {
#if SSDB_CONNECT_ONCE
   ...
   * Send sql statement.
   */
  dbReq=ssdbSendRequestTrans(dsmdbError,dbConnect,SSDB_REQTYPE_SELECT,
			     SSDB_RULE_TABLE,
  ...
  if(!dbReq)
    {
      ...
      if(verbose > VERB_INFO)
	{
	  fprintf(stderr,"SSDB(DSM:3) ssdbSendRequestTrans Error:");
	  dsm_last_error_ssdb();
	}
      num_errors_in_dsm_ssdb++;
#endif
#if !SSDB_CONNECT_ONCE
      if(close_connection)
	ssdbCloseConnection(dsmdbError,dbConnect);
#endif
      return DSM_ERROR(DSM_MAJ_SSDB_ERR,DSM_MIN_DB_NORULE);
    }
   ...
   */
  sem_mem_free(Pchar_rule);
  if(!ssdbFreeRequest(dsmdbError,dbReq))
    {
      /*
       * failed.
       */
                                         ...
                                         ssdb_Connection_Handle dbConnect)
{
  ...
  if(!dbReq)
      ...
      if(!dbReq)
	{
	  ...
	  if(verbose > VERB_INFO)
	    {
	      fprintf(stderr,"SSDB(DSM:4) ssdbSendRequestTrans Error:");
	      dsm_last_error_ssdb();
	    }
	  num_errors_in_dsm_ssdb++;
#endif
  ...
  else
    {
       ...
       * Is this rule a dso ?.
       */
      if(!ssdbGetNextField(dsmdbError,dbReq,&Itemp,sizeof(int)))
	{
#if EVENTS_DEBUG
	  if(verbose > VERB_INFO)
	    {
...
__uint64_t dsm_init_host_rule_ssdb(struct rule *rule,int Iobjid,
				   ssdb_Connection_Handle dbConnect)
  {
	...
	if(verbose > VERB_INFO)
	  {
	    fprintf(stderr,"SSDB(DSM:5) ssdbSendRequestTrans Error:");
	    dsm_last_error_ssdb();
	  }
	num_errors_in_dsm_ssdb++;
#endif
					       ...
					       struct event_private *Pev)
{
  ...
  dbReq=ssdbSendRequestTrans(dsmdbError,DSMdbConnect,SSDB_REQTYPE_INSERT,
  ...
  if(dbReq)
  ...
  else
    {
      ...
      if(verbose > VERB_INFO)
	{
	  fprintf(stderr,"SSDB(DSM:7) ssdbSendRequestTrans Error:");
	  dsm_last_error_ssdb();
	}
      num_errors_in_dsm_ssdb++;
#endif
  ...
  dbReq=ssdbSendRequestTrans(dsmdbError,DSMdbConnect,SSDB_REQTYPE_UPDATE,
  ...
  if(dbReq)
  ...
  else
    {
      ...
      if(verbose > VERB_INFO)
	{
	  fprintf(stderr,"SSDB(DSM:8) ssdbSendRequestTrans Error:");
	  dsm_last_error_ssdb();
	}
      num_errors_in_dsm_ssdb++;
#endif
...
void dsm_ssdb_clean()
{
  ...
  ssdbCloseConnection(dsmdbError,DSMdbConnect);
#endif
  ssdbDeleteErrorHandle(dsmdbError);
}
