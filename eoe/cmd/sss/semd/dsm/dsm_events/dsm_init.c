...
/*
 * Should contain any dsm specific stuff when the program becoes a daemon.
 */
static __uint64_t dsm_become_daemon()
{
  return 0;
}
 ...
 */
__uint64_t dsm_check_arguments(int argc,char **argv)
{
  int c;
  
  while ( -1 != (c = getopt(argc,argv,"v:h")) )
    {
      switch(c)
	{
	case 'v':
...
/*
 * Release any allocated memory. Clean up all the interfaces.
 * Called at exit time.
 */
void dsm_clean_everything()
{
  alarm(0);			/* cancel any previous alarm requests. */
  dsm_seh_clean();		/* close interface. */
  dsm_ssdb_clean();		/* close interface. */
  dsm_events_clean();		/* free memory allocated to events. */
  dsm_rules_clean();		/* free memory allocated to rules. */
}
