		             ...
		             int qualifier)
{
    ...
    if ( !ssdbIsClientValid(ErrorHandle, Client) ) {
    ...
    if ( !(Connection = ssdbOpenConnection(ErrorHandle, Client, 
    ...
    if ( !(Request = ssdbSendRequest(ErrorHandle, Connection, 
    ...
    if ( (NumRows = ssdbGetNumRecords(ErrorHandle, Request)) != 0 ) {
	...
	if ( (NumCols = ssdbGetNumColumns(ErrorHandle, Request)) == 0 ) {
	...
	for ( iTmpVar2 = 0; iTmpVar2 < NumCols; iTmpVar2++) {
		...
		if ( ((iFieldVal = 
		       ssdbGetFieldMaxSize(ErrorHandle,
					   Request, iTmpVar2)) <= 0) &&
		     iImportant[iTmpVar2])
		{
                    /*-----------------------------------------------*/
	...
	for ( iTmpVar1 = 0; iTmpVar1 < NumRows; iTmpVar1++ ) {
		...
		switch (iTmpVar2) {
		    case 0:
			if ( !ssdbGetNextField(ErrorHandle, Request, 
					       /*&pSysInfo->system_id,*/
					       tmpSysID,
    ...
    ssdbFreeRequest(ErrorHandle, Request);

    /* Close Connection that we opened */

    end1:
    ssdbCloseConnection(ErrorHandle, Connection);
    return(ErrCode);
}
...
{
    printf("Error in ssdb%s: %s\n", loc, ssdbGetLastErrorString(ErrorHandle));
}
