...
/*
 * A note on the way locking of events and rules work.
 *
 * There are two locks when the SEH & DSM share address spaces as is the
 * case with the pthreaded case -- rules_events lock and memory lock.
 *
 * The rules_events lock controls access to both the rules and events data structures.
 */
...
#if !KLIB_LIBRARY
  memset(RUN(Pev),0,sizeof(struct event_runtime));
#endif
