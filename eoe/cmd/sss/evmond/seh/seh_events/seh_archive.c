 ...
 */
char *semdAcceptMessage(const char *buffer,int *len)
{
  static char status_buffer[16*MAX_MESSAGE_LENGTH];
   ...
   */
  if(*len == (strlen(SEMD_QUIT)+1) && !strncasecmp(SEMD_QUIT,buffer,*len))
  {
      exit_flag=1;
  }
