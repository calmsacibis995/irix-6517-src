...
/* ------------------------------------------------------------------------- */
extern void semdInit(void);
extern void semdDone(void);
extern void semdOn  (void);
extern int semdDeliverEvent(const char *_hostNameFrom,long eventTimeStamp,int eventClass,int eventType,int eventPri,int eventCounter,int eventDataLength,void *eventData);
extern const char *semdAcceptMessage(const char *msgbuf,int *msgsize);
/* ------------------------------------------------------------------------- */
...
#define EVENTMONSOCK_PATH   "/tmp/.eventmond.events.sock"      /* default syslogd socket name for events */
...
#define EVMCMD_BMSGROK 'C'  /* Response: Message command */
#define EVMCMD_SEMON   'Y'  /* Priv: Event handle subsystem "on" command */
#define EVMCMD_SEMOFF  'y'  /* Priv: Event handle subsystem "off" command */
/* ------------------------------------------------------------------------- */
...
typedef struct s_statinfo_struct {
  ...
  unsigned long volatile statInTotalSockChmodError;      /* Stat: Info/Cmd thread, total chmod error counter */
  ...
  unsigned long volatile statInTotalBinaryMsgRequest;    /* Stat: Info thread, total binary_messages request counter */
  unsigned long volatile statInTotalSemOnRequest;        /* Stat: Info thread, total sem_subsystem_on request counter */
  unsigned long volatile statInTotalSemOffRequest;       /* Stat: Info thread, total sem_subsystem_off request counter */
  ...
  unsigned long volatile statInTotalOpenSndBMsgSockError;/* Stat: Info thread, total open_send_binary_msg_socket_error counter */
  unsigned long volatile statInTotalSendBMsgSuccess;     /* Stat: Info thread, total send_binary_msg_success counter */
  unsigned long volatile statInTotalSendBMsgError;       /* Stat: Info thread, total send_binary_msg_error counter */
...
#define EVMSEQN_RESERVED00       0x0020011Bl  /* Seq.Number for "" syslog message */
#define EVMSEQN_RESERVED01       0x0020011Cl  /* Seq.Number for "" syslog message */
...
static const char szEventMonInfoSockPath[] = "/tmp/.eventmond.info.sock";    /* EventMon Info Socket name (non secured) */
...
static int volatile sem_enable                  = 0;                /* Enable event handle subsystem flag */
static int volatile seminited_flg               = 0;                /* Event handle subsystem inited flag */
...
FPssdbInit               *fp_ssdbInit               = NULL;
FPssdbDone               *fp_ssdbDone               = NULL;
FPssdbCreateErrorHandle  *fp_ssdbCreateErrorHandle  = NULL;
FPssdbDeleteErrorHandle  *fp_ssdbDeleteErrorHandle  = NULL;
FPssdbIsErrorHandleValid *fp_ssdbIsErrorHandleValid = NULL;
FPssdbNewClient          *fp_ssdbNewClient          = NULL;
FPssdbDeleteClient       *fp_ssdbDeleteClient       = NULL;
FPssdbIsClientValid      *fp_ssdbIsClientValid      = NULL;
FPssdbOpenConnection     *fp_ssdbOpenConnection     = NULL;
FPssdbCloseConnection    *fp_ssdbCloseConnection    = NULL;
FPssdbIsConnectionValid  *fp_ssdbIsConnectionValid  = NULL;
FPssdbSendRequest        *fp_ssdbSendRequest        = NULL;
FPssdbSendRequestTrans   *fp_ssdbSendRequestTrans   = NULL;
FPssdbFreeRequest        *fp_ssdbFreeRequest        = NULL;
FPssdbIsRequestValid     *fp_ssdbIsRequestValid     = NULL;
FPssdbGetNumRecords      *fp_ssdbGetNumRecords      = NULL;
FPssdbGetNumColumns      *fp_ssdbGetNumColumns      = NULL;
FPssdbGetNextField       *fp_ssdbGetNextField       = NULL;
FPssdbLockTable          *fp_ssdbLockTable          = NULL;
FPssdbUnLockTable        *fp_ssdbUnLockTable        = NULL;
FPssdbGetRow             *fp_ssdbGetRow             = NULL;
FPssdbGetFieldMaxSize    *fp_ssdbGetFieldMaxSize    = NULL;
FPssdbRequestSeek        *fp_ssdbRequestSeek        = NULL;
FPssdbGetLastErrorCode   *fp_ssdbGetLastErrorCode   = NULL;
FPssdbGetLastErrorString *fp_ssdbGetLastErrorString = NULL;
/* ------------------------------------------------------------------------- */
...
static pthread_t cmd_tid;             /* Command thread id */
...
static DMUTEX start_cmd_mutex;        /* Synchro Start Mutex for Command thread */
...
{ time_t t; 
  ...
  memset(&start_cmd_mutex,0,sizeof(DMUTEX));
...
{ int i = 0;
  ...
  if(i < maxbufsize) i += snprintf(&buf[i],maxbufsize-i,"\
Output filename:        \"%s\"\n\
...
ESP tag in syslog:      \"%s\"\n\
EventHandle subsystem:  \"%s\"\n\
EventHandle inited:     %d\n\
GroupMode is:           \"%s\"\n\
CurrLoadConfig timeout: %d sec.\n", 
  ...
  sem_enable ? szOn : szOff, /* Enable/Disable event handle subsystem */
  seminited_flg,      /* Event handle subsystem inited flag */
  ...
  if(i < maxbufsize) i += snprintf(&buf[i],maxbufsize-i,"%s\nAmtickerThread Status:\n\
Thread status:       \"%s\"\n\
Ticker fname:        \"%s\"\n\
...
Binary Messages:     %lu\n\
EventHandle ON req:  %lu\n\
EventHandle OFF req: %lu\n\
Unknown request cnt: %lu\n\
Open SndInfoSock err:%lu\n\
Send Info success:   %lu\n\
Send Info error:     %lu\n\
Open SndBMsgSock err:%lu\n\
Send BMsg success:   %lu\n\
Send BMsg error:     %lu\n\
Invalid Src address: %lu\n",
  ...
  est.statInTotalSockChmodError,      /* Chmod socket error */
  ...
  est.statInTotalBinaryMsgRequest,    /* Binary messages request counter */
  est.statInTotalSemOnRequest,        /* Event handle subsystem "on" request counter */
  est.statInTotalSemOffRequest,       /* Event handle subsystem "off" request counter */
  ...
  est.statInTotalOpenSndBMsgSockError,/* Open send binary message socket error counter (response) */
  est.statInTotalSendBMsgSuccess,     /* Send binary msg success counter */
  est.statInTotalSendBMsgError,       /* Send binary msg error counter */
...
/* ====================== thread_func_info ================================= */
void *thread_func_info(void *secured_flg)
{ char buf[RCVINFOREQ_MAXSIZE+1],*c,*socket_name;
  int i,s,l,slen,buflen;
  struct sockaddr_un saddr;
  int opensock_timeout = TOUTBASE_IFOPENSOCK;

  if(secured_flg)
  { pthread_mutex_lock(&start_cmd_mutex.mutex); pthread_mutex_unlock(&start_cmd_mutex.mutex);
    socket_name = (char*)szEventMonCmdSockPath;
  }
  else
  { pthread_mutex_lock(&start_info_mutex.mutex); pthread_mutex_unlock(&start_info_mutex.mutex);
    socket_name = (char*)szEventMonInfoSockPath;
  }
  ...
  { if(s < 0)
    { est.statInTotalOpenSocket++;
      if((s = socket(AF_UNIX,SOCK_DGRAM,0)) >= 0)
      { unlink_file(socket_name); est.statInTotalOpenSocketSuccess++; est.statInTotalBindSocket++;
        if(bind(s,(struct sockaddr*)&saddr,make_sockaddr(&saddr,socket_name)) < 0)
        { close_unlink(&s,socket_name); est.statInTotalBindSocketError++;
        }
        else
        { if(secured_flg)
	  { if(chmod(szInpSockName,0700) == (-1)) est.statInTotalSockChmodError++;
	  }
    ...
    { memset(&saddr,0,sizeof(saddr)); slen = sizeof(saddr);  est.statInTotalReadSocket++;
      ...
      { buf[l] = 0; est.statInTotalReadSocketSuccess++;
        ...
        { switch(buf[3]) {
          case EVMCMD_STOP    : if(secured_flg)
          ...
          case EVMCMD_TEST    : est.statInTotalTestRequest++;
                                break;
          case EVMCMD_RFILT   : if(secured_flg)
	                        { est.statInTotalReloadFilterRequest++;
                                ...
                                break;
          case EVMCMD_POUT    : if(secured_flg)
	                        { est.statInTotalRefreshOutputRequest++;
          ...
          case EVMCMD_SETTICK : if(secured_flg)
				  ...
				  { pthread_mutex_lock(&amticker_mutex.mutex);
				    amticker_timeout = i; pthread_cond_signal(&amticker_condition.cond);
                                ...
                                break;
          case EVMCMD_SETSTAT : if(secured_flg)
	                        { est.statInTotalAmtSetExecInterval++;
                                  if((i = atoi(&buf[4])) >= 0)
				  { pthread_mutex_lock(&amticker_mutex.mutex);
                                ...
                                break;
          case EVMCMD_SETGRP  : if(secured_flg)
	                        { est.statInTotalSetGrpModeRequest++;
                                  enable_group_mode = 1;
				}
                                break;
          case EVMCMD_CLRGRP  : if(secured_flg)
	                        { est.statInTotalClrGrpModeRequest++;
                                  enable_group_mode = 0;
				}
                                break;
          case EVMCMD_BMSG    : if(secured_flg)
	                        { est.statInTotalBinaryMsgRequest++;
                                  if((slen > sizeof(saddr.sun_family)) && saddr.sun_path[0])
                                  { if((i = socket(AF_UNIX,SOCK_DGRAM,0)) >= 0)
                                    { buf[3] = EVMCMD_BMSGROK; l -= 4;
				      if((c = (char *)semdAcceptMessage((const char *)&buf[4],&l)) != 0)
				      { if(l > (sizeof(buf)-4)) l = sizeof(buf)-4;
				        memcpy(&buf[4],c,l); l += 4;
				      }
				      else l = 4;
				      buflen = l;
				      if((l = sendto(i,buf,buflen,0,(struct sockaddr*)&saddr,slen)) != buflen) est.statInTotalSendBMsgError++;
                                      else est.statInTotalSendBMsgSuccess++;
                                      close_socket(i);
                                    }
                                    else est.statInTotalOpenSndBMsgSockError++;
                                  }
                                  else est.statInTotalInvSrcAddr++;
				}
                                break;
          case EVMCMD_SEMON   : if(secured_flg)
	                        { est.statInTotalSemOnRequest++;
                                  i = getFilterInfoIsReadyFlag();
                                  pthread_mutex_lock(&output_queue_mutex.mutex); sem_enable = 1;
                                  /* Initialize SEM only if SSDB is ready */
                                  if(i) 
                                  {  if ( !seminited_flg) 
                                        { semdInit(); seminited_flg++; }
                                     else    
                                          semdOn();
                                  }
                                ...
                                break;
          case EVMCMD_SEMOFF  : if(secured_flg)
	                        { est.statInTotalSemOffRequest++;
                                  pthread_mutex_lock(&output_queue_mutex.mutex); sem_enable = 0; pthread_cond_signal(&outnotempt_condition.cond);
                                  pthread_mutex_unlock(&output_queue_mutex.mutex);
				}
                                break;
      ...
      if(l < 0) { close_unlink(&s,socket_name); est.statInTotalReadSocketError++; }
    }
  }
  close_unlink(&s,socket_name);
#ifdef INCDEBUG
  dprintf("*** %s thread exit ***\n",secured_flg ? "Command" : "Info");
#endif
  return 0;
}
...
{ int bsize,reptcnt,eclass,etype,epri;
  ...
  while(!exit_phase)
  { /* Wite message or signal */
    pthread_mutex_lock(&output_queue_mutex.mutex);
    while(!exit_phase && (!sem_enable || (link = pickUpOutputEmsgList()) == 0))
     pthread_cond_wait(&outnotempt_condition.cond,&output_queue_mutex.mutex);
    if(!seminited_flg && !exit_phase ) { semdInit(); seminited_flg++; }
    pthread_mutex_unlock(&output_queue_mutex.mutex);
    ...
    if(link)
    { if(SEM_EVENTDATA_MAX_SIZE >= bsize)
      { semdDeliverEvent(hostname,(long)timestamp,eclass,etype,epri,reptcnt,bsize,buffer);
        semapi_retcode = SEMERR_SUCCESS;
      }
      else  semapi_retcode = SEMERR_FATALERROR;
  ...
  if(seminited_flg) { semdDone(); seminited_flg = 0; }
...
/*  if((ssdb_dlhandle = dlopen(lpszSSDBLibraryName,RTLD_NOW)) == NULL) FIXME */
  ...
  if((fp_ssdbIsErrorHandleValid = (FPssdbIsErrorHandleValid*)dlsym(ssdb_dlhandle,"ssdbIsErrorHandleValid")) == 0) lfp_error++;
  ...
  if((fp_ssdbIsClientValid      = (FPssdbIsClientValid*)dlsym(ssdb_dlhandle,"ssdbIsClientValid")) == 0) lfp_error++;
  ...
  if((fp_ssdbIsConnectionValid  = (FPssdbIsConnectionValid*)dlsym(ssdb_dlhandle,"ssdbIsConnectionValid")) == 0) lfp_error++;
  ...
  if((fp_ssdbIsRequestValid     = (FPssdbIsRequestValid*)dlsym(ssdb_dlhandle,"ssdbIsRequestValid")) == 0) lfp_error++;
  ...
  if((fp_ssdbLockTable          = (FPssdbLockTable*)dlsym(ssdb_dlhandle,"ssdbLockTable")) == 0) lfp_error++;
  if((fp_ssdbUnLockTable        = (FPssdbUnLockTable*)dlsym(ssdb_dlhandle,"ssdbUnLockTable")) == 0) lfp_error++;
  if((fp_ssdbGetRow             = (FPssdbGetRow*)dlsym(ssdb_dlhandle,"ssdbGetRow")) == 0) lfp_error++;
  if((fp_ssdbGetFieldMaxSize    = (FPssdbGetFieldMaxSize*)dlsym(ssdb_dlhandle,"ssdbGetFieldMaxSize")) == 0) lfp_error++;
  if((fp_ssdbRequestSeek        = (FPssdbRequestSeek*)dlsym(ssdb_dlhandle,"ssdbRequestSeek")) == 0) lfp_error++;
  ...
  { timeout = 0;
    ...
    { est.statRfCurrentLoadLibraryTimeout = TOUTBASE_LLIB;
      ...
      { est.statRfCurrentInitLibraryTimeout = TOUTBASE_ILIB;
        ...
        /* Initialize SEM if SEM functionality enabled */
        pthread_mutex_lock(&output_queue_mutex.mutex);
        if(sem_enable && !seminited_flg)
        { semdInit(); seminited_flg++;
          pthread_cond_signal(&outnotempt_condition.cond);
        }
        pthread_mutex_unlock(&output_queue_mutex.mutex);
...
/* -------------------------- send_test_request ---------------------------- */
static void send_test_request(int secured)
{ struct sockaddr_un su_addr;
  char buf[16];
  int s;

  if((s = socket(AF_UNIX, SOCK_DGRAM, 0)) >= 0)
  { sendto(s,buf,prepare_cmdbuffer(buf,EVMCMD_TEST),0,(struct sockaddr*)&su_addr,make_sockaddr(&su_addr,secured ? szEventMonCmdSockPath : szEventMonInfoSockPath));
    close_socket(s);
  }
}
/* ---------------------------- send_test_event ---------------------------- */
...
{ pthread_mutex_unlock(&start_rsock_mutex.mutex);       /* Start RcvSocket thread */
  ...
  pthread_mutex_unlock(&start_cmd_mutex.mutex);         /* Start Command thread */
  ...
  /* reanimate cmd thread (send test request message) */
  send_test_request(1);
  ...
  pthread_join(cmd_tid,NULL);      /* wait exit for info thread */
...
/* ------------------------ isr_reapchild ---------------------------------- */
void isr_reapchild(int sign)
{ union wait status;
  while(wait3((int *)&status,WNOHANG,(struct rusage *)NULL) > 0);
  signal(sign,isr_reapchild);
}
/* ------------------------- daemon_init ----------------------------------- */
...
{ pid_t pid;
  ...
  signal(SIGCHLD,isr_reapchild);                        /* Child process status change, flush all process stats */
...
{ /* Init RcvSocket thread */
  ...
  if(pthread_create(&info_tid,NULL,thread_func_info,(void*)0))
  ...
  /* Init Command thread */
  pthread_mutex_lock(&start_cmd_mutex.mutex);
  if(pthread_create(&cmd_tid,NULL,thread_func_info,(void*)1))
  { exit_phase++; pthread_mutex_unlock(&start_cmd_mutex.mutex);
    syslog_msg(LOG_ERR,EVMSEQN_INITTHREADERR,szCantInitThread,"cmd"); exit(EMERR_INITTHREAD);
  }
...
{ /* init mutex resources */
  ...
  inited_mutex++;
  if(!init_dmutex(&start_cmd_mutex))        { syslog_msg(LOG_ERR,EVMSEQN_INITMUTEXERR,szCantInitMutex,"start_cmd"); exit(EMERR_INITDATA); }
  inited_mutex++;
...
{ /* Destroy all mutexes */
  ...
  { inited_mutex = 0;
    ...
    destroy_dmutex(&start_cmd_mutex);
...
{ char buf[2048];
  ...
  l = sendto(s,buf,len,0,(struct sockaddr*)&su_addr,make_sockaddr(&su_addr,szEventMonCmdSockPath)); close_socket(s);
  if(l != len)
  { if(!silenceMode) fprintf(stderr,szErrorDaemonNotInstalled3,szDaemonName,EMERR_NOTINSTALLED,errno);
    exit(EMERR_NOTINSTALLED);
  }
...
/* ------------------- declare_sem_enable_mode ----------------------------- */
static void declare_sem_enable_mode(char *_optarg)
{ sem_enable = (-1);
  if(_optarg && _optarg[0])
  { if(!strcmp(_optarg,szOn) || !strcasecmp(_optarg,szOn) || !strcmp(_optarg,"1")) sem_enable = 1;
    else if(!strcmp(_optarg,szOff) || !strcasecmp(_optarg,szOff) || !strcmp(_optarg,"0")) sem_enable = 0;
  }
  if(sem_enable < 0)
  { if(!silenceMode) fprintf(stderr,szErrorInvalidParameter3,szDaemonName,EMERR_INVPARAM,_optarg);
    exit(EMERR_INVPARAM);
  }
  if(prev_instance) declare_something(sem_enable ? EVMCMD_SEMON : EVMCMD_SEMOFF); /* declare event handle subsystem on/off */
}
/* ---------------------- declare_daemon_stop ------------------------------ */
...
[-x on|off] [-y on|off] [-p pause_sec] [-i] [-v] [-c] [-h]\n" : "%s [-i] [-v] [-c] [-h]\n",szDaemonName);
    ...
    if(privModeFlag)
    {
      ...
      fprintf(stderr," -y - set \"on/off\" for event handle sybsystem (default: \"%s\")\n",sem_enable ? szOn : szOff);
...
{ char tmp[1024];
  int c,i,err = 0;
#ifdef INCDEBUG
  char *cmdkey_string = privModeFlag ? "hHcCvViIqQrRdDLlA:a:U:u:J:j:E:e:G:g:x:X:y:Y:o:O:f:F:b:B:s:S:k:K:n:N:t:T:w:W:u:U:p:P:" : "hHcCvViIs:S:";
#else
  char *cmdkey_string = privModeFlag ? "hHcCvViIqQrRLlA:a:U:u:J:j:E:e:G:g:x:X:y:Y:f:F:b:B:s:S:k:K:n:N:t:T:w:W:u:U:p:P:" : "hHcCvViIs:S:";
#endif
  ...
  { switch(c) {
    ...
    case 'x' : set_ssstag_enable_mode(optarg);
               break;
    case 'Y' : /* Only in Priv mode */
    case 'y' : declare_sem_enable_mode(optarg);
               break;
    default  : err++;
               break;
    }; /* end of switch(c) */
