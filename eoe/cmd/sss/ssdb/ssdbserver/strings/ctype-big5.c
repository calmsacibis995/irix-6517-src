...
uint16 big5stroke(uint16 i);
uint16 big5strokexfrm(uint16 i);
...
int my_strnxfrm(uchar * dest, uchar * src, int len, int srclen)
{
  ...
  while (len--)
  {
    ...
    {
      e = big5strokexfrm(big5code(*src, *(src+1)));
      *dest++ = big5head(e);
      *dest++ = big5tail(e);
      src +=2;
      len--;
		      ...
		      uint res_length, char *min_str,char *max_str)
{
  ...
  {
    if (ptr != end+1 && isbig5code(ptr[0],ptr[1]))
    {
      *min_str++= *max_str++ = *ptr++;
      *min_str++= *max_str++ = *ptr;
      continue;
