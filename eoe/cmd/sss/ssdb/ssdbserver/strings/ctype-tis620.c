/* 
   Copyright (c) 1998 Pruet Boonma <pruet@ds90.intanon.nectec.or.th>
   Copyright (C) 1998 by Theppitak Karoonboonyanan, all rights reserved.
   Copyright (c) 1989, 1991 Samphan Raruenrom <samphan@thai.com>
...
/* $Id: ctype-tis620.c,v 1.1 1999/05/07 22:58:53 satish Exp $
...
tchar* thai2sortable(const tchar *tstr,uint len);
...
uint MY_STRXFRM_MULTIPLY = BUFFER_MULTIPLY;
...
{
	return(strcmp(thai2sortable(s1, len1), thai2sortable(s2, len2)));
}
...
{
  return(strcmp(thai2sortable(s1, (uint) strlen(s1)), thai2sortable(s2, (uint)strlen(s2))));
}
...
int my_strxfrm(uchar * dest, uchar * src, int len)
{
	uint bufSize;
	bufSize = buffsize(src);
	memcpy((uchar *)dest, (uchar *)thai2sortable(src, (uint)strlen(src)), bufSize);
  	return bufSize;
}

my_bool my_like_range(const char *ptr, uint ptr_length, pchar escape, uint res_length, char *min_str, char *max_str) 
{
	char *end;
	...
	tbuff = thai2sortable(ptr, ptr_length);
...
void ThNormalize(uchar* ptr, uint field_length, const uchar* from, uint length)
{
	...
	while(length--) {
		if((istone(*fr) || isdiacrt1(*fr)) && (islwrvowel(fr[1]) || isuprvowel(fr[1]))) {
			*p = fr[1];
			p[1] = *fr;
			fr += 2;
			p += 2;
