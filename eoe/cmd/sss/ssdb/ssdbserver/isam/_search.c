...
int _ni_check_index(N_INFO *info, int inx)
{
...
int _ni_search(register N_INFO *info, register N_KEYDEF *keyinfo, uchar *key, uint key_len, uint nextflag, register ulong pos)
{
  int error,flag;
  uint nod_flag;
  uchar *keypos,*maxpos;
  uchar lastkey[N_MAX_KEY_BUFF],*buff;
  DBUG_ENTER("_ni_search");
  ...
  if (pos == NI_POS_ERROR)
  {
    ...
    info->lastpos= NI_POS_ERROR;
    if (!(nextflag & (SEARCH_SMALLER+SEARCH_BIGGER)))
      DBUG_RETURN(-1);				/* Not found ; return error */
    DBUG_RETURN(1);				/* Search at upper levels */
  }

  if (!(buff=_ni_fetch_keypage(info,keyinfo,pos,info->buff,
  ...
  if (flag)
  {
    if ((error=_ni_search(info,keyinfo,key,key_len,nextflag,
			  _ni_kpos(nod_flag,keypos))) <= 0)
      DBUG_RETURN(error);

    if (flag >0)
    {
      if (nextflag & SEARCH_SMALLER && keypos == buff+2+nod_flag)
  ...
  else
  {
    if (nextflag & SEARCH_FIND && (!(keyinfo->base.flag & HA_NOSAME)
				   || key_len) && nod_flag)
    {
      if ((error=_ni_search(info,keyinfo,key,key_len,SEARCH_FIND,
			    _ni_kpos(nod_flag,keypos))) >= 0 ||
  ...
  if (nextflag & SEARCH_SMALLER && flag != 0)
    keypos=_ni_get_last_key(info,keyinfo,buff,lastkey,keypos);

  VOID((*keyinfo->get_key)(keyinfo,nod_flag,&keypos,lastkey));
  VOID(_ni_move_key(keyinfo,info->lastkey,lastkey));
  info->lastpos=_ni_dpos(info,nod_flag,keypos);
...
int _ni_bin_search(N_INFO *info, register N_KEYDEF *keyinfo, uchar *page, uchar *key, uint key_len, uint comp_flag, uchar **ret_pos, uchar *buff)
{
  reg4 int start,mid,end;
  int flag;
  uint totlength,nod_flag;
  DBUG_ENTER("_ni_bin_search");
  ...
  {
    mid= (start+end)/2;
    if ((flag=_ni_key_cmp(keyinfo->seg,page+(uint) mid*totlength,key,key_len,
			  comp_flag))
	>= 0)
      end=mid;
    else
  ...
  if (mid != start)
    flag=_ni_key_cmp(keyinfo->seg,page+(uint) start*totlength,key,key_len,
		     comp_flag);
  if (flag < 0)
  ...
  *ret_pos=page+(uint) start*totlength;
  ...
  DBUG_RETURN(flag);
	...
	/* Used instead of _ni_bin_search() when key is packed */
	/* Puts smaller or identical key in buff */
	/* Key is searched sequentially */

int _ni_seq_search(N_INFO *info, register N_KEYDEF *keyinfo, uchar *page, uchar *key, uint key_len, uint comp_flag, uchar **ret_pos, uchar *buff)
{
  int flag;
  uint nod_flag,length;
  uchar t_buff[N_MAX_KEY_BUFF],*end;
  DBUG_ENTER("_ni_seq_search");
  ...
  *ret_pos=page;
  while (page < end)
  {
    ...
    if ((flag=_ni_key_cmp(keyinfo->seg,t_buff,key,key_len,comp_flag)) >= 0)
      break;
#ifdef EXTRA_DEBUG
...
ulong _ni_kpos(uint nod_flag, uchar *after_key)
{
  after_key-=nod_flag;
  switch (nod_flag) {
  case 3:
...
void _ni_kpointer(register N_INFO *info, register uchar *buff, ulong pos)
{
  pos/=512L;
...
ulong _ni_dpos(N_INFO *info, uint nod_flag, uchar *after_key)
{
  ulong pos;
...
void _ni_dpointer(N_INFO *info, uchar *buff, ulong pos)
{
  ...
  default: abort();			/* Impossible */
	...
	*/
int _ni_key_cmp(register N_KEYSEG *keyseg, register uchar *a, register uchar *b, uint key_length, uint nextflag)
{
  ...
  if (!(nextflag & (SEARCH_FIND | SEARCH_NO_FIND)) || key_length == 0)
  ...
  for ( ; (int) key_length >0 ; key_length-= (keyseg++)->base.length)
  {
	...
	{
	  if (flag = my_strnncoll(a, (end-a), b, b_length))
	    return (keyseg->base.flag & HA_REVERSE_SORT) ? -flag : flag;
	  b+= (uint) (end-a);
	  a=end;
	}
	...
	{
	  if (flag = my_strnncoll(a, (end-a), b, (end-a)))
	    return (keyseg->base.flag & HA_REVERSE_SORT) ? -flag : flag;
	  b+= (uint) (end-a);
	  a=end;
	}
...
uint _ni_get_key(register N_KEYDEF *keyinfo, uint nod_flag,
		 register uchar **page, register uchar *key)
{
  ...
  *page+=length;
  return((uint) (key-start_key)+keyseg->base.length);
	...
	/* same as _ni_get_key but used with fixed length keys */

uint _ni_get_static_key(register N_KEYDEF *keyinfo, uint nod_flag, register uchar **page, register uchar *key)
{
...
uchar *_ni_get_last_key(N_INFO *info, N_KEYDEF *keyinfo, uchar *keypos, uchar *lastkey, uchar *endpos)
{
  uint nod_flag;
  uchar *lastpos;
  ...
  else
  {
    ...
    {
      lastpos=keypos;
      VOID(_ni_get_key(keyinfo,nod_flag,&keypos,lastkey));
    }
  }
  return lastpos;
} /* _ni_get_last_key */
...
uint _ni_keylength(N_KEYDEF *keyinfo, register uchar *key)
{
  reg1 N_KEYSEG *keyseg;
  uchar *start;
...
uchar *_ni_move_key(N_KEYDEF *keyinfo, uchar *to, uchar *from)
{
  reg1 uint length;
  memcpy((byte*) to, (byte*) from,
	 (size_t) (length=_ni_keylength(keyinfo,from)));
  return to+length;
}
...
int _ni_search_next(register N_INFO *info, register N_KEYDEF *keyinfo,
		    uchar *key, uint nextflag, ulong pos)
{
  int error;
  uint nod_flag;
  uchar lastkey[N_MAX_KEY_BUFF];
  DBUG_ENTER("_ni_search_next");
  ...
  if ((nextflag & SEARCH_BIGGER && info->int_keypos >= info->int_maxpos) ||
    ...
    DBUG_RETURN(_ni_search(info,keyinfo,key,0,nextflag | SEARCH_SAVE_BUFF,
			   pos));

  if (!(info->update & HA_STATE_BUFF_SAVED))
  {
    if (!_ni_fetch_keypage(info,keyinfo,info->int_pos,info->buff,0))
    {
      info->lastpos= NI_POS_ERROR;
      DBUG_RETURN(-1);
    }
  ...
  VOID(_ni_move_key(keyinfo,lastkey,key));

  if (nextflag & SEARCH_BIGGER)					/* Next key */
  {
    ulong pos=_ni_kpos(nod_flag,info->int_keypos);
    if (pos != NI_POS_ERROR)
    {
      if ((error=_ni_search(info,keyinfo,key,0,nextflag | SEARCH_SAVE_BUFF,
			    pos)) <=0)
	DBUG_RETURN(error);
    }
  ...
  {
    info->int_keypos=_ni_get_last_key(info,keyinfo,info->buff,lastkey,
				      info->int_keypos);
    if (info->int_keypos == info->buff+2)
      DBUG_RETURN(_ni_search(info,keyinfo,key,0,nextflag | SEARCH_SAVE_BUFF,
			     pos));
    if ((error=_ni_search(info,keyinfo,key,0,nextflag | SEARCH_SAVE_BUFF,
			  _ni_kpos(nod_flag,info->int_keypos))) <= 0)
      DBUG_RETURN(error);
  }
  ...
  info->lastpos=_ni_dpos(info,nod_flag,info->int_keypos);
...
int _ni_search_first(register N_INFO *info, register N_KEYDEF *keyinfo, register ulong pos)
{
  uint nod_flag;
  uchar *page;
  DBUG_ENTER("_ni_search_first");
  ...
  do
  {
    if (!_ni_fetch_keypage(info,keyinfo,pos,info->buff,0))
    {
      info->lastpos= NI_POS_ERROR;
      DBUG_RETURN(-1);
    }
...
int _ni_search_last(register N_INFO *info, register N_KEYDEF *keyinfo, register ulong pos)
{
  uint nod_flag;
  uchar *buff,*page;
  DBUG_ENTER("_ni_search_last");
  ...
  do
  {
    if (!_ni_fetch_keypage(info,keyinfo,pos,buff,0))
    {
      info->lastpos= NI_POS_ERROR;
      DBUG_RETURN(-1);
    }
  ...
  VOID(_ni_get_last_key(info,keyinfo,buff,info->lastkey,page));
  info->lastpos=_ni_dpos(info,nod_flag,page);
