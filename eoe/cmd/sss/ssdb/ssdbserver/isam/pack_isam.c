...
static int compare_huff_elements(byte *a,byte *b);
...
static my_string load_default_groups[]= { "pack_isam",0 };
...
{
  printf("%s  Ver 5.6 for %s on %s\n",my_progname,SYSTEM_TYPE,MACHINE_TYPE);
}

static void usage(void)
{
  print_version();
  puts("Copyright (C) 1994-1998 TcX AB & Monty Program KB & Detron HB.");
...
static N_INFO *open_isam_file(char *name,int mode)
{
  ...
  if (!(isam_file=ni_open(name,mode,(opt_wait ? 1 : 0))))
  {
  ...
  if (! force_pack && share->state.records != 0 &&
      (share->state.records <= 1 ||
       share->state.data_file_length < 1024))
  {
    VOID(fprintf(stderr,"%s is too small to compress\n",name));
    VOID(ni_close(isam_file));
    DBUG_RETURN(0);
  }
  VOID(ni_lock_database(isam_file,F_WRLCK));
...
static int compress(MRG_INFO *mrg,char *result_table)
{
  ...
  if (init_queue(&queue,256,0,0,compare_huff_elements))
    goto err;
  ...
  if (!test_only)
  {
    ...
    if (!result_table)
    {
      ...
      isam_file->dfile= -1;		/* Tell ni_close file is closed */
    }
  }
  ...
  {
    if (old_length)
      printf("%.4g%%     \n",((double) old_length-(double) new_length)*100.0/
	     old_length);
    else
...
static HUFF_COUNTS *init_huff_count(N_INFO *info,my_off_t records)
{
					...
					MYF(MY_ZEROFILL | MY_WME))))
  {
	...
	count[i].max_zero_fill= count[i].field_length;
      init_tree(&count[i].int_tree,-1,(qsort_cmp) compare_tree,NULL);
      if (records)
...
static int make_huff_tree(HUFF_TREE *huff_tree, HUFF_COUNTS *huff_counts)
{
  ...
  {
    delete_queue(&queue);
    if (init_queue(&queue,found,0,0,compare_huff_elements))
      return -1;
  }
		       ...
		       ulong crc)
{
  ...
  share->state.data_file_length=new_length;
  share->state.del=share->state.empty=0;
  share->state.dellink= (ulong) HA_POS_ERROR;
  share->state.splitt=mrg->records;
  share->state.version=(ulong) time((time_t*) 0);
  share->state.keys=0;
  share->state.key_file_length=share->base.keystart;

  isam_file->update|=HA_STATE_CHANGED;
			  ...
			  ulong crc)
{
  ...
  state.data_file_length=new_length;
  state.del=state.empty=0;
  state.dellink= (ulong) HA_POS_ERROR;
  state.records=state.splitt=mrg->records;
...
static void mrg_reset(MRG_INFO *mrg)
{
  if (mrg->current)
  {
    ni_extra(*mrg->current,HA_EXTRA_NO_CACHE);
    mrg->current=0;
  }
}
...
static int mrg_rrnd(MRG_INFO *info,byte *buf)
{
  ...
  if (!info->current)
  {
    ...
    ni_extra(isam_info,HA_EXTRA_RESET);
  ...
  for (;;)
  {
	...
	my_errno != HA_ERR_END_OF_FILE)
      return (error);
    ni_extra(isam_info,HA_EXTRA_NO_CACHE);
