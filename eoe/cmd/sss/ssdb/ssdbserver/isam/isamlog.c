...
static int examine_log(my_string file_name, char **table_names)
{
  ...
  init_tree(&tree,sizeof(file_info),(qsort_cmp) file_info_compare,
	 ...
	 !my_b_read(&cache,(byte*) head,9))
  {
	...
	if (!(file_info.isam= ni_open(isam_file_name,O_RDWR,1)))
	  goto com_err;
	...
	goto err;
      memcpy(&extra_command,head,sizeof(extra_command));
      if (verbose && !record_pos_file &&
	  (!table_names[0] || curr_file_info && curr_file_info->used))
	printf_log("%s: %s(%d) -> %d",FILENAME(curr_file_info),
      ...
      {
	if (ni_extra(curr_file_info->isam,extra_command) != (int) result)
	{
	  VOID(fprintf(stderr,
	...
	goto err;
      memcpy(&filepos,head,sizeof(filepos));
      if (verbose && (!record_pos_file ||
	...
	printf_log("%s: %s at %ld -> %d",FILENAME(curr_file_info),
      ...
      if (update && curr_file_info && !curr_file_info->closed)
      {
	if (ni_rrnd(curr_file_info->isam,curr_file_info->record,filepos))
	{
	  if (!recover)
	    goto com_err;
	  com_count[command][2]++;		/* Mark error */
	}
	ni_result=ni_delete(curr_file_info->isam,curr_file_info->record);
	if (ni_result == 0 && result || ni_result && (uint) my_errno != result)
	{
	  if (!recover)
	    goto com_err;
	  if (ni_result)
	...
	if ((enum nisam_log_commands) command == LOG_UPDATE)
	{
	  ...
	  ni_result=ni_update(curr_file_info->isam,curr_file_info->record,
			      buff);
	...
	{
	  ni_result=ni_write(curr_file_info->isam,buff);
	  if ((ni_result == 0 && result) || ni_result &&
	      (uint) my_errno != result)
	  {
	    if (!recover)
	      goto com_err;
	    if (ni_result)
	...
	printf_log("%s: %s(%d) -> %d\n",FILENAME(curr_file_info),
      ...
      if (update && curr_file_info && !curr_file_info->closed)
      {
	if (ni_lock_database(curr_file_info->isam,lock_command) !=
	    (int) result)
	  goto com_err;
      }
      break;
...
uint maximum_files;
{
  ...
  if (!getrlimit(RLIMIT_NOFILE,&rlimit))
  {
    ...
    if (maximum_files && maximum_files > old_max)
      rlimit.rlim_max=maximum_files;
    rlimit.rlim_cur=rlimit.rlim_max;
    if (setrlimit(RLIMIT_NOFILE,&rlimit))
    {
      if (old_max != maximum_files)
...
TREE *tree;
{
  ...
  if (ni_close(access_param.found->isam))
    return 1;
...
struct file_info *fileinfo;
{
  ...
  if (!(fileinfo->isam= ni_open(name,O_RDWR,1)))
    return 1;
  fileinfo->closed=0;
  re_open_count++;
  return 0;
...
byte *record;
{
  ...
  for (key=0 ; key < info->s->state.keys ; key++)
  {
    ...
    {
      VOID(_ni_make_key(info,key,tmp_key,record,0L));
      return ni_rkey(info,file_info->record,(int) key,(char*) tmp_key,0,
		     HA_READ_KEY_EXACT);
    }
  }
  return 1;
