...
typedef struct st_isam_share {		/* Shared between opens */
  ...
  int (*read_record)(struct st_isam_info*, ulong, char*);
  int (*write_record)(struct st_isam_info*, const char*);
  int (*update_record)(struct st_isam_info*, ulong, const char*);
  int (*delete_record)(struct st_isam_info*);
  int (*read_rnd)(struct st_isam_info*, char*, ulong, int);
  int (*compare_record)(struct st_isam_info*, const char *);
#ifdef THREAD
  THR_LOCK lock;
...
#define USE_HOLE_KEY	0		/* Use hole key in _ni_search() */
...
extern File nisam_log_file;
	...
	/* This is used by _ni_get_pack_key_length och _ni_store_key */
...
extern int _ni_read_dynamic_record(N_INFO *info,ulong filepos,byte *buf);
extern int _ni_write_dynamic_record(N_INFO*, const byte*);
extern int _ni_update_dynamic_record(N_INFO*, ulong, const byte*);
extern int _ni_delete_dynamic_record(N_INFO *info);
extern int _ni_cmp_dynamic_record(N_INFO *info,const byte *record);
extern int _ni_read_rnd_dynamic_record(N_INFO *, byte *,ulong, int);
extern int _ni_write_blob_record(N_INFO*, const byte*);
extern int _ni_update_blob_record(N_INFO*, ulong, const byte*);
extern int _ni_read_static_record(N_INFO *info,ulong filepos,byte *buf);
extern int _ni_write_static_record(N_INFO*, const byte*);
extern int _ni_update_static_record(N_INFO*, ulong, const byte*);
extern int _ni_delete_static_record(N_INFO *info);
extern int _ni_cmp_static_record(N_INFO *info,const byte *record);
extern int _ni_read_rnd_static_record(N_INFO*, byte *,ulong, int);
extern int _ni_ck_write(N_INFO *info,uint keynr,uchar *key);
extern int _ni_enlarge_root(N_INFO *info,uint keynr,uchar *key);
extern int _ni_insert(N_INFO *info,N_KEYDEF *keyinfo,uchar *key,
		      uchar *anc_buff,uchar *key_pos,uchar *key_buff,
		      uchar *father_buff, uchar *father_keypos,
		      ulong father_page);
extern int _ni_splitt_page(N_INFO *info,N_KEYDEF *keyinfo,uchar *key,
			   uchar *buff,uchar *key_buff);
extern uchar *_ni_find_half_pos(N_INFO *info,N_KEYDEF *keyinfo,uchar *page,
				uchar *key);
extern uint _ni_get_pack_key_length(N_KEYDEF *keyinfo,uint nod_flag,
				    uchar *key_pos,uchar *key_buff,
				    uchar *key, S_PARAM *s_temp);
extern void _ni_store_key(N_KEYDEF *keyinfo,uchar *key_pos,
			  S_PARAM *s_temp);
extern int _ni_ck_delete(N_INFO *info,uint keynr,uchar *key);
extern int _ni_readinfo(N_INFO *info,int lock_flag,int check_keybuffer);
extern int _ni_writeinfo(N_INFO *info,pbool update_keyfile);
extern int _ni_test_if_changed(N_INFO *info);
extern int _ni_check_index(N_INFO *info,int inx);
extern int _ni_search(N_INFO *info,N_KEYDEF *keyinfo,uchar *key,uint key_len,uint nextflag,ulong pos);
extern int _ni_bin_search(struct st_isam_info *info,N_KEYDEF *keyinfo,uchar *page,uchar *key,uint key_len,uint comp_flag,uchar * *ret_pos,uchar *buff);
extern int _ni_seq_search(N_INFO *info,N_KEYDEF *keyinfo,uchar *page,uchar *key,uint key_len,uint comp_flag,uchar * *ret_pos,uchar *buff);
extern ulong _ni_kpos(uint nod_flag,uchar *after_key);
extern void _ni_kpointer(N_INFO *info,uchar *buff,ulong pos);
extern ulong _ni_dpos(N_INFO *info, uint nod_flag,uchar *after_key);
extern void _ni_dpointer(N_INFO *info, uchar *buff,ulong pos);
extern int _ni_key_cmp(N_KEYSEG *keyseg,uchar *a,uchar *b,
		       uint key_length,uint nextflag);
extern uint _ni_get_key(N_KEYDEF *keyinfo,uint nod_flag,uchar * *page,uchar *key);
extern uint _ni_get_static_key(N_KEYDEF *keyinfo,uint nod_flag,uchar * *page,uchar *key);
extern uchar *_ni_get_last_key(N_INFO *info,N_KEYDEF *keyinfo,uchar *keypos,uchar *lastkey,uchar *endpos);
extern uint _ni_keylength(N_KEYDEF *keyinfo,uchar *key);
extern uchar *_ni_move_key(N_KEYDEF *keyinfo,uchar *to,uchar *from);
extern int _ni_search_next(N_INFO *info,N_KEYDEF *keyinfo,uchar *key,uint nextflag,ulong pos);
extern int _ni_search_first(N_INFO *info,N_KEYDEF *keyinfo,ulong pos);
extern int _ni_search_last(N_INFO *info,N_KEYDEF *keyinfo,ulong pos);
extern uchar *_ni_fetch_keypage(N_INFO *info,N_KEYDEF *keyinfo,my_off_t page,
				uchar *buff,int return_buffer);
extern int _ni_write_keypage(N_INFO *info,N_KEYDEF *keyinfo,my_off_t page,
			     uchar *buff);
extern int _ni_dispose(N_INFO *info,N_KEYDEF *keyinfo,my_off_t pos);
extern ulong _ni_new(N_INFO *info,N_KEYDEF *keyinfo);
extern uint _ni_make_key(N_INFO *info,uint keynr,uchar *key,
			 const char *record,ulong filepos);
extern uint _ni_pack_key(N_INFO *info,uint keynr,uchar *key,uchar *old,uint key_length);
extern int _ni_read_key_record(N_INFO *info,ulong filepos,byte *buf);
extern int _ni_read_cache(IO_CACHE *info,byte *buff,ulong pos,
			  uint length,int re_read_if_possibly);
extern byte *fix_rec_buff_for_blob(N_INFO *info,uint blob_length);
extern uint _ni_rec_unpack(N_INFO *info,byte *to,byte *from,
			   uint reclength);
my_bool _ni_rec_check(N_INFO *info,const char *from);
extern int _ni_write_part_record(N_INFO *info,ulong filepos,uint length,
				 ulong next_filepos,byte **record,
				 uint *reclength,int *flag);
extern void _ni_print_key(FILE *stream,N_KEYSEG *keyseg,const uchar *key);
extern my_bool _ni_read_pack_info(N_INFO *info,pbool fix_keys);
extern int _ni_read_pack_record(N_INFO *info,ulong filepos,byte *buf);
extern int _ni_read_rnd_pack_record(N_INFO*, byte *,ulong, int);
extern int _ni_pack_rec_unpack(N_INFO *info,byte *to,byte *from,
			       uint reclength);
...
typedef struct st_block_info {	/* Parameter to _ni_get_block_info */
  ...
  uint second_read;
	...
	/* bits in return from _ni_get_block_info */
...
#define nisam_log(a,b,c,d) if (nisam_log_file >= 0) _nisam_log(a,b,c,d)
...
extern uint _ni_get_block_info(BLOCK_INFO *,File, ulong);
extern uint _ni_rec_pack(N_INFO *info,byte *to,const byte *from);
extern uint _ni_pack_get_block_info(BLOCK_INFO *, uint, File, ulong);
...
extern void _nisam_log_record(enum nisam_log_commands command,N_INFO *info,
			      ...
			      int result);
extern ulong checksum(byte *mem,uint count);
extern byte *_ni_memmap_file(N_INFO *info);
extern void _ni_unmap_file(N_INFO *info);
