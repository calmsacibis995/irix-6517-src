...
static int write_dynamic_record(N_INFO *info,const byte *record,
				uint reclength);
static int _ni_find_writepos(N_INFO *info,uint reclength,ulong *filepos,
			     uint *length);
...
static int delete_dynamic_record(N_INFO *info,ulong filepos,
				 uint second_read);
static int _ni_cmp_buffer(File file, const byte *buff, ulong filepos,
			  uint length);
...
int _ni_write_dynamic_record(N_INFO *info, const byte *record)
{
  uint reclength=_ni_rec_pack(info,info->rec_buff,record);
  return (write_dynamic_record(info,info->rec_buff,reclength));
}

int _ni_update_dynamic_record(N_INFO *info, ulong pos, const byte *record)
{
  uint length=_ni_rec_pack(info,info->rec_buff,record);
  return (update_dynamic_record(info,pos,info->rec_buff,length));
}

int _ni_write_blob_record(N_INFO *info, const byte *record)
{
  byte *rec_buff;
  int error;
  ...
  if (!(rec_buff=(byte*) my_alloca(info->s->base.pack_reclength+
				    ...
				    extra)))
    return(-1);
  reclength=_ni_rec_pack(info,rec_buff+ALIGN_SIZE(MAX_DYN_BLOCK_HEADER),
			 record);
...
int _ni_update_blob_record(N_INFO *info, ulong pos, const byte *record)
{
  byte *rec_buff;
  int error;
  uint reclength,extra;
...
int _ni_delete_dynamic_record(N_INFO *info)
{
...
static int write_dynamic_record(N_INFO *info, const byte *record, uint reclength)
{
  ...
  while (reclength)
  {
    if (_ni_find_writepos(info,reclength,&filepos,&length))
      goto err;
    if (_ni_write_part_record(info,filepos,length,info->s->state.dellink,
			     ...
			     uint *length)   /* length of block at filepos */
{
  ...
  if (info->s->state.dellink != NI_POS_ERROR)
  {
    *filepos=info->s->state.dellink;
    ...
    if (!(_ni_get_block_info(&block_info,info->dfile,info->s->state.dellink) &
	   BLOCK_DELETED))
    {
...
int _ni_write_part_record(N_INFO *info,
			  ...
			  int *flag)		/* *flag == 0 if header */
{
  ...
  DBUG_ENTER("_ni_write_part_record");
...
static int update_dynamic_record(N_INFO *info, ulong filepos, byte *record, uint reclength)
{
  ...
  while (reclength > 0)
  {
    if (filepos != info->s->state.dellink)
    {
      ...
      if ((error=_ni_get_block_info(&block_info,info->dfile,filepos))
    ...
    if (_ni_write_part_record(info,filepos,length,block_info.next_filepos,
...
static int delete_dynamic_record(N_INFO *info, ulong filepos, uint second_read)
{
  ...
  do
  {
    if ((b_type=_ni_get_block_info(&block_info,info->dfile,filepos))
	& (BLOCK_DELETED | BLOCK_ERROR | BLOCK_SYNC_ERROR |
	   BLOCK_FATAL_ERROR) || block_info.block_len < 8)
    {
      my_errno=HA_ERR_WRONG_IN_RECORD;
      DBUG_RETURN(1);
    }
...
uint _ni_rec_pack(N_INFO *info, register byte *to, register const byte *from)
{
  ...
  for (i=info->s->base.fields ; i-- > 0; from+= (rec++)->base.length)
  {
	...
	else
	{
	  ...
	  memcpy(&temp_pos,from+length,sizeof(char*));
...
uint _ni_rec_unpack(register N_INFO *info, register byte *to, byte *from,
		    uint found_length)
{
       ...
       rec < end_field ; to+= rec_length, rec++)
  {
	...
	from+=rec_length;
	memcpy((byte*) to+rec_length,(byte*) &from,sizeof(char*));
	from+=blob_length;
	to+=sizeof(char*);
      }
      else
...
int _ni_read_dynamic_record(N_INFO *info, ulong filepos, byte *buf)
{
  int flag;
  ...
  if (filepos != NI_POS_ERROR)
  {
	...
	goto err;
      info->rec_cache.seek_not_done=1;
      if ((b_type=_ni_get_block_info(&block_info,file,
    ...
    DBUG_RETURN(_ni_rec_unpack(info,buf,info->rec_buff,block_info.rec_len) !=
		MY_FILE_ERROR ? 0 : -1);
  }
  VOID(_ni_writeinfo(info,0));
...
int _ni_cmp_dynamic_record(register N_INFO *info, register const byte *record)
{
  uint flag,reclength,b_type;
  ulong filepos;
  byte *buffer;
  BLOCK_INFO block_info;
  DBUG_ENTER("_ni_cmp_dynamic_record");
  ...
  {						/* If check isn't disabled  */
    ...
    reclength=_ni_rec_pack(info,buffer,record);
    record= buffer;
    ...
    while (reclength > 0)
    {
      if ((b_type=_ni_get_block_info(&block_info,info->dfile,
				    block_info.next_filepos))
	  & (BLOCK_DELETED | BLOCK_ERROR | BLOCK_SYNC_ERROR |
	     BLOCK_FATAL_ERROR))
      {
	...
	goto err;
      ...
      if (_ni_cmp_buffer(info->dfile,record,block_info.filepos,
			 block_info.data_len))
      {
	my_errno=HA_ERR_RECORD_CHANGED;
	goto err;
...
static int _ni_cmp_buffer(File file, const byte *buff, ulong filepos, uint length)
{
  uint next_length;
  char temp_buff[IO_SIZE*2];
  DBUG_ENTER("_ni_cmp_buffer");
...
int _ni_read_rnd_dynamic_record(N_INFO *info, byte *buf, register ulong filepos, int skipp_deleted_blocks)
{
  ...
  DBUG_ENTER("_ni_read_rnd_dynamic_record");
  ...
  do
  {
    ...
    {
      if (_ni_read_cache(&info->rec_cache,(byte*) block_info.header,filepos,
			sizeof(block_info.header),
			test(!flag && skipp_deleted_blocks) | 2))
	goto err;
      b_type=_ni_get_block_info(&block_info,-1,filepos);
    }
    else
    {
      if (info->opt_flag & WRITE_CACHE_USED &&
	...
	DBUG_RETURN(-1);
      info->rec_cache.seek_not_done=1;
      b_type=_ni_get_block_info(&block_info,info->dfile,filepos);
    }
    ...
    if (info->opt_flag & READ_CACHE_USED)
    {
      if (_ni_read_cache(&info->rec_cache,(byte*) to,block_info.filepos,
			block_info.data_len,
  ...
  VOID(_ni_writeinfo(info,0));
  if (_ni_rec_unpack(info,buf,info->rec_buff,block_info.rec_len) !=
      MY_FILE_ERROR)
    DBUG_RETURN(0);
...
uint _ni_get_block_info(BLOCK_INFO *info, File file, ulong filepos)
{
  ...
  case 0:
    info->block_len=(uint) uint3korr(header+1);
