...
#ifdef HAVE_MMAP
static void _ni_mempack_get_block_info(BLOCK_INFO *info,uint ref_length,
				       uchar *header);
#endif
...
my_bool _ni_read_pack_info(N_INFO *info, pbool fix_keys)
{
  File file;
  int diff_length;
  ...
  if (my_read(file,disk_cache,(uint) (share->pack.header_length-sizeof(header)),
	      MYF(MY_NABP)))
  {
    my_free((gptr) share->decode_trees,MYF(0));
	       ...
	       MYF(MY_HOLD_ON_ERROR));
  {
    ...
    for (i=0 ; i < trees ; i++)
      share->decode_trees[i].table=ADD_TO_PTR(share->decode_trees[i].table,diff,
					     uint16*);
  }
...
static void read_huff_table(BIT_BUFF *bit_buff, DECODE_TREE *decode_tree, uint16 **decode_table, byte **intervall_buff, uint16 *tmp_buff)
{
...
int _ni_read_pack_record(N_INFO *info, ulong filepos, byte *buf)
{
  BLOCK_INFO block_info;
  File file;
  DBUG_ENTER("ni_read_pack_record");

  if (filepos == NI_POS_ERROR)
    DBUG_RETURN(-1);			/* _search() didn't find record */

  file=info->dfile;
  if (_ni_pack_get_block_info(&block_info,info->s->pack.ref_length,file,
			      filepos))
    goto err;
  if (my_read(file,(byte*) info->rec_buff,block_info.rec_len,MYF(MY_NABP)))
    goto panic;
  info->update|= HA_STATE_AKTIV;
  DBUG_RETURN(_ni_pack_rec_unpack(info,buf,info->rec_buff,block_info.rec_len));
panic:
  my_errno=HA_ERR_WRONG_IN_RECORD;
err:
  DBUG_RETURN(-1);
...
int _ni_pack_rec_unpack(register N_INFO *info, register byte *to, byte *from, uint reclength)
{
  byte *end_field;
  reg3 N_RECINFO *end;
  N_RECINFO *current_field;
  ISAM_SHARE *share=info->s;
  DBUG_ENTER("_ni_pack_rec_unpack");
...
static void uf_endspace(N_RECINFO *rec, BIT_BUFF *bit_buff, uchar *to, uchar *end)
{
  uint spaces;
  if ((spaces=get_bits(bit_buff,rec->space_length_bits))+to > end)
  {
...
static void uf_constant(N_RECINFO *rec, BIT_BUFF *bit_buff, uchar *to, uchar *end)
{
...
/*ARGSUSED*/
static void uf_zero(N_RECINFO *rec, BIT_BUFF *bit_buff, uchar *to, uchar *end)
{
  bzero((char*) to,(uint) (end-to));
}
...
int _ni_read_rnd_pack_record(N_INFO *info, byte *buf, register ulong filepos, int skipp_deleted_blocks)
{
  uint b_type;
  BLOCK_INFO block_info;
  ISAM_SHARE *share=info->s;
  DBUG_ENTER("_ni_read_rnd_pack_record");
  ...
  if (info->opt_flag & READ_CACHE_USED)
  {
    if (_ni_read_cache(&info->rec_cache,(byte*) block_info.header,filepos,
      ...
      goto err;
    b_type=_ni_pack_get_block_info(&block_info,share->pack.ref_length,-1,
				   filepos);
  }
  else
    b_type=_ni_pack_get_block_info(&block_info,share->pack.ref_length,
				   info->dfile,filepos);
  if (b_type)
    goto err;
#ifndef DBUG_OFF
  ...
  if (info->opt_flag & READ_CACHE_USED)
  {
    if (_ni_read_cache(&info->rec_cache,(byte*) info->rec_buff,
  ...
  DBUG_RETURN (_ni_pack_rec_unpack(info,buf,info->rec_buff,
				   block_info.rec_len));
 err:
  DBUG_RETURN(-1);
}
...
uint _ni_pack_get_block_info(BLOCK_INFO *info, uint ref_length, File file, ulong filepos)
{
...
static int _ni_read_mempack_record(N_INFO *info,ulong filepos,byte *buf);
static int _ni_read_rnd_mempack_record(N_INFO*, byte *,ulong, int);
...
byte *_ni_memmap_file(N_INFO *info)
{
  byte *file_map;
  ISAM_SHARE *share=info->s;
  DBUG_ENTER("ni_memmap_file");
  ...
  if (file_map != (byte*) MAP_FAILED)
  {
    ...
    info->read_record=share->read_record=_ni_read_mempack_record;
    share->read_rnd=_ni_read_rnd_mempack_record;
  }
  else
  {
  ...
  DBUG_RETURN(file_map);
}


void _ni_unmap_file(N_INFO *info)
{
...
static int _ni_read_mempack_record(N_INFO *info, ulong filepos, byte *buf)
{
  ...
  _ni_mempack_get_block_info(&block_info,share->pack.ref_length,
			     (uchar*) share->file_map+filepos);
  DBUG_RETURN(_ni_pack_rec_unpack(info,buf,share->file_map+
...
/*ARGSUSED*/
static int _ni_read_rnd_mempack_record(N_INFO *info, byte *buf, register ulong filepos, int skipp_deleted_blocks)
{
  BLOCK_INFO block_info;
  ISAM_SHARE *share=info->s;
  DBUG_ENTER("_ni_read_rnd_mempack_record");
