...
my_string	tempname;
{
  ...
  qsort((byte*) sort_keys,count,sizeof(byte*),(qsort_cmp) info->key_cmp);
...
reg2 uint count;
{
  ...
  qsort((gptr) sort_keys,(size_t) count,sizeof(byte*),
	(qsort_cmp) info->key_cmp);
  while (count--)
...
static void closetemp(name,stream)
char *name;
FILE *stream;
{
