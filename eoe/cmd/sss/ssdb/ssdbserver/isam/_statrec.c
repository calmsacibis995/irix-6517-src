...
int _ni_write_static_record(N_INFO *info, const byte *record)
{
...
int _ni_update_static_record(N_INFO *info, ulong pos, const byte *record)
{
  info->rec_cache.seek_not_done=1;		/* We have done a seek */
...
int _ni_delete_static_record(N_INFO *info)
{
...
int _ni_cmp_static_record(register N_INFO *info, register const byte *old)
{
  DBUG_ENTER("_ni_rectest");
...
int _ni_read_static_record(register N_INFO *info, register ulong pos,
			   register byte *record)
{
  ...
  VOID(_ni_writeinfo(info,0));	/* No such record */
  return(-1);
} /* _ni_read_record */


int _ni_read_rnd_static_record(N_INFO *info, byte *buf, register ulong filepos,
			       int skipp_deleted_blocks)
{
  int locked,error,cache_read;
  uint cache_length;
  ISAM_SHARE *share=info->s;
  DBUG_ENTER("_ni_read_rnd_static_record");
  ...
  if (info->lock_type == F_UNLCK)
  {
    ...
    {						/* Test if new records */
      if (_ni_readinfo(info,F_RDLCK,0))
	DBUG_RETURN(-1);
      locked=1;
    }
    else
  ...
  {
    error=_ni_read_static_record(info,filepos,buf);
    if (error > 0)
      my_errno=HA_ERR_RECORD_DELETED;
    DBUG_RETURN(error);
  }
...
#ifndef NO_LOCKING
  if (locked)
    VOID(_ni_writeinfo(info,0));		/* Unlock keyfile */
#endif
  if (!error)
  {
    if (!buf[0])
