...
static my_string type_names[]={ "?","text","binary", "short", "long", "float",
...
static char temp_filename[FN_REFLEN],*isam_file_name;
static IO_CACHE read_cache;
static SORT_INFO sort_info;
static int tmpfile_createflag=O_RDWR | O_TRUNC | O_EXCL;

static my_string load_default_groups[]= { "isamchk",0 };

	/* Functions defined in this file */

extern int main(int argc,char * *argv);
extern void print_error _VARARGS((char *fmt,...));
static void print_warning _VARARGS((char *fmt,...));
static void print_info _VARARGS((char *fmt,...));
...
static int sort_one_index(N_INFO *info,N_KEYDEF *keyinfo,uchar *buff,
			  File new_file);
static int change_to_newfile(my_string filename,my_string old_ext,
			     my_string new_ext);
static int lock_file(File file,ulong start,int lock_type,my_string filetype,
		     my_string filename);
...
static CHANGEABLE_VAR changeable_vars[] = {
  { "keybuffer",(long*) &use_buffers,(long) USE_BUFFER_INIT,
    (long) MALLOC_OVERHEAD, (long) ~0L,(long) MALLOC_OVERHEAD,(long) IO_SIZE },
  { "readbuffer", (long*) &read_buffer_length,(long) READ_BUFFER_INIT,
      (long) MALLOC_OVERHEAD,(long) ~0L,(long) MALLOC_OVERHEAD,(long) 1L },
  { "writebuffer", (long*) &write_buffer_length,(long) READ_BUFFER_INIT,
      (long) MALLOC_OVERHEAD,(long) ~0L,(long) MALLOC_OVERHEAD,(long) 1L },
  { "sortbuffer",(long*) &sort_buffer_length,(long) SORT_BUFFER_INIT,
...
static void usage(void)
{
  ...
  printf("Possible variables for option --set-variable (-O) are:\n");
...
static int nisamchk(my_string filename)
{
  ...
  else
  {
    ...
    if (_ni_readinfo(info,lock_type,0))
    {
  ...
  if (!(testflag & T_DESCRIPT))
  {
    if (info->update & HA_STATE_CHANGED)
...
static void get_options(register int *argc,register char ***argv)
{
  ...
  while ((c=getopt_long(*argc,*argv,"adeif?lqrosSuvVw#:b:k:O:R:",
...
register N_INFO *info;
{
  ...
  if (skr != size)
  {
    info->s->state.data_file_length=size;	/* Skipp other errors */
...
register N_INFO *info;
{
       ...
       key++,keyinfo++)
  {
	...
	share->state.records == 0)
      continue;
    if (!_ni_fetch_keypage(info,keyinfo,share->state.key_root[key],info->buff,
			   0))
    {
...
uint level;
{
  ...
  for ( ;; )
  {
    if (nod_flag)
    {
      next_page=_ni_kpos(nod_flag,keypos);
	...
	if (next_page+info->s->blocksize > max_length)
	  goto err;
	info->s->state.key_file_length=(max_length &
					~ (my_off_t) (info->s->blocksize-1));
      }
      if (!_ni_fetch_keypage(info,keyinfo,next_page,temp_buff,0))
      {
	...
	(*keyinfo->get_key)(keyinfo,nod_flag,&keypos,key) == 0)
      break;
    if ((*keys)++ &&
	(flag=_ni_key_cmp(keyinfo->seg,info->lastkey,key,0,comp_flag)) >=0)
    {
      DBUG_DUMP("old",(byte*) info->lastkey,
		_ni_keylength(keyinfo,info->lastkey));
      DBUG_DUMP("new",(byte*) key,_ni_keylength(keyinfo,key));
    ...
    {
      if (*keys == 1L ||
	  _ni_key_cmp(keyinfo->seg,info->lastkey,key,0,SEARCH_FIND))
	unique_count++;
    }
    VOID(_ni_move_key(keyinfo,(uchar*) info->lastkey,key));
    record= _ni_dpos(info,nod_flag,keypos);
...
int extend;
{
  ...
  while (pos < info->s->state.data_file_length)
  {
    ...
    case DYNAMIC_RECORD:
      ...
      do
      {
	if (_ni_read_cache(&read_cache,(byte*) block_info.header,
			  (start_block=block_info.next_filepos),
			  sizeof(block_info.header),test(! flag) | 2))
	  goto err;
	b_type=_ni_get_block_info(&block_info,-1,start_block);
	...
	if (_ni_read_cache(&read_cache,(byte*) to,block_info.filepos,
	...
	if (left_length)
	{
      ...
      if (! got_error)
      {
	if (_ni_rec_unpack(info,record,info->rec_buff,block_info.rec_len) ==
	    MY_FILE_ERROR)
	{
    ...
    case COMPRESSED_RECORD:
      if (_ni_read_cache(&read_cache,(byte*) block_info.header,pos, 3,1))
	goto err;
      start_recpos=pos;
      splitts++;
      VOID(_ni_pack_get_block_info(&block_info,info->s->pack.ref_length,-1,
				   start_recpos));
	...
	break;
      ...
      if (_ni_read_cache(&read_cache,(byte*) info->rec_buff,
			block_info.filepos, block_info.rec_len,1))
	goto err;
      if (_ni_pack_rec_unpack(info,record,info->rec_buff,block_info.rec_len))
      {
	print_error("Found wrong record at %lu",(ulong) start_recpos);
	got_error=1;
      }
	...
	for (key=0,keyinfo= info->s->keyinfo; key<info->s->state.keys;
	     key++,keyinfo++)
	{
	  VOID(_ni_make_key(info,key,info->lastkey,record,start_recpos));
	  if (_ni_search(info,keyinfo,info->lastkey,0,SEARCH_SAME,
...
my_string name;
{
  ...
  info->update= (short) HA_STATE_CHANGED;
  ...
  while (!(error=sort_get_next_record()))
  {
    if (writekeys(info,(byte*) sort_info.record,sort_info.filepos))
    {
      ...
      if (testflag & T_VERBOSE)
      {
	VOID(_ni_make_key(info,(uint) info->errkey,info->lastkey,
		     sort_info.record,0L));
	_ni_print_key(stdout,share->keyinfo[info->errkey].seg,info->lastkey);
      }
      sort_info.dupp++;
      if (rep_quick == 1)
      {
...
static int writekeys(register N_INFO *info,byte *buff,ulong filepos)
{
  ...
  {
    VOID(_ni_make_key(info,i,key,buff,filepos));
    if (_ni_ck_write(info,i,key)) goto err;
  }
  DBUG_RETURN(0);
 ...
 err:
  if (my_errno == HA_ERR_FOUND_DUPP_KEY)
  {
    ...
    {
      VOID(_ni_make_key(info,i,key,buff,filepos));
      if (_ni_ck_delete(info,i,key)) break;
    }
  }
...
my_string name;
{
  ...
  if (testflag & T_VERBOSE)
  {
    ...
    printf("Datafile pointer (bytes):%7d  Keyfile pointer (bytes):%6d\n",
...
uint prot_key;
{
  ...
  for (i=0 ; i < info->s->state.keys; i++)
  {
    if (i != prot_key)
    {
      VOID(_ni_make_key(info,i,key,record,oldpos));
      if (info->s->keyinfo[i].base.flag & HA_NOSAME)
      {					/* Change pointer direct */
	uint nod_flag;
	N_KEYDEF *keyinfo;
	keyinfo=info->s->keyinfo+i;
	if (_ni_search(info,keyinfo,key,USE_HOLE_KEY,
		       (uint) (SEARCH_SAME | SEARCH_SAVE_BUFF),
		       info->s->state.key_root[i]))
	  DBUG_RETURN(-1);
	nod_flag=test_if_nod(info->buff);
	_ni_dpointer(info,info->int_keypos-nod_flag-
		     info->s->rec_reflength,newpos);
	if (_ni_write_keypage(info,keyinfo,info->int_pos,info->buff))
	  DBUG_RETURN(-1);
      }
      else
      {					/* Change old key to new */
	if (_ni_ck_delete(info,i,key))
	  DBUG_RETURN(-1);
	VOID(_ni_make_key(info,i,key,record,newpos));
	if (_ni_ck_write(info,i,key))
	  DBUG_RETURN(-1);
      }
    }
  }
...
File new_file;
{
  ...
  for ( ;; )
  {
	...
	(*keyinfo->get_key)(keyinfo,nod_flag,&keypos,lastkey) == 0)
      break;
    rec_pos= _ni_dpos(info,nod_flag,keypos);
    ...
    {
      _ni_dpointer(info,keypos-nod_flag-info->s->rec_reflength,
		   sort_info.filepos);
...
my_string name;
{
       ...
       key++,keyinfo++)
  {
    if (info->s->state.key_root[key] != NI_POS_ERROR)
    {
      ...
      if (!_ni_fetch_keypage(info,keyinfo,info->s->state.key_root[key],
			     info->buff,0))
      {
	print_error("Can't read indexpage from filepos: %lu",
  ...
  VOID(lock_file(info->s->kfile,0L,F_UNLCK,"indexfile",name));
...
File new_file;
{
  ...
  if ((nod_flag=test_if_nod(buff)))
  {
    ...
    for ( ;; )
    {
      ...
      {
	next_page=_ni_kpos(nod_flag,keypos);
	_ni_kpointer(info,keypos-nod_flag,new_file_pos); /* Save new pos */
	if (!_ni_fetch_keypage(info,keyinfo,next_page,temp_buff,0))
	{
...
my_string filename,old_ext,new_ext;
{
...
my_string filetype,filename;
{
  ...
  {
    print_error(" %d when locking %s '%s'",my_errno,filetype,filename);
...
static int filecopy(File to,File from,ulong start,ulong length,my_string type)
{
...
my_string name;
{
  ...
  got_error=1;
  new_file= -1;
  sort_info.key_block=0; sort_info.record=0;
  ...
  sort_info.dupp=0;
  read_cache.end_of_file=sort_info.filelength=
    my_seek(read_cache.file,0L,MY_SEEK_END,MYF(0));
  ...
  for (sort_info.key=0 ; sort_info.key < share->state.keys ; sort_info.key++)
  {
    ...
    reinit_io_cache(&read_cache,READ_CACHE,share->pack.header_length,1);
  }
...
void *key;
{
  ...
  VOID(_ni_make_key(info,sort_info.key,key,sort_info.record,
		    sort_info.filepos));
...
static int sort_get_next_record()
{
  ...
  case DYNAMIC_RECORD:
	...
	if (_ni_read_cache(&read_cache,(byte*) block_info.header,pos,
	...
	if (((b_type=_ni_get_block_info(&block_info,-1,pos)) & BLOCK_ERROR) ||
	    ((b_type & BLOCK_FIRST) &&
	...
	if (_ni_read_cache(&read_cache,to,block_info.filepos,
	...
	if (pos + BLOCK_INFO_HEADER_LENGTH > read_cache.end_of_file)
	{
      ...
      if (_ni_rec_unpack(info,sort_info.record,info->rec_buff,
	...
	{
	  if (_ni_rec_check(info, sort_info.record))
	  {
  ...
  case COMPRESSED_RECORD:
    ...
    {
      if (_ni_read_cache(&read_cache,(byte*) block_info.header,sort_info.pos,
	...
	DBUG_RETURN(1);		/* Something wrong with data */
      ...
      sort_info.start_recpos=sort_info.pos;
      VOID(_ni_pack_get_block_info(&block_info,share->pack.ref_length,-1,
				   sort_info.pos));
	...
	continue;
      ...
      if (_ni_pack_rec_unpack(info,sort_info.record,info->rec_buff,
			      block_info.rec_len))
      {
	if (! searching)
...
static int sort_write_record()
{
  ...
  if (sort_info.fix_datafile)
  {
	...
	from=sort_info.buff+ALIGN_SIZE(MAX_DYN_BLOCK_HEADER);
      ...
      reclength=_ni_rec_pack(info,from,sort_info.record);
      block_length=reclength+ 3 +test(reclength > 65532L);
      if (block_length < share->base.min_block_length)
	block_length=share->base.min_block_length;
      flag=0;
      info->update|=HA_STATE_WRITE_AT_END;
      if (_ni_write_part_record(info,0L,block_length,NI_POS_ERROR,
				&from,&reclength,&flag))
      {
	print_error("%d when writing to datafile",my_errno);
	DBUG_RETURN(1);
...
static int sort_key_cmp(a,b)
const void *a,*b;
{
  return (_ni_key_cmp(sort_info.keyseg,*((uchar**) a),*((uchar**) b),0,
		      SEARCH_SAME));
} /* sort_key_cmp */
...
const void *a;
{
  int cmp=sort_info.key_block->inited ?
    _ni_key_cmp(sort_info.keyseg,sort_info.key_block->lastkey,(uchar*) a,
	       0,SEARCH_FIND) : -1L;
      ...
      cmp == 0)
  {
    ...
    if (testflag & T_VERBOSE)
      _ni_print_key(stdout,sort_info.keyseg,(uchar*) a);
...
uchar *key;
{
  return _ni_dpos(info,0,key+_ni_keylength(keyinfo,key));
} /* get_record_for_key */
...
ulong prev_block;
{
  ...
  if (nod_flag)
    _ni_kpointer(info,key_block->end_pos,prev_block);

  t_length=_ni_get_pack_key_length(sort_info.keyinfo,nod_flag,
				   (uchar*) 0,lastkey,key,&s_temp);
  _ni_store_key(sort_info.keyinfo,key_block->end_pos+nod_flag,&s_temp);
  a_length+=t_length;
  putint(anc_buff,a_length,nod_flag);
  key_block->end_pos+=t_length;
  if (a_length <= sort_info.keyinfo->base.block_length)
  {
    VOID(_ni_move_key(sort_info.keyinfo,key_block->lastkey,key));
  ...
  if ((filepos=_ni_new(info,sort_info.keyinfo)) == NI_POS_ERROR)
    return 1;
...
static int sort_delete_record()
{
  ...
  if (sort_info.key)
  {
    ...
    {
      VOID(_ni_make_key(info,i,key,sort_info.record,info->lastpos));
      if (_ni_ck_delete(info,i,key))
      {
...
static int flush_pending_blocks()
{
  ...
  for (key_block=sort_info.key_block ; key_block->inited ; key_block++)
  {
    ...
    if (nod_flag)
      _ni_kpointer(info,key_block->end_pos,filepos);
    if ((filepos=_ni_new(info,sort_info.keyinfo)) == NI_POS_ERROR)
      DBUG_RETURN(1);
...
static void print_info(my_string fmt,...)
{
  va_list args;
...
static void print_warning(my_string fmt,...)
{
  va_list args;
...
char *filename;
{
  ...
  memcpy((byte*) &info,*org_info,sizeof(info));
  memcpy((byte*) &share,(*org_info)->s,sizeof(share));
  ...
  if (share.base.options & HA_OPTION_COMPRESS_RECORD)
    ...
    max_records=my_seek(info.dfile,0L,MY_SEEK_END,MYF(0)) /
  ...
  VOID(ni_close(*org_info));
  if (ni_create(fn_format(name,filename,"",N_NAME_IEXT,
			  4+ (opt_follow_links ? 16 : 0)),
		share.base.keys,keyinfo,recinfo,
		max(max_records,share.base.records),share.base.reloc,
		HA_DONT_TOUCH_DATA,
		share.base.options | (unpack ? HA_OPTION_TEMP_COMPRESS_RECORD
				     : 0),
		my_seek(info.dfile,0L,MY_SEEK_END,MYF(0))))
  {
  ...
  *org_info=ni_open(name,O_RDWR,
  ...
  VOID(_ni_readinfo(*org_info,F_WRLCK,0));
