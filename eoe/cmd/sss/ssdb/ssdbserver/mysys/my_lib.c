...
/* my_stat - returns a status structure holding info about file path */
...
MY_STAT *my_stat(const char *path, MY_STAT *stat_area, myf my_flags)
{
  ...
  if (m_used= (stat_area == NULL))
    if (!(stat_area = (MY_STAT *) my_malloc(sizeof(MY_STAT), my_flags)))
      goto error;
  if ( ! stat((my_string) path, stat_area) )
...
my_string directory_file_name (my_string dst, my_string src)
{
#ifndef VMS
