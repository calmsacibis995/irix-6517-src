...
static void DBUGOpenFile(char *name);
#ifndef THREAD
	/* Open profile output stream */
static FILE *OpenProfile(char *name);
...
static BOOLEAN InList(struct link *linkp,char *cp);
static void dbug_flush(void);
static void DbugExit(char *why);
...
static pthread_key(CODE_STATE,THR_KEY_dbug_state);

static void init_dbug_state(void)
{
  pthread_key_create(&THR_KEY_dbug_state,free);
...
{
  CODE_STATE *state;
  if (!(state=my_pthread_getspecific(CODE_STATE*,THR_KEY_dbug_state)))
  {
    state=(CODE_STATE*) DbugMalloc(sizeof(*state));
    bzero(state,sizeof(*state));
    state->func="?func";
    state->file="?file";
    pthread_setspecific(THR_KEY_dbug_state,(void*) state);
  }
  return state;
}
#else	/* __WIN32__ */
...
__declspec( thread ) static CODE_STATE	static_code_state =
