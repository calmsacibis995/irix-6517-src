     ...
     int start;
{
  ...
  while (1)
    {
      optch = getopt_long (argc, argv, "lciqsvwm", long_options, &long_index);
     ...
     char *argv[];
{
  ...
  if (env_value != NULL)
      ...
      while (*env_value != 0)
	{
      ...
      parse_args (env_argc, env_argv, 0);
    }

  /* Command line arguments. */
  parse_args (argc, argv, 1);

  /* mutually exclusive options test */
  if(std_only && multi_name ) {
        fprintf(stderr,"bc: -w and -m options mutually exclusive\n");
        exit(1);
  }
  if(std_only && warn_not_std ) {
        fprintf(stderr,"bc: -w and -s options mutually exclusive\n");
        exit(1);
  }
