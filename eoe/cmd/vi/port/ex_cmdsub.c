...
#ident	"@(#)vi:port/ex_cmdsub.c	1.17.1.2"
...
error(":40", "Out of memory|Out of memory - too many lines in file");
			}
		}
		nline++;
		a1 = truedol + 1;
...
error(":41", "Line overflow|Result line of join would be too long");
		cp--;
	}
	if ((strlen(linebuf) + strlen(genbuf) + 1) > sizeof (genbuf))
error(":41", "Line overflow|Result line of join would be too long");
	strcLIN(genbuf);
	ldelete(0);
	jcount = 1;
...
vi_move()
{
	...
	adt = address((char*)0);
	if (adt == 0)
		serror(":42", "\"%s\" where?|\"%s\" command requires a trailing address", 
			Command);
	donewline();
	move1(iscopy, adt);
	killed();
	...
	line *addrt;
{
	...
	if (adt < ad1) {
		if (adt + 1 == ad1 && !cflag && !inglobal)
			error(":43", "That move would do nothing!");
		/* XPG4: After the move, the current line indicator
		 * will be set to the first of the moved lines.
		 */
		dot = adt + 1;
		if (++adt != ad1) {
			reverse(adt, ad1);
...
put()
{
	...
	if (!FIXUNDO)
		error(":45", "Cannot put inside global/macro");
	cnt = unddol - dol;
...
error(":46", "Line too long|Result line after shift would be too long");
		CP(cp, dp);
		strcLIN(genbuf);
		putmark(addr);
	}
...
#ifdef sgi
/* Each time we look for a tag, save the current file and position in the
 * tag stack. The pop command (^T in visual mode) restores the file and 
 * position.
 */
...
static struct {
    int		lineNum;
    char	file[FNSIZE];
} tagStack[TAG_STACK_SIZE];
static int	tagStackTop = -1;
int		tagLineNum = 0;		/* global because filename() needs to
					 * look at it. */

pop(quick)
    bool quick;		/* if 1, the user typed pop! to discard changes. */
{
	if (tagStackTop < 0) {
	    error(_SGI_MMX_vi_tagstkempt, "Tag stack empty");
	} else if (strcmp(tagStack[tagStackTop].file, savedfile) || !edited) {
/*
	     * The current and saved file are different.
	     */
	    unsigned char *oglobp;
	    unsigned char cmdbuf[FNSIZE + 10];
	    int  d;

	    if (!quick) {
		ckaw();
		if (chng && dol > zero)
		    error(_SGI_MMX_vi_nowrite,
			"No write@since last change (:pop! overrides)");
	    }
	    oglobp = globp;
	    strcpy(cmdbuf, "e! ");
	    strcat(cmdbuf, tagStack[tagStackTop].file);
	    tagLineNum = tagStack[tagStackTop].lineNum;
	    globp = cmdbuf;
	    d = peekc; ungetchar(0);
	    commands(1, 1);
	    peekc = d;
	    ...
	    /*
	     * Same file -- just restore the old position.
	     */
	    markpr(dot);
	    dot = zero + tagStack[tagStackTop].lineNum;
	}
	tagStackTop--;
}
#endif /* sgi */
	...
	bool quick;
{
	...
	if (!skipend()) {
		...
		*lp++ = 0;
		if (!endcmd(peekchar()))
badtag:
			error(":47", "Bad tag|Give one tag per line");
	} else if (lasttag[0] == 0)
		error(":48", "No previous tag");
	c = getchar();
	if (!endcmd(c))
		goto badtag;
	if (c == EOF)
...
goleft:
			...
			if (!*cp)
badtags:
				serror(":49", "%s: Bad tags file entry", 
					lasttag);
			lp = filebuf;
			while (*cp && *cp != ' ' && *cp != '\t') {
...
#ifdef sgi
			if (tagStackTop >= (TAG_STACK_SIZE-1)) {
			    error(_SGI_MMX_vi_tagstkfull, "Tag stack full");
			} else {
			    tagStackTop++;
			    strcpy(tagStack[tagStackTop].file, savedfile);
			    tagStack[tagStackTop].lineNum = lineDOT(dot);
			}
#endif /* sgi */
			strcpy(cmdbuf, cp);
...
#endif /* sgi */
						error(":50", "No write|No write since last change (:tag! overrides)");
#ifdef sgi
					}
#endif /* sgi */
				}
	...
	if (tfcount <= 0)
		error(":51", "No tags file");
	else
		serror(":52",
			"%s: No such tag|No such tag in tags file", lasttag);
}
...
yank()
{
	...
	if (!FIXUNDO)
		error(":53", "Cannot yank inside global/macro");
	save12();
	undkind = UNDNONE;
	...
	register struct maps *mp;
{
	...
	if (dest && mp==arrows) {
		...
		if (isalpha(src[0])  && isascii(src[0]) && src[1] || any(src[0],":"))
			error(":61", "Too dangerous to map that");
	}
	else if (dest) {
	...
	if (dest == (unsigned char *)NOSTR) {
		...
		if (mp[slot].cap) {
			...
			error(":63", "Not mapped|That macro was not mapped");
		}
		return;
	}
