...
#ident	"@(#)vi:port/ex_unix.c	1.19.1.3"
	...
	register int mode;
{
	...
	if (mode & 2) {
		...
		io = pvec[0];
#ifdef sgi
	/* pid for pid2 below looks just to be a plain bug. Daveh */
		if (pid2 < 0) {
#else
		if (pid < 0) {
#endif
		...
		if (pid2 == 0) {
			...
			io = pvec[1];
#ifdef sgi
                        /* Since the child process shares its tfile descriptor
                         * with its parent by default, and since both the
                         * parent and the child access the file simultaneously,
                         * we have to use a different file descriptor to avoid
                         * strange behavior.
                         */
                        close(tfile);
                        tfile = gfile;
