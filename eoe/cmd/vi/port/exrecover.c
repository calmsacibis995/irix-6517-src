...
#ident	"@(#)vi:port/exrecover.c	1.28.1.4"
...
int tfile = -1;	/* ditto */
const	char	EXmlostlines[] = ":256: [Lost line(s):";
const	char	errmsg[] = ":240:%s: %s\n";
const	char	EXmpercs[] = ":257:%s:\n";
const	char	EXmnofilesaved[] = ":258:No files saved.\n";
	...
	unsigned char *argv[];
{
	...
	(void)setlabel("UX:exrecover");
	(void)setcat("uxed.abi");
	cp = string;
	strcpy(mydir, USRPRESERVE);
	if (pp == NULL) {
		pfmt(stderr, MM_ERROR, ":259:Unable to get user's id\n");
		exit(-1);
	}
	...
	iblock = oblock = -1;
        ...
        while ((c=getopt(argc, (char **)argv, "d:rx")) != EOF)
		switch (c) {
                        case 'd':       /* from 'set dir' in EXINIT or .exrc */
                                exdir = optarg;
                                break;
			case 'r':
				rflg++;
	 ...
	 */
	if (rflg && argc == 0) {
		pfmt(stderr, MM_INFO, EXmpercs, mydir);
		listfiles(mydir);
		pfmt(stderr, MM_INFO, EXmpercs, TMPDIR);
		listfiles(TMPDIR);
                if(exdir && strcmp(exdir, TMPDIR)) {
                        /* if -d given, and different dir than default */
                        fprintf(stderr,"%s:\n", exdir);
                        listfiles(exdir);
                }
		exit(0);
	...
	if (argc != 2)
		error(":260", " Wrong number of arguments to exrecover", 0);
	 ...
	 */
	(void)cftime((char *)cp, gettxt(":261", "%a %h %d %T"), &H.Time);
	pfmt(stderr, MM_NOSTD, ":262: [Dated: %s", cp);
	if (vercnt == 1)
		fputc(']', stderr);
	else
		pfmt(stderr, MM_NOSTD, ":263:, newest of %d saved]", vercnt);
	fprintf(stderr, "\r\n");
	...
	if(H.encrypted) {
		if(xflag) {
			...
			kflag = run_setkey(perm, mypass(gettxt(":90", 
				"Enter encryption key: ")));
		if(kflag == -1) {
			kflag = 0;
			xflag = 0;
			pfmt(stderr, MM_ERROR, 
				":91:Encryption facility not available\n");
			exit(-1);
		}
                xtflag = 1;
                if (makekey(tperm) != 0) {
			xtflag = 0;
			pfmt(stderr, MM_ERROR, 
				":6:Warning--Cannot encrypt temporary buffer\n");
			exit(-1);
        	}
	}
	pfmt(stderr, MM_NOSTD, ":264:\r\n [Hit return to continue]");
	fflush(stderr);
	...
	if ((int) sbrk((int) (H.Flines * sizeof (line))) == -1)
		error(":265", " Not enough core for lines", 0);
#ifdef DEBUG
	...
	int inf;
{
	...
	lookedup = (*id ? gettxt((char *)id, (char *)str) : (char *)str);
	if (inf)
		pfmt(stderr, MM_ERROR|MM_NOGET, lookedup, inf);
	else
		pfmt(stderr, MM_ERROR|MM_NOGET, lookedup);
	...
	unsigned char *dirname;
{
	...
	int ecount, qucmp();
	register int f;
	unsigned char cp1[128];
	unsigned char cp2[128];
	...
	{
		pfmt(stderr, MM_INFO, EXmnofilesaved);
		return;
	}
	if (chdir((char *)dirname) < 0) {
		pfmt(stderr, MM_ERROR, errmsg, (char *)dirname, strerror(errno));
		return;
	}
	...
	for (fp = &svbuf[0]; fp < &svbuf[ecount]; fp++) {
		(void)cftime((char *)cp1, gettxt(":274", "%a %b %d"), &fp->sf_time);
		(void)cftime((char *)cp2, gettxt(":275", "%R"), &fp->sf_time);
		pfmt(stderr, MM_INFO, ":273:On %s at %s", cp1, cp2);
		if (fp->sf_lines == 1)
			pfmt(stderr, MM_NOSTD,
				":271: saved 1 line of file \"%s\" ",
				fp->sf_name);
		else
			pfmt(stderr, MM_NOSTD,
				":267: saved %d lines of file \"%s\" ",
				fp->sf_lines, fp->sf_name);
		if (fp->sf_encrypted)
			pfmt(stderr, MM_NOSTD, ":251:[ENCRYPTED]");
		fputc('\n', stderr);
	}
}
	...
	unsigned char *dir;
{
	 ...
	 */
	searchdir(dir);
	if (chdir((char *)mydir) == 0)
		searchdir(mydir);
        if(exdir && chdir(exdir) == 0)
                searchdir(exdir);
	if (bestfd != -1) {
		/*
		 * Gotcha.
	...
	for (ip = one; ip <= dol; ip++)
		if (*ip > maxt) {
			...
			*ip = ((HBLKS*BUFSIZ)-8) >> SHFT;
			...
			if (bad == 0)
				pfmt(stderr, MM_NOSTD, EXmlostlines);
	...
	int (*iofcn)();
{
	...
	if ((rc =(*iofcn)(tfile, buf, BUFSIZ)) != BUFSIZ) {
		(void)pfmt(stderr, MM_ERROR,
			":272:Failed on BLK: %d with %d/%d: %s\n",
			b,rc,BUFSIZ, strerror(errno)); 
		dirtcnt = 0;
	}
}
