...
#ident	"@(#)vi:port/ex_put.c	1.22.1.1"
	...
	int i;
{
	...
	if (shudclob)
		slobber(' ');
	/*
	 * make sure that list mode isn't used to print
	 * the number, or the tab will become "^I".
	 */
	nprintf("%6d\t", i);
	normline();
}
...
/*
 * Printf (temporarily) in normal mode.
 */
/*VARARGS2*/
nprintf(cp, dp)
	unsigned char *cp, *dp;
...
ostart()
{
	...
	/*
	if (!intty)
		error(":0", "Open and visual must be used interactively");
	*/
	gTTY(2);
	normtty++;
	f = tty;
...
#ifdef sgi
        /*
         * Prevent SIGTSTP signals from the tty driver by turning off the
         * suspend character.  We will now treat ^Z as a command, a la BSD.
         */
        tty.c_cc[VSUSP] = '\0';
#endif
	ttcharoff();
