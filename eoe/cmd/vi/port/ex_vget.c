...
#ident	"@(#)vi:port/ex_vget.c	1.20.1.5"
...
/*
 *	Here are the strings which have to form single semantic units for
 *	the translations (a total of forty-two, the ultimate answer to the
 *	ultimate ...).  Pointers to these are then entered into an array
 *	for which an index is built from values obtained from the first
 *	characters of, say, notenam.  This is vile, but these strings have
 *	to be single semantic units.  Also, there is probably redundancy
 *	here ("1 less line joined"?!), but setting some wierd combination
 *	of ex variables could possibly cause trouble, so better to give the
 *	translators a bit of work than drop core on the user :-)
 */
const	struct	msg	messages[]	={
	":163", "1 line",	/* Catchall message for unnamed commands */
	":164", "%d lines",	/* idem. */
	":208", "1 line joined",
	":209", "%d lines joined",
	":210", "1 line deleted",
...
char	*
pastmesg(cnt, cmd, sgn)
int cnt;
char *cmd, *sgn;
{
	register char c;
	char *retstr;
	const static char notevalid[] = "jdhy><=omp!";
	...
	if ((c = sgn[0]) == '\0'){
		...
		register int msgindex;
		char *strchr();
		register char *str_index;

		if ((c = cmd[0]) == 'c')	/* Copy or change            */
			c = cmd[1];		/* Is a unique identifier    */

		if (inopen && c == 't' && cmd[1] == '\0') /* 't' copies lines */
			c = 'o';

		if (cmd[0] && (str_index = strchr(notevalid, c)) != 0)
			msgindex = (cnt == 1 ? 0 : 1) + 2 *
				   (1 + (int)(str_index - notevalid));
		else
			msgindex = (cnt == 1 ? 0 : 1); /* Catchall */

		retstr = gettxt(messages[msgindex].id,messages[msgindex].text);

	} else if (c == 'm'){
		retstr = (cnt == 1) ? gettxt (":230", "1 more line")
				    : gettxt (":231", "%d more lines");
	} else {
		retstr = (cnt == 1) ? gettxt (":232", "1 fewer line")
				    : gettxt (":233", "%d fewer lines");
	}
	return (retstr);
}
...
#ifdef sgi
	/* Once, we looked at vmacp before vglobp. That was switched to fix 
	 * another bug, alas, vgetline when trying to wrap lines from a 'p'
	 * insert relies on a macpush of \n. So we need to modify */
	...
	bool must;
{
	...
	printf(pastmesg(notecnt, notenam, notesgn), notecnt);
	vclreol();
	notecnt = 0;
	if (state != VISUAL)
		vcnt = vcline = 0;
			 ...
			 */
			 ...
			 && strcmp(maps[d].descr, maps[d].cap)) {
				...
				if(strlen(vmacbuf)  + strlen(st) > BUFSIZ) 
					error(EXmmactoolongid, EXmmactoolong);
				else
					/* 
...
#ifdef sgi
struct itimerval setalarmtimer;
#else
static int ftfd;
struct requestbuf {
	short time;
 ...
 * On SGI systems, the "timeout" options sets the number of
 * milliseconds to delay.
 * Note: I have left the ftfd code here ifdef'd out in case there is ever
 * a merge/replacement of SGI ftimers with ft pseudo-devices.
 */
setalarm()
{
#ifdef sgi
        register long ms = value(vi_TIMEOUT);

        setalarmtimer.it_value.tv_sec = 0;
        while (ms >= 1000) {
                setalarmtimer.it_value.tv_sec++;
                ms -= 1000;
        }
        setalarmtimer.it_value.tv_usec = ms * 1000;
        setitimer(ITIMER_REAL, &setalarmtimer, (struct itimerval *)0);
#else
