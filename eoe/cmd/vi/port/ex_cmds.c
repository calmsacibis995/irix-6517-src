...
#ident	"@(#)vi:port/ex_cmds.c	1.22.1.4"
#include "ex.h"
#include "ex_argv.h"
#include "ex_temp.h"
#include "ex_tty.h"
	...
	bool noprompt, exitoneof;
{
	...
	int nextc1;
	line	*savdot = 0;
	unsigned char   cur_file[FNSIZE];
	extern struct glob G;
	static bool first_wq_flag = 1;
...
error(":18", "Offset out-of-bounds|Offset after command too large");
				setdot1();
				goto print;
			}
		}
		 ...
		 * Keep the last name for use by pastmesg.
		 */
		if (c != EOF)
			tailspec(c);
...
/* crypt */
			...
			ent_crypt:
				...
				permflag = 1;
				if ((kflag = run_setkey(perm, (key = vgetpass(
					gettxt(EXmenterkeyid, EXmenterkey))))) == -1) {
					xflag = 0;
					kflag = 0;
					crflag = 0;
...
editcmd:
			...
			if (strcmp(file,cur_file))
				...
				savdot = dot;
			if ((c == 'E') &&
				(!(value(vi_AUTOWRITE) && !value(vi_READONLY)))) {
				ungetchar(lastchar());
				(void)quickly();
			}
			setnoaddr();
...
casek:
			...
			c = getchar();
			if (endcmd(c))
serror(":20", "Mark what?|%s requires following letter", Command);
			donewline();
			if (!islower(c))
				error(":21", 
...
wq:
			if (skipwh() && peekchar() == '!') {
				...
				if (c == 'q') {
					write_quit = 1;
					if (first_wq_flag == 1) {
						/* For a :wq command, a setty is done
						   in wop() which messes up the screen
						   addressing (happens only for 1st :wq).
						   Hence set write_quit = 2 to skip
						   doing that in wop(). Bug #352835.
						*/
						first_wq_flag = 0;
						write_quit = 2;
					}
...
/* EOF */
		...
		ungetchar(c);
		{
			...
			if(length < 0)
				length = -length;
			multic[length] = '\0';
			error(":28", "What?|Unknown command character '%s'",
				multic);
		}
	}
}
