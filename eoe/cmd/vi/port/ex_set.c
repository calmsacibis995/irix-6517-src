...
#ident	"@(#)vi:port/ex_set.c	1.14.1.2"
...
dontset:
		...
		for (op = options; op < &options[vi_NOPTS]; op++)
			if (eq(op->oname, cp) || op->oabbrev && eq(op->oabbrev, cp))
				break;
		if (op->oname == 0)
serror(":157", "%s: No such option|%s: No such option - 'set all' gives all option values", cp);
		c = skipwh();
		if (peekchar() == '?') {
			ignchar();
printone:
		...
		if (no)
			serror(":158", "Option %s is not a toggle", op->oname);
		if (c != 0 || setend())
			goto printone;
		if (getchar() != '=')
			serror(":159",
		...
		case NUMERIC:
			...
			} else if (op == &options[vi_SCROLL]) {		/* SGI */
		...
		case OTERM:
			...
			while (!setend()) {
				if (cp >= &optname[ONMSZ])
error(":161", "String too long|String too long in option assignment");
 ...
 */
				if (inopen)
error(":162", "Cannot change type of terminal from within open/visual");
				unterm();
				setterm(optname);
			} else {
