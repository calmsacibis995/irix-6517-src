	...
	line *limit;
{
	...
	if (pastatom >= 2) {
		if (pastatom == 3) {
			/* move to end first, so we do the right thing
			 * even if already at end of word; this also is part
			 * of the fix for single letter words. */
			if(eend(f))
				cnt--;
		}
		if(cnt) {
		...
		else if(pastatom==2 && dir>0 && wcursor<linebuf) {
			/* without this, in some corner cases, we
			* can drop an insert of just one char!  In particular,
			* xpg4.2 vi assertions 279 and 282; w on a word wrap
			* to a blank line, then 1 char insert.  */
			wcursor = linebuf;
		}
	}
 ...
 * NOTE: xpg4 defines paragraph as superset of section, and
 * start of section is both { or formfeed as first char of line (plus
 * values of sections variable, of course).  OLDPARA keeps the pre-xpg4
 * definition.
 */
endPS()
{
	...
	return (linebuf[0] == 0 ||
	(!value(vi_OLDPARA) && ( linebuf[0] == '\f' || linebuf[0] == '{')) ||
...
/* NOTE: xpg4 defines start of section is both { or formfeed as first char of
 * line (plus values of sections variable, of course).  OLDPARA keeps the
 * pre-xpg4 definition.
*/
lbrack(c, f)
	register int c;
