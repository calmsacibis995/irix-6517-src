...
#ident	"@(#)vi:port/ex_vmain.c	1.22.1.5"
...
vmain()
{
	...
	/*
	 * The following need to be declared static so any assigned
	 * values will still be defined across a longjmp()
	 *
	 */
	static int onumber, olist, (*OPline)(), (*OPutchar)();
	...
	vshowmode(NULL, "");		/* As a precaution */
	/*
	 * NB:
	 *
	...
	for (;;) {
		if(savefirst) {
			/* this replicates what we do with initev, and is done because
			 * posix/xpg4 require that -t be done before the -c... */
			oglobp = globp;
			globp = savefirst;
			savefirst = NULL;
			hadcnt = cnt = 0;
...
reread:
		 ...
		 *
		 *		The scroll option is updated to reflect
		 *		this new count value.  --sgi--
		 *
...
insrt:
		...
		case 'q':	/* quit */
			if (initev) {
				vsave();
				CATCH
error(":235", "Q gets ex command mode, :q leaves vi");
				ENDCATCH
				splitw = 0;
				getDOT();
				vrepaint(cursor);
