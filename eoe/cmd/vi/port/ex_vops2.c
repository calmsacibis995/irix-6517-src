...
#ident	"@(#)vi:port/ex_vops2.c	1.21.1.3"
	...
	int cnt, indent;
{
	...
	case 'r':
		break;
	case 'a':
		vshowmode(EXmappmodeid, EXmappmode);
		break;
	case 's':
		vshowmode(EXmsubmodeid, EXmsubmode);
		break;
	case 'c':
		vshowmode(EXmchamodid, EXmchamod);
		break;
	case 'R':
		vshowmode(EXmrepmodeid, EXmrepmode);
		break;
	case 'o':
		vshowmode(EXmopnmodeid, EXmopnmode);
		break;
	case 'i':
		vshowmode(EXminsmodeid, EXminsmode);
		break;
	default:
		vshowmode(EXminpmodeid, EXminpmode);
	}
	ixlatctl(1);
...
vbackup:
			...
			case CTRL('v'):
				...
				if (c != NL) {
					if (doomed >= 0)
						doomed++;
					if(c == ATTN) /* did ^V^C for example... */
						multic[0] = wchar = tty.c_cc[VINTR];
					else
						multic[0] = wchar = c;
					length = 1;
					goto def;
		...
		if (c != NL && !splitw) {
			...
			if (value(vi_WRAPMARGIN) &&
				...
				*gcursor = 0;
				...
				macpush("\n", 0);
#ifdef sgi
				forcewrap = 1; /* to cause getbr to see the
						* pushed string correctly. */
#endif
				/*
...
def:
			...
			if (gcursor + length - 1 > &genbuf[LBSIZE - 2])
				error(EXmlitoolongid, EXmlitoolong);
