...
/* #endif PTR_ADDRESSES */
	...
	case 'w':
		wdkind = c & ' ';
		forbid(vlfind(2, cnt, opf, (line *)0) < 0);
		vmoving = 0;
		break;
	...
	case 'e':
		wdkind = 1;
ein:
		/* was cnt-1, but that stopped one word short, if we started
		 * at the end of a word (also mishandled one char words) */
		forbid(vlfind(3, cnt, opf, (line *)0) < 0);
		vmoving = 0;
		break;
	 ...
	 */
	case ')':
		forbid(vlfind(0, cnt, opf, (line *) 0) < 0);
		markDOT();
		break;
	 ...
	 */
	case '}':
		forbid(vlfind(1, cnt, opf, (line *) 0) < 0);
		markDOT();
		break;
...
slerr:
		...
		splitw = 0;
		vmoving = 0;
		wcursor = (unsigned char *)vloc1;
		if (i != 0)
			vsetsiz(i);
		if (opf == vmove) {
			if (state == ONEOPEN || state == HARDOPEN)
				outline = destline = WBOT;
			if (addr != dot || (unsigned char *)vloc1 != cursor)
				markDOT();
			if (vloc1 > (char *)linebuf && *vloc1 == 0)
				vloc1 = (char *)lastchr(linebuf, vloc1);
			if (c)
				vjumpto(addr, vloc1, c);
			else {
				vmoving = 0;
				if (vloc1) {
					vmoving++;
					vmovcol = column(vloc1);
				}
				getDOT();
