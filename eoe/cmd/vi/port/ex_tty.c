...
#ident	"@(#)vi:port/ex_tty.c	1.12.1.2"
...
gettmode()
{
...
#ifdef sgi
			    /*
			     * assume that if speed is __INVALID_BAUD
			     * then it is actually > B1200
			     */
					   && ((tty.c_cflag & CBAUD) != __INVALID_BAUD)
#endif
					   );
	...
	if ((tty.c_oflag & TABDLY) == TAB3 || teleray_glitch)
		GT = 0;
#ifdef sgi
        NONL = ((tty.c_oflag & ONLCR) == 0) && ((tty.c_iflag & ICRNL) == 0);
#else
	...
	unsigned char *type;
{
	...
	extern void setsize();
#ifdef sgi
        extern char _called_before;     /* 'magic' flag to setupterm().
                If non-zero, setupterm() won't really do all the work.
                This means that changing the term type from inside ex
                has no effect...  Olson, 6/88 */
#endif
	unknown = 0;
	...
	if (errret != 1) {
		...
		cur_term = 0;
#ifdef sgi
                strcpy(termtype, "dumb");
                _called_before = 0;
                setupterm("dumb", 1, &errret);
                resetterm();
#else
		if (errret == 0)
	...
	kpadd(immacs, key_ic, "\033", "inschar");
        /* old definition for key_eic, new def below  */
        /* kpadd(arrows, key_eic, "i", "inschar");    */
        /* kpadd(immacs, key_eic, "\033", "inschar"); */
	
	kpboth(arrows, immacs, key_eic, "L", "end");  
...
#ifdef sgi
                if (value(vi_NOVICE) || tty.c_cc[VSUSP] == CNSWTCH)
                        /* no job control */
                        dosusp = 0;
                else {
                        sc[0] = tty.c_cc[VSUSP];
	...
	if (unknown)
		serror(":180", "%s: Unknown terminal type", type);
}
