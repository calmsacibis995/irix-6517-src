...
#ident	"@(#)vi:port/ex_temp.c	1.19.1.3"
#include "ex.h"
#include "ex_temp.h"
#include "ex_vis.h"
#include "ex_tty.h"
...
#ifdef sgi
unsigned char	tfname[PATH_MAX];
unsigned char	rfname[PATH_MAX];
#else
...
dumbness:
	...
	for (p = strend(tfname), i = 10, j = getpid(); i > 0; i--, j /= 10)
		*--p = j % 10 | '0';
	tfile = open(tfname, O_EXCL|O_CREAT|O_RDWR, 0600);
	if (tfile < 0)
		goto dumbness;
#ifdef VMUNIX
	{
	...
	int iof;
{
	...
	if (bno >= NMBLKS) {
		...
		dol--;
		unddol--;
		error(EXmtmptoobigid, EXmtmptoobig);
	}
	nleft = BUFSIZ - off;
	if (bno == iblock) {
		ichanged |= iof;
...
synctmp()
{
	...
	if (dol == zero)
		return;
#ifdef sgi
        if ( ((dol - zero + 1)/(BUFSIZ/sizeof(line))) >= LBLKS )
                (void)exitclean("tmp file error: file has too many lines\n");
#endif
	/*
...
REGblk()
{
	...
	error(":177", "Out of register space (ugh)");
	/*NOTREACHED*/
}
	...
	unsigned char c;
{
	...
	if (rnext == 0) {
		...
		vreg = -1;
		error(EXmemptyregid, EXmemptyreg, c);
	}
	if (inopen && partreg(c)) {
		if (!FIXUNDO) {
			splitw++; vclean(); vgoto(WECHO, 0); vreg = -1;
			error(":178", "Cannot put partial line inside macro");
		}
		squish();
		addr1 = addr2 = dol;
	}
...
int buflen;
{
	...
	while (getREG()==0) {
		for (lp=linebuf; *lp;) {
			if (p >= &buf[buflen])
error(":179", "Register too long|Register too long to fit in memory");
			*p++ = *lp++;
		}
		*p++ = '\n';
	}
...
shownam()
{
	...
	gprintf(EXmregcontid, EXmregcont);
	gprintf(EXmequalsmid, EXmequalsm);
	for (k = 'a'; k <= 'z'; k++) {
		rbuf = &putrbuf;
		rnleft = 0;
		rblock = 0;
		...
		printf(" %c:",k);
		if (rnext == 0)
			gprintf(EXmemptyregtwoid, EXmemptyregtwo);
		while (getREG() == 0) {
			printf("\t\t%s\n", linebuf);
		}
	}
