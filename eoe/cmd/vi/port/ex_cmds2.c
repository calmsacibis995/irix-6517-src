...
#ident	"@(#)vi:port/ex_cmds2.c	1.13.1.2"
...
eol()
{
	...
	if (!skipend())
		error(":29", "Extra chars|Extra characters at end of command");
	ignnEOF();
}
	...
	int i;
{
	...
	merror(id, str, i);
	if (writing) {
		serror(":30", " [Warning - %s is incomplete]", file);
		writing = 0;
	}
	error1(str);
}
...
next()
{
	...
	if (argc == 0)
		error(":32", "No more files|No more files to edit");
	morargc = argc;
...
donewline()
{
	...
	for (;;) {
		...
		default:
			if (!endcmd(c))
serror(":33", "Extra chars|Extra characters at end of \"%s\" command", Command);
			if (c == EOF)
				ungetchar(c);
			setflav();
			return;
...
nomore()
{
	...
	if (argc == 1)
	{
		serror(":34", "1 more file|1 more file to edit");
	} else
	{
		serror(":35", "%d more files|%d more files to edit", argc);
	}
}
	...
	bool ask;
{
	...
	if (inopen) {
		...
		if (ask) {
			merror(":39", "[Hit return to continue] ");
			flush();
		}
#ifndef CBREAK
		vraw();
