...
#endif
{
	if(sigok)
		...
		windowchg++;
	(void)signal(SIGWINCH, winch_sig);
}
...
{
	struct winsize jwin;
	static int xpg4check = -1;

	if(xpg4check == -1) {
		char *xpg = getenv("_XPG");
		xpg4check = xpg && (atoi(xpg) > 0);
		if(xpg4check) {
			/* this hack is necessary because we have always had the ioctl override the
			 * env variables, but xpg4 wants the reverse behavior.  Since we don't know
			 * whether the lines and columns variables were set by curses from the env
			 * variables or the terminfo info, we have to check here... */
			if(!getenv("COLUMNS") && !getenv("LINES"))
				xpg4check = 0;	/* use ioctl, just like always */
		}
	}
...
oop()
{
	...
	if (peekchar() == '/') {
		...
		savere(scanre);
		if (execute(0, dot) == 0)
			error(":193", "Fail|Pattern not found on addressed line");
		ic = (unsigned char *)vloc1;
		if (ic > linebuf && *ic == 0)
			ic--;
	} else {
		getDOT();
...
ovbeg()
{
	...
	if (inopen)
		error(":194", "Recursive open/visual not allowed");
	Vlines = lineDOL();
	fixzero();
	setdot();
	pastwh();
...
merror(":195",
	...
	if (!clear_screen) {
		if (initev)
			goto toopen;
		error(":199", "Visual requires clear screen capability");
	}
	if (!scroll_forward) {
		if (initev)
			goto toopen;
		error(":200", "Visual requires scrolling");
	}
	windowchg = 0;
	(void)signal(SIGWINCH, winch_sig);
	ovbeg();
...
setwind()
{
	...
	case VISUAL:
		...
		if (ZERO < 0)
			ZERO = 0;
		if (ZERO > basWTOP)
			error(":201", "Screen too large for internal buffer");
	...
	register wchar_t *atube;
{
	...
	if (WCOLS == 1000)
		serror(":202", "Do not know enough about your terminal to use \"%s\"",
			Command);
	if (WCOLS > TUBECOLS)
		error(":203", "Terminal too wide");
	if (WLINES >= TUBELINES || WCOLS * (WECHO - ZERO + 1) > TUBESIZE)
		error(":204", "Screen too large");
...
begin_ca_mode()
{
	...
	if ((ca_mode != 0) || !enter_ca_mode) return;

	ca_mode = 1;

	(void) tputs(enter_ca_mode,0,putch);
}
...
end_ca_mode()
{
	...
	if ((ca_mode != 1) || !exit_ca_mode) return;

	ca_mode = 0;

	(void) tputs(exit_ca_mode,0,putch);
}
