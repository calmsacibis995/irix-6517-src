...
#ident	"@(#)vi:port/ex_vput.c	1.16.1.2"
...
vclreol()
{
	...
	if (destline < 0 || destline > WECHO)
		error(":237", "Internal error: vclreol");
	i = WCOLS - destcol;
...
unsigned char *id, *msg;
{
	...
	if (*msg) {
		if (id && *id)
			strncpy(locmsg, gettxt((char *)id, (char *)msg), LOCMSIZ);
		else
			strncpy(locmsg, msg, LOCMSIZ);
		if (value(vi_TERSE))
	...
	register int y, x;
{
	...
	if (y < 0) {
		error(":238", "Internal error: vgoto");
	}
	if (outcol >= WCOLS) {
	...
	if (state == HARDOPEN || state == ONEOPEN) {
		if (y != outline)
			error(":239", "Line too long for open");
