...
char	*av[];
{
	...
	if (ac <= 1) {
usage:
		fprintf(stderr,"Usage: ctags [-BFatuwvx] [-f tagsfile] file ...\n");
		exit(1);
	}
	if( (xpg = getenv("_XPG")) != NULL && atoi(xpg) > 0)
		_XPG = 1;

	while ((c=getopt(ac,av,"BFatuwvxf:")) != EOF) {
		switch (c) {
			case 'B':
				searchar='?';
				break;
	...
	if(_XPG)
		++tflag;	/* POSIX: Set this as default */
...
badone:			bad = TRUE;
	...
	firsttok = FALSE;
	parennest = 0;
	while ((*++sp=c=getc(inf)) != ')' || (parennest > 0)) {
		if (c == '(')
			parennest++;
		else if (c == ')' && (parennest > 0))
			parennest--;
		else if (c == EOF)
			goto badone;
		else if (c == '\n') {
...
getit()
{
	...
	*--cp = 0;	/* zap newline */
	...
	if (*dbp == 0 || !isalpha(*dbp) || !isascii(*dbp))
		return;
	/* allow _ and $ in names: bug 132568. */
