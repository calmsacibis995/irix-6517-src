/*
 * acctcvt/extacct64.c
 *	acctcvt functions for dealing with IRIX 6.4 extended acct records
 *
...
/*
 * ParseRecord_EA64
 *	Parses the specified image_t as an IRIX 6.4 extended accounting
...
*/
int
ParseRecord_EA64(const image_t *Image, record_t *Record)
{
	const char *recbufptr;
	int  recsize;
	const struct sat_hdr *rechdr = (struct sat_hdr *) Image->data;

	/* Validate the image */
	if (Image->format != fmt_ea64) {
		InternalError();
		return -1;
	}
	...
	if (rechdr->sat_outcome & SAT_CHK) {
		Record->gotpriv = 1;
		Record->priv    = rechdr->sat_cap;
		if (rechdr->sat_outcome & SAT_SUSER) {
			Record->privhow |= SAT_SUSERPOSS;
		}
		if (rechdr->sat_outcome & SAT_CAPABILITY) {
			Record->privhow |= SAT_CAPPOSS;
		}
	}

	recbufptr = Image->data + sizeof(struct sat_hdr);
	...
	/* Capability set */
	if (rechdr->sat_cap_size > 0) {
		if (rechdr->sat_cap_size != sizeof(cap_set_t)) {
			if (Debug & DEBUG_INVALID) {
				fprintf(stderr,
					"%s: invalid cap_size %d, s/b %d\n",
					MyName,
					rechdr->sat_cap_size,
					sizeof(cap_set_t));
			}
			...
			bcopy(recbufptr, &Record->caps, sizeof(cap_set_t));
			Record->gotcaps = 1;
		}

		recbufptr += rechdr->sat_cap_size;
	}
	else {
		Record->gotcaps = 0;
	}
	...
	switch (Record->rectype) {
		...
		if (rechdr->sat_recsize > 0) {
			...
			if (Debug & DEBUG_RECORD) {
				fprintf(stderr, "%s: SAT_AE_CUSTOM text = '%s',"
					" size(%d).\n", MyName, Record->custom,
...
/*
 * ReadRecord_EA64
 *	Reads an IRIX 6.4 extended accounting record from the specified
 *	input_t into the specified image_t. Returns 0 if successful, -1
 *	if not.  
 */
int
ReadRecord_EA64(input_t *Input, image_t *Image)
{
	char *recbufptr;
	int  recsize;
	struct sat_hdr *rechdr;

	/* Read the record header */
	if (Read(Input, Image->data, sizeof(struct sat_hdr)) != 0) {
		if (!InputEOF(Input)) {
			fprintf(stderr,
	...
	rechdr = (struct sat_hdr *) Image->data;
	...
	if (Read(Input,
		 Image->data + sizeof(struct sat_hdr),
		 Image->len - sizeof(struct sat_hdr)) != 0)
	{
		if (!InputEOF(Input)) {
			fprintf(stderr,
 ...
 *	IRIX 6.4 extended accounting format. Returns 0 if successful,
 *	-1 if not.
 */
int
WriteRecord_EA64(output_t *Output, record_t *Record)
{
	char recbuf[4096];
	char *recbufptr;
	int  recsize;
	...
	bzero(recbuf, 4096);
	rechdr = (struct sat_hdr *) recbuf;
	recsize = sizeof(struct sat_hdr);
	...
	/* Set up capability used if necessary */
	if (Record->gotpriv) {
		rechdr->sat_cap = Record->priv;
		rechdr->sat_outcome |= SAT_CHK;
		if (Record->privhow & SAT_CAPPOSS) {
			rechdr->sat_outcome |= SAT_CAPABILITY;
		}
		else if (Record->privhow & SAT_SUSERPOSS) {
			rechdr->sat_outcome |= SAT_SUSER;
		}
	}
	...
	/* Set up the capability set, if we have one */
	if (Record->gotcaps) {
		rechdr->sat_cap_size = sizeof(cap_set_t);
		bcopy(&Record->caps, recbufptr, rechdr->sat_cap_size);

		recbufptr += rechdr->sat_cap_size;
		recsize   += rechdr->sat_cap_size;
	}
