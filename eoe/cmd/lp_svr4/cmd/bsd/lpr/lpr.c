 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/bsd/lpr/RCS/lpr.c,v 1.1 1992/12/14 11:27:43 suresh Exp $"
		...
		if (chkbits) {
			if (chkbits & PCK_TYPE)		/* No terminfo */
				...
				buf[0] = NULL;
					...
					strcat(buf, "-h ");
	    			lp_fatal(E_LP_PTRCHK, NOLOG, buf);
			}
		} else 
			lp_fatal(E_LP_DENYDEST, NOLOG, Printer);
		/*NOTREACHED*/
		break;
    	case MNOFILTER:
		lp_fatal(E_LP_NOFILTER, NOLOG);
		/*NOTREACHED*/
		break;
    	case MERRDEST:
