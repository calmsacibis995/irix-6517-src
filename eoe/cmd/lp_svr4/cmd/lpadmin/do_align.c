 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/lpadmin/RCS/do_align.c,v 1.1 1992/12/14 11:29:06 suresh Exp $"
...
Done:		if (!(printer_status & PS_DISABLED))
	...
	if (setjmp(pipe_env) == 0) {
		...
		cancel (req_id);
                LP_OUTMSG(INFO, E_ADM_INTERRUPT);
	}
