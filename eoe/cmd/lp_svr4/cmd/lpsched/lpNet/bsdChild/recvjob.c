 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/lpsched/lpNet/bsdChild/RCS/recvjob.c,v 1.1 1992/12/14 11:31:03 suresh Exp $"
...
static  int       smail(char *, int, long, int, ...);
...
#endif
{
	...
	*reqid = '-';			/* restore file name */
	...
	if (putrequest(file, rqp) < 0 || putsecure(file, &secure) < 0) {
		...
		smail(user, E_LP_PUTREQUEST, NOLOG, error);
	...
	if (msend(Msg) == -1 || mrecv(Msg, MSGMAX) != R_PRINT_REQUEST) {
		smail(user, E_LP_NEEDSCHED, NOLOG); 
	...
	case MOK:
		break;
	case MNOMEM:
		smail(user, E_LP_MNOMEM, NOLOG);
		break;
    	case MDENYDEST:
		if (chkbits) {
			if (chkbits & PCK_TYPE)
                		smail(user, E_LP_PGONE, NOLOG, rqp->destination);
			else {
	    			char buf[20];
				...
				buf[0] = NULL;
					...
					strcat(buf, "-h ");
	    			smail(user, E_LP_PTRCHK, NOLOG, buf);
			}
		} else 
			smail(user, E_LP_DENYDEST, NOLOG, rqp->destination);
		break;
    	case MNOFILTER:
		smail(user, E_LP_NOFILTER, NOLOG);
		break;
    	case MERRDEST:
		smail(user, E_LP_REQDENY, NOLOG, rqp->destination);
		break;
   	case MNOOPEN:
		smail(user, E_LPP_NOOPEN, NOLOG);
		frecverr("lpsched can't open %s", file);	/* cleans-up */
		/*NOTREACHED*/
	default:
 		smail(user, E_LP_BADSTATUS, NOLOG, status);
		break;
	}
}
...
static
#if defined (__STDC__)
smail(char *user, int seqnum, long msgid, int logind, ...)
#else
smail(user, seqnum, msgid, logind, va_alist)
char	*user;
int      seqnum;
long	 msgid;
int      logind;
