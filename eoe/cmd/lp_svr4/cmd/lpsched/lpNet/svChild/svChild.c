 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/lpsched/lpNet/svChild/RCS/svChild.c,v 1.2 1993/01/08 19:49:35 suresh Exp $"
...
static char	*current_sendjob_req_id = NULL;
/*-----------------------------------------------------------------*/
/*
*	Local functions.
*/
...
s_send_job:
		...
		*/
		...
		jobId = ReceiveJob (CIP, &dataList_p, &fileList_p, 0);
		...
		if (jobId == -1)
		{
			...
			if (errno == 0) {
				...
				putmessage(buf,R_PRINT_REQUEST,
					   MUNKNOWN,
					   (current_sendjob_req_id  != NULL
					    	? current_sendjob_req_id
					   	: ""),
					   0);
				NotifyLpExecOfJobStatus(MOK,buf);
			} else
				NotifyLpExecOfJobStatus (MTRANSMITERR, 0);
			break;
		}
...
_POLLIN:
	...
	*/
	...
	jobId = ReceiveJob (CIP, &dataList_p, &fileList_p, 1);
...
char	*jobMsg_p;
{
	...
	**
	*/
	if (current_sendjob_req_id != NULL) {
		free(current_sendjob_req_id);
		current_sendjob_req_id = NULL;
	}
...
#ifdef	DEBUG
{
	...
	case	1:
		...
		secbufp = getsecure(requestFileName_p);
		if (secbufp != NULL) {
			current_sendjob_req_id = secbufp->req_id;
			secbufp->req_id = NULL;
			freesecure(secbufp);
	        }
