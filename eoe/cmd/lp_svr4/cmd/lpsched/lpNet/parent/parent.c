 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/lpsched/lpNet/parent/RCS/parent.c,v 1.1 1992/12/14 11:31:29 suresh Exp $"
	...
	*/
	...
	case	S_NEW_CHILD:
		...
		if (sip == NULL) {
			...
			(void)	sprintf (emsgBuffer,
			"Cannot find system info for '%s'.", systemName_p);

			TrapError (NonFatal, Internal, FnName, emsgBuffer);
	...
	*/
	default:
		...
		else
		{
			...
			(void)
			sprintf (emsgBuffer, "Started child for %s, pid = %d",
				sip->systemName_p, childPid);
			WriteLogMsg (emsgBuffer);
		}
		(void)	close (pipeFds [0]);
		(void)	close (pipeFds [1]);
	} /* switch */
