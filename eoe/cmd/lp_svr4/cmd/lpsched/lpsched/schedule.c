 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/lpsched/lpsched/RCS/schedule.c,v 1.1 1992/12/14 11:34:41 suresh Exp $"
...
#ifdef _RISCOS
	/*
	 * Check if we need to send a status request
	 * to this system:
	 */
		...
		if (prs != NULL) {
			...
			if (prs == NULL) {
				...
				if ((now - pss->laststat) > 
						USER_STATUS_EXPIRED) {
					pss->laststat = now;
					schedlog ("Scheduling automatic status call to %s\n",
						  pss->system->name);
					mkreq(pss);
				}
			}
