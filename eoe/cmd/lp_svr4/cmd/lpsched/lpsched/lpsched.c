 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/lpsched/lpsched/RCS/lpsched.c,v 1.1 1992/12/14 11:33:44 suresh Exp $"
...
#ifdef _RISCOS
    /* Use Bourne shell if available to prevent incompatible syntax
       among shells. */
    if (stat(DEFAULT_SHELL, &stbuf) == 0)
	SHELL = DEFAULT_SHELL;
    else
#endif  /* _RISCOS */
...
startup()
{
	...
	if (errno == EAGAIN)
	    lpfail (ERROR, E_SCH_PRACTIVE);
	else
	    lpfail (ERROR, E_SCH_TROUBLE, NB(Lp_Schedlock), PERROR);
    close_lpfile(LockFp);
    background();
    /*
