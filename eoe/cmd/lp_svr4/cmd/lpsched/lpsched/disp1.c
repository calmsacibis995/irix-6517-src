 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/lpsched/lpsched/RCS/disp1.c,v 1.2 1993/04/06 23:54:56 miken Exp $"
...
#else
void s_start_change_request ( m, md )
char	*m;
MESG	*md;
#endif
{
...
#endif
{
    ...
    */
	...
	if (STRNEQU(file, Lp_Spooldir, strlen(Lp_Spooldir)))
	    ...
	    if(STRNEQU(file, "/var/spool/lp", 13))
		cp = file + strlen("/var/spool/lp") + 1;
	    else
	    {
		*idnumber = NULL;
		return(NULL);
