 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/lpstat/RCS/lpstat.c,v 1.1 1992/12/14 11:35:53 suresh Exp $"
...
#endif
{
	...
	if ((name = getdefault()))
                LP_OUTMSG1(INFO, E_STAT_DEFDEST, name);
	else
                LP_OUTMSG(INFO, E_STAT_NODEFDEST);
...
{
        if (scheduler_active)
             LP_OUTMSG(INFO, E_STAT_SCHED);
        else
             LP_OUTMSG(INFO, E_STAT_NOSCHED);
