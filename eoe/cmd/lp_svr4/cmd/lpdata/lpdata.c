 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/lpdata/RCS/lpdata.c,v 1.1 1992/12/14 11:30:05 suresh Exp $"
...
main(int argc, char * argv[])
{
	...
	switch(c)
	{
	    ...
	    default:
            LP_ERRMSG1(ERROR, E_DAT_OPTION, c);
	}
	...
	if (index >= MAXCMD)
	{
            LP_OUTMSG1(ERROR, E_DAT_COMMAND, buff);
	    fflush(stdout);
	    continue;
	}
...
void newfile(void)
{
    ...
    {
        LP_ERRMSG2(ERROR, E_DAT_NOOPEN, Datafile, errno);
...
void check(void)
{
    ...
    if (!testswitch('t'))
    {
        LP_OUTMSG(ERROR, E_DAT_MISCHECK);
	return;
    }
	...
	check_stuff(cp - Chktab + 1);
        ...
        LP_OUTMSG1(ERROR, E_DAT_UNREC, target);
}
...
void check_stuff(int type)
{
	...
	if (CS_STREQU(p, NAME_ALL) || CS_STREQU(p, NAME_NONE) || CS_STREQU(p, NAME_ANY))
	{
	    if (type != 3)
            {
                msg_text = retmsg(E_DAT_RESWORD);
		printf(msg_text, more++ ? ", " : "", p);
            }
	    continue;
	}
	...
	if (syn_name(p))
	{
            msg_text = retmsg(E_DAT_NAMES);
	    printf(msg_text, more++ ? ", " : "");
	    continue;
	}
	...
	if (type & 01)
	{
	    ...
	    if (reject && (check & 1))
	    {
                msg_text = retmsg(E_DAT_REJECTING);
		printf(msg_text, more++ ? ", " : "", p);
		continue;
	    }
	    ...
	    if (!reject && (check & 2))
	    {
                msg_text = retmsg(E_DAT_ACCEPTING);
		printf(msg_text, more++ ? ", " : "", p);
		continue;
	    }
	}
...
printers:
	...
	if (type & 02)
	{
	    ...
	    if (!(pstat & PS_DISABLED) && (check & 2))
	    {
                msg_text = retmsg(E_DAT_ENABLED);
		printf(msg_text, more++ ? ", " : "", p);
		continue;
	    }
	    if ((pstat & PS_DISABLED) && (check & 4))
	    {
                msg_text = retmsg(E_DAT_DISABLED);
		printf(msg_text, more++ ? ", " : "", p);
		continue;
	    }
	    if (!(pstat & PS_REJECTED) && (check & 8))
