 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/cmds/RCS/lp.c,v 1.1 1992/12/14 11:28:25 suresh Exp $"
...
ack_job()
{
    ...
    if(nfiles > 1) {
        if (stdinp > 0)
           LP_OUTMSG2(INFO, E_LP_REQID6, reqid, nfiles);
        else
           LP_OUTMSG2(INFO, E_LP_REQID5, reqid, nfiles);
    }
    else {
        if (nfiles > 0) {
           if (stdinp > 0)
              LP_OUTMSG1(INFO, E_LP_REQID4, reqid);
           else
              LP_OUTMSG1(INFO, E_LP_REQID3, reqid);
           }
           else {
              if (stdinp > 0)
                 LP_OUTMSG1(INFO, E_LP_REQID2, reqid);
              else
                 LP_OUTMSG1(INFO, E_LP_REQID1, reqid);
           }
    }
}
main(argc, argv)
...
*/
                ...
                LP_OUTMSG(INFO|MM_NOSTD, E_LP_USAGE1);
                LP_OUTMSG(INFO|MM_NOSTD, E_LP_USAGE2);
                LP_OUTMSG(INFO|MM_NOSTD, E_LP_USAGE3);
                LP_OUTMSG(INFO|MM_NOSTD, E_LP_USAGE4);
		exit(0);
	    }
	    (p = "-X")[1] = optopt;
