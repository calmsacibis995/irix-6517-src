 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/cmds/RCS/lpforms.c,v 1.2 1993/07/20 02:37:30 raghav Exp $"
	...
	char *			argv[];
{
	...
	while ((c = getopt(argc, argv, OPT_LIST)) != -1) {
		...
		default:
			if (optopt == '?') {
/*
 * The usage: message of the command is a very long message that does
 * not fit into the message buffer of 512 bytes.  The message was broken
 * into 4 messages USAGE, USAGE1, and USAGE2. The last two of
 * them are displayed with MM_NOSTD option to exclude the label and make
 * all 3 messages look like one.
*/
                                LP_OUTMSG(INFO, E_FOR_USAGE);
                                LP_OUTMSG(INFO|MM_NOSTD, E_FOR_USAGE1);
                                LP_OUTMSG(INFO|MM_NOSTD, E_FOR_USAGE2);
				exit (0);
			}
			(cp = "-X")[1] = optopt;
...
#else
add_alert (form, input, p_new_alert, u)
	char *			form;
	FILE *			input;
	FALERT *		p_new_alert;
	char *			u;
...
#else
add_form (form, input, p_new_alert, u)
	char *			form;
	FILE *			input;
	FALERT *		p_new_alert;
	char *			u;
#endif
{
	...
	if (input) {
		...
		if (rdform(form, &new_fbuf, input, onerror, which_set) == -1) {
		...
		if (which_set[FO_ALIGN]) {
			...
			path = tempnam(TMPDIR, "lpf.");
			align_fp = open_lpfile(path, "w+", MODE_NOREAD);
			if (!align_fp) {
				LP_ERRMSG (ERROR, E_FOR_CTMPFILE);
				exit (1);
			}
			Unlink (path);	/* So it disappears on close. */
	...
	while (return_code == -1) {
		...
		if (getform(form, &fbuf, &alert, (FILE **)0) == -1)
			switch (errno) {
...
#endif
{
	...
	if (STREQU(NAME_ALL, form)) {
		...
		while (getform(form, &fbuf, &alert, &align_fp) == 0) {
                        LP_OUTMSG2(INFO|MM_NOSTD, E_FOR_FORMS, nl, fbuf.name);
...
#endif
{
	...
	wrform (pf->name, pf, stdout, onerror, which_set);
	if (align_fp)
