 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/cmd/cmds/RCS/lpfilter.c,v 1.1 1992/12/14 11:28:28 suresh Exp $"
	...
	for (ps = store; ps->name; ps++) {
		...
		if (putfilter(ps->name, ps) == -1) {
			...
			case LP_EREGEX:
			{
				...
				switch (regerrno) {
				case 11:
					why = retmsg(E_FL_CASE11);
					break;
				case 16:
					why = retmsg(E_FL_CASE16);
					break;
				case 25:
					why = retmsg(E_FL_CASE25);
					break;
				case 36:
					why = retmsg(E_FL_CASE36);
					break;
				case 41:
					why = retmsg(E_FL_CASE41);
					break;
				case 42:
					why = retmsg(E_FL_CASE42);
					break;
				case 43:
					why = retmsg(E_FL_CASE43);
					break;
				case 44:
					why = retmsg(E_FL_CASE44);
					break;
				case 45:
					why = retmsg(E_FL_CASE45);
					break;
				case 46:
					why = retmsg(E_FL_CASE46);
					break;
				case 49:
					why = retmsg(E_FL_CASE49);
					break;
				case 50:
					why = retmsg(E_FL_CASE50);
					break;
				}
	...
	char			*filter;
{
	...
	if (STREQU(NAME_ALL, filter)) {
		...
		while ((pf = getfilter(filter))) {
                        LP_OUTMSG2(INFO|MM_NOSTD, E_FL_FILTERS, nl, pf->name);
			_list_filter (pf);
			nl = "\n";
		}
