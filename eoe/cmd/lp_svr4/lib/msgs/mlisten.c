 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/lib/msgs/RCS/mlisten.c,v 1.1 1992/12/14 13:31:21 suresh Exp $"
...
#endif
{
	...
	*/
	if (NumEvents <= 0)
	{
	    ...
	    **	set errno to EAGAIN.
	    */
/* fprintf(stderr,"DBG: mlisten before poll errno %d\n",errno); */
	    if ((NumEvents = poll(PollFdList, NumCons, -1)) < 0)
	    {
		errno = EAGAIN;
		return(NULL);
	    }
/* fprintf(stderr,"DBG: mlisten after poll errno %d\n",errno); */
	}
