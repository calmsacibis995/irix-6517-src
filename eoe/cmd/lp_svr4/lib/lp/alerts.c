 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/lp_svr4/lib/lp/RCS/alerts.c,v 1.1 1992/12/14 13:27:47 suresh Exp $"
...
#endif
{
	if (!alertp->shcmd) {
		if (isfault)
                        LP_OUTMSG(INFO||MM_NOSTD, E_ADM_ALERT13);
		else
                        LP_OUTMSG(INFO||MM_NOSTD, E_ADM_ALERT14);
		...
		if (
		   ...
		   if (isfault)
		      if (alertp->W > 0)
                         LP_OUTMSG3(INFO||MM_NOSTD, E_ADM_ALERT1, copy, cp, alertp->W);
		      else
                         LP_OUTMSG2(INFO||MM_NOSTD, E_ADM_ALERT2, copy, cp);
                   else
                      if (alertp->Q > 1)
		         if (alertp->W > 0)
                            LP_OUTMSG4(INFO||MM_NOSTD, E_ADM_ALERT5, alertp->Q, copy, cp, alertp->W);
                         else
                            LP_OUTMSG3(INFO||MM_NOSTD, E_ADM_ALERT6, alertp->Q, copy, cp);
                      else
		         if (alertp->W > 0)
                            LP_OUTMSG3(INFO||MM_NOSTD, E_ADM_ALERT9, copy, cp, alertp->W);
                         else
                            LP_OUTMSG2(INFO||MM_NOSTD, E_ADM_ALERT10, copy, cp);
                else
		   if (isfault)
		      if (alertp->W > 0)
                         LP_OUTMSG2(INFO||MM_NOSTD, E_ADM_ALERT3, alertp->shcmd, alertp->W);
		      else
                         LP_OUTMSG1(INFO||MM_NOSTD, E_ADM_ALERT4, alertp->shcmd);
                   else
                      if (alertp->Q > 1)
		         if (alertp->W > 0)
                            LP_OUTMSG3(INFO||MM_NOSTD, E_ADM_ALERT7, alertp->Q, alertp->shcmd, alertp->W);
                         else
                            LP_OUTMSG2(INFO||MM_NOSTD, E_ADM_ALERT8, alertp->Q, alertp->shcmd);
                      else
		         if (alertp->W > 0)
                            LP_OUTMSG2(INFO||MM_NOSTD, E_ADM_ALERT11, alertp->shcmd, alertp->W);
                         else
                            LP_OUTMSG1(INFO||MM_NOSTD, E_ADM_ALERT12, alertp->shcmd);
