...
static void	process(), validate(), read_to(), report(), preprocess();
static int	getstats(), do_delt(), read_mod(), invalid(), opt_need_arg();
void	ffree(), exit();
void	addq(), remq();
...
char	*argv[];
{
	...
	*/
	...
	/* Preprocess command line to removes spaces betw. opts/args */
