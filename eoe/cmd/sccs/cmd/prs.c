...
static char	tempskel[]   =   "/tmp/prXXXXXX";	
/* used to generate temp file names */
        ...
        for (i = 0, j = -1, fg = n = 0; argv[i]; i++){
			...
			if (j == -1)
                ...
                if (j == -1){
                        ...
                        if (fg){
                                argvtmp = malloc(strlen(argv[i])+ strlen(argv[j])+1);
