...
int	stat64(), setuid(), setgid(), xcreat();
...
void	preprocess(),increment_had();
char	*strcpy(), *strcat();
char	del_ab();

static	char	*compose_elist();
...
char *file;
{
	...
	*/
	...
	if (gpkt.p_verbose) {
		...
		/* Below line commented out to satisfy xpg4 */
		if (!HADL)
	...
	if (!HADG || HADL) {
		mode_t FileMode; /* mode of file on creation */
		if (gpkt.p_stdout) {
		...
		*/
		if (Sflags[EXECFLAG - 'a'])  /* set mask to turn execute permission on */
			FileMode = (mode_t)0111;
		else 
			FileMode = (mode_t)0;
		...
		if (gpkt.p_gout == 0) {
			if (HADP)
				...
				if (HADK) 
					FileMode |= (mode_t)0644;
				else
					FileMode |= (mode_t)0444;

				gpkt.p_gout = xfcreat(Gfile, FileMode);
			}
		}
		...
		/* Below line commented out to satisfy xpg4 test  ***/
		if (gpkt.p_verbose && !HADL)
			fprintf(gpkt.p_stdout,"%d\n",gpkt.p_glnno);
		/* Above line commented out to satisfy xpg4 test  ***/
		if (!Did_id && !HADK)
			if (Sflags[IDFLAG - 'a'])
				if(!(*Sflags[IDFLAG - 'a']))
...
register char *old, *new;
{
	...
	sprintf(new, "%d/%d/%d", time.tm_mon, time.tm_mday, time.tm_year);
	time.tm_mon--;
	/*
...
int     *argc;
{
        ...
        for (i = 0, j = -1, fg = n = 0; argv[i]; i++){
                if (!strcmp(argv[i], "--")){
                        n = 1;
                        continue;
                }
                if (n && argv[i][0] == '-'){
                        argv[i] = (char *) strdup(&argv[i][1]);
                }
                if (j == -1){
                        if (opt_need_arg(cmd, argv[i])){
                                j = i;
                                fg = 1;
                        }
                }
...
static  void cvtserial_string(serial, string, pkt)
int     serial;
char    *string;
struct  packet  *pkt;
{
        int     s_rel = pkt->p_idel[serial].i_sid.s_rel;
        int     s_br  = pkt->p_idel[serial].i_sid.s_br;
        int     s_lev = pkt->p_idel[serial].i_sid.s_lev;
        int     s_seq = pkt->p_idel[serial].i_sid.s_seq;
        char    s_rel_str[20];  
        char    s_br_str[20];   
        char    s_lev_str[20];  
        char    s_seq_str[20];  

        sprintf(s_rel_str, "%d", s_rel);
        sprintf(s_br_str,  "%d", s_br);
        sprintf(s_lev_str, "%d", s_lev);
        sprintf(s_seq_str, "%d", s_seq);
        strcpy(string, s_rel_str);      
        if (s_br){      
                strcat(string, ".");
                strcat(string, s_br_str);
        }
        if (s_lev){
                strcat(string, ".");
                strcat(string, s_lev_str);
        }
        if (s_seq){
                strcat(string, ".");
                strcat(string, s_seq_str);
        }
        return;
}

static  char *compose_elist(pkt)
struct  packet  *pkt;
{
        int     i, last;
        int     count, serial;
	...
	extern	char	*malloc();
        ...
        e_list = (char *) malloc(200*sizeof(char));
	strcpy(e_list, "");	
	if (!pkt->p_idel)
		return (e_list);
        last  = pkt->p_idel->i_pred;
	if (!pkt->p_idel[last].i_ixg)
		return (e_list);
        count = pkt->p_idel[last].i_ixg[0].i_cnt;
	if (!count)
		return (e_list);
	if (pkt->p_idel[last].i_ixg[0].i_type != 'g')
		return (e_list);
       	for (i = 0; i < count; i++){
               	serial = pkt->p_idel[last].i_ixg[0].i_ser[i];
               	cvtserial_string(serial, string, pkt);
               	if (!strcmp(e_list, ""))
                       	strcpy(e_list, string);
               	else {
                       	strcat(e_list, ",");
                       	strcat(e_list, string);
               	}
	}
        return (e_list);
}
