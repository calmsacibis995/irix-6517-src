...
#define rename presumptuous_ansi_c_stdio_rename_prototype
#ident	"$Revision: 1.9 $"
/* #ident	"@(#)sccs:cmd/admin.c	6.14.1.1" */
	...
	for(j=1; j<argc; j++)
		if(argv[j][0] == '-' && (c = argv[j][1])) {
			...
			case 'f':	/* flags to be added */
				...
				case EXECFLAG: /* file should have execute 
						* permission on get
						*/
					break; 
...
char	*afile;
{
	...
	/*
	 * new version of admin ought to expect -r without -i
	 *
	if (!HADI && HADR)
	...
	*/
	if (!HADH) {
		if (!HADN) stat64(gpkt.p_file,&sbuf);
        ...
        for (i = 0, j = -1, fg = n = 0; argv[i]; i++){
			...
			if (j == -1)
                ...
                if (j == -1){
                        ...
                        if (fg){
                                argvtmp = (char *) malloc(strlen(argv[i])+ strlen(argv[j])+1);
