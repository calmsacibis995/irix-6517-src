/* lightly hacked from gzip-1.2.4 distribution */
...
#ifdef RCSID
static char rcsid[] = "$Id: unlzw.c,v 1.2 1995/09/05 01:04:04 doucette Exp $";
#endif
...
int unlzw(int in, int out)	/* input and output file descriptors */
#else
int unlzw(in, out) 
...
/*  REG10  int        insize; (global) */
    ...
    if (maxbits > BITS) {
	fprintf(stderr,
		"\nuncompress: compressed with %d bits, can only handle %d bits\n",
		maxbits, BITS);
	exit_code = ERROR;
	return ERROR;
    }
	...
	if (insize < INBUF_EXTRA) {
	    rsize = 0;
	    if (exit++)	/* if we get here a second time, leave */
	...
	while (inbits > posbits) {
	    ...
	    input(inbuf,posbits,code,n_bits,(long)bitmask);
