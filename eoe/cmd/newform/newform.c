...
#ident	"@(#)newform:newform.c	1.14.1.1"
...
char	pachar = ' ',		/* Prepend/append character		*/
	...
	*wlast  = &work[3*NCOLS], /* Pointer to end of work area	*/
	...
	savchr[COL8],		/* Holds char stripped from line start  */
	...
	if (mcolumn > effll) {
		...
		while (n) {
			...
			if (ISASCII(*plast)) {
				...
				for(i = 0; i < maxeucw; i++, plast--) {
					if(ISSET2(*plast)) {
						scrw = wp._scrw2;
						break;
					} else if (ISSET3(*plast)) {
						scrw = wp._scrw3;
						break;
					}
				}
				if (!ISSET2(*plast) && !ISSET3(*plast)) {
					plast++;
					scrw = wp._scrw1;
					plast += (maxeucw - wp._eucw1);
				}
				if (n >= scrw) {
					plast--;
					n -= scrw;
...
sstrip()
{
	...
	while (i <= 7) {
		if (ISASCII(*c)) {
			...
			if ((i+scrw > 8) || ((i+scrw == 8) && (savek+eucw < k))) {
				while (i++ < 8)
					savchr[++savek] = '*';
				break;
			}
	...
	pfirst += (k+2);		/* increase tab and pointer */
}
/* ================================================================= */
