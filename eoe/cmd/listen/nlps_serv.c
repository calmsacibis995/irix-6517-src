...
#ident	"@(#)listen:nlps_serv.c	1.3.9.1"
				...
				if (dbp->dbf_sflags & CFLAG) {
					...
					if (*dbp->dbf_scheme != '\0') {
						if (invoke(0, dbp->dbf_scheme) != 0) {
							sprintf(msgbuf, "NLPS: invoke of <%s> failed", dbp->dbf_scheme);
							logmessage(msgbuf);
							(void) exit(2);
						}
					}
...
register char **o_argv;
{
	...
	if (*wdbp->dbf_scheme != '\0') {
		if (invoke(0, wdbp->dbf_scheme) != 0) {
			sprintf(msgbuf, "invoke of <%s> failed for <%s>", wdbp->dbf_scheme, wdbp->dbf_svc_code);
			logmessage(msgbuf);
			exit(2);
		}
	}
	...
	if (*wdbp->dbf_id != '\0') {
		if (set_id(wdbp->dbf_id) != 0) {
			sprintf(msgbuf, "set_id(%s) failed for <%s>", wdbp->dbf_id, wdbp->dbf_svc_code);
			logmessage(msgbuf);
			exit(2);
		}
	}
		...
		if (set_id(NULL) != 0) {
			sprintf(msgbuf, "set_id(NULL) failed for <%s>", wdbp->dbf_svc_code);
			logmessage(msgbuf);
			exit(2);
		}
	}
	...
	if ((pwdp = getpwuid(getuid())) == NULL) {
		sprintf(msgbuf, "NLPS: Missing or bad passwd entry for <%d>", getuid());
		logmessage(msgbuf);
		exit(2);
	}
...
register char *svc_code_p;
{
	...
	while (rd_dbf_line(dbfp, Dbf_line_buf, &Dbf_entry.dbf_svc_code, 
		...
		&Dbf_entry.dbf_res2, &Dbf_entry.dbf_scheme,&Dbf_entry.dbf_prv_adr,
