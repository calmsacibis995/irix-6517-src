	...
	if (re_read)  {
		...
		if (Newdbf)
			for (dbf_p = Newdbf; dbf_p->dbf_svc_code; ++dbf_p)
				DEBUG((7, "svc code <%s>; id: %s; scheme : %s; private address: %s; modules: %s; cmd line: %s; sflags: %x, prognum: %d version: %d",
				dbf_p->dbf_svc_code,dbf_p->dbf_id,
				dbf_p->dbf_scheme, dbf_p->dbf_prv_adr,
				dbf_p->dbf_modules, dbf_p->dbf_cmd_line,
				dbf_p->dbf_sflags, dbf_p->dbf_prognum,
				dbf_p->dbf_version));
#endif	/* DEBUGMODE */
	}
	else {
		...
		if (Dbfhead)
			for (dbf_p = Dbfhead; dbf_p->dbf_svc_code; ++dbf_p)
				DEBUG((7, "svc code <%s>; id: %s; r1: %s; r2: %s; scheme: %s; private address: %s; modules: %s; cmd line: %s; sflags: %x, prognum: %d version: %d",
				dbf_p->dbf_svc_code, dbf_p->dbf_id, dbf_p->dbf_res1, dbf_p->dbf_res2, dbf_p->dbf_scheme, dbf_p->dbf_prv_adr, dbf_p->dbf_modules, dbf_p->dbf_cmd_line, dbf_p->dbf_sflags, dbf_p->dbf_prognum, dbf_p->dbf_version));
#endif	/* DEBUGMODE */
	}
...
register char *cmd_p;
{
	...
	while (n = rd_dbf_line(dbfilep,p,&svc_code_p,&flags,&id_p,&res1_p,&res2_p,&scheme_p,&private_p,&prognum,&vernum,&module_p,&sflags,&cmd_line_p,NOSUPPRESS))  {
		...
		strcpy(cmd_p, scheme_p);
		dbf_p->dbf_scheme = cmd_p;
		cmd_p += strlen(scheme_p) + 1;
...
register char *path;
{
	...
	do {
		n = rd_dbf_line(dbfilep,p,&svc_code_p,&flags,&id_p,&res1_p,&res2_p,&scheme_p,&private_p,&prognum,&vernum,&module_p,&sflags,&cmd_line_p,SUPPRESS);
		if (n == -1)  {
			fclose(dbfilep);
			return(-1);
		}
 ...
 *				*scheme_p = authentication scheme;
...
int
rd_dbf_line(fp, bp, svc_code_p, flags_p, id_p, res1_p, res2_p, scheme_p, private_p, prognum, vernum, module_p, sflags, cmd_line_p, mflag)
register FILE *fp;
register char *bp;
char **svc_code_p;
int *flags_p;
	...
	do {
		...
		*(bp + strlen(bp) -1) = (char)0; /* delete newline	*/
		...
		if (strlen(bp)) {		/* anything left?	*/
		   ...
		   if (!(length=scan_line(bp,svc_code_p,flags_p,id_p,res1_p,res2_p,scheme_p,private_p,prognum,vernum,module_p,sflags,cmd_line_p,mflag))) {
 ...
 *
 *	service_code: flags: id: res1: res2: scheme: private address: rpcdata: sflags: modules: cmd_line # comments
 *
...
int
scan_line(bp, svc_code_p, flags_p, id_p, res1_p, res2_p, scheme_p, private_p, prognum, vernum, module_p, sflags, cmd_line_p, mflag)
register char *bp;
char **svc_code_p;
...
int mflag;
{
	...
	*res2_p = nexttok;
	...
	if (!(p = strchr(nexttok, DBFTOKSEP ))) {
		DEBUG((9,"scan_line failed scheme strchr"));
		return(0);
	}
	*p = NULL;
	*scheme_p = nexttok;
	nexttok = ++p;
	if (**id_p == '\0' && **scheme_p == '\0') {
		if (!mflag) {
			/* messages aren't suppressed */
			sprintf(scratch, "service code <%s> missing id and scheme - entry ignored", *svc_code_p);
			logmessage(scratch);
		}
		ignore = 1;
	}
	...
	*p = NULL;
	if (ignore)
		/* setting addr to nothing causes entry to be ignored */
		*private_p = p;
	else
		*private_p = nexttok;
	nexttok = ++p;
	...
	*cmd_line_p = nexttok;
	...
	DEBUG((9,"scan_line: modules: %s; line: %s; len: %d", *module_p, *cmd_line_p, strlen(*svc_code_p)+strlen(*id_p)+strlen(*res1_p)+strlen(*res2_p)+strlen(*scheme_p)+strlen(*private_p)+strlen(*module_p)+strlen(*cmd_line_p)+9));
	/*
	 * return the length of the buffer.  Add 9 for the NULLs after each
	 * string
	 */
	return(strlen(*svc_code_p)+strlen(*id_p)+strlen(*res1_p)+strlen(*res2_p)+strlen(*scheme_p)+strlen(*private_p)+strlen(*module_p)+strlen(*cmd_line_p)+9);
}
