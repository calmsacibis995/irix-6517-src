...
#ident	"@(#)listen:listen.c	1.19.18.1"
 ...
 */
	...
	logmessage("@(#)listen:listen.c	1.19.18.1");
...
char **ap;
{
	...
	if (t_bind(fd, req, ret))  {
		...
		if ((t_errno == TNOADDR) || ((t_errno == TSYSERR) && (errno == EAGAIN))) {
			...
			logmessage("no more addresses");
			tli_error(E_T_BIND, EXIT);
		}
		...
		if (clen == -1) {
			...
			*ap = (char *) malloc(((ret->addr.len) << 1) + 3);
			...
			if ( t_free(ret, T_BIND) )
				tli_error(E_T_FREE, EXIT);
			logexit(1, "failed to bind properly");
		}
...
#ifdef i386
		if (poll(Pollfds, (unsigned long)(Valid_addrs + 1), -1) < 0) {
#else
...
#ifndef sgi
			/*
			 * invoke authentication scheme if there is one
			 */

			if (*dbp->dbf_scheme != '\0') {
				if (invoke(Acceptfd, dbp->dbf_scheme) != 0) {
					sprintf(scratch, "invoke of <%s> failed", dbp->dbf_scheme);
					logmessage(scratch);
					goto cleanup;
				}
			}
 ...
 * Note: call was accepted on fd 0.
 */
...
register dbf_t *dbp;
{
	...
	DEBUG((9, "Running doconfig on %s", dbp->dbf_svc_code));
	if ((i = doconfig(0, dbp->dbf_svc_code, 0)) != 0) {
		DEBUG((9, "doconfig exited with code %d", i));
...
#ifndef sgi
	/*
	 * At SGI we removed ALL calls to the general
	 * Identification and Authentication Facility (IAF) since:
	 * 1) It is totally non-standard
	 * 2) It has NO existing installed clients or any visible user demand
	 *    for this type of security/authentication AND lastly
	 * 3) It provides a rather poor solution to the general
	 *    authentication problem and begs to provide a solution to
	...
	if (*dbp->dbf_scheme != '\0') {
		if (invoke(0, dbp->dbf_scheme) != 0) {
			sprintf(msgbuf, "invoke of <%s> failed for <%s>", dbp->dbf_scheme, dbp->dbf_svc_code);
			logmessage(msgbuf);
			exit(2);
		}
	}
	...
	if (*dbp->dbf_id != '\0') {
		if (set_id(dbp->dbf_id) != 0) {
			sprintf(msgbuf, "set_id(%s) failed for <%s>", dbp->dbf_id, dbp->dbf_svc_code);
			logmessage(msgbuf);
			exit(2);
		}
	}
		...
		if (set_id(NULL) != 0) {
			sprintf(msgbuf, "set_id(NULL) failed for <%s>", dbp->dbf_svc_code);
			logmessage(msgbuf);
			exit(2);
		}
	}
	...
	if ((pwdp = getpwuid(getuid())) == NULL) {
		sprintf(msgbuf, "Missing or bad passwd entry for <%d>", getuid());
		logmessage(msgbuf);
		exit(2);
	}
