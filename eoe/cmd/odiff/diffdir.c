...
#define MAXREGEXPS 10
static char *revec[MAXREGEXPS];
static char **next_re = &revec[0];

/* add a directory name to the list to exclude
 */
int				/* <0 if bad or too many */
setxpat(char *pat)
{
	char *new_re;

	if (next_re >= &revec[MAXREGEXPS]) {
		fprintf(stderr, "diff: too many -x patterns\n");
		return (-1);
	}
	*next_re = regcmp(pat, 0);
	if (!next_re) {
		fprintf(stderr, "diff: \"%s\" is an illegal pattern\n", pat);
		return (-1);
	}
	++next_re;
}
	...
	register struct dir *dp;
{
	...
	fstat64(f1, &stb1); fstat64(f2, &stb2);
...
ascii(f)
	int f;
{
	/* sgi rewritten: previous ascii(){} contents discarded - pj */
	char buf[BUFSIZ];
	int cnt, i;
...
register char *cp;
{
	...
	/*
	 * check first (since most common) if a name should be excluded
	 */
	for (rep = &revec[0]; rep < next_re && *rep; rep++) {
		if (regex(*rep, cp))
			return (1);
	}
