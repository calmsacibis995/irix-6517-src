...
int	iflag;
/*
 * In the case of multiple attrs (e.g. ul + bold), output bold instead of
 * the default reverse video.  This is used by the man command to output
 * the SVR4 man pages in a more attractive manner.
 */
int	bflag;
	...
	char **argv;
{
	...
	if (termtype == NULL || (argv[0][0] == 'c' && !isatty(1)))
		termtype = "lpr";
	while ((c=__getopt(argc, argv, "ibt:T:")) != EOF)
		switch(c) {
		...
		default:
			fprintf(stderr,
				"Usage: %s [ -ib ] [ -tTerm ] file...\n",
				argv[0]);
			exit(1);
		}

	/* hack attempt to keep libcurses termcap emulation from doing ioctls
	 * that change tty modes, since we don't need or want them */
	c = dup(2);
	if (c != -1) close(2);
	tgv = tgetent(termcap, termtype);
	if (c != -1) dup2(c,2);
...
FILE *f;
{
	...
	/*
	 * keep track of whether the last character was a 'special'
	 * character and therefore didn;t advance the column counter.
	 * IF we attempt to backspace over one of these, we ignore the
	 * backspace command. This allows sequences like:
	 * SI ^H B ^H  SO ^H SI ...
	 *
	 * There are 2 column counters - one is the index into the line
	 * buffer (col), the other is the 'true' column counter -
	 * these differ when overstriking. We need the true counter to
	 * properly handle tabs
	 */
	while((c = getc(f)) != EOF) switch(c) {

	case '\b':
		if (col > 0 && !lastwasspec)
			col--, tcol--;
		lastwasspec = 0;
		continue;
	...
	case '\t':
		{
		/* if due to overstrikes we are already at the 'correct'
		 * character position then at least move forward 1 space
		 */
		int tomove = ((tcol+8) & ~07) - tcol;
		if (tomove == 0)
			tomove++;
		col += tomove;
		tcol += tomove;
	...
	case '_':
		/*
		 * an _ aftger a character means underline that char and
		 * advance
		 */
		if (obuf[col].c_char) {
	...
	default:
		...
		if (obuf[col].c_char == '\0') {
			...
			/* overstrike -- output only the second character */
			obuf[col].c_char = c;
			obuf[col].c_mode = mode;
		}
		col++, tcol++;
...
fwd()
{
	register oldcol, oldtcol, oldmax;
