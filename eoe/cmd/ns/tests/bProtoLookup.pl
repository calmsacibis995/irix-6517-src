...
#	1. To run on ficus to scale  getprotobyname
#		./bProtoLookup.pl -ficus -o 1 -n <protocol_name>
#	2. To run on ficus to scale getprotobynumber
#		./bProtoLookup.pl -ficus -o 2 -n <protocol_number>
#	3. To run on kudzu to scale getprotobyname
#		./bProtoLookup.pl -kudzu -o 1 -n <protocol_name>
#	4. To run on kudzu to scale getprotobynumber
#		./bProtoLookup.pl -kudzu -o 2 -n <protocol_number>
...
sub run_benchmark 
{
	...
	for($j=0; $j < $samplingCnt ; $j++) {
		system "./bProtoLookup -r 1 -t $timeToRun -o $fn_no -n $ent{$i} > /tmp/bb";
		$i++;
		if($i > $argCnt -1)  { $i = 0; }
