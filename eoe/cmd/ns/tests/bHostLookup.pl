...
#       1. To run on ficus to scale  gethostbyname
#               ./bHostLookup.pl -ficus -o 1 -n <host_name>
#       2. To run on ficus to scale gethostbyaddr
#               ./bHostLookup.pl -ficus -o 2 -n <host_addr>
#       3. To run on kudzu to scale gethostbyname
#               ./bHostLookup.pl -kudzu -o 1 -n <host_name>
#       4. To run on kudzu to scale gethostbyaddr
#               ./bHostLookup.pl -kudzu -o 2 -n <host_addr>
...
{
        if ($> != 0) {
                die "You need to be root to run the benchmark as it involves restarting nsd.\n";
        }
	...
	for($j=0; $j < $samplingCnt ; $j++) {
		system "./bHostLookup -r 1 -t $timeToRun -o $fn_no -n $ent{$i} > /tmp/bb";
		$i++;
		if($i > $argCnt -1)  { $i = 0; }
