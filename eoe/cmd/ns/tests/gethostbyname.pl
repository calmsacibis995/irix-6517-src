...
#		./gethostbyname.pl -repeat 100 -hostname <comma_separated_host_names>
#
#	2. Change the timestamp of the /etc/hosts file so that it
#	   will cause the cache to be rebuild.
#		./gethostbyname.pl -touch -repeat 100 -hostname <comma_separated_host_names>
#
...
#	 	./gethostbyname.pl -add <comma_separated_host_names> -repeat 100 -hostname <comma_separated_host_names>
#
#	4. Stop the nsd, add new entries and retrive the information.
#		./gethostbyname.pl -add <comma_separated_host_names> -stopnsd -repeat 100 -hostname <comma_separated_host_names>
#
#	5. Wait for timeout and then retrieve the information, this will
#	   cause cache to be rebuild
#		./gethostbyname.pl -wait <time_val_in_seconds> -repeat 100 -hostname <comma_separated_host_names>
#
#	6.Add an arbitrary entry in the /etc/hosts file and look for it
#		./gethostbyname.pl -entry <a_host_entry> -hostname <host_name>
...
print "Start Script:gethostbyname.pl\n";
print "####CMD: ./gethostbyname.pl ", join(' ', @options), "\n";

($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime(time);
#print "Date:/$mon/$mday/$year  ", "Time:$hour:$min:$sec\n";

for($i=0; $i < $repeatCount ; $i++) {
	print STDOUT "==================gethostbyname() - Begin :", $i,"=============\n";
	if($touchflg ) {
	...
	##Call the unit test 
	print  "./gethostbyname ", join(' ', @list_of_hostnames), "\n";
        system "./gethostbyname @list_of_hostnames";

	print  "./gethostbyname_r ", join(' ', @list_of_hostnames), "\n";
        system "./gethostbyname_r @list_of_hostnames";
...
#        print STDOUT "Time - user = $luser, system = $lsystem, cuser = $lcuser, csystem = $lcsystem\n";
	...
	$prevCuser = $cuser;
	$prevCsystem = $csystem;
	print STDOUT "==================gethostbyname() - End :", $i,"===============\n";
}
close(STDOUT);
close(STDERR);
open(STDOUT, ">&SAVEOUT");
...
sub print_usage()
{
	print "Usage: ./gethostbyname.pl \n\t[-add <comma_separated_host_names>] \n\t[-touch] \n\t[-stopnsd]\n\t[-entry <entry_to_add>]\n\t[-wait <wait_for_seconds>]\n\t[-repeat <n_times>]\n\t-hostname <comma_separated_host_names>\n";
	exit(-1);
}
