...
void myGetrpcbyname(void *argv); 
void myGetrpcbynumber(void *arg);
...
main(int argc, char **argv)
{
	...
	for(i = 0; i < no_of_readers; i++) {
		...
		if(f_no ==1 ) {
			...
			strcpy(pg_name, "getrpcbynumber()");
			pthread_create(&threadArray[i],
						NULL,
						(void *)myGetrpcbynumber,
						(void*) &(threadValArray[i]));
		}
	}
...
myGetrpcbyname(void *arg)
{
	...
	while(run) {
		r = getrpcbyname(entName);
		if (!r) {
			fprintf(stderr, "Thread %d->failed getrpcbyname: %s\n",th, entName);
		} else {
			threadArg->count++;
		}
	}
...
myGetrpcbynumber(void *arg)
{
	...
	while(run) {
        	r = getrpcbynumber(strtol(entName, 0, 10));
		if (!r) {
			fprintf(stderr, "Thread %d-> failed getrpcbynumber: %s\n",th, entName);
		} else {
			threadArg->count++;
		}
	}
