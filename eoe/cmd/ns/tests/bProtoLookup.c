...
void myGetprotobyname(void *argv); 
void myGetprotobynumber(void *arg);
...
main(int argc, char **argv)
{
	...
	for(i = 0; i < no_of_readers; i++) {
		...
		if(f_no ==1 ) {
			strcpy(pg_name, "getprotobyname()");
			pthread_create(&threadArray[i],
						NULL,
						(void *)myGetprotobyname,
						(void*) &(threadValArray[i]));
		}
		else {
			strcpy(pg_name, "getprotobynumber()");
			pthread_create(&threadArray[i],
						NULL,
						(void *)myGetprotobynumber,
						(void*) &(threadValArray[i]));
		}
	}
...
myGetprotobyname(void *arg)
{
	...
	while(run) {
		r = getprotobyname(entName);
		if (!r) {
			fprintf(stderr, "Thread %d->failed getprotobyname: %s\n",th, entName);
		} else {
			threadArg->count++;
		}
	}
...
myGetprotobynumber(void *arg)
{
	...
	while(run) {
        	r = getprotobynumber(strtol(entName, 0, 10));
		if (!r) {
			fprintf(stderr, "Thread %d-> failed getprotobynumber: %s\n",th, entName);
		} else {
			threadArg->count++;
		}
	}
