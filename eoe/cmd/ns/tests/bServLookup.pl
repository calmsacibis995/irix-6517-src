...
#       1. To run on ficus to scale  getservbyname
#               ./bServLookup.pl -ficus -o 1 -n <serv_name>
#       2. To run on ficus to scale getservbyport
#               ./bServLookup.pl -ficus -o 2 -n <serv_port>
#       3. To run on kudzu to scale getservbyname
#               ./bServLookup.pl -kudzu -o 1 -n <serv_name>
#       4. To run on kudzu to scale getservbyport
#               ./bServLookup.pl -kudzu -o 2 -n <serv_port>
...
sub run_benchmark 
{
	...
	for($j=0; $j < $samplingCnt ; $j++) {
		system "./bServLookup -r 1 -t $timeToRun -o $fn_no -n $ent{$i} > /tmp/bb";
		$i++;
		if($i > $argCnt -1)  { $i = 0; }
