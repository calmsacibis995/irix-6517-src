...
#       1. To run on ficus to scale  getgrnam
#               ./bGroupLookup.pl -ficus -o 1 -n <grp_name>
#       2. To run on ficus to scale getgrgid
#               ./bGroupLookup.pl -ficus -o 2 -n <grp_id>
#       3. To run on kudzu to scale getgrnam
#               ./bGroupLookup.pl -kudzu -o 1 -n <grp_name>
#       4. To run on kudzu to scale getgrgid
#               ./bGroupLookup.pl -kudzu -o 2 -n <grp_id>
...
sub run_benchmark 
{
	...
	for($j=0; $j < $samplingCnt ; $j++) {
		system "./bGroupLookup -r 1 -t $timeToRun -o $fn_no -n $ent{$i} > /tmp/bb";
		$i++;
		if($i > $argCnt -1)  { $i = 0; }
