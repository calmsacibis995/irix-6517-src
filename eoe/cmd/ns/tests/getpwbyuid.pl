...
#		./getpwbyuid.pl -repeat 100 -userid <comma_separated_user_uids>
#
#	2. Change the timestamp of the /etc/passwd file so that it
#	   will cause the cache to be rebuild.
#		./getpwbyuid.pl -touch -repeat 100 -userid <comma_separated_user_uids>
#
...
#	 	./getpwbyuid.pl -add <comma_separated_user_names> -repeat 100 -userid <comma_separated_user_uids>
#
#	4. Stop the nsd, add new entries and retrive the information.
#		./getpwbyuid.pl -add <comma_separated_user_names> -stopnsd -repeat 100 -userid <comma_separated_user_uids>
#
#	5. Wait for timeout and then retrieve the information, this will
#	   cause cache to be rebuild
#		./getpwbyuid.pl -wait <time_val_in_seconds> -repeat 100 -userid <comma_separated_user_uids>
#
#	6.Add an arbitrary entry in the /etc/passwd file and look for it
#		./getpwbyuid.pl -entry <a_user_entry> -userid <comma_separated_user_uids>
...
while(@ARGV) {
  ...
  if(/^-repeat(.*)/) {
	...
	@list_of_userids = split(/,/, shift(@ARGV));
	$useridFlg = 1;
  } elsif (/^-entry(.*)/) {
...
print "Start Script:getpwbyuid.pl\n";
print "####CMD: ./getpwbyuid.pl ", join(' ', @options), "\n";

($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime(time);
#print "Date:/$mon/$mday/$year  ", "Time:$hour:$min:$sec\n";

for($i=0; $i < $repeatCount ; $i++) {
	print STDOUT "==================getpwbyuid() - Begin :", $i,"=============\n";
	if($touchflg ) {
	...
	##Call the unit test 
	print  "./getpwuid ", join(' ', @list_of_userids), "\n";
        system "./getpwuid @list_of_userids";

	print  "./getpwuid_r ", join(' ', @list_of_userids), "\n";
        system "./getpwuid_r @list_of_userids";
...
sub print_usage()
{
	print "Usage: ./getpwbyuid.pl \n\t[-add <comma_separated_user_names>] \n\t[-touch] \n\t[-stopnsd]\n\t[-entry <entry_to_add>]\n\t[-wait <wait_for_seconds>]\n\t[-repeat <n_times>]\n\t-userid <comma_separated_user_uids>\n";
	exit(-1);
}
