...
#		./getprotobyname.pl -repeat 100 -protoname <comma_separated_protocol_names>
#
#	2. Change the timestamp of the /etc/protocols file so that it
#	   will cause the cache to be rebuild.
#		./getprotobyname.pl -touch -repeat 100 -protoname <comma_separated_protocol_names>
#
...
#	 	./getprotobyname.pl -add <comma_separated_protocol_names> -repeat 100 -protoname <comma_separated_protocol_names>
#
#	4. Stop the nsd, add new entries and retrive the information.
#		./getprotobyname.pl -add <comma_separated_protocol_names> -stopnsd -repeat 100 -protoname <comma_separated_protocol_names>
#
#	5. Wait for timeout and then retrieve the information, this will
#	   cause cache to be rebuild
#		./getprotobyname.pl -wait <time_val_in_seconds> -repeat 100 -protoname <comma_separated_protocol_names>
#
#	6.Add an arbitrary entry in the /etc/protocols file and look for it
#		./getprotobyname.pl -entry <a_protocol_entry> -protoname <protocol_name>
...
while(@ARGV) {
  ...
  if(/^-repeat(.*)/) {
	...
	@list_of_protocolnames = split(/,/, shift(@ARGV));
	$protocolnameFlg = 1;
  } elsif (/^-entry(.*)/) {
...
print "Start Script:getprotobyname.pl\n";
print "####CMD: ./getprotobyname.pl ", join(' ', @options), "\n";

($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime(time);
#print "Date:/$mon/$mday/$year  ", "Time:$hour:$min:$sec\n";

for($i=0; $i < $repeatCount ; $i++) {
	print STDOUT "==================getprotobyname() - Begin :", $i,"=============\n";
	if($touchflg ) {
	...
	##Call the unit test 
	print  "./getprotobyname ", join(' ', @list_of_protocolnames), "\n";
        system "./getprotobyname @list_of_protocolnames";

	print  "./getprotobyname_r ", join(' ', @list_of_protocolnames), "\n";
        system "./getprotobyname_r @list_of_protocolnames";
...
#        print STDOUT "Time - user = $luser, system = $lsystem, cuser = $lcuser, csystem = $lcsystem\n";
	...
	$prevCuser = $cuser;
	$prevCsystem = $csystem;
	print STDOUT "==================getprotobyname() - End :", $i,"===============\n";
}
close(STDOUT);
close(STDERR);
open(STDOUT, ">&SAVEOUT");
...
sub print_usage()
{
	print "Usage: ./getprotobyname.pl \n\t[-add <comma_separated_protocol_names>] \n\t[-touch] \n\t[-stopnsd]\n\t[-entry <entry_to_add>]\n\t[-wait <wait_for_seconds>]\n\t[-repeat <n_times>]\n\t-protoname <comma_separated_protocol_names>\n";
	exit(-1);
}
