...
#       1. To run on ficus to scale  getrpcbyname
#               ./bRpcLookup.pl -ficus -o 1 -n <rpc_name>
#       2. To run on ficus to scale getrpcbynumber
#               ./bRpcLookup.pl -ficus -o 2 -n <rpc_number>
#       3. To run on kudzu to scale getrpcbyname
#               ./bRpcLookup.pl -kudzu -o 1 -n <rpc_name>
#       4. To run on kudzu to scale getrpcbynumber
#               ./bRpcLookup.pl -kudzu -o 2 -n <rpc_number>
...
sub run_benchmark 
{
	...
	for($j=0; $j < $samplingCnt ; $j++) {
		system "./bRpcLookup -r 1 -t $timeToRun -o $fn_no -n $ent{$i} > /tmp/bb";
		$i++;
		if($i > $argCnt -1)  { $i = 0; }
