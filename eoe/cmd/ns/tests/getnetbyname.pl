...
#		./getnetbyname.pl -repeat 100 -networkname <comma_separated_network_names>
#
#	2. Change the timestamp of the /etc/networks file so that it
#	   will cause the cache to be rebuild.
#		./getnetbyname.pl -touch -repeat 100 -networkname <comma_separated_network_names>
#
...
#	 	./getnetbyname.pl -add <comma_separated_network_names> -repeat 100 -networkname <comma_separated_network_names>
#
#	4. Stop the nsd, add new entries and retrive the information.
#		./getnetbyname.pl -add <comma_separated_network_names> -stopnsd -repeat 100 -networkname <comma_separated_network_names>
#
#	5. Wait for timeout and then retrieve the information, this will
#	   cause cache to be rebuild
#		./getnetbyname.pl -wait <time_val_in_seconds> -repeat 100 -networkname <comma_separated_network_names>
#
#	6.Add an arbitrary entry in the /etc/networks file and look for it
#		./getnetbyname.pl -entry <a_network_entry> -networkname <network_name>
...
while(@ARGV) {
  ...
  if(/^-repeat(.*)/) {
	...
	@list_of_networknames = split(/,/, shift(@ARGV));
	$networknameFlg = 1;
  } elsif (/^-entry(.*)/) {
...
print "Start Script:getnetbyname.pl\n";
print "####CMD: ./getnetbyname.pl ", join(' ', @options), "\n";

($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime(time);
#print "Date:/$mon/$mday/$year  ", "Time:$hour:$min:$sec\n";

for($i=0; $i < $repeatCount ; $i++) {
	print STDOUT "==================getnetbyname() - Begin :", $i,"=============\n";
	if($touchflg ) {
	...
	##Call the unit test 
	print  "./getnetbyname ", join(' ', @list_of_networknames), "\n";
        system "./getnetbyname @list_of_networknames";

	print  "./getnetbyname_r ", join(' ', @list_of_networknames), "\n";
        system "./getnetbyname_r @list_of_networknames";
...
#        print STDOUT "Time - user = $luser, system = $lsystem, cuser = $lcuser, csystem = $lcsystem\n";
	...
	$prevCuser = $cuser;
	$prevCsystem = $csystem;
	print STDOUT "==================getnetbyname() - End :", $i,"===============\n";
}
close(STDOUT);
close(STDERR);
open(STDOUT, ">&SAVEOUT");
...
sub print_usage()
{
	print "Usage: ./getnetbyname.pl \n\t[-add <comma_separated_network_names>] \n\t[-touch] \n\t[-stopnsd]\n\t[-entry <entry_to_add>]\n\t[-wait <wait_for_seconds>]\n\t[-repeat <n_times>]\n\t-networkname <comma_separated_network_names>\n";
	exit(-1);
}
