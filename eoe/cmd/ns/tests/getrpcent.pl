...
#	 	./getrpcent.pl -add a,b,c -repeat 100
#
#	4. Stop the nsd, add new entries and retrive the information.
#		./getrpcent.pl -add <comma_separated_protocol_name> -stopnsd -repeat 100
#
#	5. Wait for timeout and then retrieve the information, this will
#	   cause cache to be rebuild
#		./getrpcent.pl -wait <time_val_in_seconds> -repeat 100
#
...
print "####CMD: ./getrpcent.pl ", join(' ', @options), "\n";

($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime(time);
#print "Date:/$mon/$mday/$year  ", "Time:$hour:$min:$sec\n";

for($i=0; $i < $repeatCount ; $i++) {
	print STDOUT "==================Allrpc() - Begin :", $i,"=============\n";
	if($touchflg ) {
