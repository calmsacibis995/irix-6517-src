...
main(int argc, char **argv)
{
	...
	for(i = 0; i < no_of_readers; i++) {
		...
		if(f_no ==1 ) {
			strcpy(pg_name, "gethostbyname()");
			pthread_create(&threadArray[i],
						NULL,
						(void *)myGethostbyname,
						(void*) &(threadValArray[i]));
		}
		else {
			strcpy(pg_name, "gethostbyaddr()");
			pthread_create(&threadArray[i],
						NULL,
						(void *)myGethostbyaddr,
						(void*) &(threadValArray[i]));
		}
	}
...
myGethostbyaddr(void *arg)
{
	...
	while(run) {
		...
		if (!h2) {
			fprintf(stderr, "Thread %d->gethostbyaddr: failed gethostbyaddr: %s\n",th, entName);
		} else {
			threadArg->count++;
		}
	}
