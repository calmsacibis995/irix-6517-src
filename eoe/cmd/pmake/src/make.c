...
static char     *rcsid = "Id: make.c,v 1.61 90/01/05 19:15:22 adam Exp $ SPRITE (Berkeley)";
#endif lint
    ...
    register GNode *gn;	      /* the node to check */
{
	...
	oodate = TRUE;
#ifdef sgi
    } else if (unconditional || (gn->mtime < gn->cmtime) ||
#else
    ...
    register GNode *cgn;	/* the child node */
{
	...
	if (DEBUG(MAKE)) {
	    printf("update time of %s: %s\n", cgn->name, Targ_FmtTime(cgn->mtime));
	}
#ifdef NEVER
	/*
	 * This basically breaks the above comment - we inherit
	 * the childs mtime which means that even though the command
	 * didn't touch the target we will think that the target's time
	 * has been moved forward and will therefore trigger out-of-date
	 * status for targets above it
	 */
	if (cgn->mtime < cgn->cmtime) {
	    if (DEBUG(MAKE)) {
		printf("child newer by %d secs\n", cgn->cmtime - cgn->mtime);
	    }
	    cgn->mtime = cgn->cmtime;
	}
#endif
#endif
    }
...
#ifdef sgi
	} else if (pgn->type & OP_LIB) {
		/*
		 * For OP_LIB we don't check that the child was modified
		 * during this pmake run. This fixes a case where
		 * we have a .o that is newer than the .o in the archive
		 * but the archive's mtime is newer than the .o's mtime
		 * In this case we want to go ahead and archive the .o
		 * cgn->made has been set (cgn in this case is libx(foo.o))
		 * since the arch module understood that it was out of date
		 */
		if (pgn->mtime < cgn->mtime || cgn->made == MADE)
			Var_Append(OODATE, child, pgn);
#endif
	} else if ((pgn->mtime < cgn->mtime) || unconditional ||
