...
{
    return Dir_FindFileSCCS(name,path,0,NULL);
}
char *
Dir_FindFileSCCS (name, path, flag, issccs)
    char    	  *name;    /* the file to find */
    Lst           path;	    /* the Lst of directories to search */
    int		  flag;	    /* whether to scan for SCCS files */
    int	 	  *issccs;  /* set to 1 if found an SCCS file */
#else
char *
Dir_FindFile (name, path)
    char    	  *name;    /* the file to find */
...
#else
    /*
     * If we're checking for SCCS stuff we need to ALWAYS check if an
     * SCCS file exists since we need to construct an implicit
     * dependency if it does.
     */
    if (flag & DIR_FINDSCCS)
	*issccs = 0;
    if ((!hasSlash || (cp - name == 2 && *name == '.'))) {
	if ((flag & DIR_FINDSCCS) && (Lst_Open (sccsSearchPath) != FAILURE)) {
	    /* look in SCCS directory */
	    while ((ln = Lst_Next (sccsSearchPath)) != NILLNODE) {
		p = (Path *) Lst_Datum (ln);
		if (strcmp("SCCS", p->name) == 0)
		    break;
	    }
	    if (ln != NILLNODE) {
		char *sf, *sname, *tsname;

		/*
		 * We may be asked to look up a file of the form: xx.S_C_C_S_
		 * in this case we strip off the .S_C_C_S_ first
		 */
		tsname = Str_New(cp);
		if ((sf = rindex(tsname, '.')) != NULL)
		    if (strcmp(sf, ".S_C_C_S_") == 0)
			*sf = '\0';

		sname = Str_Concat ("s.", tsname, 0);
		free(tsname);
		if (DEBUG(DIR)) {
		    printf("%s...", sname);
		}
		if (Hash_FindEntry (&p->files, (Address)sname) != (Hash_Entry *)NULL) {
		    if (DEBUG(DIR)) {
			printf("in SCCS/\n");
		    }
		    Lst_Close (sccsSearchPath);
		    hits += 1;
		    p->hits += 1;
		    *issccs = 1;
		    /* XXX should we bother prepending 'SCCS/' ??? */
		    file = Str_Concat (p->name, sname, STR_ADDSLASH);
		    free(sname);
		    return (file);
		}
		free(sname);
	...
	if (Hash_FindEntry (&dot->files, (Address)cp) != (Hash_Entry *)NULL) {
	    if (DEBUG(DIR)) {
		printf("in '.'\n");
	    }
	    hits += 1;
...
#ifdef sgi
    register dirent_t *dp;	/* entry in directory */
#else
...
#ifdef sgi
	    while ((dp = readdir (d)) != (dirent_t *) NULL)
#else
...
#endif sun
#if sgi
		/* guess what - '.' && '..' don't have to be first! */
		if (dp->d_name[0] == '.' &&
		    (dp->d_name[1] == '\0' ||
