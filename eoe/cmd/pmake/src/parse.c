...
#ifdef sgi
static unsigned char funny[256];
#endif
/*
...
#ifdef SUNSCCS
GNode *sccsgn;	/* holds command for SCCS transformation */
#endif
...
{ ".NULL",  	  Null,	    	0 },
#ifdef sgi	/* reno */
{ ".OPTIONAL",	  Attribute,   	OP_DONTCARE },
#endif
{ ".ORDER", 	  Order,    	0 },
{ ".PATH",	  Path,		0 },
...
#ifdef sgi
    char     	  *arg1;/* First optional argument for the fmt string */
    char	  *arg2;/* Second optional argument for the fmt string */
#else
...
{
#ifdef sgi
    /* don't let targets that begin with '.' be main targets */
    if (*gn->name == '.' && gn->name[1] != '.' && gn->name[1] != '/')
	return (0);
#endif
	...
	*cp = '\0';
	...
	if (*line == '.' && isupper (line[1])) {
		...
		switch (specType) {
			...
			(void)Lst_AtEnd(paths, (ClientData)sccsSearchPath);
			sccsgn = Targ_FindNode(line, TARG_CREATE);
			sccsgn->type |= OP_NOTMAIN|OP_TRANSFORM;
			(void)Lst_AtEnd(targets, (ClientData)sccsgn);
			Suff_InitSccs();
			break;
#endif
		    case Begin:
...
#ifdef SUNSCCS
	    case Sccs_get:
		/* doesn't matter whether empty or not */
		Lst_ForEach(paths, ParseAddDir, (ClientData)"SCCS");
		break;
#endif
	}
    ...
    register char  *line;	/* the line to check */
{
	 ...
	 * Recognize
	 * $(FOO:.c=.o):foo
	 * foo:$(FOO:c=o) (with no spaces)
	 * foo:$$(FOO:c=o) (with no spaces)
	 * as a dependency lines, not as a variable assignments.
	 */
	} else if (*line == '$') {
	    /*
	     * Call the Var module to parse the puppy so we can safely 
	     * advance beyond it.
	     */
#if 1
	    extern char *Var_Skip();
	    ...
	    result = Var_Parse(line, VAR_CMD, FALSE, &length, &freeIt);
...
static void
#ifdef sgi
ParseDoInclude (file, inclAtBOL, softincl)
    char          *file;        /* file specification */
    Boolean       inclAtBOL;    /* SYSV "include" style (vs #include style) */
    Boolean       softincl;     /* don't complain if file doesn't exist */
#else
ParseDoInclude (file)
    char          *file;	/* file specification */
#endif /* sgi */
{
...
#ifdef sgi
    char	  *orig_file;	/* filename before processing */
#endif
    ...
    if (inclAtBOL) {
	isSystem = FALSE;
	/* Like make(1), find terminating whitespace */
	for (cp = file; *cp && !isspace(*cp); cp++) {
	    continue;
	}
    } else {
#endif /* sgi */
	...
	Parse_Error (PARSE_FATAL,
		     ...
		     (char *)(__psint_t)SPECIAL_CHAR, (char *)(__psint_t)endc);
#else
	Parse_Error (PARSE_FATAL,
		     "Unclosed %cinclude filename. '%c' expected",
		     SPECIAL_CHAR, endc);
    ...
    *cp = '\0';
    ...
    if (strlen(file) == 0) {
	if (!softincl) {
	    Parse_Error (PARSE_FATAL, "Could not find include file '%s'", 
			orig_file);
	}
	return;
    }
...
#ifdef sgi
	/* all these changes are for performance */
	while ((c = ParseReadc ()) != EOF) {
test_char:
	    if (funny[(unsigned char)c]) {
#else
	while (((c = ParseReadc ()) != '\n' || (lastc == '\\')) &&
	       (c != EOF))
	{ /* } */
test_char:
	    ...
	    case '#':
		if (!ignComment) {
#ifdef sgi 	/* "backwards" sysv make recognizes \# */
		    if ((lastc != '\\')) { /* } */
#else
		    if (backwards || (lastc != '\\')) {
#endif
    ...
    FILE *	  stream;   	/* Stream open to makefile to parse */
{
	...
	while (line = ParseReadLine ()) {
		...
		ParseDoInclude(line + 8, TRUE, FALSE);
		goto nextLine;
	    } else if (strncmp(line, "sinclude", 8) == 0 && isspace(line[8])) {
		ParseDoInclude(line + 9, TRUE, TRUE);
		goto nextLine;
	    } else
#endif /* sgi */
...
#ifdef sgi
		    ParseDoInclude (cp + 7, FALSE, FALSE);
#else
		    ParseDoInclude (cp + 7);
#endif
		    goto nextLine;
#ifdef sgi
		} else if (strncmp (cp, "sinclude", 8) == 0) {
		    ParseDoInclude (cp + 8, FALSE, TRUE);
		    goto nextLine;
#endif
...
#ifdef sgi
    /* If a transformation is at the end of say system.mk, we need to
     * close it off
     */
    ParseFinishLine();
#endif
...
Parse_Init ()
{
    ...
    char    	*start;
#ifdef sgi
    static char defsyspath[] = DEFSYSPATH;	/* Avoid faults on read-only string
					 * constant... */
    char *syspath;
#else
...
#ifdef sgi
    syspath = Var_Subst (defsyspath, VAR_CMD, FALSE);
#endif
    /*
...
#ifdef sgi
    free((Address)syspath);
    { char *s;
    for (s = "\n;=#:!"; *s ; ++s)
	funny[(unsigned char)*s] = 1;
    }
#endif
}

