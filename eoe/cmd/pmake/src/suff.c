...
static char     *rcsid = "Id: suff.c,v 1.72 90/03/16 13:31:07 adam Exp $ SPRITE (Berkeley)";
#endif lint
...
#ifdef SUNSCCS
    int		    issccs;	/* found as an SCCS file */
#endif
} Src;
...
#ifdef SUNSCCS
static Suff	    *suffSCCS;	/* The SCCS suffix */
int chksccs;
#endif
...
/*
 * Suff_InitSccs
 * Initialize special SCCS suffix.
 * We use the non-nullness of suffSCCS as an indicator that we
 * should be looking for SCCS files
 */
void
Suff_InitSccs(void)
{
...
#endif
    Suff_AddSuffix(".S_C_C_S_");
    ln = Lst_Find(sufflist, (ClientData)".S_C_C_S_", SuffSuffHasNameP);
    suffSCCS = (Suff *)Lst_Datum(ln);
    chksccs = 1;
}

/*
 * SuffAddSccs
 * Makes sure that the suffSCCS is initialized - we have to do this
 * AFTER all the parsing since we need to look up the commands
 * associated with .SCCS_GET and create a transformation with them
 */
static void
SuffAddSccs(void)
{
    GNode         *gn, *tgn;		/* GNode of transformation rule */

    if (suffSCCS == NULL)
	return;
    gn = Targ_NewGN (".S_C_C_S_");
    gn->type = OP_TRANSFORM;
    (void)Lst_AtEnd (transforms, (ClientData)gn);
    if ((tgn = Targ_FindNode(".SCCS_GET", TARG_NOCREATE)) != NILGNODE) {
	(void) Lst_Concat (gn->commands, tgn->commands, LST_CONCNEW);
    }
}
#endif
    ...
    char          *line;	/* name of transformation to add */
{
    ...
    /*
     * Parse transform line now - in case it is a NULL suffix
     * 'line' will not equal s->name ## t->name
     * This means that later on ApplyTransform won't be able to find it
     */
...
#ifdef sgi
	/*
	 * a .DEFAULT rules with no commands can come through here...
	 */
    ...
    char	  *sname;     /* Name of suffix to mark */
{
	...
	s->flags |= SUFF_INCLUDE;
	    ...
	    Parse_Error (PARSE_WARNING, "suffix %s for .INCLUDES ignored since not in .SUFFIXES list", sname);
#endif
}

/*-
    ...
    char	  *sname;     /* Name of suffix to mark */
{
	...
	s->flags |= SUFF_LIBRARY;
	    ...
	    Parse_Error (PARSE_WARNING, "suffix %s for .LIBS ignored since not in .SUFFIXES list", sname);
#endif
}
...
Suff_ClearLib ()
{
    Lst_ForEach (sufflist, SuffClearLib, (ClientData)NULL);
}
#endif

...
#ifdef SUNSCCS
	    ((junk = Dir_FindFileSCCS (s->file, s->suff->searchPath,
				suffSCCS ? DIR_FINDSCCS : 0,
				&issccs)) != (char *) NULL))
#elif sgi
	    ((junk = Dir_FindFile (s->file, s->suff->searchPath)) != (char *) NULL))
#else
    ...
    Suff    	*s; 	    /* Source suffix */
{
     ...
     */
    if (Lst_IsEmpty(tGn->commands) || amMake) {
...
#ifdef SUNSCCS
	/*
	 * If we found an SCCS file - add a src for it here
	 */
	if (bottom->issccs) {
	    char *file;
	    targ = (Src *)malloc(sizeof(Src));
	    file = Str_Concat(gn->name, bottom->suff->name, 0);
	    targ->file = Str_Concat(file, ".S_C_C_S_", 0);
	    free(file);
	    /*
	    targ->file = Str_Concat("s.", gn->name, 0);
	    targ->file = Str_Concat(targ->file, bottom->suff->name, 0);
	    */
	    targ->suff = suffSCCS;
	    targ->node = NILGNODE;
	    targ->parent = bottom;
	    targ->pref = Str_New(gn->name);
	    targ->issccs = 0;
	    targ->children = 1;
	    bottom = targ;
	}
...
#ifdef SUNSCCS
	    { auto int issccs;
	    gn->path = Dir_FindFileSCCS(gn->name,
				    (targ == NULL ? dirSearchPath :
				     targ->suff->searchPath),
				     chksccs ? DIR_FINDSCCS : 0,
				     &issccs);
	    }
#else
...
#ifdef sgi
	    /* we really have no idea what this is - suffix MUST be set on
	     * return
	     */
	    gn->suffix = NULL;
#endif
...
#ifndef sgi
		/* this is a bug - freeing memory now is a bad idea since
		 * 'bottom' still exists on the 'srcs' list and will be
		 * freed later on
		SuffFreeSrc(bottom);
		*/
#endif
...
#ifdef SUNSCCS
	/*
	 * If targ->issccs then src->suff better be .S_C_C_S_
	 */
	if (!SuffApplyTransform(targ->node, src->node,
			   targ->issccs ? emptySuff : targ->suff, src->suff) &&
#else
	if (!SuffApplyTransform(targ->node, src->node,
			   targ->suff, src->suff) &&
#endif
    ...
    GNode         *gn;	      	/* node we're dealing with */
{
    ...
    if (gn->type & OP_ARCHV) {
	SuffFindArchiveDeps(gn);
    } else if (!sysVmake && (gn->type & OP_LIB)) {
	/*
