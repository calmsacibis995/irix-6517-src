...
static char     *rcsid = "Id: var.c,v 1.59 90/03/12 15:50:11 adam Exp $ SPRITE (Berkeley)";
#endif lint
...
#define VAR_JUNK  	4   	    /* Variable is a junk variable that
				     ...
				     * modified variables */
#ifdef sgi
#define VAR_LOCAL	8	    /* a local sysV modified variable */
#endif
}  Var;

/*-
    ...
    char    *name;
{
    ...
    v = (Var *)malloc(sizeof(Var) + strlen(name) + 1);
    ...
    GNode          *ctxt;	/* context in which to set it */
{
...
#ifdef sgi
    /*
     * For POSIX - SHELL is special - it NEVER affects the
     * environment. We use this opportunity to get the SHELL
     * value and set up the job shell
     * In this case we do NOT set the new value in the environ (per POSIX)
     * XXX this leaks memory if SHELL is set multiple times since
     * Job_ParseShell needs the memory.
     */
    if (*name == 'S' &&
		*(name+1) == 'H' &&
		*(name+2) == 'E' &&
	...
	/* we only want to update the Job Shell if in fact this var setting
	 * sets the highest precedence variable. So we look it up here
	 * So this may well set our shell to what it current is (if
	 * for example the makefile sets SHELL but it has already been
	 * set on the command line
	 * Note that due to POSIX, we never look in the ENVironment
	 */
	v = VarFind ("SHELL", VAR_CMD, FIND_GLOBAL | FIND_CMD);
	if (v == (Var *) NIL)
		v = VarFind ("SHELL", VAR_GLOBAL, FIND_GLOBAL | FIND_CMD);
	val = (char *)Buf_GetAll(v->val, (int *)NULL);
	str = Var_Subst(val, VAR_CMD, 0);
	line = malloc(5 + 1 + strlen(str));
	/* get SHELL=csh to work */
	if (strchr(val, '/') == NULL)
		strcpy(line, "name=");
	else
		strcpy(line, "path=");
...
#else
	/*
	 * let shells beware! we want to be able to pass '+' etc inside
	 * variables
	 */
	setenv(name, val);
#endif
	    ...
	    ((tstr-str) == 4) && (str[3] == 'F' || str[3] == 'D'))
	{
		...
		case '<':
		{
			...
			*tstr = endc;
			return(val);
#else /* sgi */
			/* @F and @D should do modifications like := */
			*freePtr = TRUE;
			v = VarAlloc(&str[1]);
			/*Buf_AddBytes(v->val, strlen(str), (Byte *)str);*/
			v->val = Buf_Init(1);
			str = val;
			v->flags = VAR_LOCAL;
			goto domod;
...
#ifdef sgi
		/* break out here so don't get in infinite loop */
		break;
#endif
	    } else if (termc == ':') {
		*cp++ = termc;
	...
	if ((*str == '$') && (str[1] == '$')) {
		...
		if (oldVars) {
			...
			Parse_Error (PARSE_WARNING,
				     "Undefined variable \"%.*s\"",(char *)(__psint_t)length,str);
#else
			Parse_Error (PARSE_WARNING,
 ...
 * Var_Skip --
 *
 *	Skip over variables in a string. Returns the pointer to
 *	the first character past the variable.
 *
 *	XXX This version is incomplete: it doesn't handle all types
    ...
    char    	  *str;	    	/* The string to parse */
{
	...
	/*
	 * If it's not bounded by braces of some sort, life is much simpler.
	 * We just need to skip 2 characters. 
	 */
	return(++str);
    } else {
	endc = *str == '(' ? ')' : '}';
