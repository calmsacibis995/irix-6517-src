...
#include    <sgidefs.h>
static int gfd;
static int globalFull;
static void initglobaljobs(void);
static void Jobackglobal(void);
static Boolean Jobgetglobal(void);
static void Jobreleaseglobal(void);
static void Job_releaseallglobal(void);
static void Jobcheckglobal();
void Job_exitglobal(void);
#endif
     ...
     */
{
...
#if (defined(sun) && !defined(Sprite)) || defined(SYSV) || defined(sgi)
    TRUE, "set -e", "set +e",
#else
    FALSE, "echo \"%s\"\n", "sh -c '%s || exit 0'\n",
#endif
    ...
    /*
     * KSH description. Echo  & Error control is possible
     */
{
    "ksh", (char *)0,
    TRUE, "set +v", "set -v", "set +v", 6,
    ...
    /*
     * TCSH description. The tcsh can do echo control by playing
     * with the setting of the 'echo' shell variable. Sadly,
     * however, it is unable to do error control nicely.
     */
{
...
#define DBPRINTF(fmt, arg) if (DEBUG(JOB)) printf (fmt, arg); fprintf (job->cmdFILE, fmt, arg)
    ...
    if (!shutUp &&
		...
		(*commandShell->echo == '\0') &&
		commandShell->hasEchoCtl) {
	/* to support ksh which reads .kshrc files which we don't want echoed
         * we always turn on verbose explicitly if we want it
         */
	DBPRINTF ("%s\n", commandShell->echoOn);
    }
#endif
...
#ifdef sgi
    fflush(job->cmdFILE);
    if (ferror(job->cmdFILE)) {
	Punt ("Error writing to shell command file %s (file system full??).",
				tfile);
	/* NOTREACHED */
    }
#endif
...
#if sgi
	if (!usePipes && job->outFile)
	    free((Address)job->outFile);
#endif
	free((Address)job);
...
#ifdef GLOBALJOBS
    while (!errors && !globalFull && !jobFull &&
		(aborting != ABORT_INTERRUPT) && !Lst_IsEmpty(stoppedJobs)) {
#else
    ...
    char    	  **argv;
{
     ...
     */
#ifdef sgi	/* sysv make is less verbose... */
    if (!backwards)
#endif
...
#ifdef sgi
	if (shellPath)
		(void) write (2, shellPath, strlen(shellPath));
	(void) write (2, "\n", 1);
#endif
	_exit (1);
    } else {
    ...
    char	  **argv;
{
    ...
    if (commandShell->noninter)
	argv[argc++] = commandShell->noninter;
    ...
    Job 	  *job;    	/* Job to restart */
{
	    ...
	    )
	{
		...
		return;
#ifdef GLOBALJOBS
	    } else if (!Jobgetglobal()) {
		if (DEBUG(JOB)) {
		    printf("holding - due to global max\n");
		}
		(void)Lst_AtFront(stoppedJobs, (ClientData)job);
		return;
#endif
			       ...
			       * if any. */
{
...
#ifdef sgi
	/* do this on demand to avoid wastful creat/unlink */
	job->cmdFILE = NULL;
#else
     ...
     * NOTE: we create the pipe once for this job, even though we might
     * fork lots of shells (in smake mode). Thus we can't close EITHER
     * end of the pipe until we're done. This means the JobDoOutput must
     * deal with potential hanging if it calls read after the child
     * has died.
     */
    if (!backwards || (job->flags & JOB_FIRST)) {
	if (usePipes) {
	    ...
	    (void)fcntl (job->inPipe, F_SETFL, FNONBLOCK);
...
#if sgi
	    if (job->outFile == NULL)
		job->outFile = malloc(strlen(tfile) + 1 + 3 + 1);
#endif
	    sprintf (job->outFile, "%sJ%03d", tfile, jobno);
	    jobno = (jobno + 1) % 1000;
...
#ifdef GLOBALJOBS
    /*
     * NOTE: if successful, Jobgetglobal allocates a global job slot. It
     * MUST be given back at some time!
     */
    } else if (local && !Jobgetglobal()) {
	if (DEBUG(JOB))
	    printf("Marking %s for restart due to global max exceeded\n",
		job->node->name);
	job->flags |= JOB_RESTART;
	(void)Lst_AtEnd(stoppedJobs, (ClientData)job);
#endif
...
#if OUTDEBUG
	if (logfile == NULL) {
		logfile = fopen("/usr/tmp/PMAKEDEBUG", "a");
		setlinebuf(logfile);
	}
	fprintf(logfile, "Start JobDoOutput: %s\n", job->node->name);
#endif
...
end_loop:
...
#if OUTDEBUG1
	{ int rv;
	if ((rv = ioctl(job->inPipe, FIONREAD, &nRead)) != 0)
		fprintf(logfile, "JobDoOutput(%s):ioctl error %d:%s\n",
			job->node->name, rv, strerror(errno));
	else
		fprintf(logfile, "JobDoOutput(%s):avail %d\n",
			job->node->name, nRead);
	}
#endif
...
#if OUTDEBUG
	fprintf(logfile, "JobDoOutput(%s): nr %d fd %d curpos %d buffull %d\n",
		job->node->name, nRead, job->inPipe, job->curPos, bufFull);
	if (nRead > 0)
		fprintf(logfile, "Contents(%s):<%.*s>\n",
			job->node->name, nRead, &job->outBuf[job->curPos]);
#endif

	/*
	 * The tricky part of this code is that we (the parent pmake) have
	 * the pipe open for write, we can't close it until we've finished
	 * the last command.
	 * However, we do close it just before we call this function
	 * for the last time (with finish==TRUE). So that should be
	 * good enough to guarantee that at least once we can tell the
	 * difference between nothing to read and noone alive..
	 */
	if ((nRead < 0) && errno == EAGAIN) {
	    fd_set ofd;
	    /*
	     * nothing to read, but still a writer - if this is finish==TRUE
	     * then we kind of have to wait until the writer goes away.
	     * We can get in this situation in smake mode when the commands
	     * are placed in the background, since the shell finishes but
	     * not the job. Sigh.
	     */
	    if (!finish)
		    return;
	    FD_ZERO(&ofd);
	    FD_SET(job->inPipe, &ofd);
	    select(job->inPipe + 1, &ofd, NULL, NULL, NULL);
	    goto end_loop;
	} else if (nRead < 0) {
	    if (DEBUG(JOB)) {
	 ...
	 * If last time we had a full buffer, and now we got nothing
	 * we need to add a newline.
	 */
	if ((nr == 0) && ((job->curPos != 0) || bufFull)) {
	...
	for (i = job->curPos + nr - 1; i >= job->curPos; i--) {
		...
		gotNL = TRUE;
		/*
		 * If we get a single newline we want to print it always
		 * if we don't then lines of exactly BUFSIZE don't get
		 * a newline ..
		 */
		if (i == 0) {
			bufFull = FALSE;
	...
	if (!gotNL) {
		...
		/*
		 * if we had a full buffer last time, we printed it but
		 * there isn't a newline - we really need to have one.
		 * We always dump as much as possible since that
		 * will keep as much of the output looking as good as possible
		 * XXX would be nice to delay this .. in case the
		 * next input is for the same job
		 */
		if (bufFull) {
			i = job->curPos;
		        job->outBuf[job->curPos] = '\n';/* XXX don't need */
	...
	if (gotNL || bufFull) {
#if OUTDEBUG
		fprintf(logfile, "Scanning(%s): gotNL %d bufFull %d\n",
			job->node->name, gotNL, bufFull);
		...
		if (commandShell->noPrint) {
			...
			if (cp != &job->outBuf[i]) {
			     ...
			     * command....
			     */
			    cp++; /* skips newline that we always place after
				   * the non-printable command
				   */
#if sgi
			    /* don't mess up printing of multi-line commands */
			    if (!backwards)
#endif
			    while (*cp == ' ' || *cp == '\t' || *cp == '\n') {
				cp++;
		...
		if (*cp != '\0') {
		    ...
		    printf ("%s", cp);
		    /* don't print newline if we're here cause of buffer
		     * full..
		     */
onlynl:
		    if (gotNL)
	...
	if (finish || bufFull) {
	     ...
	     *
	     * If the buffer was full we loop at least once.
	     */
	    goto end_loop;
	}
    } else {
			     ...
			     * be running at once. */
{
    ...
    do {
      if (((tmpdir = getenv("TMPDIR")) != NULL) &&
		(stat(tmpdir, &sb) == 0) &&
		S_ISDIR(sb.st_mode)) {
	tfile = malloc(strlen(tmpdir) + strlen("/make") + 20 + 1);
        sprintf (tfile, "%s/make%d.%d", tmpdir, getpid(), rand());
      } else {
	tfile = malloc(strlen("/tmp/make") + 20 + 1);
        sprintf (tfile, "/tmp/make%d.%d", getpid(), rand());
      }
      ret = stat(tfile, &sb);
    } while (ret != -1 && errno != ENOENT);	/* file exists, try again */
    }
#else
#ifdef Sprite
    ...
    maxJobs = 	  maxproc;
#ifdef sgi	/* ignore -L flag, allow -J to set # of jobs */
    maxLocal = 	  maxproc;
#else
    maxLocal = 	  maxlocal;
#endif
...
#ifdef sgi
    /*
     * catch SIGCHLD - used just to break us out of any pending select
     * we don't do any processing in the CLD handler
     */
    signal(SIGCHLD, JobCld);
#endif
...
{
#ifdef GLOBALJOBS
    return (aborting || jobFull || globalFull);
#else
...
Job_Empty ()
{
	...
	if (!Lst_IsEmpty(stoppedJobs) && !aborting) {
	    ...
	    jobFull = FALSE;
#ifdef GLOBALJOBS
	    while (!globalFull && !jobFull && !Lst_IsEmpty(stoppedJobs)) {
#else
    ...
    char	  *line;  /* The shell spec */
{
    ...
    for (path = (char *)NULL, argc = wordCount - 1, argv = words + 1;
	 ...
	 argc--, argv++) {
		 ...
		 } else if (strncmp (*argv, "noninterFlag=", 13) == 0) {
		     newShell.noninter = &argv[0][13];
		 } else {
				 ...
				 * target should be executed */
{
    ...
    Lst_Close (jobs);
#ifdef GLOBALJOBS
    /* release any global job permits we have */
    Job_releaseallglobal();
#endif
...
#define MAXMAKES 256
#define AREANSIZE (64*1024)
extern int maxUserJobs;
extern int pmake_depth;

/*
 * The idea here is to limit, per user id, the total number of jobs that all
 * makes that the user have fired off.
 *
 * XXX - there are a few things that can easily mess all this up:
 * 1) invoking 2 or makes in the background from within a makefile - this
 *	will cause each child make to do a vsema on the belief that the parent
 *	has used a global slot to invoke the make. This is recoverable -
 *	it just permits too many global jobs
 */
static struct ginfo {
	usema_t *jobs;
} *ginfo;

static int curglobaljobs;
static int outstandingP;	/* debugging */
static int inthebank;
static fd_set waitg;
static int diddepth;
...
initglobaljobs(void)
{
	...
	/*
	 * we need the select that is done when using pipes - for
	 * now don't support global job max unless using pipes
	 */
	if (!usePipes || amMake)
		maxUserJobs = -1;
	if (maxUserJobs < 0)
	...
	usconfig(CONF_INITUSERS, MAXMAKES);
	usconfig(CONF_INITSIZE, AREANSIZE);
#if (_MIPS_SIM == _ABIN32)
	/* yuck - usinit can't mix o32 and n32 */
	sprintf(name, "/usr/tmp/smaken32G%d", getuid());
#else
	sprintf(name, "/usr/tmp/smakeG%d", getuid());
#endif
	if ((up = usinit(name)) == NULL) {
		Error("usinit of global arena failed:%s - ignoring global limit\n",
			strerror(errno));
		maxUserJobs = -1;
		return;
	}
	while ((ginfo = usgetinfo(up)) == NULL) {
		if ((ginfo = usmalloc(sizeof(struct ginfo), up)) == NULL) {
			Punt("usmalloc of ginfo failed\n");
		}
		if ((ginfo->jobs = usnewpollsema(up, maxUserJobs)) == NULL) {
			usfree(ginfo, up);
			Punt("usnewsema failed\n");
		}
		if (uscasinfo(up, NULL, ginfo) == 0) {
			/* may have a race - free up everything and try again */
			usfreesema(ginfo->jobs, up);
			usfree(ginfo, up);
		}
	}
	if ((gfd = usopenpollsema(ginfo->jobs, 0600)) < 0) {
		Punt("usopenpollsema failed\n");
	}
	FD_ZERO(&waitg);

	/*
	 * If we're a descendent of a make, then we don't really want
	 * to count ourselves as a 'job'. Alas, when the parent make invoked
	 * us, they didn't know they were invoking a make and so used up a
	 * precious global slot. We give back a slot if we're a descendent
	 */
	if (pmake_depth > 0) {
		if (usvsema(ginfo->jobs) < 0) {
			Error("Global job semaphore depth-vsema failure\n");
			DieReallyHorribly();
		}
		diddepth = 1;
	}
...
Jobgetglobal(void)
{
	...
	if (outstandingP) {
		/*
		 * we can get here via JobFinish if the target has multiple
		 * commands
		 */
		return FALSE;
	}
	if (inthebank) {
		inthebank = 0;
		curglobaljobs++;
	} else if ((rv = uspsema(ginfo->jobs)) < 0) {
		Error("Global job semaphore psema failure\n");
		DieReallyHorribly();
	} else if (rv == 0) {
		/*
		...
		outstandingP++;
		if (DEBUG(JOB))
			printf("Global Limit Exceeded:maxUserJobs:%d nLocal:%d nJobs:%d\n",
				maxUserJobs, nLocal, nJobs);
		return FALSE;
	} else {
		/* got a job slot */
		curglobaljobs++;
...
/*
 * called when select returns our pollsema fd
 */
static void
Jobackglobal(void)
{
	/*
	 * one can get here with maxUserJobs reset if we were out of
	 * global resources, then got an interrupt, AND we have a .INTERRUPT
	 * handler
	 */
	if (maxUserJobs < 0)
		return;
	if (outstandingP != 1) {
		Error("outstandingP != 1:%d\n", outstandingP);
		DieReallyHorribly();
	}
	outstandingP--;
	FD_CLR(gfd, &outputs);
	globalFull = FALSE;
	if (DEBUG(JOB))
		printf("Global Limit Relieved:maxUserJobs:%d nLocal:%d nJobs:%d\n",
			maxUserJobs, nLocal, nJobs);

	/*
	 * Mark us as having a global resource 'in-the-bank'.
	 * Can't just give back the semaphore here since that can lead
	 * to live-locks when there are lots more smakes than global limits
	 */
	if (inthebank != 0) {
		Error("inthebank != 0:%d\n", inthebank);
		DieReallyHorribly();
...
/*
 * called on each job finish.
 * We attempt to relieve a globalFull situation as aggressively as we can -
 * it is of course
 * possible that someone else gets 'our' slot - that will just cause us
 * to re-queue a job.
 */
static void
Jobreleaseglobal(void)
{
	if (maxUserJobs < 0)
		return;
	if (curglobaljobs <= 0) {
		Error("Jobreleaseglobal:curglobaljobs:%d\n", curglobaljobs);
		/* reset maxUserJobs so when we're called again from
		 * Job_Abort, nothing worse happens!
		 */
		maxUserJobs = -1;
		DieReallyHorribly();
	}
	if (usvsema(ginfo->jobs) < 0) {
		Error("Global job semaphore vsema failure\n");
		DieReallyHorribly();
	}
	curglobaljobs--;
	if (globalFull) {
		...
		tv.tv_usec = 0;
		FD_SET(gfd, &waitg);
		if (select(gfd + 1, &waitg, NULL, NULL, &tv) == 1) {
			Jobackglobal();
		}
	}
	return;
...
static void
Job_releaseallglobal(void)
{
	if (maxUserJobs < 0)
		return;
	if (curglobaljobs < 0) {
		Error("curglobaljobs:%d\n", curglobaljobs);
		return;
	}
	while (curglobaljobs > 0) {
		if (usvsema(ginfo->jobs) < 0) {
			Error("Global job semaphore vsema-all failure\n");
			return;
		}
		curglobaljobs--;
	}
	if (globalFull) {
		...
		outstandingP = 0;
		/*
		 * release sema - since globalFULL was set we must have
		 * requested a slot - if we got here and haven't used it
		 * yet (e.g. due to an interrupt), release it now.
		 */
		usvsema(ginfo->jobs);
	}
	if (inthebank) {
	...
	/*
	 * reset mechanism - if we're called from JobInterrupt we might
	 * end up running another job, lets not subject that to any
	 * global management
	 */
	maxUserJobs = -1;
}
...
{
	if (curglobaljobs != 0) {
		Error("Jobcheckglobal:curglobaljobs %d != 0\n", curglobaljobs);
		return;
	}
}

/*
 * Called on ALL exit points to be sure that if we gave a resource on startup
 * we remove it now - otherwise the global max could slowly increase!
 */
void
Job_exitglobal(void)
{
	...
	if (inthebank) {
		inthebank = 0;
		if (usvsema(ginfo->jobs) < 0)
			Error("Global job semaphore vsema-exit failure\n");
	}
	if (diddepth) {
		if ((rv = uspsema(ginfo->jobs)) < 0)
			Error("Global job semaphore psema-exit failure\n");
		else if (rv == 0) {
			/* couldn't get it immediately - wait for it */
			struct timeval tv;

			tv.tv_sec = 40; /* some safety net */
			tv.tv_usec = 0;
			FD_SET(gfd, &waitg);
			rv = select(gfd + 1, &waitg, NULL, NULL, &tv);
			if (rv == 0) {
				Error("Timed out trying to retire my global slot\n");
			} else if (rv < 0) {
				Error("error:%s while trying to retir emy global slot\n",
					strerror(errno));
			}
		}
		diddepth = 0;
