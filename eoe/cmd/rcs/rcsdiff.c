...
/*
 * $Log: rcsdiff.c,v $
 * Revision 1.6  1997/03/31 21:28:08  msf
 ...
 *
 * Revision 1.2  1997/01/27  22:02:06  msf
 * Add -O option to diff between indicated revision and Older (preceeding) one.
 *
 ...
 *
 * Revision 1.5.1.2  1996/06/28  23:18:55  msf
 * Cast 2nd argument to pairnames to const char **.
 *
 * Revision 1.5.1.1  1996/06/25  19:45:51  msf
...
mainProg(rcsdiffId, "rcsdiff", "$Id: rcsdiff.c,v 1.6 1997/03/31 21:28:08 msf Exp $")
{
    static char const cmdusage[] =
	    "\nrcsdiff usage: rcsdiff -ksubst -q -rrev1 [-rrev2] -O -Vn -xsuff -zzone [diff options] file ...";

    int  revnums;                 /* counter for revision numbers given */
    int  oldrev = 0;		  /* -O (diff against older rev) flag */
    ...
    *diffp++ = DIFF;
	...
	while ((c = *a++)) switch (c) {
	    case 'r':
			...
			case 2: rev2=a;
				if (oldrev) {
				    error("too many revision numbers (-r) to use -O");
				    oldrev = 0;
				}
				break;
	    ...
	    case 'C': case 'F': case 'I': case 'L': case 'W':
			...
			*diffp++ = *argv++;
		    ...
		    break;
	    case 'O':	/* diff against old revision before specified one */
		    oldrev = 1;
		    if (revnums > 1) {
			error("too many revision numbers with -O");
			oldrev = 0;
		    }
		    break;
	    case 'y':
...
#   endif
	...
	for (;  0 < argc;  cleanup(), ++argv, --argc) {
	    ...
	    if (pairnames(argc, (const char **)argv, rcsreadopen, true, false)
			  <=  0)
		    continue;
	    diagnose("===================================================================\nRCS file: %s\n",RCSname);
	    if (!rev2 && !oldrev) {
		...
		*diff_label1 = setup_label(&labelbuf[0], target->num, target->date);
	    ...
	    if (revnums==2) {
		    ...
		    if (countnumflds(numericrev.string) < 3) 
			    target = target->next;

		    else { /* previous revision is in gendeltas entry before
			    * last one
			    */
			    for(target = NULL; gendeltas->rest;
					gendeltas = gendeltas->rest) {
				target = gendeltas->first;
			    }
		    }
		    if (!target) {
			    error("No revision prior to %s", xrev1);
			    continue;
		    }
		    xrev2 = target->num;
		    if (no_diff_means_no_output && xrev1 == xrev2)
