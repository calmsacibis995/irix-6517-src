...
/*
 * $Log: rcsutil.c,v $
 * Revision 1.9  1997/03/31 21:29:35  msf
 ...
 *
 * Revision 1.3  1997/03/07  19:49:42  pj
 ...
 *
 * Revision 1.2  1997/01/27  22:24:38  msf
 * Allow running of asynchronous children - runv_async() (like runv()),
 * trap SIGCHLD, add rcswaitpid() (like waitpid(), but handles
 * child exit status even if child exited, and waited on, before thie
 * routine called); add rcswait1() (wait for one child, any child - used in loop
 * to see if flag has changed value)
 * Add sigsetchld() (allow caller to specify callback upon catching SIGCHLD)
 *
 ...
 *
 * Revision 1.8.1.3  1996/07/25  00:37:50  msf
 * Add fastread(), which will copy part (rather than all) of an Ifile
 * to a stream.
 *
 * Revision 1.8.1.2  1996/06/29  00:34:10  msf
 * Changes for RCS library:
 * 1) Add hstr_save() - save a string using an arbitrary heap
 * 2) Enhance catchsigaction() to call user's signal handler (rcsusrhdlr)
 *    if nonnull.
 * 3) Add stream output routines - swrite(): write to stream
 * 				fast2stream(): like fastcopy, but to stream
 * 4) Enhance write_stderr to use finform, rather than stderr.
 *
 * Revision 1.8.1.1  1996/06/25  18:41:15  msf
...
libId(utilId, "$Id: rcsutil.c,v 1.9 1997/03/31 21:29:35 msf Exp $")

static int (*sigchldprocess) P((pid_t, int)) = NULL;  /* called when SIGCHLD */
static pid_t last_pid = 0;    /* pid of synchronous child that died too fast */
static int last_w;	      /* wait(2) status of above child process */
static int sigchldnum = 0;    /* number of pending (held) SIGCHLDs */
	...
	size_t n;
{
	...
	pid_t
rcswaitpid(pid, statptr, options)
	pid_t pid;
	int *statptr;
	int options;
/* Like waitpid, except that it can handle the case where the child
 * died before routine is even called.  (In that case, the signal
 * handler records the fact, along with the child exit code, and this
 * routine merely checks that data, with interrupts ignored.)
 */
{
	pid_t w;
	ignoreints();	/* ignore SIGCHLD while waiting for specific child */

	if (last_pid == pid) {	/* caught child dying before this fcn called */
	    *statptr = last_w;
	    last_pid = 0;
	    w = pid;
	}
...
#if !has_waitpid
	    for(;;) {
		if ((w = wait(statptr)) < 0) break;
		if (w == pid || pid == -1) break;
		sigchldnum--;
#ifdef DEBUG
		printf("rcswaitpid decremented sigchldnum to %d\n", sigchldnum);
#endif
		if (sigchldprocess) 
		    (*sigchldprocess)(pid, *statptr);
	    }
#else
	    w = waitpid(pid, statptr, options);
#endif
	    if (--sigchldnum < 0) sigchldnum = 0;
#ifdef DEBUG
	    printf("rcswaitpid last reduced sigchldnum to %d\n", sigchldnum);
#endif
	}
	restoreints();	/* process any pending interrupt */
	return w;
}
...
rcswait1 P((void))
{
	...
	int stat;
	ignoreints();
	if ((w = wait(&stat)) < 0) return;
	if (sigchldprocess) (*sigchldprocess)(w, stat);
	if (--sigchldnum < 0) sigchldnum = 0;
#ifdef DEBUG
	printf("rcswait1 (waithdl) decremented sigchldnum to %d\n", sigchldnum);
#endif
	restoreints();
}
...
hstr_save(heap, s)
	struct rcsHeap *heap;
	char const *s;
/* Save s in storage that is obtained from the specified heap */
{
	size_t len;
	char *ret;

	len = strlen(s)+1;
	return memcpy(okalloc((malloc_type)rcsHeapAlloc(heap, len)),s, len);
}
#endif /* RCSLIBRARY */
...
signal_type (*rcsusrhdlr) P((int)) = NULL;	/* user-provided handler */
...
#         if bad_wait_if_SIGCHLD_ignored
	    if (s == SIGCHLD) VOID signal(SIGCHLD, SIG_DFL); 
	    else
#         endif
...
#	endif
	if (s == SIGCHLD) sigchldnum++;
#ifdef DEBUG
	if (s == SIGCHLD)
	    printf("catchsig increaed sigchldnum to %d\n", sigchldnum);
#endif
	return;
    }
	...
	/* Harvest all outstanding SIGCHLDs */

	/* Note: This routine may have been called by the signal handler
	 * or by restoreints(). 
	 *
	 * In the former case, we have sigchldnum pending SIGCHLDs, plus
	 * one for the current SIGCHLD which trapped to here.
	 *
	 * In the latter case, we have only sigchldnum SIGCHLDs (no current
	 * SIGCHLD trapped).   This is handled by restoreint(), which
	 * automatically decrements sigchldnum before calling us.   That
	 * way, we always wait() sigchldnum + 1 times.
	 */
	do {
	    p = wait(&w);
	    if (p < 0) {
		if (errno != ECHILD ) efaterror("wait");
		break;
	    }
	    /* If not an expected (async) child, may be a synchronous
	     * child we haven't gotten around to waiting for (died too
	     * fast).  We'll remember it here.
	     */
	    if (!sigchldprocess || !(*sigchldprocess)(p,w)) {
		last_pid = p;
		last_w = w;
	    }
#ifdef DEBUG
	    printf("catchsig decremented sigchldnum to %d\n", sigchldnum-1);
#endif
	...
	restoreints();
#if !has_sigaction
	/* reset handler - may harvest any SIGCHLD that OS held */
...
restoreints()
{
	if (!--holdlevel && heldsignal) {
	    if (heldsignal == SIGCHLD) sigchldnum--;  /* not raising NEW sig */
#ifdef DEBUG
	    printf(
		"restoring to holdlevel = %d, heldsig = %d, sigchldnum = %d\n",
		holdlevel, heldsignal, sigchldnum);
#endif
#	    ifdef SA_SIGINFO
...
#	endif
	...
	int
fast2stream (writeMethod, outStream, inf)
	size_t (*writeMethod)(RCSusrcontext *, const void *, size_t);
	RCSusrcontext *outStream;
	register RILE *inf;
/* Function: copies the remainder of file inf to output stream.
 * Returns: 0 (success), negative error codes.
 */
{
	int rc;

/* write n bytes from buf to stream os with write method wm.
 * "returns" rc ERR_WRITE_STREAM (failed to write), or 0 (success)
 */
#define sswrite(wm, os, buf, n) \
	{ int nbytes = n; const char *ptr = (char *)buf; \
	  for(; nbytes > 0 && (rc = wm(os, ptr, nbytes)) > 0; \
                nbytes -= rc, ptr += rc); \
	  rc = nbytes > 0 ? ERR_WRITE_STREAM : 0; \
	}
...
#if large_memory
#	if maps_memory
	    sswrite(writeMethod, outStream,
		   (char const*)inf->ptr, (size_t)(inf->lim - inf->ptr));
	    inf->ptr = inf->lim;
	    return rc;
#	else
	    for (;inf->ptr != inf->lim; Igetmore(inf)) {
		sswrite(writeMethod, outStream,
		       (char const*)inf->ptr,(size_t)(inf->readlim - inf->ptr));
		inf->ptr = inf->readlim;
		if (rc < 0) return rc;
	    }
	    return 0;
#	endif
	...
	while (!feof(inf)) {
		...
		sswrite(writeMethod, outStream, buf, (size_t)rcount);
		if (rc < 0) return rc;
        }
	return 0;
#endif
	...
	int
fastread (writeMethod, outStream, inf, nbytes)
	size_t (*writeMethod)(RCSusrcontext *, const void *, size_t);
	RCSusrcontext *outStream;
	register RILE *inf;
	int nbytes;
/* Function: responds to request for the next nbytes of data in the inf file,
 *	     by writing at most nbytes of data (not past EOF) to the stream,
 *	     returning the number of bytes of data written (and read),
 *	     advancing inf past the number of bytes returned.
 *
 * Returns:  number of bytes read (<= nbytes).
 *	     0 in case of EOF (otherwise, at least one byte is written)
 *	     ERR_WRITE_STREAM in case the output stream cannot accept nbytes
 *	     negative number in case of other errors
 */
{
	int rc;
	size_t num;	/* number of bytes to read (min of nbytes and fsize) */

#if large_memory
	    if ((num = inf->lim - inf->ptr) > nbytes) num = nbytes;

#	if maps_memory
	    sswrite(writeMethod, outStream, (char const*)inf->ptr, num);
	    inf->ptr += num;
	    return (rc < 0 ? rc : num);
#	else
	{
	    size_t numwrite,	/* number to write on this iteration */
		   numblock,	/* number read into this block */
		   numleft;	/* number of bytes left to write */
	    for (numleft = num; inf->ptr != inf->lim; Igetmore(inf))
	    {
		/* Safety - in case we have exhausted current buffer */
		if (!(numblock = (size_t)(inf->readlim-inf->ptr))) continue;

		/* Write min(bytes in buffer block, remaining requested bytes)*/
		numwrite = numblock > numleft ? numleft : numblock;
		sswrite(writeMethod, outStream, (char const*)inf->ptr,numwrite);

		/* Adjust RILE pointer */
		inf->ptr += numwrite;
		if (rc < 0) return rc;

		/* Adjust count of bytes left to write */
		if (!(numleft -= numwrite)) break;
	    }

	    /* Number read is requested number, less number past EOF */
	    return rc < 0 ? rc : num - numleft;
	}
#	endif
#else
    {
	...
	while (!feof(inf)) {
		...
		/* Read in requested bytes, but not more than buffer size */
		rcount = num < sizeof(buf) ? num : buf;
		if (!(rcount = Fread(buf,sizeof(*buf),rcount,inf))) {
			testIerror(inf);
			return nbytes - num;
		}

		/* Write the partial result to the stream */
		sswrite(writeMethod, outStream, buf, (size_t)rcount);
		if (rc < 0) return rc;

		/* Adjust remaining bytes to read */
		if (!(num -= rcount)) return nbytes;
        }
	return nbytes - num;
    }
#endif
	...
	FILE *f;
{
	...
	int
swrite(wm, os, buf, n)
	size_t (*wm)P((RCSusrcontext *, const void *, size_t));
	RCSusrcontext *os;
	const char *buf;
	size_t n;
{
	size_t nwritten;
	for (;; n-= nwritten, buf += nwritten) {
	    if ((nwritten = wm(os, buf, n))<= 0) return ERR_WRITE_STREAM;
	    if (n <= nwritten) return 0;
	}
}
	...
	char const *s;
{
	...
	if (write(fileno(finform), s, slen) != slen)
		_exit(EXIT_TROUBLE);
}
#endif
...
#		endif
	...
	if (pid < 0)
		efaterror("fork");
	if (rcswaitpid(pid, &wstatus, 0) < 0 && errno != ECHILD)
		efaterror("waitpid");
#else
	static struct buf b;
	char const *p;
...
/*
 * Run a command, but do not wait for results.   Only works if has_fork.
 * infd, if not -1, is the input file descriptor.
 * outname, if nonzero, is the name of the output file.
 * [if null, stdout should be closed or sent to /dev/null; not implemented yet.]
 * errname, if nonzero, is the name of the stderr file.
 * if null, is sent to /dev/null.
 * args[1..] form the command to be run; args[0] might be modified.
 */
	int
runv_async(infd, outname, errname, args)
	int infd;
	char const *outname, *errname, **args;
{
#if !has_fork
	return ERR_BAD_FORK;
#else
...
#		    endif
		...
		if (!errname) errname = "/dev/null";
		if (fdreopen(
			STDERR_FILENO, errname,
			O_CREAT | O_TRUNC | O_WRONLY
		    ) < 0) {
			/* Avoid perror since it may misuse buffers.  */
			write_stderr(args[1]);
			write_stderr(": ");
			write_stderr(errname);
			write_stderr(": cannot create\n");
			_exit(EXIT_TROUBLE);
		}
