...
/*
 * $Log: rcsfnms.c,v $
 * Revision 1.12  1997/03/31 21:28:18  msf
 ...
 *
 * Revision 1.4  1997/03/20  20:45:50  pj
 * * Replaced the "bool" type in librcs with "rbool", to avoid conflict
 *   with the future C++ keyword "bool".
 *
 * Revision 1.3  1997/03/05 18:04:12  msf
 * Bug fix - async checkout sometimes failed, because errno wasn't initialized
 * to zero.
 *
 * Revision 1.2  1997/01/27  22:08:47  msf
 * Centralize waiting for children to complete in rcsutil.c (do not do
 * a wait()/waitpid() in other files).
 * Enable pairnames() to simply locate the RCS file and stat() it
 * without opening it.   It will do this if given a NULL rcsopen() fcn pointer.
 *
 ...
 *
 * Revision 1.11.1.3  1996/07/10  19:21:08  msf
 * Add call to rcsParseReset() when reusing an archive file (to reinitalize
 * the "selector" field of deltas, which is ordinarily initialized when
 * parsing).  Set rcsSelectorChg to false, indicating that no further
 * resetting needs to be done.
 *
 * Revision 1.11.1.2  1996/06/28  23:52:20  msf
 * Changes for RCS library:
 * 1) Add rcsreopen() - converts an archive from opened for read, to opened
 *                      for update
 * 2) Generalize handling of temp files - instead of 5 temp files, which
 *    must be carefully coordinated, provide a pool of temp files which
 *    the library may use as its "virtual temp files"
 *
 * General cleanup - remove unused variables, change pointers to const ptrs.
 *
 * Revision 1.11.1.1  1996/06/25  19:42:47  msf
...
libId(fnmsId, "$Id: rcsfnms.c,v 1.12 1997/03/31 21:28:18 msf Exp $")
...
char *volatile tpnames[TEMPNAMES+TMPRESERVED];
...
#	else
	    static char tpnamebuf[TEMPNAMES+TMPRESERVED][L_tmpnam];
	    p = tpnamebuf[n];
	    if (!tmpnam(p) || !*p)
#		ifdef P_tmpdir
 ...
 */
{
	...
	for (i = TEMPNAMES+TMPRESERVED;  0 <= --i;  )
	    if ((p = tpnames[i])) {
		VOID unlink(p);
		/*
 ...
 *
 * If RCSOPEN is NULL, don't set finptr; just stat the file (into RCSstat),
 * and set RCSbuf to the name, RCSerrno to errno.
 *
 ...
 */
{
	...
	if (rcsopen) {
	    ...
	    interesting = !stat(RCSb.string, &RCSstat) || errno != ENOENT;
	}
 ...
 *
 * If no rcsopen() routine is provided, the file is not opened;
 * pairnames() simply returns after computing the filenames and stat.
 *
 ...
 */
{
	...
	register const char *p, *arg, *RCS1;
	...
	if ((x = rcssuffix(arg)))
	{
		...
		if (
			...
			workname = tempbuf.string;
			tempbuf.string[baselen] = 0;
		}
        } else {
	...
	if (RCSbase!=RCS1) {
		...
		bufscpy(&RCSbuf, RCS1);
		errno = 0;
		if (rcsopen) finptr = (*rcsopen)(&RCSbuf, &RCSstat, mustread);
		else stat(RCSbuf.string, &RCSstat);
		RCSerrno = errno;
        } else {
		int found;
		bufscpy(&RCSbuf, "");
		...
		if (RCS1)
			...
			found = fin2open(arg, (size_t)0, RCSbase, baselen,
			...
			while (! fin2open(arg, dlen, base, baselen,
				...
				x += xlen;
				if (!*x++) {
					found = 0;  /* oh, well; no file */
					break;
				}
			}
		if (!found) RCSerrno = ENOENT;
		}
        }
	RCSname = p = RCSbuf.string;
	if (!rcsopen) {
		if (!RCSerrno) return 1;
		else return -1;
	}
	if (finptr) {
...
rcsreopen(mustexist)
	rbool mustexist;
{
	/* A streamlined pairname(), to convert a single archive,
	 * already opened for read, into one opened for update.
	 * (Primarily entails creating the ,file, lockfile.)
	 */

	struct stat sbuf;

	/* Get name of "cached" file from RCSname */
	if (!RCSname) return ERR_INTERNAL;

	/* Create the lockfile, but do not reopen the RCS file (already open) */
	if (rcswriteopen(&RCSbuf, &sbuf, mustexist|0x2)) { /* RCS file exists */
	    if (!S_ISREG(sbuf.st_mode)) return ERR_ARCHIVE_CHANGED;

	    if (same_file(sbuf, RCSstat, 0) && same_attr(sbuf, RCSstat, 0)) {
					/* file not changed; don't reopen */

		if (rcsSelectorChg) rcsParseReset();
		return 0;
	    }

	    /* File has changed; close and reopen */
	    Izclose(&finptr);
	    finptr = Iopen(RCSname, FOPEN_RB, &RCSstat);
	    if (finptr) {
	       Lexinit();		/* reparse from the beginning */
	       getadmin();
	        return 0;
	    }
	}
	if (errno == EEXIST) return ERR_ARCHIVE_INUSE;
	if (errno == ENOENT && mustexist) return ERR_NO_ARCHIVE;
	if (errno != ENOENT || fdlock < 0) return ERR_INTERNAL;
...
#				endif
	...
	if (0 <= child) {
		...
		if (rcswaitpid(child, &wstatus, 0) < 0)
			wstatus = 1;
	}
	if (!fp) {
		VOID close(fd[0]);
