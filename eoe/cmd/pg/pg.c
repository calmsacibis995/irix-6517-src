...
register char *s;
{
	...
	(void) pfmt(stderr, MM_ERROR, ":392:Prompt too long\n");
	exit(1);
}
	...
	for (;;) {
		...
		if (clropt) {
			...
			if (start < old_ss.first_line)
				sopr(":393", "...skipping backward\n",0);
			else
			if (start > old_ss.last_line + 1)
				sopr(":394", "...skipping forward\n",0);
		}
	...
	for (;;) {
			...
			*/
			...
			if (setjmp(restore) != 0) {
				...
				if ((sf=fopen(cmdptr,"w")) == NULL) {
					error(":396", "Cannot open %s: %s",
						cmdptr,	strerror(errno));
					break;
				}
				kill_line();
				sopr(":397", "saving file ",1);
				sopr(NULL, cmdptr,1);
				(void) fflush(stdout);
				save_input(sf);
				error(":398", "saved");
		...
		case '!':       /* shell escape */
			if (rmode) {	/* restricted mode */
				(void) pfmt(stderr, MM_ERROR, ":399:!command not allowed in restricted mode.\n");
				break;
			}
			...
			if ((id = fork ()) < 0) {
				error(":400", "fork() failed, try again later");
				break;
			}
			if (id == (pid_t)0) {
				/*
...
help()
{
	...
	(void) fputs(dashes, stdout);
	(void) pfmt(stdout, MM_NOSTD, ":401:  h                     help\n");
	(void) pfmt(stdout, MM_NOSTD, ":402:  q or Q                quit\n");
	(void) pfmt(stdout, MM_NOSTD, ":403:  <blank> or <newline>  next page\n");
	(void) pfmt(stdout, MM_NOSTD, ":404:  l                     next line\n");
	(void) pfmt(stdout, MM_NOSTD, ":405:  d or <^D>             display half a page more\n");
	(void) pfmt(stdout, MM_NOSTD, ":406:  . or <^L>             redisplay current page\n");
	(void) pfmt(stdout, MM_NOSTD, ":407:  f                     skip the next page forward\n");
	(void) pfmt(stdout, MM_NOSTD, ":408:  n                     next file\n");
	(void) pfmt(stdout, MM_NOSTD, ":409:  p                     previous file\n");
	(void) pfmt(stdout, MM_NOSTD, ":410:  $                     last page\n");
	(void) pfmt(stdout, MM_NOSTD, ":411:  w or z                set window size and display next page\n");
	(void) pfmt(stdout, MM_NOSTD, ":412:  s savefile            save current file in savefile\n");
	(void) pfmt(stdout, MM_NOSTD, ":413:  /pattern/             search forward for pattern\n");
	(void) pfmt(stdout, MM_NOSTD, ":414:  ?pattern? or\n");
	(void) pfmt(stdout, MM_NOSTD, ":415:  ^pattern^             search backward for pattern\n");
	(void) pfmt(stdout, MM_NOSTD, ":416:  !command              execute command\n");
	(void) putc('\n', stdout);
	(void) pfmt(stdout, MM_NOSTD, ":417:Most commands can be preceeded by a number, as in:\n");
	(void) pfmt(stdout, MM_NOSTD, ":418:+1<newline> (next page,stdout); -1<newline> (previous page); 1<newline> (page 1).\n");
	(void) putc('\n', stdout);
	(void) pfmt(stdout, MM_NOSTD, ":419:See the manual page for more detail.\n");
	(void) fputs(dashes, stdout);
}
...
register int nskip;
{
	if (fnum + nskip < 0) {
		nskip = -fnum;
		if (nskip == 0)
			error(":420", "No previous file");
	}
	else
	if (fnum + nskip > nfiles - 1) {
		nskip = (nfiles - 1) - fnum;
		if (nskip == 0)
			error(":421", "No next file");
	}
	return(nskip);
}
...
register char *fs;
{
	...
	if ((stbuf.st_mode & S_IFMT) == S_IFDIR) {
		if (f_was_opened)
			(void) fclose(f);
		(void) pfmt(stderr, MM_ERROR, ":422:%s is a directory\n",fs);
		return ((FILE *)NULL);
	}
	if ((stbuf.st_mode & S_IFMT) == S_IFREG) {
		...
		if (f != stdin) {
			if (f_was_opened)
				(void) fclose(f);
			(void) pfmt(stderr, MM_ERROR, ":423:Special files only handled as standard input\n");
			return((FILE *)NULL);
		}
		else {
			(void) mktemp(tmp_name);
			if ((fd=creat(tmp_name,(S_IRUSR|S_IWUSR))) < 0) {
			    (void) pfmt(stderr, MM_ERROR, ":424:Cannot create temp file: %s\n",
			    	strerror(errno));
			    return((FILE *)NULL);
			}			
			(void) close(fd);
			if ((tmp_fou = fopen(tmp_name, "w")) == NULL) {
				(void) pfmt(stderr, MM_ERROR,":425:Cannot get temp file for writing: %s\n",
					strerror(errno));
				return((FILE *)NULL);
			}
			if ((tmp_fin = fopen(tmp_name, "r")) == NULL) {
				(void) pfmt(stderr, MM_ERROR, ":426:Cannot get temp file for reading: %s\n",
					strerror(errno));
				return((FILE *)NULL);
			}
			pipe_in = 1;
...
int i;
{
	...
	static struct messages {
		...
		":427",	"Pattern not found",				1,
		":173",	"Range endpoint too large",			11,
		":177",	"Bad number",					16,
		":186",	"`\\digit' out of range",			25,
		":199",	"No remembered search string",  		41,
		":200",	"'\\( \\)' imbalance",				42,
		":201",	"Too many `\\(' s",				43,
		":202",	"More than two numbers given in '\\{ \\}'",  	44,
		":203",	"'\\}' expected",				45,
		":204",	"First number exceeds second in '\\{ \\}'",  	46,
		":207",	"'[ ]' imbalance",				49,
		":208",	"Regular expression overflow",			50,
		":224",	"Illegal byte sequence",			67,
		":209",	"Regular expression error",			0
		};

	for (j = 0; re_errmsg[j].number != 0; j++ )
		if (re_errmsg[j].number == i )
			break;
	error(re_errmsg[j].messageid, re_errmsg[j].message);
...
save_pipe()
{
	if (!doliseof)
		while (fgetputc(stdin) != EOF)
			if (brk_hit) {
				brk_hit = 0;
				error(":428", "Piped input only partially saved");
				break;
			}
	(void) fclose(tmp_fou);
}
...
compact()
{
	(void) pfmt(stderr, MM_ERROR, ":429:Out of memory - line %d\n",dol->l_no);
	end_it();
}
...
terminit()	/* set up terminal dependencies from termlib */
{
	...
	if ((freopen("/dev/tty","r+",stdout)) == NULL) {
		(void) pfmt(stderr, MM_ERROR, ":430:Cannot reopen stdout: %s\n",
			strerror(errno));
		exit(1);
	}
...
error(messid, mess, a1, a2, a3)
char *messid, *mess, *a1, *a2, *a3;
{
	char buf[100];
	(void)sprintf(buf, gettxt(messid, mess), a1, a2, a3);
	kill_line();
	sopr(NULL, buf,1);
	prompt((char *) NULL);
	errors++;
...
char *filename;
{
	...
	int pagenum;
	if (filename != NULL) {
		sopr(":431", "(Next file: ",1);
		sopr(NULL, filename,1);
		sopr(NULL, ")",1);
	}
	else {
