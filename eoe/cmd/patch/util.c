...
/* Append a file, only used if specified -o output file */

int append_file(from, to)
char *from, *to;
{
    Reg1 int fromfd;
    Reg2 int tofd;
    ...
    tofd = open(to, O_WRONLY | O_APPEND);
    if (tofd < 0) 
      pfatal2("internal error, can't reopen %s", to);
    while ((i=read(fromfd, buf, sizeof buf)) > 0)
      if ((test = write(tofd, buf, i)) != i)
