/* $Header: /proj/irix6.5.7m/isms/eoe/cmd/patch/RCS/pch.c,v 1.1 1996/01/17 02:44:58 cleo Exp $
 *
 * $Log: pch.c,v $
 * Revision 1.1  1996/01/17 02:44:58  cleo
 * initial checkin.
 *
 * Revision 1.4  1995/11/06  23:42:11  lguo
 * Added a new function lookup_imtable() to look for the original
 * file to be patched. If the file is already in im_table, it means
 * an intermediate file already exists and we will do the patch on
 * the intermediate file, not the original file.
 *
 * Revision 1.3  1995/10/11  18:35:31  lguo
 * No Message Supplied
 *
...
static struct intermediate *lookup_imtable (char *);
...
intuit_diff_type()
{
    ...
    struct intermediate *entry;
    Reg12 bool have_name = TRUE;
	...
	else if (strnEQ(s, "Index:", 6)){       /* add it to the intermediate
						   table structure */
	    indtmp = savestr(s+6);
	    /* look for indtmp in im_table and if found, patch it on the
	       intermediate file */
	    if(output){
		if((entry = lookup_imtable(indtmp)) == NULL){
		    /* insert new entry */
		    entry = (struct intermediate *)malloc(sizeof(struct intermediate));
		    if (entry == NULL)
		      pfatal1("No memory available.\n");
		    entry->next = im_table;
		    im_table = entry;
		    entry->orig = savestr(indtmp);
		    have_name = FALSE;
		} 
  ...
  scan_exit:
    if (no_filearg) {
	if (indtmp != Nullch && (!output || !have_name))
	...
	if (oldname && newname) {
	    ...
	    filearg[0] = savestr(newname);
	else if (indname && (!output || !have_name))
...
/* look up the intermediate table for file "file_to_be_patched" */
struct intermediate *
lookup_imtable(file_to_be_patched)
char *file_to_be_patched;
{
    struct intermediate *imp;
    char *s;

    for (imp = im_table; imp; imp = imp->next){
	if (!strcmp(file_to_be_patched, imp->orig)){
	    filearg[0] = savestr(imp->interm);
	    return imp;
	}
    }
