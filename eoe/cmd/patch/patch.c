char rcsid[] =
	"$Header: /proj/irix6.5.7m/isms/eoe/cmd/patch/RCS/patch.c,v 1.1 1996/01/17 02:44:48 cleo Exp $";
 ...
 *
 * $Log: patch.c,v $
 * Revision 1.1  1996/01/17 02:44:48  cleo
 * initial checkin.
 *
 * Revision 1.5  1995/11/07  16:42:14  lguo
 * After the intermediate file is used, we remove the file.
 *
 * Revision 1.3  1995/10/11  18:35:17  lguo
 * No Message Supplied
 *
...
char **argv;
{
    ...
    /* initialize the intermediate table */
    im_table = NULL;              
...
"%seversed (or previously applied) patch detected!  %s -R? [y] ",
	...
	if (!skip_rest_of_patch) {
	    ...
	    /* if no output file is specified, or if it's the first patch,
	     * create the new file; otherwise, append the output file */
	    if (!output || first){
		...
		if (output)
		  copy_file(outname, TMPOUTNAME);
		first = FALSE;
	    } else {
		append_file(TMPOUTNAME, outname);
		chmod(outname, filemode);
	    }
...
reinitialize_almost_everything()
{
	...
	strcpy(temp, filearg[0]);
	if(!strcmp(tmpdir, dirname(temp)))
	    unlink(filearg[0]);
	free(filearg[0]);
	filearg[0] = Nullch;
    }

    if (!output && outname != Nullch) {
	free(outname);
	outname = Nullch;
    }
...
static char *shortopts = "-bB:cd:D:eEfF:i:lnNo:p:r:RsStuvV:x:";
