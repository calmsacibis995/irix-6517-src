...
/*
 * kill_mac.c -- MAC read tests for Kill() system call.
 */
static int kill_mac1(struct macinfo_2 *parmsptr);
...
kill_mac(void)
{
    ...
    for (i = 0; i < ncases; i++) {
	flush_raw_log();   
	switch (kill_mac1(&kill_minfo[i]) ) {
	/*
	 * Write formatted result to raw log.
	 */    
	case PASS:      /* Passed */
	    RAWPASS(kill_minfo[i].casenum, kill_minfo[i].casename);
	    break;
	case FAIL:      /* Failed */
	    RAWFAIL(kill_minfo[i].casenum, kill_minfo[i].casename,
		    kill_mdesc[i]);
	    fail = 1;
	    break;
	default:
	    RAWINC(kill_minfo[i].casenum, kill_minfo[i].casename);
	    incomplete = 1;
	    break;  /* Incomplete */
	}
	flush_raw_log();
...
static int
kill_mac1(struct macinfo_2 *parmsptr)
{
    register char i;             /* Wait loop index. */
	...
	/*
	 * Here's the test: Call Kill with O's pid as
	 * arg1.
	 */
	if (parmsptr->c_flag == 0) {
		retval = kill(forkval1, SIGINT);
	}else{
		retval = cap_kill(forkval1, SIGINT);
	}

	 /*
	  * Write to the pipe so O can go away.
	  */
	if ( (write(StoO_fd[1], buf, sizeof(buf)))
	    != (sizeof(buf)) ) {
	 ...
	 */
	if (  ( retval = kill(forkval1, SIGINT) ) != -1 ){
	    w_error(PRINTNUM, namestring, err[TEST_RETVAL], retval);
	    exit(FAIL);
	}
