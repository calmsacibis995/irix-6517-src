...
semget_obr1(void)
{
	...
	int semid;                   /* Returned by semget create call. */
	key_t key;		     /* Key value. */
	char namestring[MSGLEN];     /* Used in error messages. */

	srand(getpid());
	key = rand();
	if ((semid = semget(key, 1, 00666|IPC_EXCL|IPC_CREAT)) == -1) {
	    w_error(SYSCALL, namestring, err[F_SEMGET], errno);
	    return(INCOMPLETE);
	}
	...
	sbuf[0].sem_flg = 0;
	/*
	 * Semop should return zero.
	 */
	if (semop(semid, sbuf, 1) < 0) {
		w_error(SYSCALL, namestring, err[F_SEMOP], errno);
		return(INCOMPLETE);
	}
	return(PASS);
}
