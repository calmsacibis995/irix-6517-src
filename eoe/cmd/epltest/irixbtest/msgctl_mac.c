...
/*
 * msgctl_mac.c -- MAC read tests for Msgctl() system call.
 */
static int msgctl_mac1(struct macinfo_2 *parmsptr);
...
"pos01",  1,    1,      PASS,   0,        MSL, MIE,   MSH, MIE, SUPER,
...
msgctl_mac(void)
{
	 ...
	 */    
	flush_raw_log();   
	switch (msgctl_mac1(&msgctl_minfo[i]) ) {
	/*
	 * Write formatted result to raw log.
	 */    
	case PASS:      /* Passed */
	    RAWPASS(msgctl_minfo[i].casenum, msgctl_minfo[i].casename);
	    break;
	case FAIL:      /* Failed */
	    RAWFAIL(msgctl_minfo[i].casenum, msgctl_minfo[i].casename,
		    msgctl_mdesc[i]);
	    fail = 1;
	    break;
	default:
	    RAWINC(msgctl_minfo[i].casenum, msgctl_minfo[i].casename);
	    incomplete = 1;
	    break;  /* Incomplete */
	}
	flush_raw_log();
...
static int
msgctl_mac1(struct macinfo_2 *parmsptr)
{
    ...
    key_t key; 	 	         /* Key value. */
    struct msqid_ds msqid_ds;    /* For test call */
    struct msqid_ds *msgbuf;     /* For test call */
	...
	/*
	 * Here's the test: Call msgctl(), save
	 * retval and errno.
	 */
	if (parmsptr->c_flag == 0) {
		retval = msgctl(msqid, IPC_STAT, msgbuf);
	}else{
		retval = cap_msgctl(msqid, IPC_STAT, msgbuf);
	}
	loc_errno = errno;
