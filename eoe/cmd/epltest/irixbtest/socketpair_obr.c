...
/*
 * socketpair_obr1()  (UNIX socketpairs only)
 */
static int socketpair_obr1(struct so_obrinfo *);
...
socketpair_obr(void)
{
	...
	strcpy(testname,"socketpair_obr");
	/*
	...
	for (i = 0; i < (ncases1); i++) {
		...
		flush_raw_log();   
		/*
		 * Call socketpair_obr1() for each INET test case, passing a ptr to
		 ...
		 */
		retval = socketpair_obr1(&soc_info[i]);
...
static int 
socketpair_obr1(struct so_obrinfo *parmsptr)
{
	char testname[SLEN];                /* used to write to logfiles */
	 ...
	 * Create a socketpair.
	 */
	if ( socketpair(AF_UNIX, parmsptr->type, 0, sv)  < 0 ) {
		w_error(SYSCALL, namestring, err[F_SOCKETPAIR], errno);
		exit(INCOMPLETE);
	}
	/*
	...
	FD_ZERO(&r_set);
	FD_SET(sv[0], &r_set);
	if ( (retval = select(sv[0]+1, &r_set, (fd_set *)0, (fd_set *)0,
			      &timeout)) < 0 ) {
	 ...
	 */
	if ( fcntl(sv[0], F_SETFL, FNDELAY) < 0 ) {
		w_error(SYSCALL, namestring, err[F_FCNTL], errno);
		exit(FAIL);
	} 
	if ( read(sv[0], rbuf, sizeof(rbuf) ) > 0) {
