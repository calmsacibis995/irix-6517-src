 ...
 * Initial revision
 *
 * Revision 1.1.4.3  1993/01/03  21:38:33  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/03  14:33:27  bbelch]
 *
 * Revision 1.1.4.2  1992/12/23  18:44:35  zeliff
 * 	Embedding copyright notice
 * 	[1992/12/23  01:00:39  zeliff]
 * 
 * Revision 1.1.2.4  1992/06/05  14:09:42  harrow
 * 	Remove -[no_]bug 4 option.
 * 	[1992/06/05  13:30:13  harrow]
 * 
 * 	Correct writting to storage that wasn't allocated.
 * 	Add missing ANSI C prototype.
 * 	[1992/05/05  13:37:00  harrow]
 * 
 * Revision 1.1.2.3  1992/05/05  15:53:08  harrow
 * 	*** empty log message ***
 * 
 * Revision 1.1.2.2  1992/04/06  19:49:15  harrow
 * 	     Change default NDR generation to be -bug 4.  This prevents arrays
 * 	     of [ref] pointers contained in structures from being marshalled
 * 	     properly (no holes are shipped for the pointers), but maintians compatibility
 * 	     with DCE V1.0.1.
 * 	     [1992/04/06  13:39:32  harrow]
 * 
 * Revision 1.1  1992/01/19  03:00:50  devrcs
 * 	     Initial revision
 * 
 * $EndLog$
 */
...
OPTIONS option_table[]={
...
#endif
#ifdef DUMPERS
    "dump",         VSTRARG(MAX_DUMP_STRINGS),      FDV(dump_strings),
#endif
    ...
    "keep",         STRARG,                         FD(keep),
#ifdef __sgi
    "ksgen",        ASSERTARG,                      FD(cmd_opt[opt_ksgen]),
    "kstypes",      ASSERTARG,                      FD(cmd_opt[opt_kstypes]),
#endif
    ...
    "U",            VSTRARG(MAX_DEF_STRINGS),       FDV(undef_strings),
#endif
    "v",            ASSERTARG,                      FD(cmd_opt[opt_verbose]),
    "version",      ASSERTARG,                      FD(cmd_opt[opt_version]),
    0,              0,                              0 };

/*
...
#ifdef DUMPERS
typedef enum {bit, string, string_list, int_list} opt_kind_t;
typedef struct
{
    char *opt_name;
    opt_kind_t opt_kind;
...
#endif
#ifdef __sgi
    cmd_opt[opt_ksgen]		= FALSE;
    cmd_opt[opt_kstypes]	= FALSE;
#endif
...
#ifdef __sgi
    if (cmd_opt[opt_kstypes])
	auto_import_file = "ks/ks.idl";
    else
	auto_import_file = AUTO_IMPORT_FILE;
#endif
