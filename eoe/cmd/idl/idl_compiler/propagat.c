 ...
 * Initial revision
 *
 * Revision 1.1.4.3  1993/01/03  21:41:26  bbelch
 * 	Embedding copyright notice
 * 	[1993/01/03  14:38:10  bbelch]
 *
 * Revision 1.1.4.2  1992/12/23  18:51:13  zeliff
 * 	Embedding copyright notice
 * 	[1992/12/23  01:05:31  zeliff]
 * 
 * Revision 1.1.2.3  1992/05/05  15:20:27  harrow
 * 	Add flag to indicate that an operation requires the node alias table
 * 	[1992/05/04  20:01:59  harrow]
 * 
 * Revision 1.1.2.2  1992/03/24  14:13:11  harrow
 * 	     Add logic to move large stack surrogates into
 * 	     heap storage due to the small stack size normally
 * 	     provided by DCE threads.
 * 	     [1992/03/18  18:47:31  harrow]
 * 
 * Revision 1.1  1992/01/19  03:01:50  devrcs
 * 	     Initial revision
 * 
 * $EndLog$
 */
...

{
    ...
    /*
     * If the type has an rep as, set the flag in the propagation context
     */
    ...
    else
    {
        ...
        case AST_structure_k:
        {
                ...
                ;   field_p = field_p->next)
            {
                 ...
                 *  under_ptr flag if this it doensn't have the [ref]
                 *  attribute.
                 */
                if (!ctx->under_ptr && (field_p->type->kind == AST_pointer_k))
                    ctx->under_ptr = !AST_REF_SET(field_p);

                ctx->instance_p = (ASTP_node_t *)field_p;
                ctx->parent_type_p = type_p;
                PROP_type_info(field_p->type, ctx);
        ...
        case AST_disc_union_k:
        {
                ...
                ;   arm_p = arm_p->next)
            {
                if (arm_p->type != NULL)
                {
                    ...
                    ctx->instance_p = (ASTP_node_t *)arm_p;
                    ctx->parent_type_p = type_p;
                    PROP_type_info(arm_p->type, ctx);
        ...
        case AST_pointer_k:
        {
            ...
            if (!toplevel_ref_param)
            {
                ...
                /*
                 *  If this is an a arrayified full pointer (the array_rep
    ...
    /*
     * If we found any containing [represent_as] types, set FE_HAS_REP_AS
     * on the type.
     */
    if (ctx->has_rep_as)
...

{
    ...
    PROP_post_filter_types_list(&int_p->xa_types, FALSE);
    ...
    PROP_post_filter_types_list(&int_p->ch_types, FALSE);
