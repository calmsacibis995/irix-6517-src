...
usage() {
	fprintf(stderr,
			"ansitape: usage: ansitape -{rxtc}[flnvbF] [filename] [label] [filename] [blocksize] [files]\n");
	exit(1);
}
	...
	if (!fflag) {
		while(1) {
			...
			if(obuf+i+4 > endobuf) {  /* end of output buffer */
				...
				success = ansiwrite(tape,obufstart,blocksize);
    ...
    register char *string;
{
    ...
    if ((base = strrchr(string, '/')) == 0) base = string;
    else {
	base++;
	strcpy(string, base);
    }
...
int recordsize;
	...
	if(!(ansiread(tape,buf,80))) return(1); /* no hdr record, so second eof */
	...
	if(tflag || xflag) {
		...
		if(mode == 'D') {
			...
			while(size=ansiread(tape,ibufstart,blocksize)) {
				if(size != blocksize) {
					/*
...
#define bad(a) (!(isdigit(ibuf[a])))
			...
			while(ansiread(tape,ibufstart,blocksize)) {
				numblock++;
				ibuf = ibufstart;
...

{
	...
	if ((buf.st_mode & S_IFIFO)==S_IFIFO) {
		fprintf(stderr,"%s: named pipe - skipped\n",name);
		return(1);
	}
	*file = open(name,O_RDONLY,NULL);
	...
	if(read(*file,&sample,sizeof(struct filehdr))>=sizeof(struct filehdr)){
		if(sample.f_magic == OBJMAGIC) {
			/* executable */
   ...
   unsigned int	len;
{
   int		tmp;
	...
	if ((tmp = read(fd, buf, len)) == -1) {
		perror("ansitape read");
		return(-1);
	}
	else if (tmp != len && tmp != 0)
		fprintf(stderr, "ansitape: short read (want %d, got %d)\n",
			len, tmp);
#ifdef DEBUG
	else
		fprintf(stderr, "read %d bytes from tape\n", tmp);
#endif
	return(tmp);
}
...
#endif
	if ((tmp = ioctl(fd, cmd, mp)) == -1) {
		perror("ansitape ioctl");
		return(-1);
	}
	return(tmp);
}

ansiwrite(fd, buf, len)
   int		fd;
   char		*buf;
   unsigned int	len;
{
   int		tmp;
	...
	if ((tmp = write(fd, buf, len)) == -1) {
		perror("ansitape write");
		return(-1);
	}
	else if (tmp != len)
		/*
		 * I suppose someday we could actually handle multi-volume
		 * ansi tapes right here.
		 */
		fprintf(stderr,
			"ansitape: short write (want %d, got %d)\n", len, tmp);
#ifdef DEBUG
	else
