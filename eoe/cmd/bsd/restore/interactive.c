...
static int trap_is_set = 0;   /* true iff the reset trap is valid */
 ...
 */
struct afile {
	efs_ino_t fnum;		/* inode number of file */
...
static int mkentry(char *, efs_ino_t, struct arglist *);
static void printlist(char *, efs_ino_t, char *);
 ...
 */
static void
printlist(char *name, efs_ino_t ino, char *basename)
{
	struct afile *fp;
	struct direct *dp;
 ...
 */
static int
mkentry(char *name, efs_ino_t ino, struct arglist *ap)
{
	struct afile *fp;
...
{
	struct afile *f1 = (struct afile *)af1, *f2 = (struct afile *)af2;
