 ...
 */
long
listfile(char *name, efs_ino_t ino, int type)
{
 ...
 */
long
addfile(char *name, efs_ino_t ino, int type)
{
	struct entry *ep;
	long descend = hflag ? GOOD : FAIL;
	char buf[100];
...
/* ARGSUSED */
long
deletefile(char *name, efs_ino_t ino, int type)
{
	long descend = hflag ? GOOD : FAIL;
	struct entry *ep;
 ...
 */
long
nodeupdates(char *name, efs_ino_t ino, int type)
{
...
createfiles(void)
{
	efs_ino_t first, next, last, lowest;
	struct entry *ep;
	long curvol;
	...
	lowest = curfile.ino;
	Dprintf(stdout, "First ino = %d, last = %d\n", first, last);
	for (;;) {
		first = lowerbnd(first);
		last = upperbnd(last);
		/*
		...
		if (first > last)
			return;
		/*
		 * Watch out for missing files (ie files on the
		 * dump map but not dumped (maybe deleted while
		 * was in progress)). Report these files and 
		 * delete their request flags.
		 */
		next=first;
		while (next > lowest && next < curfile.ino) {
			missing(next);
			next =  (next < last) ? lowerbnd(next) : maxino;
		}
		if (next > last)
			return;
		/*
		...
		while (curfile.ino > last) {
			if (all_tapes_read()) {
				while (first <= last) {
					missing(first);
					first =  (first < last) ? lowerbnd(first) : maxino;
				}
				return;
			}
			Vprintf(stdout, "None of the remaining requested files are on this tape\n");
			curfile.action = SKIP;
			getvol((long)0);
			skipmaps();
			skipdirs();
		 ...
		 */
		next = lowerbnd(MAX(curfile.ino, next));
		do	{
			curvol = volno;
			while (next > curfile.ino && volno == curvol) {
 				Dprintf(stdout, "Looking for inode %d: Skipping inode %d;",
					next, curfile.ino);
				skipfile();
			}
			skipmaps();
		...
		if (next == curfile.ino && next <= last) {
			...
			if (ep == NIL)
				panic("corrupted symbol table\n");
			Dprintf(stdout, "Extracting inode %d\n", next);
			(void) extractfile(myname(ep), ep);
			ep->e_flags &= ~NEW;
			if (volno != curvol)
				skipmaps();
 ...
 */
long
verifyfile(char *name, efs_ino_t ino, int type)
{
	struct entry *np, *ep;
	long descend = GOOD;
