...
static void addino(efs_ino_t, struct entry *);
 ...
 */
static void
addino(efs_ino_t inum, struct entry *np)
{
	struct entry **epp;
 ...
 */
struct entry *
addentry(char *name, efs_ino_t inum, int type)
{
	struct entry *np, *ep;
...
savename(char *name)
{
	...
	if (np != NULL) {
		...
		cp = calloc(1, (unsigned)allocsize(len));
		if (cp == NULL)
...
initsymtable(char *filename)
{
	...
	if (filename == NULL) {
		entrytblsize = (long)(maxino / HASHFACTOR);
		entry = (struct entry **)
	...
	tblsize = (long)(stbuf.st_size - sizeof(struct symtableheader));
