 ...
 *
 *
 * kern.c,v 3.8.4.7 1997/06/09 22:25:36 fenner Exp
 */
    ...
    int t;
{
    ...
    ttl = t;
    if (cap_setsockopt(igmp_socket, IPPROTO_IP, IP_MULTICAST_TTL,
    ...
    int l;
{
    ...
    loop = l;
    if (cap_setsockopt(igmp_socket, IPPROTO_IP, IP_MULTICAST_LOOP,
    ...
    u_int32 ifa;
{
    ...
    adr.s_addr = ifa;
    if (cap_setsockopt(igmp_socket, IPPROTO_IP, IP_MULTICAST_IF,
		   (char *)&adr, sizeof(adr)) < 0) {
        if (errno == EADDRNOTAVAIL)
                log(LOG_WARNING, errno, "setsockopt IP_MULTICAST_IF %s",
                    inet_fmt(ifa, s1));
        else
    ...
    u_int32 ifa;
{
    ...
    if (cap_setsockopt(igmp_socket, IPPROTO_IP, IP_ADD_MEMBERSHIP,
    ...
    u_int32 ifa;
{
    ...
    if (cap_setsockopt(igmp_socket, IPPROTO_IP, IP_DROP_MEMBERSHIP,
		   (char *)&mreq, sizeof(mreq)) < 0)
    ...
    struct sg_count *retval;
{
	...
	retval->pktcnt = retval->bytecnt = retval->wrong_if = 0xffffffff;
	return 1;
    }
