 ...
 *
 *
 * vif.c,v 3.8.4.39 1997/06/11 22:06:35 fenner Exp
 */
...
static int info_version __P((u_char *p));
    ...
    u_int32 level;
{
    ...
    if ((pv == 3 && mv == 255) || (pv > 3 && pv != 10 && pv != 11))
	dvmrpspec = 1;
...
age_vifs()
{
	...
	for (prev_a = (struct listaddr *)&(v->uv_neighbors),
		...
		((a->al_pv > 3) && (a->al_pv != 10) && (a->al_pv != 11)))
    ...
    FILE *fp;
{
	...
	if (v->uv_groups != NULL) {
	    fprintf(fp, "           group host (time left): %-15s %-15s (%s)\n",
		    inet_fmt(v->uv_groups->al_addr, s1),
		    inet_fmt(v->uv_groups->al_reporter, s2),
		    scaletime(timer_leftTimer(v->uv_groups->al_timerid)));
	...
	if (v->uv_filter) {
	    ...
	    fprintf(fp, "             %5s %7s filter: %-18s%s\n",
			v->uv_filter->vf_flags & VFF_BIDIR ? "bidir"
							   : "     ",
			v->uv_filter->vf_type == VFT_ACCEPT ? "accept"
							    : "deny",
			inet_fmts(v->uv_filter->vf_filter->vfe_addr,
				  v->uv_filter->vf_filter->vfe_mask, s1),
			v->uv_filter->vf_filter->vfe_flags & VFEF_EXACT ?
						" (exact)" : "");
	    for (vfe = v->uv_filter->vf_filter->vfe_next;
	...
	if (!(v->uv_flags & VIFF_TUNNEL))
	    fprintf(fp, "                     IGMP querier: %-18s\n",
		    (v->uv_querier != NULL || v->uv_flags & VIFF_QUERIER) ?
			inet_fmt(v->uv_querier ? v->uv_querier->al_addr
					   : v->uv_lcl_addr, s1) : "none");
	...
	if (did_final_init)
		...
		fprintf(fp, "                   pkts/bytes in : %ld/%ld\n",
			v_req.icount, v_req.ibytes);
		fprintf(fp, "                   pkts/bytes out: %ld/%ld\n",
