 ...
 *
 *
 * prune.c,v 3.8.4.40 1997/06/11 22:10:02 fenner Exp
 */
    ...
    u_int32 old_parent_gw;
{
	...
	while ((pt = *ptnp)) {
		...
		IF_DEBUG(DEBUG_PRUNE)
		log(LOG_DEBUG, 0, "update_table_entry deleting prune for (%s %s) from %s on vif %d",
			RT_FMT(r, s1), inet_fmt(g->gt_mcastgrp, s2),
    ...
    int datalen;
{
	...
	IF_DEBUG(DEBUG_PRUNE)
	log(LOG_DEBUG, 0, "ignoring prune from %s on vif %d for (%s %s)/%d because it's too short",
...
age_table_entry()
{
	...
	if (gt->gt_timer <= 0) {
		...
		gt->gt_prsent_timer > 0) {
		IF_DEBUG(DEBUG_CACHE)
		log(LOG_DEBUG, 0, "refresh lifetim of cache entry %s%s%s(%s, %s)",
		...
		if (gt->gt_prsent_timer == -1) {
		    gt->gt_prsent_timer = 0;
		    /*
		     * XXX This is incorrect.  Further traffic flow
		     * should trigger a new prune, not just the
		     * fact that the old one timed out.
		     * However, removing this causes an occasional
		     * white hole, so there's a bug somewhere else
