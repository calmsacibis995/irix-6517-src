...
main(int argc, char *argv[])
{
  ...
  if(verbose) { /* no translation English only :-) */
   ...
   data_map_size = 0x10000 * sizeof(uint_t);
   map_table = (uint_t *) malloc (data_map_size); /* Map the whole UCS table */
  ...
  if (verbose) fprintf(verbose_fd, "max val %x\n", maxtval);
   ...
   fclose(out_fd);
   printf("\"%s\" \"%s\" \"libc.so.1\" \"__iconv_flh_bom_idxw_mbs\" \"libc.so.1\"\
 \"__iconv_control_bom\" \"libc.so.1\" \"__iconv_unicode_signature\" FILE \"%s_%s.map\";\n", from_codeset, to_codeset,
         from_codeset, to_codeset);
   printf("\"%s\" \"%s\" \"libc.so.1\" \"__iconv_utf8_idxw_mbs\" \"libc.so.1\"\
