...
*       genxlt_fl.c
*
*       genxlt for the fix length conversion tables.
*
...
*       characteristics:
*
*	SBCS to SBCS: Single Byte to Single byte
*
*	UCS to SBCS: UCS-2 to SBCS
*/
...
main(int argc, char *argv[])
{
  ...
  if(verbose) { /* no translation English only :-) */
   ...
   if (type_flag == 1) 
	...
	data_map_size = 0x10000;
   else {
	fprintf(stderr, "%s\n", Usage_msg);
	exit(1);
   }
	
   map_table = (uchar_t *) malloc (data_map_size); /* Map the whole UCS table */
   ...
   fclose(out_fd);
   if (type_flag == 1) 
        printf("\"%s\" \"%s\" \"libc.so.1\" \"__iconv_flb_idxb_flb\" \"libc.so.1\"\
 \"__iconv_control\" FILE \"%s_%s.map\";\n", from_codeset, to_codeset,
                 from_codeset, to_codeset);
   else if(type_flag == 2) {
        printf("\"%s\" \"%s\" \"libc.so.1\" \"__iconv_flh_bom_idxb_mbs\" \"libc.so.1\" \"__iconv_control_bom\" \"libc.so.1\" \"__iconv_unicode_signature\" FILE \"%s_%s.map\";\n", from_codeset, to_codeset, from_codeset, to_codeset);
        printf("\"%s\" \"%s\" \"libc.so.1\" \"__iconv_utf8_idxb_mbs\" \"libc.so.1\" \"__iconv_control\" FILE \"%s_%s.map\";\n", "UTF-8", to_codeset, from_codeset, to_codeset);
   } else {
	fprintf(stderr, "%s\n", Usage_msg);
	exit(1);
   }
   ...
   uchar_t *outbytes, uint_t outlen)
{
   ...
   for (i = 0; i < outlen; i++)
	out_int = out_int | outbytes[i] << 8* (outlen - i - 1);

   if (verbose)
	fprintf(verbose_fd, "in %x, out %x\n", (uint_t) in_int,
		(uchar_t) out_int);

   maxtval = MAX(maxtval, in_int);
   map_table [in_int] = (uchar_t) out_int;
