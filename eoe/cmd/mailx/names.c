	...
	char line[];
{
	...
	while ((cp = yankword(cp, nbuf)) != NOSTR) {
		/* SPR 88997: customer was confused by the auto-@ conversion
		 * below. Now, mailx won't convert "wjm at foo" to wjm@foo, but
		 * Dan Celidonio wanted to keep this in BSD Mail.
		 * -- wjm 8/97
		 */
		if (!ismailx && np != NIL && equal(nbuf, "at")) {
