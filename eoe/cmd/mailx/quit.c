...
quit()
{
	int mcount, p, modify, autohold, anystat, holdbit, nohold, lcnt;
	long ccnt;
	 ...
	 * can save copying mbox to temp and back.
	 *
	...
	for (mp = &message[0]; mp < &message[msgCount]; mp++)
		if (mp->m_flag & MBOX)
			if (send_msg(mp, obuf, NO_SUPPRESS, 
					&lcnt, &ccnt, 0) < 0) {
				perror(mbox);
				/*
				 *  ibuf has not been used as a file descriptor if
				 *  the 'append' variable has a value.
				 */
				if (value("append") == NOSTR)	/* fix for bug #323033 */
					fclose(ibuf);
				fclose(obuf);
				fclose(fbuf);
				unlock();
