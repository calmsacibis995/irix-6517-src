...
static void statusput(struct message *, FILE *, int, char *);
 ...
 * by suppress value.
 */
send_msg(mailp, obuf, suppress, lcnt, ccnt, cmdflag)
	...
	char *cmdflag;
{
	...
	*ccnt = 0L;
	...
	if (ismailx && cmdflag && ((*cmdflag == 'm') || (*cmdflag == 'M'))) {
		if ((msgpfix = value("indentprefix")) == NOSTR)
			msgpfix = "\t";
	}
	while (c > 0L) {
...
writeit:
	...
	fflush(obuf);			/* fix for bug #323033 */
	if (ferror(obuf))
		return(-1);
 ...
 */
static void
statusput(mp, obuf, suppress, pfix)
	register struct message *mp;
	register FILE *obuf;
	int	suppress;
	char	*pfix;
...
topdog:
	...
	if (Fflag || followflag) {
		savemail(expand(to->n_name), hp, mtf);
	} else {
		if ((value("norecord") == NOSTR) && 
				((cp = value("record")) != NOSTR)) {
			if (value("outfolder") &&
					((cp1 = value("folder")) != NOSTR)) {
				if (*cp == '/') {
				...
				for (i=0; i<sizeof(savepath); i++)
					savepath[i] = 0;
				if (*cp1 == '/') {
					snprintf(savepath, BUFSIZ, "%s/%s",
						cp1, cp);
				} else {
					snprintf(savepath, BUFSIZ, "%s/%s/%s",
						homedir, cp1, cp);
				}
				savemail(expand(savepath), hp, mtf);
			} else {
				savemail(expand(cp), hp, mtf);
			}
		}
