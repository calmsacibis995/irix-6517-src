...
char    *line;
int	linesz = 1024; /* initial size of input buffer.  512 was a bit skimpy */
char *lp;
...
char **av;
{
	...
	line = calloc(linesz, 1);
	if(line == NULL)
		fatal("can't allocate memory for line buffer\n", NULL);
...
for( ; ; )
	...
	if(lp > &line[linesz-2]) {
		...
		if(!line)
			fatal("can't reallocate memory for line buffer\n", NULL);
		lp = line + lpoff;
	}
	if(intable && c=='T')
		{
