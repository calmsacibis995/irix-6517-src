...
#ident	"@(#)acct:common/cmd/acct/runacct.sh	1.9.1.6"
#ident "$Id: runacct.sh,v 1.6 1995/03/02 18:33:54 jwag Exp $"
...
_baseerrormsg="\n\n************ ACCT ERRORS : see  ${_active}"
_MIN_BLKS=500

cd ${_adm}
#
# SGI - start out with a clean active file rather than saving all old ones
#
nulladm ${_active}
...
if test $? -ne 0; then
	...
	#
	# This doesn't make sense ... it doesn't say to look at the active
	# file, and this is the only one that doesn't move it to active.date..
...
#
# Also check /var/adm rather than /usr: could be a separate filesystem!

_blocks=`df /var/adm |awk '/avail/ {next} /.*/ {print $5}'`
if [ "$_blocks" -le $_MIN_BLKS ];then
	echo "runacct: Insufficient space in /var/adm ($_blocks blks); \c"
...
# default for case 0, 3
_date="`date +%m%d`"
_errormsg="${_baseerrormsg}${_date}********\n\n"
...
nulladm ${_wtmp}
utmp2wtmp -t	# fudge active user from utmp to wtmp
...
#
# keeping all active files is a new SVR4-ism - but it really causes
# active files to stack up, for all 'good'days. This seems a waste and
# makes it hard to spot error days..
#
#mv ${_active} ${_active}${_date}
