...
#ident	"@(#)acct:common/cmd/acct/lib/devtolin.c	1.9.2.4"
#ident "$Header: /proj/irix6.5.7m/isms/eoe/cmd/acct/lib/RCS/devtolin.c,v 1.3 1993/11/05 04:25:09 jwag Exp $"
...
char	*strncpy();
dev_t	lintodev();
static int srch_dir(dev_t device, char *path, int depth, char *skip_dirs[]);
	...
	*last_comp++ = '/';
	...
	while ((!found) && ((d = readdir(fdev)) != NULL))
		if (d->d_ino != 0) {
			...
			if ((path_len + (int)strlen(d->d_name) + 2) > MAX_DEV_PATH)
				continue;
			else
