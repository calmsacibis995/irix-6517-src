...
#ident	"@(#)acct:common/cmd/acct/lib/pnpsplit.c	1.7.3.4"
#ident "$Id: pnpsplit.c,v 1.10 1997/09/29 14:42:17 tee Exp $"
/*
 ...
 *	Assume all time is prime time unless holidays says otherwise.
 */
static struct hours {
 ...
 */
void
pnpsplit(long start, long etime, long result[2])
{
	struct tm cur, end;
	long tcur, tend;
	long tmp;
...
checkhol(void)
{
	...
	tp = localtime(&t);
	tp->tm_year += 1900;
	if((rv = inithol(tp->tm_year)) == 0) {
		fprintf(stderr, "pnpsplit: holidays table setup failed\n");
		thisyear = 0;
		holidays[0] = -1;
		return;
	}
	if (((rv == 1) && (tp->tm_year == thisyear && tp->tm_yday > 359))
		|| tp->tm_year > thisyear) {
		if (uname(&name) < 0)
			fprintf(stderr,
			...
			fprintf(stderr,
				"***UPDATE %s ON MACHINE %s WITH NEW HOLIDAYS***\n",
				HOLFILE, name.nodename);
	}
 ...
 * holidays array.
 * Returns 0 if error
 *	   1 if normal (year came from holiday file)
 *	   2 if used current year (i.e. holiday file had a '0' in it)
 */
inithol(int curyear)
{
	FILE		*fopen(), *holptr;
	...
	while(fgets(holbuf, sizeof(holbuf), holptr) != NULL) {
		...
		for (strp = holbuf; *strp == ' ' || *strp == '\t'; strp++) ;
		...
		if (*strp == '*' || *strp == '#')
			...
			/* SGI extentsion - a year of '0' means this year */
			if (thisyear == 0) {
				rv = 2;
				thisyear = curyear;
			}
		...
		} else if ((nf = sscanf(holbuf, "%d	%*s %*s	%*[^\n]\n", &doy)) != 1) {
			fprintf(stderr, "pnpsplit: invalid line <%s> ignoring\n", holbuf);
			continue;
		} else if(doy < 1 || doy > 366) {
			fprintf(stderr,
