...
#ident	"@(#)acct:common/cmd/acct/acctcom.c	1.37.3.7"
#ident "$Header: /proj/irix6.5.7m/isms/eoe/cmd/acct/RCS/acctcom.c,v 1.10 1996/06/14 21:05:37 rdb Exp $"
...
dofile(char *fname)
{
	...
	if(fname != NULL)
		if(freopen(fname, "r", stdin) == NULL) {
			...
			exitcode = 2; /* Have exit reflect error */
			return;
		}
	...
	while(aread(ver) != 0) {
		elapsed = expand64(a->ac_etime);
		if (elapsed + (unsigned long long)a->ac_btime > 0x7fffffff) {
			etime = 0x7fffffff;
		}
		else {
			etime = a->ac_btime + (time_t)SECS(elapsed);
		}
		...
		if(su_user && !SU(a->ac_flag))
			continue;
		sys = expand64(a->ac_stime);
		user = expand64(a->ac_utime);
		cpu = sys + user;
		if(cpu == 0)
			cpu = 1;
		mem = expand64(a->ac_mem);
		...
		if(syscut && syscut >= SECS(sys))
			continue;
		if(linedev != NODEV && a->ac_tty != linedev)
			continue;
