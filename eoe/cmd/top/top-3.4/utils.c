...
/* Explanation:
   ...
   the format mmm:ss.  For values that exceed 999:59, we switch to a format
   that displays hours and _decimal_ tenths:  hhh.tH.  For values that
   exceed 999.9 hours, we use Days: ddd.tD, for those that exceed 999.9 days,
   we use "???".
 */
...
char *format_time(long seconds)
{
    ...
    /* sanity protection */
    if (seconds < 0 || seconds > (9999L * 3600L * 24L)) { /* too big */
	strcpy(result, "   ???");

    } else if (seconds > (9999L * 360L)) {
	/*
	 * over 999.9 hours, print in days
	 */
	sprintf(result, "%5.1fD", (double)seconds / (double)(24L * 3600L));

    } else if (seconds > (999 * 60 + 59)) {
	/*
	 * over 999:59 MMM:SS, print in hours
	 */
	sprintf(result, "%5.1fH", (double)seconds / (double)(3600L));

    } else {
	/* standard method produces MMM:SS */
	/* we avoid printf of float as must as possible to make this quick */
...
/*
 * format_k(amt)
 *	format a kilobyte memory value, returning a string suitable for
 *	display.  Returns a pointer to a static area that changes each
 *	call.  "amt" is converted to a string with a trailing "K".
 *	If "amt" is 10000 or greater, then it is formatted as Megabytes
 *	(rounded) with a trailing "M" and if that is 10000 or greater,
 *	as Gigabytes (trailing "G"), and ... Terabytes (Trailing T).
 */
...
{
    static char retarray[NUM_STRINGS][8];
    static int index = 0;
    register char *p;
    register char *ret;
    register char tag = 'K';
