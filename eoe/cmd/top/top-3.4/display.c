...
double *avenrun;
{
    ...
    printf("%.2f %.2f %.2f", avenrun[0], avenrun[1], avenrun[2]);
}
...
{
    register int	i;
    extern char		uname_str[];	/* Ariel: added uname data */

    /* i_loadave also clears the screen, since it is first */
    if (smart_terminal)
	clear();

    printf("%-30.30s load averages: ", uname_str);
...
char *cpustates_tag()
{
    static char cpu_prefix[20];
    extern int numcpus;

    if (numcpus > 1) {
	sprintf(cpu_prefix, "%d CPUs: ", numcpus);
    } else {
	sprintf(cpu_prefix, "CPU: ");
    }

    /* set cpustates_column accordingly then return result */
    cpustates_column = strlen(cpu_prefix);
    return cpu_prefix;
}

static const char * const cpu_state_fmt = "%s%4.1f%% %s";
static const char * const null_sep = "";
static const char * const comma_sep = ", ";

void i_cpustates(int *states)
{
    const char *sep = null_sep;
	...
	if (*thisname != '\0') {
	    printf(cpu_state_fmt, sep, 0.0, thisname);
	    sep = comma_sep;
	}
    }
...
void u_cpustates(int *states)
{
	...
	if (*thisname != '\0') {
		...
		printf(cpu_state_fmt, sep, (double)value / 10.0, thisname);
		sep = comma_sep;
