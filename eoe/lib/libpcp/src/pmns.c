...
#ident "$Id: pmns.c,v 2.55 1999/04/30 04:03:45 kenmcd Exp $"
 ...
 */
#define CPP "_RLD_LIST=DEFAULT %s -Usgi -Uunix -Umips -Ulinux -I. -I/var/pcp/pmns -I/usr/pcp/pmns %s"
#else
#define CPP "%s -Usgi -Uunix -Umips -Ulinux -I. -I/var/pcp/pmns -I/usr/pcp/pmns %s"
#endif

static char	*cpp_path[] = {
#if defined(__NUTC__)
    "c:/nutcroot/bin/cpp.exe",
#else
    "/lib/cpp",
...
/* Forward reference */
static int
GetChildrenStatusRemote(int, const char *, char ***, int **);

/*
 * Retry with IRIX added to name 
 */
static int
GetChildrenStatusRetry(int pc_fd, const char *name, char ***offspring, int **statuslist)
{
    int     num_xlch = 0;
    char    **xlch = NULL;
    int     *xlstatus = NULL;
    ...
    if (newname != NULL) {
        n = GetChildrenStatusRemote(pc_fd, newname, offspring, statuslist);
        free(newname);
        if (n > 0) {
      ...
      switch(ctxp->c_type) {
	    ...
	    if ((sts = __pmFdLookupIPC(ctxp->c_pmcd->pc_fd, &ipc)) < 0) {
	      __pmNotifyErr(LOG_ERR, 
			"pmGetPMNSLocation: version lookup failed: %s", 
			ctxp->c_pmcd->pc_fd, pmErrStr(sts));
	      ...
	      pmns_location = PMNS_REMOTE;
            	...
            	__pmNotifyErr(LOG_ERR, 
			"pmGetPMNSLocation: bad version number (fd=%d, ver=%d)",
			ctxp->c_pmcd->pc_fd, ipc->version);
	      	pmns_location = PM_ERR_NOPMNS;
	    }
	    break;
		...
		pmns_location = PMNS_ARCHIVE;
	        ...
	        __pmNotifyErr(LOG_ERR, "pmGetPMNSLocation: bogus version number"); 
	        pmns_location = PM_ERR_NOPMNS;
	    }

	    break;
        default: 
	    __pmNotifyErr(LOG_ERR, "pmGetPMNSLocation: bogus context type"); 
            pmns_location = PM_ERR_NOPMNS;
      }/*switch*/
    }
    else {
...
dumptree(FILE *f, int level, __pmnsNode *rp, int verbosity)
{
	...
	if (verbosity > 1) {
	    fprintf(f, "\t[first: ");
	    if (rp->first) fprintf(f, "0x%p", rp->first);
	    else fprintf(f, "<null>");
	    fprintf(f, " next: ");
	    if (rp->next) fprintf(f, "0x%p", rp->next);
	    else fprintf(f, "<null>");
	    fprintf(f, " parent: ");
	    if (rp->parent) fprintf(f, "0x%p", rp->parent);
	    else fprintf(f, "<null>");
	    fprintf(f, " hash: ");
	    if (rp->hash) fprintf(f, "0x%p", rp->hash);
	    else fprintf(f, "<null>");
	}
	fputc('\n', f);
...
lex(int reset)
{
	...
	for (i = 0; cpp_path[i] != NULL; i++) {
		...
		continue;
	    if ((lp = (char *)malloc(strlen(CPP) + strlen(fname) +strlen(cpp_path[i]) - 4 + 1)) == NULL)
		return -errno;
...
loadbinary(int secure)
{
	...
	if (version == 0) {
	    /*
	     * TODO - expunge support for Version 0 binary PMNS format.
	     *        It can never work on anything but 32-bit int and
	     *        32-bit ptrs.
	     */
	    ...
	    if (main_pmns == NULL || htab == NULL || root == NULL || symbol == NULL) {
		__pmNoMem("loadbinary",
			 sizeof(*main_pmns) +
			 htabsize * sizeof(htab[0]) + 
			 nodecnt * sizeof(*root) + symbsize,
			 PM_FATAL_ERR);
		/*NOTREACHED*/
	    }
	    ...
	    /* swab all nodes : all pointers are 32 bits in Version 0 PMNS */
	    for (i=0; i < nodecnt; i++) {
		__pmnsNode *p = &root[i];
		p->pmid = __ntohpmID(p->pmid);
		p->parent = ntohl(p->parent);
		p->next = ntohl(p->next);
		p->first = ntohl(p->first);
		p->hash = ntohl(p->hash);
		p->name = (char *)ntohl(p->name);
	    }
#endif
...
getfname(const char *filename, int cap_id)
{
	...
	if (def_pmns != NULL)
	  ...
	  /* otherwise use the hard-coded pathname */
...
pmLookupName(int numpmid, char *namelist[], pmID pmidlist[])
{
	...
	if (n < 0) {
		 ...
		 */
	        int op_status; 
		sts = __pmDecodeIDList(pb, PDU_BINARY, 
...
#if IRIX_XLATION
                if (sts == PM_ERR_NAME) {
                    /* re-send a list of names which were not found the first
                     * time with the 'irix' node at the start.
                     * if there is a failure at any step, then just return the results
...
static int
GetChildrenStatusRemote(int pc_fd, const char *name, char ***offspring, int **statuslist)
{
    int n;

    n = __pmSendChildReq(pc_fd, PDU_BINARY, name, (statuslist==NULL) ? 0 : 1);
    if (n < 0)
        return __pmMapErrno(n);
    else {
	__pmPDU      *pb;

        n = __pmGetPDU(pc_fd, PDU_BINARY, TIMEOUT_DEFAULT, &pb);
        if (n == PDU_PMNS_NAMES) {
            int numnames;
...
pmGetChildrenStatus(const char *name, char ***offspring, int **statuslist)
{
	...
	*offspring = result;
	...
	assert(n >= 0);
	ctxp = __pmHandleToPtr(n);
        n = GetChildrenStatusRemote(ctxp->c_pmcd->pc_fd, name, offspring, statuslist);
	     ...
	     * children.
            ...
            if (CheckForXlChild(n, offspring)) {
                ...
                int  *l_statl;
                int  l_n;
                l_n = GetChildrenStatusRetry(ctxp->c_pmcd->pc_fd, name, &l_offs, &l_statl);
                if (l_n > 0) {
                    char *newname;
                    /*
            ...
            /* Retry with IRIX added to name */
            n = GetChildrenStatusRetry(ctxp->c_pmcd->pc_fd, name, offspring, statuslist);
        }
#endif /* IRIX_XLATION */
        return n;
    }
...
pmNameID(pmID pmid, char **name)
{
	...
	ctxp = __pmHandleToPtr(n);
	idlist[0] = pmid;
	n = __pmSendIDList(ctxp->c_pmcd->pc_fd, PDU_BINARY, 
              1, idlist, 0);
	if (n < 0)
   	  return __pmMapErrno(n);
	else {
...
pmTraversePMNS(const char *name, void(*func)(const char *name))
{
	...
	assert(n >= 0);
	ctxp = __pmHandleToPtr(n);
	n = __pmSendTraversePMNSReq(ctxp->c_pmcd->pc_fd, 
	       PDU_BINARY, name);
