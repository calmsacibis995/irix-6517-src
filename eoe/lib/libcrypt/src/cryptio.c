...
static int	cryptopen(int [2]);
static int	writekey(int [2], char *);
static int	_cp2close(int [2]), _cp2open(const char *, int [2]);
...
static int writekey(int p[2], char *keyarg)
{
	void (*pstat) ();
...
static int _cp2open(const char *cmd, int p[2])
			 /* file descriptor array to cmd stdin and stdout */
{
	int		tocmd[2];
	int		fromcmd[2];
	register int	pid;
