...
static int
file_protocolsByName(nsd_file_t *rq)
{
	int len, clen;
	char *line, *key, *p;
	file_domain_t *dom;
	...
	/*
	** Open the protocols file if not already done.
	*/
...
static int
file_protocolsByNumber(nsd_file_t *rq)
{
	char *p, *line, *key;
...
int
file_protocols_lookup(nsd_file_t *rq)
{
	char *map;
	...
	if (strcasecmp(map, PROTOCOLS_BYNAME) == 0) {
		return file_protocolsByName(rq);
	} else {
		return file_protocolsByNumber(rq);
	}
}

int
file_protocols_list(nsd_file_t *rq)
{
	file_domain_t *dom;
