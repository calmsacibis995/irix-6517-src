...
static hash_file_t *__hash_files = (hash_file_t *)0;
...
hash_get_file(nsd_file_t *rq)
{
	...
	*/
	...
	if (! hf) {
		nsd_logprintf(NSD_LOG_RESOURCE, "hash_open: failed malloc\n");
...
/*
** This routine simply closes and unmaps a hash file.
*/
static void
...
lookup(nsd_file_t *rq)
{
	...
	*/
	...
	kv.key.dsize = strlen(kv.key.dptr);
	...
	if (nsd_attr_fetch_bool(rq->f_attrs, "casefold", FALSE)) {
		...
		p = kv.key.dptr;
		q = alloca(kv.key.dsize);
		for (i=0; i < kv.key.dsize; q++, p++, i++) {
			*q = tolower(*p);
		}
		kv.key.dptr = q - kv.key.dsize;
	}
