...
char 	*__execute(char *, char *),
	*__advance(char *, char *);
int	__xpush(int, char *),
	__getrnge(int *, int *, char *),
	__cclass(char *, char, int),
	__bcclass(char *, char **, int), 
	__abcclass(char *, char **, int);
char 	*__xpop(int);
...
#endif
{
	...
	va_end(ap);
	__cflg = 0;
	cur = __execute((char *) addrc, (char *)addrl);
	if(cur == (char *)-1L) {
		/*
		 * -1 is returned only via __xpush - it is really an internal
		 * error
		 * There is no defined error return for regex (in either
		 * SVR4 or XPG4) but it seems like a useful differentiation
...
char *
__advance(register char *lp, register char *ep)
{
 	char *curlp;
...
int
__cclass(char *aset, char ac, int af)
{
	register char *set, c;
	register int n;
...
int
__bcclass(register char *ep, register char **rp, int neg)
{
	register char *lp;
...
int
__abcclass(register char *ep, register char **rp, int neg)
{
	register char *lp;
	register char c, d, f;
	register int n;
