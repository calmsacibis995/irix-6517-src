...
static	int	_hash_ln(chtype  *,chtype  *), _use_idln(void), _set_idln(int, int);
static	void	_find_idln(int, int), _do_idln(int, int, int, int);
...
idlok(WINDOW *win, bool bf)
{
    ...
    SP->yesidln =  (unsigned) ((delete_line || parm_delete_line ||
...
_set_idln(int topy, int boty)
{
	...
	n = scrli + 1;
	if (!(mt = (int **) malloc((size_t) n * sizeof(int *))) ||
	    !(mt[0] = (int *) malloc((size_t) (n * n) * sizeof(int))) ||
	    !(id = (IDST *) malloc((size_t) scrli * sizeof(IDST))))
	{
	    if (mt && mt[0])
		free((char *) mt[0]);
	    if (mt)
...
_hash_ln(chtype *sc, chtype *ec)
{
    ...
    return (int) (didhash ? (hv == 0 ? _THASH : (int) hv == _NOHASH ? _THASH+1 : hv) : hv);
}
