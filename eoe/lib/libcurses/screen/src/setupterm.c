...
#ident	"@(#)curses:screen/setupterm.c	1.57"
#ifndef	NOBLIT
...
#if DUMPTI
static	void	_Pr(int);
static	int	_Getsh(char *);
static	int	_Vr2getsh(char *);
static	void	_Mprint(int, char *);
static	void	_Sprint(int, char *);
#endif
...
static void
_Pr(register int ch)	/* FOR DEBUG ONLY */
{
    if (ch >= 0200)
    {
	printf("M-");
...
#ifdef	DUMPTI
static void
_Sprint(int n, register char *string)	/* FOR DEBUG ONLY */
{
    register	int	ch;
...
static void
_Mprint(register int n, register char *memory)	/* FOR DEBUG ONLY */
{
...
/* filenum is a UNIX file descriptor, not a stdio ptr. */
{
    char	tiebuf[4096];
    char	fname[512];
    register	char	*ip;
    ...
    char	*strtab;
    int		minimal = 0;	/* set if can't read dumb/unknown terminfo */
#ifdef	DUMPTI
    int		Vr2val;
#endif	/* DUMPTI */
...
minimalist:
	...
	goto corrupt;
    snames = _Getshi();
    if (minimal) snames = (int) strlen(term);
#ifdef	DUMPTI
    Vr2val = _Vr2getshi();
...
out_err:
	...
	cur_nums = &_frst_nums;
	cur_strs = &_frst_strs;
	if ((size_t) sstrtab < sizeof(_frst_tblstr))
	    strtab = _frst_tblstr;
    }
	...
	*errret = 1;
	...
	cur_term->_fl_rawmode = (char) (cur_term->_ungotten = 0);
    ...
    (void) memcpy((char *) cp, (char *) ip, ((size_t) snames * sizeof(*cp)));
    ip += snames;
	...
	if (cp < fp)
	    (void) memset(cp, 0, ((size_t) (fp - cp) * sizeof(bool)));
    }

    /* Force proper alignment */
    if (((__psunsigned_t) ip) & 1)
	ip++;
	...
	if (sp < fp)
	    (void) memset((char *) sp, '\377', ((size_t) (fp - sp) * sizeof(short)));
    }

    if (minimal) {
	columns = (short)80;
	cur_bools->_auto_right_margin = 1;
	cur_bools->_generic_type = 1;
    }
	 ...
	 */
	{
		...
		if (ioctl(filenum, TIOCGWINSZ, &w) != -1 &&
			...
			cur_nums->_lines = (short) w.ws_row;
			cur_nums->_columns = (short) w.ws_col;
#ifdef DUMPTI
	...
	if (pp < fp)
	    (void) memset((char *) pp, 0, ((size_t) (fp - pp) * sizeof(charptr)));
    }

    (void) memcpy(strtab, ip, (size_t) sstrtab);
...
next:
    ...
    (void) memcpy(&cur_term->_b1, &cur_bools->_auto_left_margin,
		(size_t) ((char *) &cur_term->_c1 - (char *) &cur_term->_b1));
    (void) memcpy((char *) &cur_term->_c1, (char *) &cur_nums->_columns,
		(size_t) ((char *) &cur_term->_Vr2_Astrs._s1 -
					(char *) &cur_term->_c1));
    (void) memcpy((char *) &cur_term->_Vr2_Astrs._s1,
		(char *) &cur_strs->strs._back_tab,
		(size_t) ((char *) &cur_term->Filedes -
					(char *) &cur_term->_Vr2_Astrs._s1));
	...
	for (n = 0; (size_t) n < sizeof(faked_attrs); n++)
	{
	    if ((!on_sequences[i = faked_attrs[n]]) ||
#ifdef _SGI_SOURCE
		(on_sequences[0] == NULL) ||
#endif
		(_STRNOTEQ(on_sequences[i], on_sequences[0]) == 0))
	    {
	...
	for (n = 0; (size_t) n < sizeof(offsets); n++)
	{
	    if ((!(off_sequences[n] = str_array[offsets[n]])) ||
#ifdef _SGI_SOURCE
		(off_sequences[0] == NULL) ||
#endif
		((n > 0) && (_STRNOTEQ(off_sequences[n], off_sequences[0]) == 0))
#ifndef _SGI_SOURCE
		||
		((n == 2) && (_STRNOTEQ(exit_attribute_mode, off_sequences[n]) == 0))
#endif
		)
	    {
...
#ifdef _SGI_SOURCE
	if (exit_attribute_mode && off_sequences[2] &&
	    (strcmp(exit_attribute_mode, off_sequences[2]) == 0))
        {
		cur_term->check_turn_off &= ~bit_attributes[2];
    }
#endif
    }
...
#ifdef	SYSV
# if _ABIAPI
    (void) ioctl(cur_term -> Filedes, TCGETS, &SHELLTTY);
# else
