...
#ident	"@(#)curses:screen/newscreen.c	1.23"
...
SCREEN	*
newscreen(char *type, int lsize, int csize, int tabsize, FILE *outfptr, FILE *infptr)
{
    ...
    _csmax = (short) (cswidth[0] > cswidth[1]+1 ?
    ...
    _scrmax = (short) (_scrwidth[0] > _scrwidth[1] ?
		(_scrwidth[0] > _scrwidth[2] ? _scrwidth[0] : _scrwidth[2]) :
		(_scrwidth[1] > _scrwidth[2] ? _scrwidth[1] : _scrwidth[2]));
    /* true multi-byte/multi-column case */
    _mbtrue = (char) (_csmax > 1 || _scrmax > 1);

    if ((curs_errno = (short) _chk_trm()) != -1)
    {
    ...
    LINES = SP->lsize = (short) (lsize > 0 ? lsize : lines) ;
    ...
    COLS = SP->csize = (short) (csize > 0 ? csize : columns);
    if (tabsize == 0)
	...
	if ((*marks = (char *) calloc((size_t) (nc * LINES), sizeof(char))) == NULL)
	    goto err;
    ...
    if ((SP->cur_hash = (int *) calloc((size_t) (2 * LINES), sizeof(int))) == NULL)
	goto err;
...
err2:
    ...
    SP->imode = (unsigned) (enter_insert_mode && exit_insert_mode);
    SP->dmode = (unsigned) (enter_delete_mode && exit_delete_mode);
    if (SP->imode && SP->dmode)
    {
    ...
    SP->ichok = (unsigned) (SP->imode || insert_character || parm_ich);
    SP->dchok = (unsigned) (delete_character || parm_dch);
