...
#ident	"@(#)curses:screen/mvcur.c	1.12"
...
static	short	Newy;
static  int	_homefirst(int, int, int, int), _mvrel(int, int, int, int, int), 
...
_mvright(int cx, int nx, int doit)
{
    register	chtype	*scp;
    register	char	*mks;
    register	int	nt, tx, x, stcost, iscont;
	...
	for (x = tx; x < nx; ++x)
	{
		...
		if (!ISMBIT(scp[x]))
		    ...
		    !(nx - x == 1 && nx < curscr->_maxx && ISCBIT(scp[nx])))
		    stcost += 1;
		else
