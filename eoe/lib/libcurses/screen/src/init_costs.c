 ...
 */
    *costptr++ = (short) (((enter_insert_mode) && (exit_insert_mode)) ?
	_cost_fn(enter_insert_mode, 0) + _cost_fn(exit_insert_mode, 0) : 0);

    *costptr++ = (short) (((enter_delete_mode) && (exit_delete_mode)) ?
	_cost_fn(enter_delete_mode, 0) + _cost_fn(exit_delete_mode, 0) : 0);

    while (i < FIRST_LOOP)
	*costptr++ = (short) _cost_fn(str_array[offsets[i++]], 1);

    while (i < SECOND_LOOP)
	*costptr++ = (short) _cost_fn(tparm(str_array[offsets[i++]], 10), 1);

    *costptr++ = (short) _cost_fn(tparm(cursor_address, 8, 10), 1);
    *costptr++ = (short) _cost_fn(tparm(row_address, 8), 1);

    xon_xoff = (char) save_xflag;
#ifdef	DEBUG
    if (outf)
    {
