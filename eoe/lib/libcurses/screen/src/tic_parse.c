...
dump_structure(short Booleans[], short Numbers[], short Strings[])
{
	...
	if (strlen(filename) > NAME_MAX )
		warning("'%s' filename too long, truncating to '%.255s'\n",filename, filename);
	...
	if (!check_only) {
		...
		DEBUG(1, "Created %.255s\n", filename);
	}
	else DEBUG(1, "Would have created %.255s\n", filename);
	...
	if (strlen(linkname) > NAME_MAX )
		warning("'%s' linkname too long, truncating to '%.255s'\n", linkname, linkname);
		...
		if (!check_only) {
			...
			if (link(filename, linkname) < 0)
			    syserr_abort("Can't link %s to %s", filename,
								linkname);
			}
#else
			if (link(filename, linkname) < 0)
		...
		else DEBUG(1, "Would have linked %.255s\n", linkname);
	    }
	}
