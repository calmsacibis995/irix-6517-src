...
static long
_timediff(struct _timeval t1, struct _timeval t2)
{
...
nap(int ticks)
{
    ...
    t.c_cc[VTIME] = tenths;
#if _ABIAPI
    (void) ioctl(ttyfd, TCSETSW, &t);
    n = read(ttyfd, &c, 1);
    (void) ioctl(ttyfd, TCSETSW, &ot);
#else
...
int
select(register int nfds, int *prfds, int *pwfds, int *pefds, struct _timeval *timeout)
{
    register	int	fd;
