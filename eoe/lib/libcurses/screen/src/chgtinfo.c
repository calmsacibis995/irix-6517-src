...
static void doremoves(int);
static void errexit(char *);
static int findtermfile(char *, char *);
...
int findtermfile(char *terminfo, char *term)
{
    char termfile[512];
...
char **argv;
{
    ...
    if (mkdir((const char *) term, 0755) < 0)
	errexit("mkdir() failed");
    rmstate++;
...
char *f1, *f2;
{
	...
	if (write(outfd, buf, (unsigned int) n) != n)
	    {
	    ret = 0;
	    break;
	    }
