...
static	void		_laddone(char *);
static	void		_keyfunc(char **, char **);
static	void		_keyfunc2(char **, char **);
...
static	void
_keyfunc(register char **keyptr, register char **lastkey)
{
...
static	void
_keyfunc2(register char **keyptr, register char **lastkey)
{
...
setkeymap(void)
{
	...
	malloc ((size_t) (key_size + (numkeys = (int) (p - keymap)))
    ...
    if ((p = (_KEY_MAP *) malloc((size_t) (numbytes = (int) sizeof(_KEY_MAP)
							* numkeys))) == NULL)
    {
...
out:
    ...
    if (key_size != 0)
    {
	(void) memcpy((char *) &(key_ptrs[numkeys]), (char *) cur_term->_keys, ((size_t) key_size * sizeof (_KEY_MAP *)));
	free(cur_term->_keys);
    }
    (void) memcpy((char *) (cur_term->internal_keys = p), (char *) keymap, (size_t) numbytes);
