...
static char	*_branchto(char *, char);
...
tparm(char *instring, ...)
{
	...
	switch (c = *cp++)
	{
		...
		*format = '\0';
		...
		if (c == 'd' && formatbuffer[2] == '\0') { /* fast path "%d" */
			char ai[16];
			char *ap;
