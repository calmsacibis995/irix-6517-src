...
#ident	"@(#)curses:screen/curshdr.h	1.12"
...
struct	screen
{
    ...
    int		baud;		/* baud rate of this tty */
...
extern	short	cswidth[],	/* byte size of multi-byte chars */
		_scrwidth[];	/* display size */
extern	short	_csmax,
		_scrmax;
extern	bool	_mbtrue;

#define	MBIT		0200		/* indicator for a multi-byte char */
#define	CBIT		0100000		/* indicator for a continuing col */
#define	RBYTE(x)	((x) & 0377)
#define	LBYTE(x)	(((x) >> 8) & 0377)
...
#define	_BLNKCHAR	' '
#define  _CTRL(c)   ((c >= 'A' && c <= 'Z') ? (c - 'A' + 1) : (c | 0100))
...
extern	int	(*_setidln)(int, int), (*_useidln)(void), (*_quick_ptr)(WINDOW *, chtype);

/* min/max functions */
#define	_MIN(a, b)	((a) < (b) ? (a) : (b))
#define	_MAX(a, b)	((a) > (b) ? (a) : (b))

extern	int	(*_do_slk_ref)(void), (*_do_slk_tch)(void), (*_do_slk_noref)(void);
extern	void	(*_slk_init)(void), (*_rip_init)(void);
extern	WINDOW	*_makenew(int , int , int , int);

/* prototype for realloc(), malloc(), calloc(), free(), exit(), getenv() */
#include <stdlib.h>

extern void perror (const char *);

/* replaces prototype for serveral calls - memset(), strcpy() strlen() */
#include <string.h>
