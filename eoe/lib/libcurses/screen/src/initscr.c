...
/* Free structures we are about to throw away so we can reuse the memory. */
    ...
    {
#ifdef	_SGI_SOURCE
	if (SP->tcap->_check_fd == 0)	/* STDIN_FILENO */
	    /*
	     * Don't bother closing the terminal input because
	     * the following call to newscreen() requires stdin.
	     */
   	    SP->tcap->_check_fd = -1;	/* now delterm() cannot close it */
#endif
	delscreen(SP);
	SP = NULL;
    }
