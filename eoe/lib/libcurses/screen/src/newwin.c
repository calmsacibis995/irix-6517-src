...
WINDOW	*
newwin(register int nlines, register int ncols, register int by, register int bx)
{
    register	WINDOW	*win;
    int		counter = 0;
...
int
_image(register WINDOW *win)
{
    register	int	i, nlines = win->_maxy,
#ifdef	_VR3_COMPAT_CODE
			oscols = win->_maxx * (int) sizeof (_ochtype),
#endif	/* _VR3_COMPAT_CODE */
			scols = win->_maxx * (int) sizeof (chtype);
