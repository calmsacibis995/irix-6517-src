...
int
prefresh(WINDOW	*pad, int pminy, int pminx,
...
int
_padjust(WINDOW *pad, int pminy, int pminx,
	 int sminy, int sminx, int smaxy, int smaxx)
{
    ...
    if (((padwin->_cury = (short) (pad->_cury - pminy)) < 0) || (padwin->_cury >= prows))
	 padwin->_cury = 0;
    if (((padwin->_curx = (short) (pad->_curx - pminx)) < 0) || (padwin->_curx >= pcols))
	 padwin->_curx = 0;
    ...
    padwin->_maxy = (short) prows;
    padwin->_maxx = (short) pcols;
