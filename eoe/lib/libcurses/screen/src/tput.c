...
#ident	"@(#)curses:screen/tput.c	1.23"
...
static int settabs(void);
static int allnumeric(char *);
static int getpad(char *);
static int outputcap(char *, int, char **);
...
#define BUFLEN 128
	...
	while (fgets (buff,BUFLEN,stdin) != NULL)
	{
...
static int
outputcap(register char *cap, int argc, char **argv)
{
...
*/
int
allnumeric(register char *string)
{
    if (*string)
    {
        while (*string)
...
*/
static void
setdelay (register int delay, struct delay delaytable[], int bits, tcflag_t *flags)
{
    ...
    *flags |= (tcflag_t) lastdelay -> d_bits;
}

/*
...
#if defined(SYSV) || defined(USG)
# if _ABIAPI
    struct termios termmode;		/* To hold terminal settings. */
# define GTTY(fd,mode)	ioctl(fd, TCGETS, mode)
# define STTY(fd,mode)	ioctl(fd, TCSETSW, mode)
# else
...
# define TAB3		XTABS
	...
	CurrentBaudRate = SPEED(termmode);

    if (xon_xoff)
	{
	OFLAG(termmode) &= (unsigned) ~(NLbits | CRbits | BSbits | FFbits | TBbits);
	}
    else
	{
