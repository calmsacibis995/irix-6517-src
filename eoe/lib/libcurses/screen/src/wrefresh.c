...
#ident	"@(#)curses:screen/wrefresh.c	1.41"
...
static void	_updateln(int), _turn_off_background(void), 
		...
		_setmark2(int, int, chtype *), 
		_rmargin(int), _useceod(int, int);
static int	_useidch(chtype *, chtype *, int, int, int *), 
		_prefix(chtype *, chtype *, int, int, int *), _getceod(int, int);
static chtype	*_shove(int);
...
wrefresh(WINDOW *win)
{
    ...
    if ((_INPUTPENDING = (bool) _chkinput()) == TRUE)
    {
	if (curwin)
	    curscr->_clear = TRUE;
	return (OK);
...
/* SS: colors */
	...
	/* there is type-ahead */
	if (!curwin && (_INPUTPENDING = (bool) _chkinput()) == TRUE)
	{
	    _VIRTTOP = (short) wy;
	    goto done;
	}
...
_shove(register int wy)
{
	...
	if (line)
	    free((char *) line);
	line = (chtype *) malloc((size_t) scrco * sizeof(chtype));
	length = line ? scrco : 0;
    }
	...
	*cp = didshift ? ' ' : _CHAR(*cp);
    ...
    if (wy == cury)
	_virtscr->_curx = (short) (curx >= scrco ? scrco - 1 : curx);
...
_updateln(int wy)
{
    ...
    chtype		wc, sc, *ep;
    int			wx_sav, multi_col;

    redraw = (bool) (_virtscr->_firstch[wy] == _REDRAW);
    endns = _ENDNS[wy];
    begns = _BEGNS[wy];
	...
	else
	{
		...
		if (_DARKCHAR(*wp))
		    break;
	    for(sp = scp+blnkx+1; blnkx < scrco - 1; ++sp,++blnkx)
		if(!ISCBIT(*sp))
		    break;
...
#define	SLACK	4
    ...
    didcolor = (bool) ((color_marks) ? FALSE : TRUE);
...
/* SS: colors */
		...
		mtch = x - wx;
		(void) memcpy((char *) scp, (char *) wcp, (size_t) mtch * sizeof(chtype));
		wcp += mtch;
		scp += mtch;
		wx = x;
	    }
...
no_change:
	    ...
	    _outwch(tilde_glitch && _CHAR(wc) == '~' ? '`' : wc);
...
do_insert_char:
	...
	for(x1 = 0; x1 < idch; ++x1)
	{
	    ...
	    if (_ATTR(wc) != curscr->_attrs)
		_VIDS(_ATTR(wc),curscr->_attrs);
	    _outwch(_CHAR(wc) == '~' && tilde_glitch ? '`' : wc);
	    ++cx;
	}
	...
	scp[x1] = scp[x2];
    (void) memcpy((char *) scp, (char *) wcp, (size_t) idch * sizeof(chtype));
...
_rmargin(int wx)
{
	...
	_outwch(tilde_glitch && _CHAR(wcp[wx]) == '~' ? '`' : wcp[wx]);
...
/* SS: colors */
	...
	w = _scrwidth[TYPE(RBYTE(curscr->_y[cy][x]))];
