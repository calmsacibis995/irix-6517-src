...
#ident	"@(#)curses:screen/testterminfo.c	1.10"
/*
...
void
dumb_gocurs(register int x, register int y)
{
...
void
gocurs(register int x, register int y)
{
	if (cursor_address) putp(tparm(cursor_address, x, y));
...
void
tst_scroll(register int top, register int bot, int pause)
{
...
void
tst_Pscroll(register int top, register int bot)
{
	...
	if (plab_norm) {
		...
		for(i = 1; i <= num_labels; ++i) {
			...
			for(k = 0; k < label_width; ++k)
				label[j * label_width + k] = (char) ('0' +(k % 10));
