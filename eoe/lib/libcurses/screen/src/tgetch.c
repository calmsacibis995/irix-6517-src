...
#ident	"@(#)curses:screen/tgetch.c	1.15"
...
static int	_getkey(int, int *), _fpk(void), _pk(void);
static int 	get_xterm_mouse(int, int *);
static void 	_map_button(int *);
static void	_catch_alarm(void);

#ifdef	FIONREAD
static	int _readchar(void);
#endif
...
#define WAIT3		333
	...
	if (interpret && cur_term->funckeystarter[inp])
	    collapse = _getkey(interpret - 1, (int *)&inp);
    }
...
_readchar(void)
{
	...
	if (cur_term->_delay > 0)
	{
	    ...
	    i = select(20, (fd_set *)&infd, (fd_set *)NULL, (fd_set *)NULL, (struct timeval *)&t);
	    if (i < 1)
		return (ERR);
	}
...
static	int
_getkey(register int blockpeek, register int *inp)
{
	...
	if (kp[key]->_sends[0] == *inp)
	{
		...
		{
		    (*chars_onQ)++;
		    switch ((int) (inputQ[i] = (chtype) ((blockpeek) ? _pk() : _fpk())))
		    {
			case -2:
			    /*
...
*/
static int
get_xterm_mouse(register int blockpeek, register int *i)
{
    chtype	*inputQ = cur_term->_input_queue;		/* ??? */
    char	*chars_onQ = &(cur_term->_chars_on_queue);
    ...
    char1 = (short) (inputQ[(*i)++] = (chtype) ((blockpeek) ? _pk() : _fpk()));
    ...
    if (char1 != '0')
    {
        char2 = (short) (inputQ[(*i)++] = (chtype) ((blockpeek) ? _pk() : _fpk()));
    ...
    for (j=0; j<2; j++)
    {
	c1 = (short) (inputQ[(*i)++] = (chtype) ((blockpeek) ? _pk() : _fpk()));
	if (c1 < 0)
	    return (c1);
        (*chars_onQ)++;
	if (c1 >= ' ' && c1 <= '~')	/* ascii char */
	{
        ...
        {
            c2 = (short) (inputQ[(*i)++] = (chtype) ((blockpeek) ? _pk() : _fpk()));
	    if (c2 < 0)
		return (c2);
            (*chars_onQ)++;
    	    if (c2 >= ' ' && c2 <= '~')
...
_fpk(void)
{
    ...
    rc = select(20, (fd_set *)&infd, (fd_set *)outfd, (fd_set *)exfd, (struct timeval *)&t);
    if (rc < 0)
...
_map_button (int *inp)
{
	...
	if (!(SP->_map_mbe_to_key & (unsigned long)
