 ...
 *
 *  $Log: tic_read.c,v $
 *  Revision 1.3  1993/09/09 00:03:07  igehy
 *  Converted to 64-bit.
 *
 * Revision 1.2  1993/08/04  22:29:40  wtk
 * Fullwarn of libcurses
 *
 * Revision 1.1  1991/12/06  14:06:14  daveh
 *
...
int
read_entry(char *filename, struct _bool_struct *bptr,
	   struct _num_struct *nptr, struct _str_struct *sptr)
{
	...
	if (swapping)
	{
	    header.magic = (short) swap(header.magic);
	    header.name_size = (short) swap(header.name_size);
	    header.bool_count = (short) swap(header.bool_count);
	    header.num_count = (short) swap(header.num_count);
	    header.str_count = (short) swap(header.str_count);
	    header.str_size = (short) swap(header.str_size);
	}
	...
	read(fd, TermNames, (unsigned) min(127, header.name_size));
	TermNames[127] = '\0';
	...
	read(fd, UB, (unsigned) min(BoolCount, header.bool_count));
	...
	if (!swapping)
	    read(fd, (char *)UN, (unsigned) min(NumCount, header.num_count) * 2);
	else
	{
		...
		if (bytebuf[1] == 0377)
			...
			UN[i] = (short) (bytebuf[0] + 256 * bytebuf[1]);
		else
		    UN[i] = (short) (bytebuf[0] + 256 * bytebuf[1]);
	    }
	}
	...
	while (num_strings > 0)
	{
	    ...
	    if (swapping)
	    {
		numread = read(fd, byte, (unsigned) (2*min(num_strings, OFFSET_BUFSIZE)));
		if (numread <= 0)
		{
		    close(fd);
		    return(-1);
	    ...
	    {
		numread = read(fd, number, (unsigned) (2*min(num_strings, OFFSET_BUFSIZE)));
		if (numread <= 0)
		{
		    close(fd);
		    return(-1);
	...
	numread = read(fd, pst, (unsigned) header.str_size);
	close(fd);
