...
scr_reset(FILE *filep, int type)
{
        ...
        if (slk != NULL)
	{
	    ...
	    for (i = 0; i < labmax; i++)
	    {
	        ...
	        if ((fread(slk->_ldis[i], sizeof(char), (size_t) lablen, filep)
							!= (size_t) lablen) ||
		    (fread(slk->_lval[i], sizeof(char), (size_t) lablen, filep)
							!= (size_t) lablen))
	        {
		    goto err;
	        }
	...
	{
	    if (fseek(filep, (long) (2 * labmax * lablen * (long) sizeof(char)), 1) != 0)
		goto err;
	}
    }
	...
	{
	    if (fseek(filep, (long) (colors * (long) sizeof(_Color) +
			color_pairs * (long) sizeof(_Color_pair)), 1) != 0)
		goto err;
	}
	else
	{
		...
		if (can_change)
		    ...
		    if ((save_ctp = (ctp = (_Color *)
				malloc((size_t) colors * sizeof (_Color))))
							== NULL)
			 goto err;

		    if (fread(ctp, sizeof(_Color), (size_t) colors, filep)
							!= (size_t) colors)
			goto err;

	            for (i=0; i<colors; i++, ctp++)
	    	    {    
		        init_color ((short) i, ctp->r, ctp->g, ctp->b);
		    }
		    free (save_ctp);
	    	}
		...
		{
		    if (fseek(filep, (long) (colors * (long) sizeof(_Color)), 1) != 0)
			goto err;
		}
	    }
	    ...
	    if ((save_ptp = (ptp = (_Color_pair *)
			malloc ((size_t) color_pairs * sizeof (_Color_pair))))
								== NULL)
		 goto err;
	    if (fread(ptp, sizeof(_Color_pair), (size_t) color_pairs, filep)
						!= (size_t) color_pairs)
	    {
err:
			if (win != NULL)
	    ...
	    {    
		 if (ptp->init)
		     init_pair ((short) i, ptp->foreground, ptp->background);
	    }
	    free (save_ptp);
	}
    }
