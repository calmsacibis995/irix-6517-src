...
int
scr_ll_dump(register FILE *filep)
{
    ...
    magic = (short) ((!slk) ? 0 : (slk->_win) ? 2 : 1);
    if (fwrite((char *) &magic, sizeof(int), 1, filep) != 1)
	goto err;
    if (magic)
    {
	short	i, labmax = slk->_num, lablen = (short) (slk->_len + 1);
	...
	for (i = 0; i < labmax; i++)
	    if ((fwrite(slk->_ldis[i], sizeof(char), (size_t) lablen, filep)
							!= (size_t) lablen) ||
		(fwrite(slk->_lval[i], sizeof(char), (size_t) lablen, filep)
							!= (size_t) lablen))
	    {
		goto err;
	    }
    ...
    magic = (short) ((cur_term->_pairs_tbl) ? 1 : 0);
    if (fwrite((char *) &magic, sizeof(int), 1, filep) != 1)
	goto err;
    if (magic)
    {
	...
	if (can_change)
	{
	    if (fwrite((char *) &(cur_term->_color_tbl->r), sizeof(_Color),
				(size_t) COLORS, filep) != (size_t) COLORS)
		goto err;
	}
	...
	if (fwrite((char *) &(cur_term->_pairs_tbl->foreground),
		sizeof(_Color_pair), (size_t) COLOR_PAIRS, filep)
		!= (size_t) COLOR_PAIRS)
	    goto err;
    }
