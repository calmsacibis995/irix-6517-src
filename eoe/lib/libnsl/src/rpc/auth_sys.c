...
#ident	"@(#)librpc:auth_sys.c	1.2.4.1"
 ...
 */
#if defined(__STDC__) 
        #pragma weak authsys_create		= _authsys_create
        #pragma weak authsys_create_default	= _authsys_create_default
#endif
...
/* Boy, here was some FINE USL-quality code.  Obviously never tested.
 * It didn't even conform to their own published prototypes.
 */
static bool_t
authsys_validate(
	register AUTH *auth,
	register struct opaque_auth *rverf)
{
	register struct audata *au;
	XDR xdrs;

	if (rverf->oa_flavor == AUTH_SHORT) {
		au = AUTH_PRIVATE(auth);
		xdrmem_create(&xdrs, rverf->oa_base, rverf->oa_length, 
			XDR_DECODE);
