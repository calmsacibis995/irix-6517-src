...
static struct hostent *_tcpip_gethostent(), *he_interpret();
static char HOSTDB[] = "/etc/hosts";

static struct servent *_tcpip_getservent(), *se_interpret();
static char SERVDB[] = "/etc/services";

char *malloc(), *calloc();

static u_long inet_addr();
static void _tcpip_sethostent(int f);
static void _tcpip_endhostent(void);
static void _tcpip_setservent(int f);
static void _tcpip_endservent(void);
