...
/*
 * 	attr_clone -- Extended Attribute library extension routine
 */
#ident	"@(#)libattr/attr_clone.c	1.1"
...
/*
 * Given two pathnames, copy all of the Extended User Attributes from
 ...
 * or not (and possibly other things in the future).
 *
...
static int
attr_clone_copy(const char *srcpath, const char *dstpath, char *list_buf,
		      char *attr_buf, int flags);
 ...
 */
int
attr_clone(const char *srcpath, const char *dstpath, int flags)
{
	 ...
	 */
	error = attr_clone_copy(srcpath, dstpath, lbuf, abuf, flags);
	if (error) {
		free(abuf);
		return(error);
	}
	 ...
	 */
	error = attr_clone_copy(srcpath, dstpath, lbuf, abuf, flags|ATTR_ROOT);
...
/*
 * Copy all the attributes (if we can see any) from one path to the other.
 *
		      ...
		      char *attr_buf, int flags)
{
	...
	do {
		if (attr_list(srcpath, list_buf, BUFSIZE, flags, &cursor) < 0)
			return(1);
		...
		for (i = 0; i < alist->al_count; i++) {
			...
			if (attr_get(srcpath, attr->a_name, attr_buf, &length,
					      flags) < 0) {
				return(1);
			}
			if (attr_set(dstpath, attr->a_name, attr_buf, length,
					      flags) < 0) {
				return(-1);
			}
		}
