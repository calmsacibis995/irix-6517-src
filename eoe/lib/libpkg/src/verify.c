...
cverify(int fix, char *ftype, char *path, struct cinfo *cinfo)
{
	...
	if(setval || (cinfo->size == BADCONT))
		cinfo->size = (long)status.st_size;
	...
	if(setval || (cinfo->cksum == BADCONT))
		cinfo->cksum = (long)(unsigned long)mycksum;
...
/* ARGSUSED */
int
averify(int fix, char *ftype, char *path, struct ainfo *ainfo, ushort shared)
{
	struct stat		status;	/* file status buffer */
	...
	/*the last comparison put in for installf compatibility */
		ainfo->mode = (long)(status.st_mode & 07777);
