...
static char	*deferr, *errmsg,
		*strtok_local(char *, char *),
		**match(CKMENU *, char *, int);
static char	*defhlp;
static int	getstr(char *, char *, char *, char *, char*),
		getnum(char *, int, int *, int *);
static struct _choice_ *next(struct _choice_ *chp);
...
int
ckitem(CKMENU *menup, char *item[], short max, char *defstr, char *error, char *help, char *prompt)
{
	int	n, i;
	char	strval[128];
	char	**list;
...
start:
	...
	if(!strcmp(strval, "??")) {
		goto reprint;
	} if(defstr && !strcmp(strval, defstr)) { 
		item[0] = defstr;
		item[1] = NULL;
	} else {
...
setitem(CKMENU *menup, char *choice)
{
	...
	chp->token = strtok_local(pt, " \t\n");
	chp->text = strtok_local(NULL, "");
	if (chp->text)
...
static char *
strtok_local(char *string, char *sepset)
{
	register char	*p, *q, *r;
	static char	*savept;
