...
static FILE *
opennewdevtab(char **pname)
/*	char  **pname;			 (ptr to temp filename's path) */
{
...
static int
lkdevtab(char *o_mode, int lktype)
...
mkdevtabent(char *alias, char **attrlist)
...
int
_putdevtabrec(FILE *stream, struct devtabent *rec)
...
int
_adddevtabrec(char *alias, char **attrval)
...
int
_moddevtabrec(char *device, char **attrval)
...
int
_rmdevtabattrs(char *device, char **attributes, char ***notfounds)
/*	char   *device;			 Device to modify */
