...
static	int locklkfile(int lkflag);
static	int getlkcnt(void);
static	int readlocks(void);
static	int getlocks(void);
static	int putlocks(char **newlist, long key);
static	int writelks(int tblcnt);
static	int appendlk(long key, char *alias);
static	int compresslks(void);
static	int freelkfile(void);
...
static	int
putlocks(char **newlist, long key)
/*	char	      **newlist;	 New devices to lock */
/*	long		key;		 Key we're locking stuff on */
{
	/* Automatic data */
 ...
 *
 *  Arguments:
 *	long key	The key on which the device was allocated
 ...
 *
 *  Arguments:
 *	long key		The key on which this device is being reserved.
 *
...
char **
devreserv(long key, char **rsvlst[])
/*	long		key;		 Key to reserve device on */
 ...
 *
 *  Arguments:
 *	long	key	The key that the device is locked on
...
int
devfree(long key, char *device)
/*	long	key;			 Key device is locked on */
