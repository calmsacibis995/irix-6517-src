...
#ident	"@(#)librpc:rpc_soc.c	1.4.5.1"
...
#if defined(__STDC__) 
#ifndef _LIBNSL_ABI
        #pragma weak authdes_create	= _authdes_create
...
extern int errno;
extern int t_errno;
static int bindresvport(int, struct sockaddr_in *, int *, int);
	...
	char *tp;
{
	...
	if (tbind == NULL) {
		(void) syslog(LOG_ERR, "clnt%s_create: out of memory", tp);
		rpc_createerr.cf_stat = RPC_SYSTEMERROR;
		rpc_createerr.cf_error.re_errno = errno;
		rpc_createerr.cf_error.re_terrno = t_errno;
		goto err;
 ...
 */
typedef bool_t (*oldresultproc_t)( void *, struct sockaddr_in *);

static oldresultproc_t clnt_broadcast_result;
/*
...
clnt_broadcast(
	...
	oldresultproc_t	eachresult)	/* call with each result obtained */
{
	...
	des_block *ckey)		/* optional conversation key to use*/
{
	...
	if (syncaddr) {
		...
		if ((nconf = _rpc_getconfip("udp")) == NULL &&
			...
			return (authdes_seccreate(servername, window,
					hostname, ckey));
