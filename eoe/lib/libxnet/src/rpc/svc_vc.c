...
*/ 
#if defined(__STDC__) 
        #pragma weak svc_vc_create	= _svc_vc_create
        #pragma weak svc_fd_create	= _svc_fd_create
#endif
...
again:
	...
	case T_LISTEN:
		...
		if (t_listen(xprt->xp_fd, t_call) == -1) {
			if (errno == EINTR) {
				/* ensure we don't leak memory here */
	...
	if (xprt->xp_tp) {
		...
		if (fstat(xprt->xp_fd, &statbuf) == -1) {
			(void) syslog(LOG_ERR,
			"rendezvous_request: cant find dev number (%d) (%m)",
			t_errno);
			goto err;
		}
	...
	if (fd == -1) {
		(void) syslog(LOG_ERR,
			"rendezvous_request: cant open connection (%d)(%m)",
			t_errno);
		goto err;
	}
	if ((tinfo.servtype != T_COTS) &&
		...
		(void) syslog(LOG_ERR,
			"rendezvous_request: illegal transport (%d)(%m)",
				t_errno);
		goto err;
	}
	...
	if (t_bind(fd, (struct t_bind *)NULL, res) == -1) {
		(void) syslog(LOG_ERR, "cannot bind svc connection (%d)(%m)",
			t_errno);
		goto err;
	}
	/*
	...
	if (t_accept(xprt->xp_fd, fd, &t_call2) == -1) {
		...
		if (t_errno == TLOOK) {
			retval = t_rcvdis(xprt->xp_fd, NULL);
			if (retval < 0 && t_errno == TNODIS) {
				retval = t_look(xprt->xp_fd);
			}
		}
		(void) syslog(LOG_ERR, 
		"cannot accept connection, t_errno: %d, errno: %d, t_look: %d",
			t_errno, errno, retval);
		goto err;
	}
	accepted = 1;
	/*
...
err:
	if (accepted == 0) {
		/* attempt to clear a pending T_CONN_IND down in the kernel */
		(void) t_snddis(xprt->xp_fd, t_call);
	}
	(void) t_free((char *)t_call, T_CALL);
	if (res)
		(void) t_free((char *)res, T_BIND);
...
svc_vc_rendezvous_ops()
{
	...
	if (ops.xp_recv == NULL) {
		...
		ops.xp_getargs = 
		    (bool_t (*)(struct __svcxprt_s *, xdrproc_t, void *))abort;
		ops.xp_reply = 
		    (bool_t (*)(struct __svcxprt_s *, struct rpc_msg *))abort;
		ops.xp_freeargs = 
		    (bool_t (*)(struct __svcxprt_s *, xdrproc_t, void *))abort;
		ops.xp_destroy = svc_vc_destroy;
	}
