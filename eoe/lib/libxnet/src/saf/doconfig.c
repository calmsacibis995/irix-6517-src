...
# ident	"@(#)libnet:saf/doconfig.c	1.6.2.1"

#ifdef __STDC__
#ifndef _LIBNSL_ABI
        #pragma weak doconfig	= _doconfig
#endif /* _LIBNSL_ABI */
#endif
...
/* forward static declarations - needed for ANSI c */
...
static int
dorun( register char *p, int waitflg)
{
	register char *tp;	/* scratch pointer */
	...
	if (!strcmp(p, "cd")) {
		...
		*tp = savech;
		...
		if ((pid = fork()) < 0) {
			...
			if (dup(0) != 2)
				return(-1);
			execle("/sbin/sh", "sh", "-c", p, 0, _environ);
