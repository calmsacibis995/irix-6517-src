#ident "$Id: nfs_server.c,v 1.16 1997/07/23 04:21:19 chatz Exp $"

/*
 * Handle metrics for cluster nfs_server (13)
 *
...
/* irix.nfs.server.badcalls */
  { (char *)&svstat.nbadcalls, { PMID(1,13,1), PM_TYPE_32, PM_INDOM_NULL, PM_SEM_COUNTER, {0,0,1,0,0,PM_COUNT_ONE} } },
/* irix.nfs.server.calls */
  { (char *)&svstat.ncalls, { PMID(1,13,2), PM_TYPE_32, PM_INDOM_NULL, PM_SEM_COUNTER, {0,0,1,0,0,PM_COUNT_ONE} } },
/* irix.nfs.server.reqs */
  { (char *)&svstat.reqs[0], { PMID(1,13,3), PM_TYPE_32, PM_INDOM_NFSREQ, PM_SEM_COUNTER, {0,0,1,0,0,PM_COUNT_ONE} } }
};

static int		nmeta = (sizeof(meta)/sizeof(meta[0]));
static int		fetched;

void nfs_server_init(int reset)
{
    int		i;
    int		indomtag;
    ...
    for (i = 0; i < nmeta; i++) {
        ...
        if (indomtag < 0 || indomtag >= PM_INDOM_NEXT) {
            __pmNotifyErr(LOG_ERR, "nfs_server_init: bad instance domain (%d) for metric %s\n",
...
void nfs_server_fetch_setup(void)
{
    fetched = 0;
}

int nfs_server_desc(pmID pmid, pmDesc *desc)
{
    int		i;
...
int nfs_server_fetch(pmID pmid, __pmProfile *profp, pmVPCB *vpcb)
{
    int		i;
    int		k;
    int		sts;
	...
	if (pmid == meta[i].m_desc.pmid) {
	    if (fetched == 0) {
#ifdef MPSA_SVSTAT
                if (sysmp(MP_SAGET, MPSA_SVSTAT, &svstat, sizeof(svstat)) < 0) {
#else
                errno = PM_ERR_PMID;
                if (1) {
#endif
                    __pmNotifyErr(LOG_WARNING, "nfs_server_fetch: %s", pmErrStr(-errno));
                    return -errno;
                }
                fetched = 1;
	    }
            ...
            switch (pmid) {
            case PMID(1,13,3): /* irix.nfs.server.reqs */
                vpcb->p_nval = 0;
		...
		vpcb->p_valfmt = sts;
            ...
            default: /* irix.nfs.server.anything_other_than_reqs */
                vpcb->p_nval = 1;
