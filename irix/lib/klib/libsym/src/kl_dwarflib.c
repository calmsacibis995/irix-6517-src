...
dw_free_die(dw_die_t *d)
{
	...
	if (d->d_name) {
		kl_free_block((k_ptr_t)d->d_name);
	}
	dnext = d->d_next;
	kl_free_block((k_ptr_t)d);
	return(dnext);
}
...
{
	if (t->t_type_str) {
		kl_free_block((k_ptr_t)t->t_type_str);
	}
...
dw_attr_name(Dwarf_Attribute dattr, int flags)
{
	...
	attr_name = (char *)kl_alloc_block(strlen(name) + 1, flags);
...
dw_type_str(dw_type_info_t *t, int flags)
{
	...
	type_str = (char *)kl_alloc_block(128, flags);
	...
	if (!(dp = t->t_dlist->d_next)) {
		...
		case DW_TAG_variable:
			kl_free_block((k_ptr_t)type_str);
			return((char *)NULL);
...
dw_type_info(Dwarf_Off off, dw_type_info_t *t, int flags) 
{
	 ...
	 */
	d = (dw_die_t *)kl_alloc_block(sizeof(dw_die_t), flags);
...
dw_type(dw_type_info_t *dti, int flag) 
{
	...
	dtp->dt_bit_size = dti->t_bit_size;
	dtp->dt_bit_offset = dti->t_bit_offset;
	return(dtp);
}
...
dw_populate_type(dw_type_t *dtp)
{
	...
	dti = (dw_type_info_t *)kl_alloc_block(sizeof(*dti), K_TEMP);
 ...
 */
dw_type_t *
dw_lkup_link(char *name, int type)
{
	...
	if ((dtp->dt_tag != DW_TAG_structure_type) && 
		...
		if(dtp->dt_tag == DW_TAG_typedef) {
			...
			dti_typedef = (dw_type_info_t *)
				kl_alloc_block(sizeof(dw_type_info_t),K_TEMP);
	    
			if(dw_type_info(dtp->dt_off, dti_typedef, K_TEMP) == 
							   (dw_type_info_t *)NULL) {
			...
			if((dti_typedef->t_actual_type == (dw_die_t *)NULL) ||
				...
				(t=dw_lkup_member(dti_typedef, name, K_TEMP)) == 
										(type_t *)NULL) {
			...
			member = dw_type((dw_type_info_t *)t->t_ptr,K_TEMP);
 ...
 */
type_t *
dw_lkup_member(dw_type_info_t *t, char *member, int flags)
{
	type_t *tp;
	dw_die_t *d;
	dw_type_info_t *dti;
	...
	while (next_off) {
		...
		if (dti->t_name && strcmp(dti->t_name, member) == 0) {
			tp = (type_t *)kl_alloc_block(sizeof(*tp), flags);
...
dw_base_value(k_ptr_t p, dw_type_t *dtp, k_uint_t *v)
{
	...
	if ((d->d_tag != DW_TAG_base_type) && 
			(d->d_tag != DW_TAG_enumeration_type)) {
		dw_free_type_info(dti);
		return(-1);
	}
...
dw_get_srcfile(kaddr_t pc)
{
	...
	src_file = (char *)kl_alloc_block(strlen(filename) + 1, K_TEMP);
