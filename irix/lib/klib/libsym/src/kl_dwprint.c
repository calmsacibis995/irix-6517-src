		    ...
		    int level, int flags, FILE *ofp)
{
	...
	if (d->d_sibling_off) {
		dw_die_t *d1;
		d1 = (dw_die_t *)kl_alloc_block(sizeof(*d1), K_TEMP);
			  ...
			  int flags, FILE *ofp)
{
	...
	d = dti->t_actual_type;
	d1 = (dw_die_t *)kl_alloc_block(sizeof(dw_die_t), K_TEMP);

	if (ptr) {
		switch (dti->t_byte_size) {
		case 1:
			val = *(char*)ptr;
			break;
		...
		if (dti->t_bit_size) {
			if ((dti->t_bit_offset + dti->t_bit_size) < 
											(dti->t_byte_size * 8)) {

				/* We need to right shift the value to align 
				 * the data properly in the value buffer
				 */
				val = (val >> ((dti->t_byte_size * 8) - 
								(dti->t_bit_offset + dti->t_bit_size)));
			}

			/* Keep only those bits which are valid (bit_size)
			 */
			val = val & ((1 << dti->t_bit_size) -1);
		}
...
dw_dump_struct(k_ptr_t buf, dw_type_info_t *t0, int level, int flags, FILE *ofp) 
{
	...
	t1 = (dw_type_info_t *)kl_alloc_block(sizeof(dw_type_info_t), K_TEMP);
	while (next_off) {
