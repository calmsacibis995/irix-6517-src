...
#define M_ADDR(symp) (PTRSZ64 ? (*(uint64*)symp) : (*(uint*)symp))
...
addr_to_mlinfo(kaddr_t addr)
{
	...
	while (mlinfo) {
		...
		kl_get_struct(mlinfo, ML_INFO_SIZE, mlp, "ml_info");

		text = kl_kaddr(mlp, "ml_info", "ml_text");
		end = kl_kaddr(mlp, "ml_info", "ml_end");
		...
		if (text && ((addr >= text) && (addr <= end))) {
			...
			mlinfo = kl_kaddr(mlp, "ml_info", "ml_next");
		}
	}
	kl_free_block(mlp);
	return((k_ptr_t)NULL);
...
ml_symname(char *strtab, k_ptr_t symp, k_ptr_t mlp)
{
	...
	if (!strtab) {
		 ...
		 */
		if (!(stringtab = kl_kaddr(mlp, "ml_info", "ml_stringtab"))) {
			return((char *)NULL);
		}
		strtabsz = kl_int(mlp, "ml_info", "ml_strtabsz", 0);
	 ...
	 */
	name = (char*)kl_alloc_block(strlen(s) + 1, K_TEMP);
	strcpy(name, s);
	if (strtab_alloced) {
...
ml_findsym(kaddr_t addr, char *name, k_ptr_t mlp)
{
	...
	nsyms = kl_int(mlp, "ml_info", "ml_nsyms", 0);
	if (nsyms == 0) {
		return((k_ptr_t)NULL);
	}
	...
	for (i = 0; i < nsyms; i++) {
		...
		if (name) {
			...
			if (addr < M_ADDR(symp)) {
				 ...
				 */
				if (i) {
					kl_get_block(lastsym, ML_SYM_SIZE, symp, "ml_sym");
					return (symp);
				}
				else {
					kl_free_block(symp);
 ...
 */
symaddr_t *
ml_insertaddr(kaddr_t lowpc, kaddr_t highpc, 
	char *name, symaddr_t *last)
{
	char *s = 0;
    symaddr_t *newsp;
     ...
     */
    newsp = (symaddr_t *)kl_alloc_block(sizeof(symaddr_t), K_PERM);
    newsp->s_lowpc = lowpc;
...
ml_addrtonm(kaddr_t addr)
{
	 ...
	 */
	if (kl_int(mlp, "ml_info", "ml_nsyms", 0) == 0) {
...
ml_nmtoaddr(char *name)
{
	...
	while (mlinfo) {
		 ...
		 */
		if (!kl_kaddr(mlp, "ml_info", "ml_text")) {
			mlinfo = kl_kaddr(mlp, "ml_info", "ml_next");
			continue;
		}
...
ml_funcaddr(kaddr_t addr) 
{
	 ...
	 */
	ml_text = kl_kaddr(mlp, "ml_info", "ml_text");
	kl_free_block(mlp);
	...
	blk = kl_alloc_block(4096, K_TEMP);

	while (start_addr >= ml_text) {
		cur = (k_ptr_t)((unsigned)blk + (size - 4));
		cur_addr = start_addr;
		kl_get_block(blk_addr, size, blk, "instr");
		for (i = 0; i < instr_cnt; i++, cur_addr -= 4) {
			...
			if (off < 0) {
				...
				if (cur_addr < sym_addr) {
					if (sym_end) {
						ml_insertaddr(sym_addr, sym_end, 
								(char *)NULL, lastsp);
						return(sym_addr);
					}
					return((kaddr_t)NULL);
...
ml_funcend(kaddr_t addr)
{
	 ...
	 */
	ml_end = kl_kaddr(mlp, "ml_info", "ml_end");
	kl_free_block(mlp);
	if (ml_end % 3) {
		 ml_end = ml_end & (~3);	
	}
	...
	blk_addr = addr;
	do {
		kl_get_block(blk_addr, 4, (char*)&instr, "instr");
