...
typedef struct trace_rec {
	...
	kaddr_t		 	kthread;
	k_ptr_t		 	ktp;
	kaddr_t			kthread2;	/* for when trace jumps to other kthread */
	k_ptr_t			ktp2;
	kaddr_t			exception;
	k_ptr_t			exp;
	kaddr_t			u_eframe;
...
#define S_INTSTACK 		2
#define S_BOOTSTACK 	3
#define S_KEXTSTACK 	4
#define S_STHREADSTACK	5
#define S_XTHREADSTACK	6
#define S_RAW_STACK     7
#define S_UNKNOWN_STACK	8
...
#define UPDATE_FRAME(FUNCNAME, PC, RA, SP, ASP, SRCNAME, LINE_NO, SIZE) \
		...
		enqueue((element_t **)&trace->frame, (element_t *)curframe); \
		trace->nframes++; \
...
#define IS_TEXT(addr) kl_funcaddr(addr)
