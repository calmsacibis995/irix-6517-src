...

{
  ...
  print_table(&perfy_options, counts, (pid_t) pid, command);
...
#define	ID_LEN			(10 + 19 + PID_LEN + PRCOMSIZ )
#ifdef FILTER
#define ID_FORMAT               ""
#else 
#define	ID_FORMAT		"Costs for pid %*d (%s) "
#endif
#define COUNT_ID_FORMAT            "Counts for pid %*d (%s)"    
...
#define	PRINT_HEADER(COST,PERFY)					\
									\
  fprintf(fpout, "\n");							\
  if (pid <= 0) {							\
    id[0]  = '\0';							\
  } else {                                                              \
    sprintf(id, PERFY ? ID_FORMAT : COUNT_ID_FORMAT , PID_LEN, pid, exec_command); \
  }									\
...

{
  ...
  if (nevents > 0) {
    PresentResults(options,pid,exec_command,tally,nevents);
  }
