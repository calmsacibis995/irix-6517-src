...
#ident	"@(#)libc-port:gen/fmtmsg.c	1.11"
 ...
 *	CONNAME		Filesystem entry name for the system (remote) console
 */
...
static char *noesc(char *);
static struct sevstr *getauxsevs(char *);
static void sevstrset(void);
static void msgverbset(void);
static int writemsg(FILE *, int , const char *, int, const char *, const char *, const char *);
...
exttok(const char *tok, const char *delims)
{
	...
	/* Automatic Data */
	const char   *tokend;		/* Ptr to the end of the token */
	const char   *p, *q;	 	/* Temp pointers */
		...
		*tokend = '\0';
		...
		if (*(tokend = exttok(current, ":,")) == ',') {
		    ...
		    *tokend = '\0';
			...
			if ( strchr(prstr,',') != ( char * ) NULL ) {
                                current = tokend;
                                current++;
                                continue;
                        }  /* If the prstr contains a comma ignore the sev */
