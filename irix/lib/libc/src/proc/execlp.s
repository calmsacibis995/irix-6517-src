 ...
 */
	.weakext	execlp, _execlp

NESTED(_execlp, FRAMESZ, ra)
        .mask   M_RA, RAOFF-FRAMESZ
	SETUP_GP
	PTR_SUBU sp,FRAMESZ
	SETUP_GP64(GPOFF,_execlp)
	SAVE_GP(GPOFF)

	REG_S	ra,RAOFF(sp)
	/* execvp is basically a varargs so need to home all args */
	REG_S	a1,A1HOME(sp)
	REG_S	a2,A2HOME(sp)
	REG_S	a3,A3HOME(sp)
#if (_MIPS_SIM != _MIPS_SIM_ABI32)
	...
	/* We will call execvp - so we need to roll up the doubleword
	 * aligned pointers to be only word aligned, since this is what
	 * execvp (and the kernel) expects. Since these are pointers,
...
1:	PTR_L	v0,0(a2)
	...
	jal	_execvp		# _execvp(file, argv)
	REG_L	ra,RAOFF(sp)
	RESTORE_GP64
	PTR_ADDU sp,FRAMESZ
	j	ra
	END(_execlp)		# _execlp(file, arg1, arg2, ..., 0);
