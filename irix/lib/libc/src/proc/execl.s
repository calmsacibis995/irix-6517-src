...
#endif
	.weakext	execl, _execl

NESTED(_execl, FRAMESZ, ra)
        .mask   M_RA, RAOFF-FRAMESZ
	SETUP_GP
	PTR_SUBU sp,FRAMESZ
	SETUP_GP64(GPOFF,_execl)
	SAVE_GP(GPOFF)
	...
	/* We will call execv - so we need to roll up the doubleword
	 * aligned pointers to be only word aligned, since this is what
	 * execv (and the kernel) expects. Since these are pointers,
...
1:	PTR_L	v0,0(a2)
	...
	jal	_execv		# _execv(file, argv)
	REG_L	ra,RAOFF(sp)
	RESTORE_GP64
	PTR_ADDU sp,FRAMESZ
	j	ra
	END(_execl)		# _execl(file, arg1, arg2, ..., 0);
