...
/*
 *	pcreatlp(name, arg,...,0)	(like pcreatl, but does path search)
 */
	.weakext	pcreatelp, _pcreatelp

NESTED(_pcreatelp, FRAMESZ, ra)
        .mask   M_RA, RAOFF-FRAMESZ
	SETUP_GP
	PTR_SUBU sp,FRAMESZ
	SETUP_GP64(GPOFF,_pcreatelp)
	SAVE_GP(GPOFF)

	REG_S	ra,RAOFF(sp)
	/* pcreatevp is basically a varargs so need to home all args */
	REG_S	a1,A1HOME(sp)
	REG_S	a2,A2HOME(sp)
	REG_S	a3,A3HOME(sp)
#if (_MIPS_SIM != _MIPS_SIM_ABI32)
	...
	/* We will call pcreatevp - so we need to roll up the doubleword
	 * aligned pointers to be only word aligned, since this is what
	 * pcreatevp (and the kernel) expects. Since these are pointers,
...
1:	PTR_L	v0,0(a2)
	...
	jal	_pcreatevp		# _pcreatevp(file, argv)
	REG_L	ra,RAOFF(sp)
	RESTORE_GP64
	PTR_ADDU sp,FRAMESZ
	j	ra
	END(_pcreatelp)		# _pcreatelp(file, arg1, arg2, ..., 0);
