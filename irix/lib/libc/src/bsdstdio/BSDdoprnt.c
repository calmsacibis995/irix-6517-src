...
#ident	"@(#)ucblibc:port/print/doprnt.c	1.3"
...
static void _mkarglst(char *, stva_list, stva_list[]);
static void _getarg(char *, stva_list *, int);
...
static void
_dowrite(register char *p, register int	n, register FILE *iop, register unsigned char **ptrptr)
{
...
int
_doprnt(const char *format, va_list in_args, register FILE *iop)
{
	...
	for ( ; ; ) {
		...
		if ((fcode = *format) != '\0' && fcode != '%') {
			...
			count += (int)(n = format - bp); /* n = no. of non-% chars */
			PUT(bp, (int)n);
		}
		if (fcode == '\0') {  /* end of format; return */
			register int nn = (int)(bufptr - iop->_ptr);
			iop->_cnt -= nn;
			iop->_ptr = bufptr;
