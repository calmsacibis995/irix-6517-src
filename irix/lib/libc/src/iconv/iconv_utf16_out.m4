...
define(`OUTPUT_SETVAL',`
    ...
    if ( ! ( ( ( unsigned int ) 'RETURN_VAR` ) >> 16 ) ) {
	'OUTPUT_CHECK_COUNT(<= 0,)`
	'OUTPUT_COUNT_ADJUST(1)`
	'OUTPUT_ASSIGN( OUTPUT_PTR_VAR, RETURN_VAR )`;
	'OUTPUT_PTR_ADJUST(1)`
    } else {
	/* UCS4 values above the UTF16 range will be truncated silently	*/
	'OUTPUT_COUNT_ADJUST(2)`
	'OUTPUT_CHECK_COUNT(< 0,OUTPUT_COUNT_ADJUST(-2))`
	'OUTPUT_PTR_ADJUST(2)`
	'RETURN_VAR` -= ( 1 << 16 );
	'OUTPUT_ASSIGN( OUTPUT_PTR_VAR - 2, ( ( ( RETURN_VAR >> UTF16_BITS ) & UTF16_MASK ) + UTF16_HIGH ) )`;
	'OUTPUT_ASSIGN( OUTPUT_PTR_VAR - 1, ( ( ( RETURN_VAR               ) & UTF16_MASK ) + UTF16_LOW  ) )`;
    }
