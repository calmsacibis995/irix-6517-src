...
#ident	"@(#)libc-port:gen/ctime.c	1.15"
#ifdef __STDC__
#ifndef  _LIBC_ABI
...
{
	struct tm res;
	return(asctime_r(localtime_r(t, &res), buf));
}
#endif
...
asctime_r(const struct tm *t, char *buf)
{
	...
	const char *Date = "Day Mon 00 00:00:00 0000\n";
	...
	*cp++ = *ncp++;
	...
	cp = ct_numb(cp, *--tp+100);
	cp = ct_numb(cp, *--tp+100);
	cp = ct_numb(cp, (t->tm_year+1900)/100);
	cp--;
