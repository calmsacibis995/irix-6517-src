...
struct qelem *elem, *pred;
{
	...
	register struct qelem *pnext;

	if (!pred) {
		/*
		 *  For XPG4 when 'pred' is NULL the forward and
		 *  backward pointers of 'elem' are initialized to
		 *  NULL pointers.
		 */
		pelem->q_forw = (struct qelem *)0;
		pelem->q_back = (struct qelem *)0;
		return;
	}
	pnext = ppred->q_forw;
