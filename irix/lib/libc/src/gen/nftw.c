...
#include <sgidefs.h>	/* for _MIPS_SIM defs */
	...
	#pragma weak nftw64 = _nftw
	#pragma weak _nftw64 = _nftw
#endif
#endif
...
static int walk(char *, int (*)(char *, struct stat *, int, struct FTW *),
						int, struct Save *, int);
...
int
nftw(const char *path, int (*fn)(), int depth, int flags)
{
	struct stat statb;
	...
	int recursive)
{
	...
	*/
	...
	if(type!=FTW_D || (curflags&FTW_DEPTH)==0) {
		...
		/* If current component is a directory
		 * and FTW_DEPTH and FTW_PHYS are not set
		 * check for previsitation.
		 */
		if(type==FTW_D && (curflags&FTW_PHYS)==0)
		{
