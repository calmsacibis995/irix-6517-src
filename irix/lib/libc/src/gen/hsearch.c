...
static unsigned int hashm(char *);
static unsigned int hash2m(char *);
#else
#ifdef DIV
#    define HASH hashd		/* Division hashing routine */
#    define HASH2(A) 1		/* Secondary hash function */
static unsigned int hashd(char *);
#endif
#endif
...
static ENTRY *build(NODE **,NODE *,ENTRY);
#else
#ifdef OPEN
...
int size;		/* Minimum size for hash table */
{
    ...
    unsize = (unsigned int) size;	/* +1 for empty table slot; -1 for ceiling */
...
POINTER key;		/* Key to be hashed */
{
    static int first;	/* Remember first time through */
    ...
    return(((crunch(key) * FACTOR) >> SHIFT));
}
...
{
    return((((crunch(key) * FACTOR) << m) >> SHIFT) | 1);
}
#endif
#endif
...
POINTER key;
{
    ...
    unsigned int s;		/* Length of the key */
