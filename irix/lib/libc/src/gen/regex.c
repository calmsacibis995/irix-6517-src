...
#ifdef __STDC__
	#pragma weak re_comp = _re_comp
	#pragma weak re_exec = _re_exec
#endif
#include "synonyms.h"
#include <stdio.h>
#include <malloc.h>
...
static	int	backref(int i, const char *lp);
...
#ifdef sgi
	if ((rgx == NULL) &&
	    (rgx = (struct regex *)calloc(1, sizeof (struct regex))) == NULL)
		return (0);
	ep = rgx->expbuf;
#endif /* sgi */
	...
	for (;;)
		...
		case CBACK|CSTAR:
			if (rgx->braelist[i = *ep++] == 0)
				return(-1);
			curlp = lp;
			ct = (int)(rgx->braelist[i] - rgx->braslist[i]);
			while (backref(i, lp))
				lp += ct;
			while (lp >= curlp) {
				if (rv = advance(lp, ep))
