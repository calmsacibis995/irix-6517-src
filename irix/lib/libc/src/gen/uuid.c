...
uuid_create(uuid_t *uuid, uint_t *status)
{
	...
	if (!uuid_have_eaddr) {
		/*
		 * This goofyness is because we can't call get_eaddr
		 * from uuid_init() because the structures it looks
		 * at aren't set up yet.
		 */
#if defined(MP) && !defined(INTR_KTHREADS)
		static lock_t dummy_lock;
		int		s;

		spinlock_init(&dummy_lock, "uuiddumy");
		s = mutex_spinlock(&dummy_lock);
		uuid_have_eaddr = get_eaddr((char *)uuid_eaddr);
		mutex_spinunlock(&dummy_lock, s);
		spinlock_destroy(&dummy_lock);
#else
		uuid_have_eaddr = get_eaddr((char *)uuid_eaddr);
#endif
		if (!uuid_have_eaddr) {
