...
#ident	"@(#)libc-port:gen/_xftw.c	1.4"

/*LINTLIBRARY*/
/***************************************************************
 *	_xftw64 - file tree walk the uses expanded stat structure
 *
 *	int _xftw64 (path, fn, depth)  char *path; int (*fn)(); int depth;
 *
 *	Given a path name, _xftw64 starts from the file given by that path
 ...
 *	contains a pointer to a stat64 buffer which will usually hold
 ...
 *			directory.  The stat64 buffer passed to fn will
 *			contain garbage.  Stat failure for any reason
 *			other than lack of permission will be
 *			considered an error and will cause _xftw64 to stop 
 *			and return -1 to its caller.
 *
 *	If fn returns nonzero, _xftw64 stops and returns the same value
 *	to its caller.  If _xftw64 gets into other trouble along the way,
 *	it returns -1 and leaves an indication of the cause in errno.
 *
 *	The third argument to _xftw64 does not limit the depth to which
 *	_xftw64 will go.  Rather, it limits the depth to which _xftw64 will
 ...
 *
 *	It is possible for _xftw64 to die with a memory fault in the event
