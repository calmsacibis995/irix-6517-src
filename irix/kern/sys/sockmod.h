...
#ifndef	_SYS_SOCKMOD_H	/* wrapper symbol for kernel use */
#define	_SYS_SOCKMOD_H	/* subject to change without notice */

#ifdef __cplusplus
extern "C" {
#endif

#ident	"@(#)uts-3b2:net/transport/sockmod.h	1.3"
...
#include "tiuser.h"	/* REQUIRED */

#endif	/* _KERNEL */

/* for sockaddr_un */
#include <sys/un.h> /* COMPATIBILITY */
...
struct so_so {
	...
	int			so_option;
	mblk_t  		*bigmsg;
        struct  so_ux		so_inuse; /* Used to link onto alloc'ed list*/
	struct	so_ux		so_ux;
	int			hasoutofband;
	mblk_t			*urg_msg;
	int			sndbuf;
