...
#ifndef	_IO_KBD_KBD_H	/* wrapper symbol for kernel use */
#define	_IO_KBD_KBD_H	/* subject to change without notice */

/* #ident	"@(#)uts-3b2:io/kbd/kbd.h	1.3"		*/
...
#define KBD_ALP 0x10	/* is an ALP function, NOT a real table */
...
/*
 * Re-use some fields for ALP when (t_flag & KBD_ALP).
 */
...
#define AF_CAST	(mblk_t *(*)(mblk_t *, caddr_t))
 ...
 * the total space that must be alloc'd for a table.  Only root can
 ...
 * in a single instantiation,
 * the name is sufficient to find the table.  If it's a composite,
 * s_nth is set to the component number.  Timer value is about 5-150 and
...
struct timeset {
	...
	unsigned short s_time;		/* timer value */
