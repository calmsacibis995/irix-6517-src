...
typedef __uint64_t	bridge_ate_t;
...
#define	b_odd_resp	b_rrb_map[1].reg	    /* 0x00028C */
	...
	uchar_t			c[0x1000 / 1];
	ushort_t		s[0x1000 / 2];
	__uint32_t		l[0x1000 / 4];
	__uint64_t		d[0x1000 / 8];
	union {
	    uchar_t		c[0x100 / 1];
	    ushort_t		s[0x100 / 2];
	    __uint32_t		l[0x100 / 4];
	    __uint64_t		d[0x100 / 8];
	}			f[8];
	...
	ushort_t		s[0x100000 / 2];
	__uint32_t		l[0x100000 / 4];
	__uint64_t		d[0x100000 / 8];
...
#define b_devio(n)	b_devio_raw[((n)<2)?(n*2):(n+2)]
	...
	ushort_t		s[0x400000 / 2];	/* read-write */
	__uint32_t		l[0x400000 / 4];	/* read-only */
	__uint64_t		d[0x400000 / 8];	/* read-only */
...
typedef struct bridge_err_cmdword_s {
	...
	__uint32_t		cmd_word;
	struct {
	    __uint32_t		    didn:4,	/* Destination ID */
				    sidn:4,	/* SOurce ID	  */
...
typedef union ate_u {
	...
	__uint64_t		barrier:1;
	__uint64_t		prefetch:1;
	__uint64_t		precise:1;
	__uint64_t		coherent:1;
	__uint64_t		valid:1;
    } field;
} ate_t;
#endif				/* LANGUAGE_C */
