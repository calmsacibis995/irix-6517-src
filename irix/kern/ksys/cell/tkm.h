...
#ident "$Id: tkm.h,v 1.4 1997/08/27 20:13:07 nar Exp $"
...
#define TK_DISPOSITION(d, c)    (((d) >> ((c) * TK_LWIDTH)) & \
					((1<<TK_LWIDTH)-1))
 ...
 * already has them.  Also try weakly to get conditional tokens.
 */
extern void tks_obtain_conditional(
	tks_state_t *,		/* object containing token */
	tks_ch_t,		/* handle for client making request */
	tk_set_t,		/* mandatory tokens */
	tk_set_t,		/* conditional tokens */
	tk_set_t *,		/* tokens granted */
	tk_set_t *,		/* tokens refused */
	tk_set_t *);		/* tokens already held by client */
 ...
 *	return 0.
 */
extern int tks_is_idle(
        tks_state_t *);


/*
 * tks_iterate_wait(cell_t cell)
 * Wait for a any ongoing tks_iterate calls to a given cell to complete.
 */
extern void tks_iterate_wait(
		cell_t);

extern void tks_make_dispall(
	tk_set_t, int, tk_disp_t *);
