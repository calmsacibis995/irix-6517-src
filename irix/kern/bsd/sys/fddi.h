 ...
 * The FDDI standard says TVX should be at least 2.5 millseconds.
 * However, it can be interesting to play with smaller values.
 */
#define FDDI_MIN_TVX	USEC2FDDICLK(-100)
#define FDDI_DEF_TVX	USEC2FDDICLK(-4000)
#define FDDI_MAX_TVX	USEC2FDDICLK(-25000)

#define FDDI_MIN_T_MAX	USEC2FDDICLK(-165000)
#define FDDI_DEF_T_REQ	USEC2FDDICLK(-165000)
...
typedef struct {unchar b[6];} LFDDI_ADDR;
typedef struct {unchar b[2];} SFDDI_ADDR;

/* filler to align tcp packets */
#define FDDI_FILLER 3

/* minimum Info field */
typedef struct {__uint32_t info[1];} FDDI_INFO;

/* the Frame Check Sequence */
typedef __uint32_t FDDI_FCS;


/* the start of any FDDI frame (using long addresses) */
struct lmac_hdr {
	unchar		filler[FDDI_FILLER-1];	/* align things reasonably */
	unchar		mac_bits;	/* status bits on input */
	unchar		mac_fc;		/* Frame Control */
...
/* a directed beacon frame */
struct d_bec_frame {
	struct lmac_hdr	hdr;
	FDDI_INFO	info;
	u_char		una[6];
};
#define D_BEC_SIZE (sizeof(struct d_bec_frame)-2)   /* adjust for C padding */

/* the start of an mbuf containing an input FDDI frame */
typedef struct {
	struct ifheader	fbh_ifh;
	struct snoopheader fbh_snoop;
} FDDI_IBUF;

/* the start of an mbuf containing an input FDDI LLC frame */
typedef struct {
	struct ifheader	fbh_ifh;
	struct snoopheader fbh_snoop;
	struct fddi	fbh_fddi;
} FDDI_IBUFLLC;
...
#define FDDI_PORT_LLC	ETHERTYPE_8023
...
#define SMT_DIRECTED_BEACON_DA	"01:80:c2:00:01:00"
#define SMT_SRF_DA		"01:80:c2:00:01:10"
#define SMT_UNKNOWN_ADDRESS	"00:00:f8:00:00:00"
#define SMT_SBA_DA		"address has been requested"
...
#define SGI_FDDI_MNFDATA_STR(ifname) "Silicon Graphics " # ifname
#define SGI_FDDI_MNFDATA(ifname) {SGI_MAC_SA0,SGI_MAC_SA1,SGI_MAC_SA2, \
	SGI_FDDI_MNFDATA_STR(ifname)}
...
#define SMT_LS_MAX	25		/* line state detection time */
#define SMT_A_MAX	200		/* max signal acquisition */
#define SMT_TL_MIN	50		/* transmit for this long */
...
#define SMT_T_SCRUB	3000		/* longer than ring latency */

#define LC_SHORT	(50*1000)	/* LCT durations */
#define LC_MEDIUM	(500*1000)
#define LC_LONG		(5*1000*1000)
#define LC_EXTENDED	(5*60*1000*1000)


/* not official */
#define SMT_DRAG	(5*1000*1000)	/* pause if neighbor is crazy */
#define SMT_TEST_DELAY	(5*1000*1000)	/* pause while "testing" */
