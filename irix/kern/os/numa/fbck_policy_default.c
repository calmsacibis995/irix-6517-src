...
static int fbck_policy_default_create(pm_t* pm, void* args, pmo_ns_t* pmo_ns);
static char* fbck_policy_default_name(void);
static void fbck_policy_default_destroy(pm_t* pm);
static pfd_t* fbck_policy_default_fallback(struct pm* pm,
                                          uint ckey,
                                          int flags,
                                          mld_t* mld,
                                          size_t* ppsz);
static int fbck_policy_default_fbck_srvc(struct pm* pm,
					 pm_srvc_t srvc,
					 void* args, ...);
 ...
 */
int
fbck_policy_default_init()
{
        pmfactory_export(fbck_policy_default_create, fbck_policy_default_name());
        pmfactory_export(fbck_policy_default_create, "FallbackLargepage");
...
/*ARGSUSED*/
static int
fbck_policy_default_create(pm_t* pm, void* args, pmo_ns_t* pmo_ns)
{
        ASSERT(pm);
        ASSERT(pmo_ns);
        
        pm->fallback = fbck_policy_default_fallback;
        pm->fbck_srvc = fbck_policy_default_fbck_srvc;
        pm->fbck_data = 0;
...
/*ARGSUSED*/
static void
fbck_policy_default_destroy(pm_t* pm)
{
        ASSERT(pm);
}
 ...
 */
static pfd_t*
fbck_policy_default_fallback(struct pm* pm,
                             uint ckey,
                             int flags,
                             mld_t* mld,
                             size_t* ppsz)
...
/*ARGSUSED*/
static int
fbck_policy_default_fbck_srvc(struct pm* pm, pm_srvc_t srvc, void* args, ...)
{
        ASSERT(pm);
        
        switch (srvc) {
        case PM_SRVC_DESTROY:
                fbck_policy_default_destroy(pm);
                return (0);

        case PM_SRVC_GETNAME:
		*(char **)args = fbck_policy_default_name();
                return (0);
	...
	case PM_SRVC_GETARG:
		*(int *)args = 0;
        ...
        default:
                cmn_err_tag(102,CE_PANIC, "[fbck_policy_default_srvc]: Invalid service type");
        }
