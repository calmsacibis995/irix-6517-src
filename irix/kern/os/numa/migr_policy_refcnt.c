...
static int migr_policy_refcnt_create(pm_t* pm, void* args, pmo_ns_t* pmo_ns);
static char* migr_policy_refcnt_name(void);
static void migr_policy_refcnt_destroy(pm_t* pm);
static int migr_policy_refcnt_migr_srvc(struct pm* pm,
					pm_srvc_t srvc,
					void* args, ...);
 ...
 */
int
migr_policy_refcnt_init()
{
        pmfactory_export(migr_policy_refcnt_create, migr_policy_refcnt_name());

        return (0);
}

/*
 * The constructor for MigrationRefcnt takes no arguments
 */

/*ARGSUSED*/
static int
migr_policy_refcnt_create(pm_t* pm, void* args, pmo_ns_t* pmo_ns)
{
        ...
        migr_as_kparms.migr_base_threshold = 0;
        ...
        pm->migr_srvc = migr_policy_refcnt_migr_srvc;
...
/*ARGSUSED*/
static char*
migr_policy_refcnt_name(void)
{
        return ("MigrationRefcnt");
}

/*ARGSUSED*/
static void
migr_policy_refcnt_destroy(pm_t* pm)
{
        ASSERT(pm);
}
...
/*ARGSUSED*/
static int
migr_policy_refcnt_migr_srvc(struct pm* pm, pm_srvc_t srvc, void* args, ...)
{
        ASSERT(pm);
        
        switch (srvc) {
        case PM_SRVC_DESTROY:
                migr_policy_refcnt_destroy(pm);
                return (0);

        case PM_SRVC_GETNAME:
		*(char **)args = migr_policy_refcnt_name();
		return (0);
	...
	case PM_SRVC_GETARG:
		*(int *)args = 0;
        ...
        default:
                cmn_err(CE_PANIC,
                        "[migr_policy_refcnt_migr_srvc]: Invalid service type");
        }
