 ...
 * Hardware Inventory
 *
 * See sys/invent.h for an explanation of the hardware inventory contents.
 *
 */
#ident "$Revision: 1.21 $"
...
{
	(void)device_inventory_add(GRAPH_VERTEX_NONE, class, type, 
...
{
	inventory_t *pinv;
	dev_t device = place->invplace_vhdl;
	graph_error_t rv;
...
{
#if _MIPS_SIM == _ABI64
	return ABI_IS_IRIX5_64(abi) ?
			sizeof(inventory_t) : sizeof(irix5_inventory_t);
#else
...
*/
inventory_t *
device_inventory_get_next(	dev_t device,
				invplace_t *invplace)
{
	inventory_t *pinv;
	graph_error_t rv;
...
void
device_controller_num_set(dev_t device, int contr_num)
{
