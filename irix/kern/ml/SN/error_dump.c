...
#define board_serial_number_output_get(_b,_sn,_sno)		\
	board_serial_number_get(_b,_sn);			\
	if (_sn[0])						\
		    sprintf(_sno,				\
			    "[serial number %s]",		\
			    _sn);				\
...
#if !defined(_STANDALONE)
	/* Write the string from the dump buffer into error buffer */
	errbuf_print(dmpbuf);
#endif /* !_STANDALONE */
	*line = oldchar;
	dmpbuf = line;
...
void
clear_hub_pi_error(hub_pierr_t *pi_err)
{
    int i;

    for (i = 0; i < 2; i++) {
	if (pi_err->hp_err_sts0[i]) {
	    if ((ERROR_TIMESTAMP - pi_err->hp_err_sts_info[i].ei_time)
		> ERROR_DISCARD_TIME) {
		pi_err->hp_err_sts0[i] = 0;
		pi_err->hp_err_sts1[i] = 0;
	    }
	}
    }
}
...
void
clear_hub_md_error(hub_mderr_t *md_err)
{
    if (md_err->hm_mem_err) {
	if ((ERROR_TIMESTAMP - md_err->hm_mem_err_info.ei_time) 	
	    > ERROR_DISCARD_TIME)
	    md_err->hm_mem_err = 0;
    }
    if (md_err->hm_dir_err) {
	if ((ERROR_TIMESTAMP - md_err->hm_dir_err_info.ei_time) 	
	    > ERROR_DISCARD_TIME)
	    md_err->hm_dir_err = 0;
    }
    if (md_err->hm_proto_err) {
	if ((ERROR_TIMESTAMP - md_err->hm_proto_err_info.ei_time) 
	    > ERROR_DISCARD_TIME)
	    md_err->hm_proto_err = 0;
    }
    if (md_err->hm_misc_err) {
	if ((ERROR_TIMESTAMP - md_err->hm_misc_err_info.ei_time)
	    > ERROR_DISCARD_TIME)
	    md_err->hm_misc_err = 0;
    }
}
/*
...
save_hub_pi_error(__psunsigned_t hub_base, hub_pierr_t *pi_err)
{
    ...
    if (HUB_REG_PTR_L(hub_base, PI_CPU_ENABLE_A) == 0)
	pi_err->hp_err_int_pend &= ~PI_ERR_CLEAR_ALL_A;
    if (HUB_REG_PTR_L(hub_base, PI_CPU_ENABLE_B) == 0)
	pi_err->hp_err_int_pend &= ~PI_ERR_CLEAR_ALL_B;
	...
	/*
	 * Need to check for the old and new forced error addresses in
	 * case we're running on old proms.
	 */
	if (WAR_ERR_STS_ENABLED && 
	    (error_addr != 
	     (ERR_STS_WAR_PHYSADDR >> ERR_STAT0_ADDR_SHFT)) &&
	    ((error_addr & 0x1fffffff) !=
	     (OLD_ERR_STS_WAR_OFFSET >> ERR_STAT0_ADDR_SHFT)))
...
save_hub_err(nasid_t nasid, klhub_err_t *klhub_err, edump_param_t *edp)
{
    ...
    if (edp->dp_flag == KL_ERROR_LOG) {
      clear_hub_pi_error(&hub_err->hb_pi);
      clear_hub_md_error(&hub_err->hb_md);
    }
...
kl_error_check_power_on(void)
{
	...
	if (hub_cf->nasid == INVALID_NASID)  continue;
...
show_err_node_comp(lboard_t *board, edump_param_t *edp)
{
    ...
    char serial_number_output[120];
    /* 
     * Acceptable components of node board are cpu, hub and memory.
     */
    get_slotname(BOARD_SLOT(board), slotname);
    board_serial_number_output_get(board,serial_number,
				   serial_number_output);
    kl_perr(SLOT_LVL, "%s in /hw/module/%d/slot/%s %s\n", 
	    NODENAME,
...
char *sie_cache_error[] = {
    ...
    "26:  D, Uncorrectable data error. Possibly MEMORY error",
    "27:  D, Uncorrectable data error. Possibly MEMORY error",
...
show_hub_md_err(hub_mderr_t *md_err)
{
	...
	kl_perr(BIT_LVL,
		"28<->3: bad BDDIR space address 0x%x (phys addr 0x%x)\n",
	...
	if (mem_err.merr_fmt.address)
	    kl_perr(BIT_LVL, "31<->03: bad addr pointer 0x%x << 3 = (0x%x)\n",
...
show_hub_io_err(klhub_err_t *klhub_err, int dp_flag, nasid_t nasid)
{
	...
	if (crba.icrba_fields_s.error == 0)
	    continue;
	/* Reserved bits (63: 58) of the iicrb entry A are being used for
	 * special purposes in the software. Mask those bits.
	 */
	kl_perr(REG_LVL, "ICRB%x_A entry Register: 0x%lx\n", 
		crb, crba.reg_value & ~(0x3full << 58));
	...
	if (crba.icrba_fields_s.mark)
	    kl_perr(BIT_LVL, "52<->55: CRB error code 0x%x (%s Error)\n",
		    crba.icrba_fields_s.ecode,
		    hubii_crb_errcode[crba.icrba_fields_s.ecode]);
...
show_hub_ni_err(hub_nierr_t *ni_err)
{
	...
	show_ni_port_error(err, (BIT_LVL | log), kl_perr);
    }
}
...
show_xbow_err(klxbow_err_t *xb_err, edump_param_t *edp)
{
    ...
    xbowreg_t err;
    int link, i;
    int	show_err_cmd_addr = 0;	/* indicates whether the xbow error command 
				 * word , error upper & lower addresses
				 * need to displayed 
				 */ 
	...
	/* If register access error or xtalk error or multiple error bit
	 * is set in the widget0 status register then we should display the
	 * error command word , error upper & lower addresses.
	 */
	if (err & (XB_WID_STAT_MULTI_ERR 	|
	...
	if ((err = xbow_err->xb_link_status[link]) == 0)
	    continue;
	/* If the illegal destination bit is set in any of the
	 * link status registers then we should display the
	 * error command word , error upper & lower addresses
	 */
	if (err & XB_STAT_ILLEGAL_DST_ERR)
	    show_err_cmd_addr++;
    ...
    /* Display error command word , error upper & lower addresses on
     * 		1. Illegal destination error in any of the 8 link statuses
     *		2. Register access error in widget0 status
     *		3. Xtalk error in widget0 status
     */
...
{
    /* No need to print the router status error register if none
     * of the lower 33 bits is set.
     */
    if (!rr_error_status)
	return;

#if defined(DEBUG)    
    /* Always print the receiver fifo overflow & recvd packet for
     * illegal port for a DEBUG kernel.
     */
...
#else
    /* Donot print the receiver fifo overflow & recvd packet for
     * illegal port messages while printing the router error
     * state during a panic since they donot help in narrowing
     * down the problem.
     * log_nonfatal 	= 1 (when called during printing HWSTATE)
     * 			= 0 (when called during hub ni error interrupt
     *			     handling)	
     */
    if (log_nonfatal) {
	if (rr_error_status & RSERR_FIFOOVERFLOW)
	...
	pf(level,"1: Port %d: RR_STATUS_ERROR = 0x%x\n",port,rr_error_status);
#endif
		      ...
		      void (*pf)(int, char *, ...))
{
   ...
   for (i = 1; i <= MAX_ROUTER_PORTS; i++) {
	...
	case RSRI_LSTAT_NOTUSED:
		/* Print the link working message only in debug mode.
		 * Right now this function is being called only while
		 * printing router error state on an ni error interrupt
		 * This code should be checked if this assumption
...
show_err_router_comp(lboard_t *board, edump_param_t *edp)
{
    ...
    board_serial_number_output_get(board,serial_number,
				   serial_number_output);
    kl_perr(SLOT_LVL, "%s in Module %d slot %s %s\n", 
	    ROUTERNAME, 
	    KLCF_MODULE_ID(board),
	    slotname,
	    serial_number_output);
        ...
        switch (KLCF_COMP_TYPE(component)) {
           case KLSTRUCT_ROU:
				...
				if (rr_error_status)
				   ...
				   continue;
				show_rr_error_status(rr_error_status, i,
					BIT_LVL, kl_perr, 1);
                             }
                             break;
           default:
