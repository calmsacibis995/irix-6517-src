...
static int
addr2sbs(uint bloc, uint offset, uint *slot, uint *bank, evcfginfo_t *ec);
int extract_bank(evcfginfo_t *ec, unsigned int addrlo, unsigned int addrhi);

/* ARGSUSED */
void
check_mc3(evbrdinfo_t *eb, everror_t *ee, everror_ext_t *eex,
...
{
    long long address;
    unsigned int bloc_num, slot, bank;
    ...
    if (addr2sbs(bloc_num, address & 0xff, &slot, &bank, ec))
	bank = NO_UNIT;
...
char
fru_bank_letter(uint leaf, uint bank) {
