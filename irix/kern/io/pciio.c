...
#if SN0 || IP30
/*
 *    For the moment, we will assume that IP27 and IP30
...
/*
 *    IP32 provides PCI support via MACE
 */
#define DEV_FUNC(dev,func)	pcimh_##func
#define CAST_PIOMAP(x)		(x)
#define CAST_DMAMAP(x)		(x)
#define CAST_INTR(x)		((pcimh_intr_t)(x))
#endif
...
alenlist_t              pciio_dmatrans_list(vertex_hdl_t, device_desc_t, alenlist_t, unsigned);
...
static pciio_info_t     pciio_cardinfo_get(vertex_hdl_t, pciio_slot_t);
int                     pciio_error_handler(vertex_hdl_t, int, ioerror_mode_t, ioerror_t *);
int                     pciio_error_devenable(vertex_hdl_t, int);
...
int                     pciio_device_attach(vertex_hdl_t);
...
alenlist_t
pciio_dmatrans_list(vertex_hdl_t dev,	/* translate for this device */
		    device_desc_t dev_desc,	/* device descriptor */
...
int
pciio_error_devenable(vertex_hdl_t pconn_vhdl, int error_code)
{
 ...
 */
pciio_slot_t
pciio_error_extract(vertex_hdl_t 	dev,
		   pciio_space_t 	*space,
		   iopaddr_t		*offset)
{
	ASSERT(dev != NODEV);
...
pciio_info_get(vertex_hdl_t pciio)
{
	...
	cmn_err(CE_PANIC, "%v: pciio_info bad fingerprint", pciio);
