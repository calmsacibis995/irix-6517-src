...
#if	defined(IP30) || defined(SN0) 	/* only IP30 or SN0 */
...
#define	MTR_IFF_FLAGS	(IFF_BROADCAST | IFF_MULTICAST | IFF_NOTRAILERS | \
			IFF_ALLCAST)
 ...
 *	Forward references:
 */
int			if_mtrattach(vertex_hdl_t);
int			if_mtrdetach(vertex_hdl_t);
...
static void 	PCI_get_dump_cfg( mtr_private_t *mtr_p, int dump_flag );
	...
	/*	By default, if_mtr_debug is off. 
	 ...
	 */
	#ifdef	DEBUG_INITIALIZATION
	#define	INIT_MTR_DBG_PRINT(_msg)	if( if_mtr_debug ) printf _msg
	#ifdef  DEBUG2
		#define	INIT_MTR_DBG_PRINT2(_msg) if(if_mtr_debug) printf _msg
	#else   /* DEBUG2 */
...
if_mtrinit(void)
{
	INIT_MTR_DBG_PRINT2( ("mtr*: if_mtrinit() called.\n"));

	/* 	
	 *	Have we already been loaded? 
	 */
	loaded_flag	= atomicAddInt(&if_mtr_loaded, 1);
...
if_mtrunload(void)
{
	...
	(void )atomicAddInt(&if_mtr_loaded, -1);
	return( error );
} /* if_mtrunload() */
...
if_mtropen(dev_t *devp, int flag, int otyp, struct cred *crp)
{
	...
	INIT_MTR_DBG_PRINT(("mtr*: 0x%x, 0x%x, 0x%x, 0x%x",
		devp, flag, otyp, crp));
	return( if_mtr_cnt == 0 ? ENODEV : NO_ERROR );
} /* if_mtropen() */
...
mtr_intr(void *arg)
{
	...
	if( mtr_p == NULL || mtr_p->tag != MTR_TAG ) {
		...
		cmn_err(CE_PANIC, "mtr*: interrupt with 0x%x?!", arg);
	}
...
int
if_mtrattach(vertex_hdl_t connect_vertex)
{
	int		error = NO_ERROR;
	int		new_mtr;		
	...
	INIT_MTR_DBG_PRINT2( ("mtr*: if_mtrattach(0x%x) called.\n", 
		connect_vertex));
	MTR_ASSERT( if_mtr_cnt < NUM_SLOTS );
	if( if_mtr_cnt >= NUM_SLOTS ){
		cmn_err(CE_WARN,
			"mtr%d: too many adapter -> ignored!", if_mtr_cnt);
		error = EIO;
		goto done;
	}
	DUMP_VERTEX(connect_vertex); 

	INIT_MTR_DBG_PRINT2( ("mtr*: if_mtrattach() kmem_zalloc(0).\n"));
	/*	
	...
	if (hwgraph_info_get_LBL(connect_vertex, MTR_INFO_LBL, 
		...
		if( (ifnet_p->if_name = kmem_zalloc(strlen(MY_NET_PREFIX)+2,
			...
			cmn_err(CE_WARN,
				...
				if_mtr_cnt, strlen(MY_NET_PREFIX)+2);
			error = ENOBUFS;
			goto done;
		}
		strcpy(ifnet_p->if_name, MY_NET_PREFIX);
		ifnet_p->if_type = IFT_ISO88025;
		ifnet_p->if_unit = if_mtr_cnt;
		if_mtr_cnt++;


		INIT_MTR_DBG_PRINT2(
			("mtr%d: if_mtrattach() hwgraph_info_add_LBL"
			 "(0x%x, 0x%x).\n", ifnet_p->if_unit,
		...
		INIT_MTR_DBG_PRINT2((
			"mtr%d: if_mtrattach() new ifmtr_t: 0x%x.%x.\n", 
	...
	if( ifmtr_p->flags != MTR_FLAG_UNINIT ){
		cmn_err(CE_WARN, "mtr%d: again for 0x%x: 0x%x.0x%x!?", 
			if_mtr_cnt, connect_vertex, mtr_p, ifmtr_p->flags);
		error = EIO;
		goto done;
	}
	...
	INIT_MTR_DBG_PRINT2( ("mtr%d: if_mtrattach() get PCI.\n",
			    mtr_p->mtr_unit));
	...
	INIT_MTR_DBG_PRINT2( 
			("mtr%d: if_mtrattach() device_desc_default_set().\n",
			    mtr_p->mtr_unit));
	...
	INIT_MTR_DBG_PRINT2( ("mtr%d: if_mtrattach() setup intr.\n",
			    mtr_p->mtr_unit));
	/* 	
	...
	INIT_MTR_DBG_PRINT2( ("mtr*: if_mtrattach() done with %d.\n", error));
	return (error);
} /* if_mtrattach() */
 ...
 */
int
if_mtrdetach(vertex_hdl_t connect_vertex)
{
	int		error = NO_ERROR;
	int		both_locked = 0;
	if_mtr_t	*ifmtr_p;
	...
	INIT_MTR_DBG_PRINT(("mtr*: if_mtrdetach(0x%x) called.\n", 
			    connect_vertex));

	if( hwgraph_info_get_LBL(connect_vertex, MTR_INFO_LBL, 
				 (arbitrary_info_t *)&ifmtr_p)
	   != GRAPH_SUCCESS ){
		cmn_err(CE_WARN, "if_mtrdetach(0x%x) connect_vertex!", 
			connect_vertex);
		error = EIO;
		goto done;
	}
	...
	mtr_p = ifmtr_p->mtr_p;
	if( mtr_p == NULL ){
		cmn_err(CE_WARN, "if_mtrdetach(0x%x): 0x%x not found!",
			connect_vertex, ifmtr_p);
		error = EIO;
		goto done;	
	}
	...
	struct rtentry *rt)
{
	...
	case AF_INET:
		...
		if( !arpresolve(&mtr_p->mtr_ac, rt, data_p, 
			...
			MTR_ASSERT(data_p != NULL && data_p->m-type != MT_FREE);
	...
	void *data)
{
	...
	case SIOCSIFADDR:
		...
		case AF_INET:
			...
			if( if_alive ){
				/*      Update ARP cache.       */
				arpwhohas((struct arpcom *)ifnet_p,
					&((struct arpcom *)ifnet_p)->ac_ipaddr);
			}
			break;
...
report_Configuration( mtr_private_t *mtr_p )
{
	...
	cmn_err(CE_CONT, "mtr%d: driver version $Revision: 1.18 $.\n", 
		mtr_p->mtr_unit);
...
hwi_initialize_adapter(mtr_private_t *mtr_p)
{
	...
	dki_dcache_inval((void *)adapter_p->scb_buff, adapter_p->test_size);
