...
#ident	"@(#)uts-comm:net/loopback/ticlts.c	1.9"
...
static
char *Kmem_alloc(unsigned int a, int b, int c, char *d, char *e)
{
char *r;
r=kmem_alloc(a,b);
...
static
void Kmem_free(void *a, int b, int c, char *d, char *e)
{
if (ticlts_tracefree)
...
STATIC int	tcl_optmgmt(queue_t *, mblk_t *);
STATIC int	tcl_bind(queue_t *, mblk_t *);
STATIC int	tcl_unconnect(tcl_endpt_t *);
STATIC int	tcl_data(queue_t *, mblk_t *);
STATIC int	tcl_ireq(queue_t *, mblk_t *);
STATIC int	tcl_fatal(queue_t *, mblk_t *);
STATIC int	tcl_ckstate(queue_t *, mblk_t *);
STATIC int	tcl_unbind(queue_t *, mblk_t *);
STATIC tcl_addr_t		*tcl_addrinit(tcl_addr_t *);
STATIC tcl_addr_t		*tcl_getaddr(int, tcl_addr_t *);
 ...
 */
STATIC int
tcl_bequal(register char *a, register char *b, register int n)
{
	register int			i;
 ...
 */
STATIC int
tcl_olink(register tcl_endpt_t *te)
{
 ...
 */
STATIC int
tcl_unolink(register tcl_endpt_t *te)
{
	ASSERT(te != NULL);
	/*
 ...
 */
STATIC int
tcl_blink(register tcl_endpt_t *te, register tcl_addr_t *ta)
{
 ...
 */
STATIC int
tcl_unblink(register tcl_endpt_t *te)
{
 ...
 */
STATIC int
tcl_sumbytes(register char *a, register int n)
{
	register char			*cp,*ep;
 ...
 */
STATIC int
tcl_endptinit(minor_t min, tcl_endpt_t **tepp)
{
 ...
 */
STATIC tcl_addr_t *
tcl_addrinit(register tcl_addr_t *ta)
{
	...
	if (ta == NULL) {
		...
		ASSERT(TCL_DEFAULTADDRSZ >= sizeof(int));
 ...
 */
STATIC int
tcl_getendpt(int flg, minor_t min, tcl_endpt_t **tep)
{
 ...
 */
STATIC tcl_addr_t *
tcl_getaddr(int flg, register tcl_addr_t *ta)
{
 ...
 */
STATIC int
tcl_ckopt(unsigned char *obuf, unsigned char *ebuf)
{
	...
	;    ohdr = ohdr1) {
		...
		if ((unchar *)ohdr + sizeof(struct tcl_opt_hdr) > ebuf) {
			STRLOG((TCL_ID,-1,4,SL_TRACE,
		...
		switch (opt->opt_type) {
			...
			break;
		    case TCL_OPT_NOOP:
			if ((unchar *)opt + sizeof(struct tcl_opt_noop) > ebuf) {
				STRLOG((TCL_ID,-1,4,SL_TRACE,
				    "tcl_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCL_BADFORMAT);
			}
			...
			break;
		    case TCL_OPT_SETID:
			if ((unchar *)opt + sizeof(struct tcl_opt_setid) > ebuf) {
				STRLOG((TCL_ID,-1,4,SL_TRACE,
				    "tcl_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCL_BADFORMAT);
			}
			...
			break;
		    case TCL_OPT_UID:
			if ((unchar *)opt + sizeof(struct tcl_opt_uid) > ebuf) {
				STRLOG((TCL_ID,-1,4,SL_TRACE,
				    "tcl_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCL_BADFORMAT);
			}
			...
			break;
		    case TCL_OPT_GID:
			if ((unchar *)opt + sizeof(struct tcl_opt_gid) > ebuf) {
				STRLOG((TCL_ID,-1,4,SL_TRACE,
				    "tcl_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCL_BADFORMAT);
			}
			...
			break;
		    case TCL_OPT_RUID:
			if ((unchar *)opt + sizeof(struct tcl_opt_ruid) > ebuf) {
				STRLOG((TCL_ID,-1,4,SL_TRACE,
				    "tcl_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCL_BADFORMAT);
			}
			...
			break;
		    case TCL_OPT_RGID:
			if ((unchar *)opt + sizeof(struct tcl_opt_rgid) > ebuf) {
				STRLOG((TCL_ID,-1,4,SL_TRACE,
				    "tcl_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCL_BADFORMAT);
			}
 ...
 */
STATIC int
tcl_wropt(int idflg, tcl_endpt_t *te, unsigned char *obuf)
{
...
/*ARGSUSED*/
STATIC int
tcl_open(register queue_t *q, register dev_t *dev, int oflg, int sflg,
	 cred_t *crp)
{
	tcl_endpt_t			*te;
	minor_t				min;
	...
	if (te != NULL) {
		...
		/* XXX MAC check here & creat audit record */
		_SAT_SVR4NET_CREATE(0,NULL,0,0,0);
                *dev = makedevice(getemajor(*dev), (int)tcl_min(te));
                return 0;
	}
	/*
	...
	if (sflg == CLONEOPEN) {
		...
		ASSERT((long)min >= 0);
		if (min >= TCL_NENDPT) {
			_SAT_SVR4NET_CREATE(0,NULL,0,0,ENODEV);
			return(ENODEV);
		}
	}
	/* XXX MAC check here & creat audit record */
        err = tcl_getendpt(TCL_OPEN,min,&te);
	if (err) {
		STRLOG((TCL_ID,-1,3,SL_TRACE,
		    "tcl_open _%d_: cannot allocate endpoint, q=%lx",__LINE__,q));
		_SAT_SVR4NET_CREATE(0,NULL,0,0,err);
		return(err);
	}
	/*
...
/*ARGSUSED*/
STATIC int
tcl_close(register queue_t *q, int unused, cred_t *crp)
{
	...
	/* Sat audit record */
	_SAT_SVR4NET_SHUTDOWN(te->te_descr,(void *)te,0,0);
 ...
 */
STATIC int
tcl_wput(register queue_t *q, register mblk_t *mp)
{
	register tcl_endpt_t		*te;
...
#define JUMP	/* nothing */
		    ...
		    case T_ADDR_REQ:
			STRLOG((TCL_ID,tcl_min(te),4,SL_TRACE,
			    "tcl_wput _%d_: got T_ADDR_REQ msg",__LINE__));
			JUMP;
#ifdef DEBUG
    jump:
#endif
#undef JUMP
			...
			STR_UNLOCK_SPL(spl);
			_SAT_SVR4NET_ADDR(te->te_descr,(void *)te,NULL,0);
			return(UNIX_PASS);
		    case T_UNITDATA_REQ:
			/*
			 *	if endpt is hosed, do nothing
...
tcl_wsrv(register queue_t *q)
{
	...
	ASSERT(q != NULL);
	ASSERT(((tcl_endpt_t *)q->q_ptr) != NULL);
	/*
	...
	while ((mp = getq(q)) != NULL) {
		...
		switch (mp->b_datap->db_type) {
			...
			switch (prim->type) {
				...
				ASSERT(0);	/* internal error */
			    case T_UNITDATA_REQ:
				STRLOG((TCL_ID,tcl_min((tcl_endpt_t *)q->q_ptr),4,SL_TRACE,
				    "tcl_wsrv _%d_: got T_UNITADTA_REQ msg",__LINE__));
				if (tcl_data(q,mp) != 0) {
					STRLOG((TCL_ID,tcl_min(((tcl_endpt_t *)q->q_ptr)),3,SL_TRACE,
 ...
 */
STATIC int
tcl_okack(queue_t *q, register mblk_t *mp)
{
	register union T_primitives	*prim;
	mblk_t				*mp1;
	int				type;
 ...
 */
STATIC int
tcl_errack(queue_t *q, register mblk_t *mp, int tli_err, int unix_err)
{
	register union T_primitives	*prim;
	mblk_t				*mp1;
	int				type;
 ...
 */
STATIC int
tcl_fatal(register queue_t *q, register mblk_t *mp)
{
	register tcl_endpt_t		*te;
 ...
 */
STATIC int
tcl_ckstate(register queue_t *q, register mblk_t *mp)
{
	register tcl_endpt_t		*te;
	register union T_primitives	*prim;
	register char			ns;
 ...
 */
STATIC int
tcl_ireq(queue_t *q, register mblk_t *mp)
{
	register tcl_endpt_t		*te;
 ...
 */
STATIC int
tcl_bind(queue_t *q, register mblk_t *mp)
{
	register tcl_endpt_t		*te;
 ...
 */
STATIC int
tcl_unbind(queue_t *q, register mblk_t *mp)
{
	register tcl_endpt_t		*te;
 ...
 */
STATIC int
tcl_optmgmt(queue_t *q, register mblk_t *mp)
{
	register tcl_endpt_t		*te;
 ...
 */
STATIC int
tcl_unconnect(register tcl_endpt_t *te)
{
	register queue_t		*q;
 ...
 */
STATIC int
tcl_uderr(register queue_t *q, register mblk_t *mp, int err)
{
	register tcl_endpt_t		*te;
	register union T_primitives	*prim,*prim1;
	int				msz1;
 ...
 */
STATIC int
tcl_data(queue_t *q, register mblk_t *mp)
{
...
static void
tcl_link(register queue_t *q, register mblk_t *mp)
{
	tcl_addr_t			addr;
	register tcl_addr_t		*ta;
