...
#ident	"@(#)uts-comm:net/transport/sockmod.c	1.14"
...
#define	RECOVER(A, B, C) \
			{ \
			putbq((A), (B)); \
			if (!bufcall((C), BPRI_MED, qenable, (long)(A))) \
				(void)STREAMS_TIMEOUT((strtimeoutfunc_t)qenable, \
				(caddr_t)(A), SIMWAIT); \
			}

int sockmoddevflag = 0;	/* New style module */
 ...
 * Also head of list of allocated so_so entries, 
 * used to find the new endpoint's queue on
 * T_CONN_REQ messages.
 */
struct so_so	*so_ux_list = (struct so_so *)NULL;
struct so_so	*so_inuse_list = (struct so_so *)NULL;
...
STATIC	void		list_dellink(struct so_so *, int);
STATIC	void		list_addlink(struct so_so *, int);
	...
	register cred_t		*crp;
{
	...
	if (q->q_ptr)
		return (1);
	if ((so = (struct so_so *)kmem_zalloc(sizeof(struct so_so), 
	          KM_NOSLEEP)) == NULL) 
		return(ENOMEM);
	list_addlink(so, INUSE_LIST);
...
socklog(so, "sockmodclose: Entered\n", 0);
	 ...
	 */
#ifdef AF_UNIX
	list_dellink(so, UNIX_DOMAIN_LIST);
#endif /* AF_UNIX */

	socklog(so, "sockmodclose: so->esbcnt %d\n", so->esbcnt);
	if (so->esbcnt) { /* outstanding mesages */
		so->flags |= S_WCLOSE;
	} else {
		list_dellink(so, INUSE_LIST);
                kmem_free(so, sizeof(struct so_so));
...
socklog(so, "sockmodrsrv: Got T_BIND_ACK\n", 0);
			...
			ASSERT(so->iocsave != NULL);
			if (*(int *)so->iocsave->b_cont->b_rptr !=
							T_BIND_REQ) {
				putnext(q, mp);
				goto rgetnext;
			}
...
socklog(so, "sockmodwput: Got SI_GETINTRANSIT q %x\n", (__psunsigned_t)q);
			...
			if (MSGBLEN(mp->b_cont) < sizeof (uint))
				error = EINVAL;
			else
			if (((sockaddr_t)so->laddr.buf)->sa_family != AF_UNIX)
				error = EINVAL;
...
socklog(so, "sockmodwput: Got M_FLUSH\n", 0);
		...
		/*
		 * This is a kludge - yes another one.
...
socklog(so, "sockmodwsrv: Got T_CONN_RES\n", 0);
			...
			while ((soq = soq->q_next) != NULL) {
				...
				for (oso = so_inuse_list; oso != NULL;
				      oso = oso->so_inuse.next) {
					if ((oso->flags & USED) == 0)
						continue;
					if (soq->q_ptr == (void *)oso) {
						/*
						 * Found it
						 */
						goto found;
...
socklog(so, "sockmodwsrv: BIND length %d\n", bind_req->ADDR_length);
					 ...
					 * domain endpoints.
					 */
					list_addlink(so, UNIX_DOMAIN_LIST);
#endif /* AF_UNIX */
				}
			}
 ...
 */
STATIC mblk_t *
_s_makeopt(register struct so_so	*so)
{
	register mblk_t		*bp;
...
STATIC void
strip_zerolen(register mblk_t	*mp)
{
	register mblk_t *bp;
...
snd_ZERO(register queue_t	*q)
{
	...
	if ((mp = _s_getmblk((mblk_t *)NULL, (size_t)1)) == (mblk_t *)NULL) {
		if (!bufcall(1, BPRI_MED, snd_ZERO, (long)q))
			(void)STREAMS_TIMEOUT((strtimeoutfunc_t)snd_ZERO,
					      (caddr_t)q,
					      SIMWAIT);
	} else	{
...
STATIC void
snd_ERRORW(register queue_t	*q)
{
	register mblk_t		*mp;
	register struct so_so	*so;

	if ((mp = _s_getmblk((mblk_t *)NULL, (size_t)2)) == (mblk_t *)NULL) {
		if (!bufcall(2, BPRI_MED, snd_ERRORW, (long)q))
			(void)STREAMS_TIMEOUT((strtimeoutfunc_t)snd_ERRORW,
					      (caddr_t)q, SIMWAIT);
		return;
...
STATIC void
snd_FLUSHR(register queue_t	*q)
{
	if (putctl1(q, M_FLUSH, FLUSHR) == 0)
		if (!bufcall(1, BPRI_MED, snd_FLUSHR, (long)q))
			(void)STREAMS_TIMEOUT((strtimeoutfunc_t)snd_FLUSHR,
					      (caddr_t)q, SIMWAIT);
	return;
}

STATIC void
snd_HANGUP(register queue_t	*q)
{
	if (putctl(q, M_HANGUP) == 0)
		if (!bufcall(sizeof (mblk_t), BPRI_MED, snd_HANGUP, (long)q))
			(void)STREAMS_TIMEOUT((strtimeoutfunc_t)snd_HANGUP,
					      (caddr_t)q, SIMWAIT);
	return;
}
        ...
        int whichlist)
{
        ...
        case INUSE_LIST: /* add to inuse list */
                so->so_inuse.next = so_inuse_list;
                so->so_inuse.prev = NULL;
                if (so_inuse_list)
                        so_inuse_list->so_inuse.prev = so;
                so_inuse_list = so;
                break;

        default: /* who know's what list */
                break;
        }
        return;
}
        ...
        int whichlist)
{
        ...
        case INUSE_LIST: /* delete from inuse list */
                if ((oso = so->so_inuse.next) != (struct so_so *)NULL)
                        oso->so_inuse.prev = so->so_inuse.prev;

                if ((oso = so->so_inuse.prev) != (struct so_so *)NULL)
                        oso->so_inuse.next = so->so_inuse.next;
                else
                        so_inuse_list = so->so_inuse.next;
                break;
	...
	register int		size)
{
	 ...
	 */
	if ((bp = esballoc((unsigned char *)buf, size, BPRI_MED,
...
/*
		else	(void)strlog(SIMOD_ID, -1, 0, SL_TRACE, str, arg);
*/
	}
}
