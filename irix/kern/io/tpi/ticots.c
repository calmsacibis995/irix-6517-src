...
#ident	"@(#)uts-comm:net/loopback/ticots.c	1.10"
...
#if ! defined(TICOTSORD) && ! defined(TICOTS)
#define TICOTS 1	/* default, if TICOTSORD not defined */
#endif
...
STATIC int	tco_dreq(queue_t *q, register mblk_t *mp);
STATIC int	tco_cres(queue_t *q, register mblk_t *mp);
STATIC int	tco_creq(queue_t *q, register mblk_t *mp);
STATIC int	tco_ckstate(register queue_t *q, register mblk_t *mp);
STATIC int	tco_unconnect(register tco_endpt_t *te);
STATIC int	tco_fatal(register queue_t *q, register mblk_t *mp);
STATIC int	tco_data(queue_t *q, register mblk_t *mp, int evtype);
STATIC int	tco_bind(queue_t *q, register mblk_t *mp);
STATIC int	tco_optmgmt(queue_t *q, register mblk_t *mp);
STATIC int	tco_wropt(int idflg, tco_endpt_t *te, unsigned char *obuf);
STATIC int	tco_ireq(queue_t *q, register mblk_t *mp);
STATIC int	tco_unbind(queue_t *q, register mblk_t *mp);
STATIC int	tco_wput(register queue_t *q, register mblk_t *mp);
STATIC int	tco_wsrv(register queue_t *q);
STATIC int	tco_open(register queue_t *q, dev_t *dev, int oflg, int sflg,
			 cred_t *crp);
STATIC int	tco_close(register queue_t *q, int unused, cred_t *crp);
STATIC int	tco_rsrv(register queue_t *q);
 ...
 */
STATIC int
tco_bequal(register char *a, register char *b, int n)
{
	register int			i;
 ...
 */
STATIC int
tco_olink(register tco_endpt_t *te)
{
 ...
 */
STATIC int
tco_unolink(register tco_endpt_t *te)
{
	ASSERT(te != NULL);
	/*
 ...
 */
STATIC int
tco_blink(register tco_endpt_t *te, register tco_addr_t *ta)
{
 ...
 */
STATIC int
tco_unblink(register tco_endpt_t *te)
{
 ...
 */
STATIC int
tco_sumbytes(register char *a, register int n)
{
	register char			*cp,*ep;
 ...
 */
STATIC int
tco_endptinit(minor_t min, tco_endpt_t **tepp, cred_t *crp)
{
 ...
 */
STATIC tco_addr_t *
tco_addrinit(register tco_addr_t *ta)
{
	...
	if (ta == NULL) {
		...
		/* following assertion so we don't have to worry about
		   wrap-around; sizeof(int) is big enough, because
		   num of addresses <= num of endpts <= num of minor numbers */
		ASSERT(TCO_DEFAULTADDRSZ >= sizeof(int));
 ...
 */
STATIC int
tco_getendpt(int flg, minor_t min, queue_t *rq, tco_endpt_t **tep, cred_t *crp)
{
 ...
 */
STATIC tco_addr_t *
tco_getaddr(int flg, register tco_addr_t *ta, unsigned qlen)
{
 ...
 */
STATIC int
tco_ckopt(unsigned char *obuf, unsigned char *ebuf)
{
	...
	;
	;    ohdr = ohdr1) {
		if ((__psunsigned_t)ohdr%NBPW != 0) {	/* alignment */
			STRLOG((TCO_ID,-1,4,SL_TRACE,
			    "tco_ckopt _%d_: bad alignment",__LINE__));
			return(retval|TCO_BADFORMAT);
		}
		if ((unchar *)ohdr + sizeof(struct tco_opt_hdr) > ebuf) {
			STRLOG((TCO_ID,-1,4,SL_TRACE,
		...
		if ((__psunsigned_t)opt%NBPW != 0) {	/* alignment */
			STRLOG((TCO_ID,-1,4,SL_TRACE,
			    "tco_ckopt _%d_: bad alignment",__LINE__));
			return(retval|TCO_BADFORMAT);
		}
		switch (opt->opt_type) {
			...
			break;
		    case TCO_OPT_NOOP:
			if ((unchar *)opt+sizeof(struct tco_opt_noop) > ebuf) {
				STRLOG((TCO_ID,-1,4,SL_TRACE,
				    "tco_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCO_BADFORMAT);
			}
			...
			break;
		    case TCO_OPT_SETID:
			if ((unchar *)opt+sizeof(struct tco_opt_setid) > ebuf) {
				STRLOG((TCO_ID,-1,4,SL_TRACE,
				    "tco_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCO_BADFORMAT);
			}
			...
			break;
		    case TCO_OPT_GETID:
			if ((unchar *)opt+sizeof(struct tco_opt_getid) > ebuf) {
				STRLOG((TCO_ID,-1,4,SL_TRACE,
				    "tco_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCO_BADFORMAT);
			}
			...
			break;
		    case TCO_OPT_UID:
			if ((unchar *)opt+sizeof(struct tco_opt_uid) > ebuf) {
				STRLOG((TCO_ID,-1,4,SL_TRACE,
				    "tco_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCO_BADFORMAT);
			}
			...
			break;
		    case TCO_OPT_GID:
			if ((unchar *)opt + sizeof(struct tco_opt_gid) > ebuf) {
				STRLOG((TCO_ID,-1,4,SL_TRACE,
				    "tco_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCO_BADFORMAT);
			}
			...
			break;
		    case TCO_OPT_RUID:
			if ((unchar *)opt+sizeof(struct tco_opt_ruid) > ebuf) {
				STRLOG((TCO_ID,-1,4,SL_TRACE,
				    "tco_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCO_BADFORMAT);
			}
			...
			break;
		    case TCO_OPT_RGID:
			if ((unchar *)opt+sizeof(struct tco_opt_rgid) > ebuf) {
				STRLOG((TCO_ID,-1,4,SL_TRACE,
				    "tco_ckopt _%d_: bad offset",__LINE__));
				return(retval|TCO_BADFORMAT);
			}
 ...
 */
STATIC int
tco_wropt(int idflg, tco_endpt_t *te, unsigned char *obuf)
{
	...
	ASSERT(((__psunsigned_t)obuf)%NBPW == 0);
	oohdr = &hdr;
...
#if _MIPS_SIM == _ABI64
	ASSERT(sizeof(struct T_conn_res) == 32);
#else
...
/*ARGSUSED*/
STATIC int
tco_open(register queue_t *q, dev_t *dev, int oflg, int sflg, cred_t *crp)
{
	minor_t				min;
	tco_endpt_t			*te;
        int                             err = 0;
	...
	if (te != NULL) {
		STRLOG((TCO_ID,tco_min(te),3,SL_TRACE,
		    "tco_open _%d_: re-open",__LINE__));
                *dev = makedevice(getemajor(*dev), (int)tco_min(te));
                return 0;
	}
	/*
	...
	if (sflg == CLONEOPEN) {
        ...
        err = tco_getendpt(TCO_OPEN,min,NULL, &te, crp);
	if (err) {
		STRLOG((TCO_ID,-1,3,SL_TRACE,
		    "tco_open _%d_: cannot allocate endpoint, q=%lx",__LINE__,q));
		return(err);
	}
	/*
...
/*ARGSUSED*/
STATIC int
tco_close(register queue_t *q, int unused, cred_t *crp)
{
 ...
 */
STATIC int
tco_wput(register queue_t *q, register mblk_t *mp)
{
	register tco_endpt_t		*te;
	...
	switch (mp->b_datap->db_type) {
		...
		ASSERT((__psunsigned_t)(mp->b_rptr)%NBPW == 0);	/* alignment */
...
#undef JUMP
			...
			tco_addr_t addr;
			int size = sizeof(*tack);
			if (te->te_state != TS_UNBND) {
				size += tco_alen(te->te_addr);
				if (te->te_state == TS_DATA_XFER) {
					size += tco_alen(te->te_con->te_addr);
				}
			}
			if ((mp2 = allocb(size, BPRI_HI)) == 0) {
				STR_UNLOCK_SPL(spl);
				tco_fatal(q,mp);
				return UNIX_FAIL;
			}
			mp2->b_datap->db_type = M_PCPROTO;
			tack = (struct T_addr_ack *)mp2->b_rptr;
			tack->PRIM_type = T_ADDR_ACK;
			mp2->b_wptr += sizeof(*tack);
			if (te->te_state != TS_UNBND) {
				tack->LOCADDR_length = tco_alen(te->te_addr);
				tack->LOCADDR_offset = sizeof(*tack);
				addr.ta_abuf = (char *)mp2->b_wptr;
				addr.ta_alen = tco_alen(te->te_addr);
				tco_cpabuf(&addr, te->te_addr);
				mp2->b_wptr += tack->LOCADDR_length;
				if (te->te_state == TS_DATA_XFER) {
					tack->REMADDR_length = 
						tco_alen(te->te_con->te_addr);
					tack->REMADDR_offset = 
					    tack->LOCADDR_offset +
					    tack->LOCADDR_length;
					addr.ta_abuf = (char *)mp2->b_wptr;
					addr.ta_alen = 
						tco_alen(te->te_con->te_addr);
					tco_cpabuf(&addr, te->te_con->te_addr);
					mp2->b_wptr += tack->REMADDR_length;
				} else {
					tack->REMADDR_length = 0;
					tack->REMADDR_offset = 0;
				}
			} else {
				tack->LOCADDR_length = 0;
				tack->LOCADDR_offset = 0;
				tack->REMADDR_length = 0;
				tack->REMADDR_offset = 0;
			}
			STR_UNLOCK_SPL(spl);
 ...
 */
STATIC int
tco_okack(queue_t *q, register mblk_t *mp)
{
	register union T_primitives	*prim;
	mblk_t				*mp1;
	long				type;
 ...
 */
STATIC int
tco_errack(queue_t *q, register mblk_t *mp, int tli_err, int unix_err)
{
	mblk_t				*mp1;
	long				type;
	register union T_primitives	*prim;
 ...
 */
STATIC int
tco_fatal(register queue_t *q, register mblk_t *mp)
{
	register tco_endpt_t		*te;
 ...
 */
STATIC int
tco_ckstate(register queue_t *q, register mblk_t *mp)
{
	register union T_primitives	*prim;
	register tco_endpt_t		*te;
	register char			ns;
 ...
 */
STATIC int
tco_ireq(queue_t *q, register mblk_t *mp)
{
	register union T_primitives	*prim1;
	tco_endpt_t			*te;
	mblk_t				*mp1;
	...
	ASSERT(te != NULL);
	ASSERT((__psunsigned_t)(mp->b_rptr)%NBPW == 0);	/* alignment */
	ASSERT(((union T_primitives *)mp->b_rptr) != NULL);
	ASSERT(((union T_primitives *)mp->b_rptr)->type == T_INFO_REQ);
	/*
	 *	prepare ack msg
	 */
	...
	ASSERT((__psunsigned_t)(mp1->b_rptr)%NBPW == 0);	/* alignment */
	...
	prim1->info_ack.PROVIDER_flag = SENDZERO | XPG4_1;
	(void)freemsg(mp);
	/*
	 *	send ack msg
	 */
 ...
 */
STATIC int
tco_bind(queue_t *q, register mblk_t *mp)
{
	register tco_endpt_t		*te;
	...
	ASSERT((__psunsigned_t)(mp2->b_rptr)%NBPW == 0);	/* alignment */
 ...
 */
STATIC int
tco_unbind(queue_t *q, register mblk_t *mp)
{
	register tco_endpt_t		*te;
	...
	ASSERT((__psunsigned_t)(mp->b_rptr)%NBPW == 0);	/* alignment */
	ASSERT(((union T_primitives *)mp->b_rptr) != NULL);
	ASSERT(((union T_primitives *)mp->b_rptr)->type == T_UNBIND_REQ);
	/*
	 *	flush queues
	 */
 ...
 */
STATIC int
tco_optmgmt(queue_t *q, register mblk_t *mp)
{
	register tco_endpt_t		*te;
	...
	switch (prim->optmgmt_req.MGMT_flags) {
		...
		ASSERT((__psunsigned_t)&prim->optmgmt_req.PRIM_type - (__psunsigned_t)&prim->optmgmt_req \
		    == (__psunsigned_t)&prim->optmgmt_ack.PRIM_type - (__psunsigned_t)&prim->optmgmt_ack);
		ASSERT((__psunsigned_t)&prim->optmgmt_req.OPT_length - (__psunsigned_t)&prim->optmgmt_req \
		    == (__psunsigned_t)&prim->optmgmt_ack.OPT_length - (__psunsigned_t)&prim->optmgmt_ack);
		ASSERT((__psunsigned_t)&prim->optmgmt_req.OPT_offset - (__psunsigned_t)&prim->optmgmt_req \
		    == (__psunsigned_t)&prim->optmgmt_ack.OPT_offset - (__psunsigned_t)&prim->optmgmt_ack);
		ASSERT((__psunsigned_t)&prim->optmgmt_req.MGMT_flags - (__psunsigned_t)&prim->optmgmt_req \
		    == (__psunsigned_t)&prim->optmgmt_ack.MGMT_flags - (__psunsigned_t)&prim->optmgmt_ack);
		mp->b_datap->db_type = M_PCPROTO;
		prim1 = prim;
 ...
 */
STATIC int
tco_creq(queue_t *q, register mblk_t *mp)
{
	register tco_endpt_t		*te,*te2,**tep,**etep;
	register union T_primitives	*prim,*prim2;
	tco_addr_t			addr,*ta;
	mblk_t				*mp1,*mp2;
	int				alen,aoff,olen,olen2,ooff,msz,msz2,
					err;
	long				idflg2;
	int				cseq;
	...
	ASSERT((__psunsigned_t)te != BADSEQNUM);
	ASSERT(te != NULL);
	cseq = atomicIncWithWrap(&te2->te_connseq, INT_MAX - 1);
	prim2->conn_ind.SEQ_number = te->te_connseq = cseq;
 ...
 */
STATIC int
tco_cres(queue_t *q, register mblk_t *mp)
{
	register tco_endpt_t		*te,*te3,**tep,**etep;
        tco_endpt_t                     *te1;
	...
	;    tep < etep
	;    tep += 1) {
		if ((*tep) && (*tep)->te_connseq == prim->conn_res.SEQ_number) {
			te3 = *tep;
			*tep = NULL;
			break;
		}
	...
	if (te3->te_state != TS_WCON_CREQ) {
		...
		ASSERT((__psunsigned_t)(mp3->b_rptr)%NBPW == 0);	/* alignment */
 ...
 */
STATIC int
tco_dreq(queue_t *q, register mblk_t *mp)
{
	register union T_primitives	*prim,*prim2;
	register tco_endpt_t		*te,*te2,**tep,**etep;
	mblk_t				*mp1,*mp2;
	...
	if (te->te_nicon > 0) {
		 ...
		 */
		if (prim->discon_req.SEQ_number == (int)NULL) {
			STRLOG((TCO_ID,tco_min(te),2,SL_TRACE,
		...
		;    tep < etep
		;    tep += 1) {
			if ((*tep) && (*tep)->te_connseq == prim->discon_req.SEQ_number) {
				te2 = *tep;
				*tep = NULL;
				break;
			}
	...
	if (te->te_nicon > 0) {
		...
		prim2 = (union T_primitives *)mp2->b_rptr;
		ASSERT(prim2 != NULL);
		prim2->discon_ind.SEQ_number = te->te_connseq;
		te->te_ocon = NULL;
 ...
 */
STATIC int
tco_unconnect(register tco_endpt_t *te)
{
	register union T_primitives	*prim;
	...
	if (te->te_nicon > 0) {
		...
		prim = (union T_primitives *)mp1->b_rptr;
		ASSERT(prim != NULL);
		prim->discon_ind.SEQ_number = te->te_connseq;
		prim->discon_ind.PRIM_type = T_DISCON_IND;
		prim->discon_ind.DISCON_reason = TCO_PROVIDERINITIATED;
		(void)putnext(te1->te_rq,mp1);
	} else if (te->te_con != NULL) {
 ...
 */
STATIC int
tco_data(queue_t *q, register mblk_t *mp, int evtype)
{
	...
	if ((__psunsigned_t)(mp->b_rptr)%NBPW != 0) {
		STRLOG((TCO_ID,tco_min(te),1,SL_TRACE,
	...
	switch (mp->b_datap->db_type) {
		...
		switch (evtype) {
			...
			ASSERT((__psunsigned_t)&prim->data_req.PRIM_type - (__psunsigned_t)&prim->data_req \
			    == (__psunsigned_t)&prim->data_ind.PRIM_type - (__psunsigned_t)&prim->data_ind);
			ASSERT((__psunsigned_t)&prim->data_req.MORE_flag - (__psunsigned_t)&prim->data_req \
			    == (__psunsigned_t)&prim->data_ind.MORE_flag - (__psunsigned_t)&prim->data_ind);
			...
			ASSERT((__psunsigned_t)&prim->exdata_req.PRIM_type - (__psunsigned_t)&prim->exdata_req \
			    == (__psunsigned_t)&prim->exdata_ind.PRIM_type - (__psunsigned_t)&prim->exdata_ind);
			ASSERT((__psunsigned_t)&prim->exdata_req.MORE_flag - (__psunsigned_t)&prim->exdata_req \
			    == (__psunsigned_t)&prim->exdata_ind.MORE_flag - (__psunsigned_t)&prim->exdata_ind);
			...
			ASSERT((__psunsigned_t)&prim->ordrel_req.PRIM_type - (__psunsigned_t)&prim->ordrel_req \
			    == (__psunsigned_t)&prim->ordrel_ind.PRIM_type - (__psunsigned_t)&prim->ordrel_ind);
