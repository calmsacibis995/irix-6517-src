...
#ident	"@(#)uts-comm:net/transport/timod.c	1.8"
/*
 ...
 */
#define	OLD_INFO_ACK_SZ	(sizeof(struct T_info_ack)-sizeof(int))

STATIC int	tim_setname(queue_t *q, mblk_t *mp);
STATIC void	tim_bcopy(mblk_t *frommp, caddr_t to, int len);
STATIC mblk_t	*tim_filladdr(queue_t *q, mblk_t *mp);

/* stream data structure definitions */

int timodopen(queue_t *, dev_t *, int, int, cred_t *);
int timodclose(queue_t *, int, cred_t *);
int timodput(queue_t *, mblk_t *);
int timodrsrv(queue_t *);
int timodwsrv(queue_t *);
...
/*ARGSUSED*/
int
timodopen(register queue_t *q, dev_t *devp, int flag, int sflag, cred_t *crp)
{
	register struct tim_tim *tp;
	register int s;
	void timtime();
	...
	if (q->q_ptr)
        ...
        if ((tp = (struct tim_tim *)kmem_zalloc(sizeof(struct tim_tim),
                        KM_NOSLEEP)) == NULL) 
                return (ENOMEM);
	...
	while ((tp->tim_myname = kmem_alloc(PRADDRSZ, KM_NOSLEEP)) == NULL) {
		...
		tp->tim_mylen = STREAMS_TIMEOUT((strtimeoutfunc_t)timtime, tp,
						TIMWAIT);
		if (sleep((caddr_t)tp, STOPRI)) {
			tp->tim_flags = 0;
	...
	while ((tp->tim_peername = kmem_alloc(PRADDRSZ, KM_NOSLEEP)) == NULL) {
		...
		STR_LOCK_SPL(s);
		tp->tim_flags |= OPENWAIT;
		tp->tim_peerlen = STREAMS_TIMEOUT((strtimeoutfunc_t)timtime,
						  tp, TIMWAIT);
		if (sleep((caddr_t)tp, STOPRI)) {
			tp->tim_flags = 0;
        ...
        {
                tmp_tim->inuse.next = tp;
                tp->inuse.prev = tmp_tim;
        }
...
/*ARGSUSED*/
int
timodclose(register queue_t *q, int flag, cred_t *crp)
{
	register struct tim_tim *tp, *tmp;
	register mblk_t *mp;
	register mblk_t *nmp;
	...
	tp->tim_flags = 0;
        ...
        if (tp->inuse.prev != NULL)
                if (tp->inuse.next == NULL)
                {
                        tmp_tim = tp->inuse.prev;
                        tmp_tim->inuse.next = NULL;
                }
                else
                {
                        tmp = tp->inuse.prev;
                        tmp->inuse.next = tp->inuse.next;
                        tmp = tp->inuse.next;
                        tmp->inuse.prev = tp->inuse.prev;
                }
        else
                if (tp->inuse.next != NULL)
                {
                        tim_tim = tp->inuse.next;
                        tim_tim->inuse.prev = NULL;
                }
 ...
 */
int
timodput(register queue_t *q, register mblk_t *mp)
{
	queue_t *rdq;
	queue_t *wrq;
...
rgetnext:
	...
	case M_PCPROTO:
	    ...
	    ASSERT((mp->b_wptr - mp->b_rptr) >= sizeof(int));
...
error_ack:
		...
		if (tp->tim_flags & WAITIOCACK) {
		    ...
		    if (pptr->error_ack.ERROR_prim !=
		      *(int *)tp->tim_iocsave->b_cont->b_rptr) {
			putnext(q, mp);
			goto rgetnext;
		    }
		...
		if (tp->tim_flags & WAITIOCACK) {
		    ...
		    if (*(int *)tp->tim_iocsave->b_cont->b_rptr !=
		      T_BIND_REQ) {
			putnext(q, mp);
			goto rgetnext;
		    }
			...
			if (ackp->ADDR_length > tp->tim_mymaxlen) {
			    ...
			    if (p == NULL) {
				putbq(q, mp);
				(void) STREAMS_TIMEOUT((strtimeoutfunc_t)timodqenable,
							q, TIMWAIT);
				return (0);
			    }
			    kmem_free(tp->tim_myname, tp->tim_mymaxlen);
		...
		if (tp->tim_flags & WAITIOCACK) {
		    ...
		    ASSERT((size == sizeof(struct T_info_ack)) || (size == OLD_INFO_ACK_SZ));
		    if (*(int *)tp->tim_iocsave->b_cont->b_rptr!=T_INFO_REQ) {
			putnext(q, mp);
			return (0);
		    }
		     ...
		     * a T_info_ack.
                     *
                     * If we are emulating the old TI_GETNAME, we make
                     * the size match the old size.
                     */
                    if (((struct iocblk *) tp->tim_iocsave->b_rptr)->ioc_cmd ==
                                        _SVR3_TI_GETINFO) {
                        if (size > OLD_INFO_ACK_SZ) {
                            size = OLD_INFO_ACK_SZ;
                            mp->b_wptr = mp->b_rptr + size;
                        }
                    } else if (size == OLD_INFO_ACK_SZ) {
			if (mp->b_datap->db_lim - mp->b_wptr < sizeof(int)) {
			    tmp = allocb(sizeof(struct T_info_ack), BPRI_HI);
			    if (tmp == NULL) {
	 			ASSERT((mp->b_datap->db_lim - mp->b_datap->db_base) < sizeof(struct T_error_ack));
...
out:
		...
		if (tp->tim_peermaxlen != 0) {
			...
			putbq(q, mp);
			if (!bufcall((uint) sizeof(mblk_t), BPRI_MED,
			    timodqenable, (long)q))
				(void) STREAMS_TIMEOUT((strtimeoutfunc_t)timodqenable, 
						       (caddr_t)q, TIMWAIT);
			return (0);
...
wgetnext:
	...
	case M_IOCTL:
		...
		if ((tmp = copymsg(mp->b_cont)) == NULL) {
			...
			i += (__psint_t)(tmp->b_wptr - tmp->b_rptr);
		    if (!bufcall(i, BPRI_MED, timodqenable, (long)q))
			 (void) STREAMS_TIMEOUT((strtimeoutfunc_t)timodqenable,
						q, 
						TIMWAIT);
		    return (0);
		...
		tp->tim_flags |= WAITIOCACK;
                if (iocbp->ioc_cmd == TI_GETINFO ||
                    iocbp->ioc_cmd == _SVR3_TI_GETINFO)
...
getname:
		if ((tmp = copymsg(mp)) == NULL) {
		    ...
		    for (tmp = mp; tmp; tmp = tmp->b_next)
			i += (__psint_t)(tmp->b_wptr - tmp->b_rptr);
		    if (!bufcall((uint) i, BPRI_MED, timodqenable, (long)q))
			 (void) STREAMS_TIMEOUT((strtimeoutfunc_t)timodqenable,
						q, 
						TIMWAIT);
		    return (0);
 ...
 */
int
ti_doname(queue_t *q, mblk_t *mp, void *lname, uint llen, void *rname,
	  uint rlen)
{
	struct iocblk *iocp;
	struct copyreq *cqp;
	struct copyresp *csp;
	struct irix5_netbuf *np;
	int ret;
	mblk_t *bp;
	...
	case M_IOCTL:
		...
		if ((iocp->ioc_count != TRANSPARENT) ||
		    ...
		    mp->b_cont->b_rptr) != sizeof(app32_ptr_t))) {
			iocp->ioc_error = EINVAL;
			freemsg(mp->b_cont);
			mp->b_cont = NULL;
			mp->b_datap->db_type = M_IOCNAK;
		...
		cqp->cq_addr = (caddr_t)(__psint_t)
			*(app32_ptr_t *)mp->b_cont->b_rptr;
		mp->b_cont = NULL;
		cqp->cq_size = sizeof(struct irix5_netbuf);
		cqp->cq_flag = 0;
	...
	case M_IOCDATA:
		...
		if (csp->cp_private->b_cont == NULL) {	/* got netbuf */
			ASSERT(mp->b_cont);
			np = (struct irix5_netbuf *)mp->b_cont->b_rptr;
			if (csp->cp_cmd == TI_GETMYNAME) {
				if (llen == 0) {
		...
		np = (struct irix5_netbuf *)csp->cp_private->b_cont->b_rptr;
		if (np->len == 0) {
			if (np->maxlen == 0) {
			...
			if (csp->cp_cmd == TI_GETMYNAME)
				...
				np->len = rlen;
			if ((bp = allocb(sizeof(struct irix5_netbuf), BPRI_MED))
			    == NULL) {
			...
			bp->b_wptr += sizeof(struct irix5_netbuf);
			bcopy((caddr_t) np, (caddr_t) bp->b_rptr,
			    sizeof(struct irix5_netbuf));
			cqp->cq_addr = (caddr_t)(__psint_t)
				*(app32_ptr_t *)csp->cp_private->b_rptr;
			cqp->cq_size = sizeof(struct irix5_netbuf);
			cqp->cq_flag = 0;
		...
		cqp->cq_addr = (caddr_t)(__psint_t)np->buf;
		cqp->cq_size = np->len;
		cqp->cq_flag = 0;
		mp->b_datap->db_type = M_COPYOUT;
		mp->b_cont = bp;
...
STATIC int
tim_setname(register queue_t *q, register mblk_t *mp)
{
	register struct iocblk *iocp;
	register struct copyreq *cqp;
	register struct copyresp *csp;
	struct tim_tim *tp;
	struct irix5_netbuf *netp;
	unsigned int len;
	caddr_t p;
	...
	case M_IOCDATA:
		...
		if (csp->cp_private == NULL) {	/* got netbuf */
			netp = (struct irix5_netbuf *)mp->b_cont->b_rptr;
			csp->cp_private = mp->b_cont;
			mp->b_cont = NULL;
			cqp->cq_addr = (caddr_t)(__psint_t)netp->buf;
 ...
 */
STATIC void
tim_bcopy(mblk_t *frommp, register caddr_t to, register int len)
{
	register mblk_t *mp;
	register int size;
 ...
 */
STATIC mblk_t *
tim_filladdr(register queue_t *q, register mblk_t *mp)
{
	register mblk_t *bp;
	register struct tim_tim *tp;
