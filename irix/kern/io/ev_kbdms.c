...
#define HIWAT_RBUF(bp) (bp->b_wptr >= (bp->b_rptr+(RBUF_LEN/2)))
...
#define	NUMDUARTS 15		/* max number of DUARTs */
#define MAX_GFXHEADS 4 
#define MASTER_MOUSEPORT  0
#define MASTER_GFXKEYPORT 1 
#define IS_GFXKEYPORT(_x)	(gfxkeyports & (1 << (_x)))
#define SET_GFXKEYPORT(_x)	(gfxkeyports |= (1 << (_x)))
#define IS_MOUSEPORT(_x)	(mouseports & (1 << (_x)))
#define SET_MOUSEPORT(_x)	(mouseports |= (1 << (_x)))
...
#define LOCK_TX_BUF(dp)		mutex_spinlock_spl(&dp->dp_tx_buflock, spl7)
#define UNLOCK_TX_BUF(dp, s)	mutex_spinunlock(&dp->dp_tx_buflock, s)
#define INIT_TX_BUFLOCK(dp) \
		init_spinlock(&dp->dp_tx_buflock, "dut", dp->dp_index)

/* RX buffer */

#define LOCK_RX_BUF(dp)		mutex_spinlock_spl(&dp->dp_rx_buflock, spl7)
#define UNLOCK_RX_BUF(dp, s)	mutex_spinunlock(&dp->dp_rx_buflock, s)
...
/* EVEREST DUART's must have their addresses assigned dynamically
 * since there are no guarantees about which padap is that of the
 * EPC.
 */
static dport_t dports[NUMDUARTS*DUARTPORTS-1];
static int max_kbdms_port;
static int ev_kbdms_portmask; /* Bits indicating the IO4 DUART units present */


static int maxport;		/* maximum number of ports */
static int fifo_size = 4;	/* on chip transmit FIFO size */
...
static int mips_du_putchar(dport_t *, u_char);
static int mips_du_putchar_nolock(dport_t *, u_char);
...
static int du_rx(dport_t *);
static int du_tx(dport_t *);
...
du_log_act(dport_t *dp, enum duact act, int info)
{
	...
	/*
	 * Don't use any locks in this routine, as it could get
	 * called from mutex_spinlock()->du_conpoll path
	 * which is trying to grab a spinlock. Trying to get
	 * another lock here, could lead to recursively calling
	 * this routine, and filling up the stack completely.
	 *
	 * Instead, use atomic ops to get the log entry to be updated.
	 */

	nanotime(&tv);

	/* Add one, and get back old value. Mod with DU_ACTLOG_SIZE
	 * to get right value.
	 */
	wind = atomicAddUint((uint *)&dp->dp_al_wind, 1) % DU_ACTLOG_SIZE;
...
du_lock_port(dport_t *dp, int try)
{
    ...
    if (try) {
	s = mutex_spintrylock_spl(&dp->dp_port_lock, spl7);
	if (s == 0)
	    return(-1);
    }
    else
	s = mutex_spinlock_spl(&dp->dp_port_lock, spl7);
...
static int
du_rx_buf_peekchar(dport_t *dp, int *c)
{
	register int s;
...
static void
du_rx_buf_advance_char(dport_t *dp)
{
    int s = LOCK_RX_BUF(dp);

    /* may have called CLEAR_RX_BUF between peekchar and advance_char
     * if a break arrived. If so the buf is clear so we should not
...
static int
du_rx_buf_putchar(dport_t *dp, int c)
{

	register int s;
	extern lock_t strintr_lock;
	int hiwat;
...
du_putchar(int port, unsigned char c)
{
	...
	while(!(RD_RR0(dp->dp_cntrl) & RR0_TX_EMPTY));
...
/* ARGSUSED */
void
evintr_kbdms(eframe_t *ep, void *arg)
{
    int i;
    int spurious;
    int first;
    ...
    /* Keep iterating until no work was done in the last pass or
     * until we've already made three passes.
     */ 
    first = 1;
    do {
	spurious = 1;
	for (i = 0; i < max_kbdms_port; i += 2) {
	    if ((ev_kbdms_portmask & (1<< i)) && du_handle_intr(i))
		spurious = 0;
	}
	if (!spurious)
	    first = 0;
    ...
    } while (!spurious && (iterations < 3));

    /* If we exited after the first path without doing anything,
     * we must have gotten a spurious interrupt.
     */	
    if (first)
	spurious_kbdms_intr++;
    ...
    for (i = 0; i < max_kbdms_port; i += 2)
	if (ev_kbdms_portmask & ( 1 << i)) /* Only if it's a valid port */
	    EV_SET_REG(dports[i].dp_swin + EPC_IMSET, EPC_INTR_DUART0);
}
...
evkm_portinit(void)
{
    ...
    extern int count_xadaps(unsigned);

    /* Invalidate the port_map array */
    ...
    base = 2 * count_xadaps(EPC_SERIAL);
    max_kbdms_port = 0;
    ev_kbdms_portmask = 0;
	...
	/* By convention the master io4 is in io4map[0] */ 
	if (unit < MAX_GFXHEADS) {        
	    /* Set up the Mouse UART information (DUART 0B) */
	    dp = &dports[2*unit];
	    dp->dp_cntrl = (u_char*) (d_base + EPC_DUART0 + CHNB_CNTRL_OFFSET);
	    dp->dp_data  = (u_char*) (d_base + EPC_DUART0 + CHNB_DATA_OFFSET);
	    dp->dp_index = 2*unit;
	    dp->dp_bitmask = 0xff;
	    dp->dp_swin = swin;
	    port_map[31 - 2*unit] = 2*unit;
	    SET_MOUSEPORT(2*unit);

	    /* Set up keyboard information (DUART 0A) */
	    dp = &dports[2*unit+1];
	    dp->dp_cntrl = (u_char*) (d_base + EPC_DUART0 + CHNA_CNTRL_OFFSET);
	    dp->dp_data  = (u_char*) (d_base + EPC_DUART0 + CHNA_DATA_OFFSET);
	    dp->dp_index = 2*unit+1;
	    dp->dp_bitmask = 0xff;
	    dp->dp_swin    = swin;
	    port_map[30 - 2*unit] = 2*unit+1;
	    SET_GFXKEYPORT(2*unit+1);

	    /* Connect the interrupt and enable it on the EPC */
	    evintr_connect((evreg_t *)(swin + EPC_IIDDUART0),
                        EVINTR_LEVEL_EPC_KBDMS,
                        SPLHIDEV,
                        EVINTR_DEST_EPC_KBDMS,
                        evintr_kbdms, /* handler for kbd/mouse */
                        (void*)0);
	    EV_SET_REG(swin + EPC_IMSET, EPC_INTR_DUART0);
	    max_kbdms_port = MAX(2*unit, max_kbdms_port+2);
	    /* ev_kbdms_portmask has 2 bits/unit, and Set both bits on */
	    ev_kbdms_portmask |= ( 3 << (unit * 2)); 
	}
...
evkb_init(void)
{
	...
	for (port = 0; port < maxport; port++) {
		if (IS_GFXKEYPORT(port)) {
			dp = &dports[port];
			if (!dp->dp_cntrl) continue;
			dp->dp_iflag = IGNBRK;
			mips_du_config(dp,DEF_GFXKEY_CFLAG,DEF_GFXKEY_OSPEED,(struct termio *)0);
			du_zap(dp, HUPCL);
		}
	}
}
...
evkb_lateinit()
{
    ...
    /* look for kbd/mouse ports and create the nodes. unit is in range
     * [0-3], with 2 ports per unit at offset 31-2*unit and 30-2*unit.
     */
    for(x = 0; x < MAX_GFXHEADS; x++) {
	vertex_hdl_t dev_vhdl;

	/* check for a mouse */
	if (port_map[31-2*x] != 255 &&
	    dports[port_map[31-2*x]].dp_cntrl) {

	    /* create a node in /hw/inputX */
	    sprintf(name, x > 0 ? "input%d/mouse" : "input/mouse", x);
	    rc = hwgraph_char_device_add(hwgraph_root, name, "evkb", &dev_vhdl);
	    ASSERT(rc == GRAPH_SUCCESS);
	    hwgraph_chmod(dev_vhdl, HWGRAPH_PERM_MOUSE);
	    hwgraph_fastinfo_set(dev_vhdl,
				 (arbitrary_info_t)&dports[port_map[31-2*x]]);
	}

	/* check for a keyboard */
	if (port_map[30-2*x] != 255 &&
	    dports[port_map[30-2*x]].dp_cntrl) {

	    /* create a node in /hw/inputX */
	    sprintf(name, x > 0 ? "input%d/keyboard" : "input/keyboard", x);
	    rc = hwgraph_char_device_add(hwgraph_root, name, "evkb", &dev_vhdl);
	    ASSERT(rc == GRAPH_SUCCESS);
	    hwgraph_chmod(dev_vhdl, HWGRAPH_PERM_KBD);
	    hwgraph_fastinfo_set(dev_vhdl,
				 (arbitrary_info_t)&dports[port_map[30-2*x]]);
	}
    }
}
...
{
    return((dport_t*)device_info_get(dev_to_vhdl(minordev)));
}
...
du_open(queue_t *rq, dev_t *devp, int flag, int sflag, struct cred *crp)
{
	...
	if (dev_is_vertex(*devp))
	    dp = (dport_t*)device_info_get(*devp);
	else
	    return(ENXIO);
...
du_rsrv(queue_t *rq)
{
	...
	while(du_rx_buf_peekchar(dp, &c)) {

	    /* If we fail to process the char due to buffering problems,
	     * just leave the chars in the rx buffer instead of chucking
	     * them on the floor. They may yet be picked up successfully
	     * on the next try
	     *
	     * XXXtcl how long might we wait before these other chars
	     * are finally sent upstream if there is not a subsequent
	     * interrupt to kick the stream again? In any case it's better
	     * to get the chars late than never. I'm betting this will
	     * only happen under very heavy input anyway, in which case there
	     * will very probably be another interrupt shortly.
	     */
	    if (du_process_inchar(dp, c))
		break;
	    du_rx_buf_advance_char(dp);
	}
...
#ifdef DEBUG
		int i, size = (int)(bp->b_wptr - bp->b_rptr);
...
#define	CCUART_INPUT_ENABLE	EV_SET_LOCAL(EV_UART_CMD, I8251_TXENB|I8251_RXENB)
#define	CCUART_INPUT_DISABLE	EV_SET_LOCAL(EV_UART_CMD, I8251_TXENB)
#define	CCUART_INPUT_RDY	((char)EV_GET_LOCAL(EV_UART_CMD) & I8251_RXRDY)
#define	CCUART_INPUT		((char)EV_GET_LOCAL(EV_UART_DATA))
#define	CCUART_OUTPUT_RDY	((char)EV_GET_LOCAL(EV_UART_CMD) & I8251_TXRDY)


/*
 * The PROM will have initialized all CC UARTs properly
...
ccuart_output(char c)
{
	...
	while (timeleft-- &&
		!(EV_GET_LOCAL(EV_UART_CMD) & I8251_TXRDY))
		/* spin */ ;

	EV_SET_LOCAL(EV_UART_DATA, c);
}

/*
 * put string to caller's ccuart.  Machines in the field will 
 * not have anything attached to this UART, but putstr to this 
 * port is convenient during initial bringup of Everest systems.  
 * It's one step above the LED display!
 */
void
ccuart_putstr(char *string)
{
	char c;
	char *bufptr = string;
...
#if SABLE
void
ccuartintr(struct eframe_s *ep)
{
    int state;

#if !IP21_NO_CCUART_INT

	ccuart_handle_intr(2);
...
ccuart_service()
{
	 ...
	 * is called.
	 */
  	CCUART_INPUT_ENABLE;		/* Enable receiver, SABLE slows down*/
#if IP21_NO_CCUART_INT
	/* If not using CCUART interrupts (i.e. masked off in SR) then we
...
#endif
	CCUART_INPUT_DISABLE;		/* Disable receive, SABLE speeds up */
}
...
int
ccuart_handle_intr(int duartnum)
{
	u_char du_cntrl;	/* XXXrs - for RD_CNTRL */
	...
	hwstate = EV_GET_LOCAL(EV_UART_CMD);

	/* receive character/error interrupt */
	if (hwstate & I8251_RXRDY) {
		SYSINFO.rcvint++;
		if ((i = du_rx(dp0)) == SCHED_RX)
			dp0rxsched++;
		else if (i == SCHED_TX)
			dp0txsched++;
	}
...
du_handle_intr(int duartnum)
{
	...
	if (rr3 & RR3_CHNB_EXT_IP) {
		...
		DU_LOG_ACT(dp0, DU_ACT_HANDLE_INTR_ESTAT_INTR);
...
du_process_inchar(dport_t *dp, int c)
{
	...
	if (!(bp = dp->dp_cur_rbp)	/* get a buffer if we have none */
	    && !(bp = du_getbp(dp, BPRI_HI))) {
		dp->dp_allocb_fail++;	/* YIKES */
		/*
		 * Note that du_getbp() has already activated flow
		 * control.
		 */
	...
	/*
	 * If above high water mark, activate flow control.  Flow control
	 * will be released when the read service procedure finally
	 * sends characters upstream.
	 */
	if (HIWAT_RBUF(bp)) {
		dp->dp_rb_hiwat++;
		mips_du_iflow(dp, SEND_XOFF, !IGNORE_IXOFF);
...
du_rx(dport_t *dp)
{
	...
	return(du_get_inchars(dp) ? SCHED_RX : SCHED_NOTHING);
}
...
du_tx(dport_t *dp)
{
	...
	while (count) {
		...
		if (dp->dp_state & DP_TX_TXON) {	/* send XON or XOFF */
		    ...
		    c = dp->dp_cc[VSTART];
		    if (mips_du_putchar_nolock(dp, c))
...
/*
 * transmit a character if the transmitter is ready, else do nothing.
 * returns 1 if the byte is actually written, 0 otherwise
 */
...
#endif
	s = LOCK_PORT(dp);
	if ((RD_RR0(cntrl) & RR0_TX_EMPTY) == 0) {
	    UNLOCK_PORT(dp, s);
	    return(0);
	}
...
mips_du_getchar(dport_t *dp, int ignerr)
{
	...
	if (RD_RR0(cntrl) & RR0_RX_CHR) {
		...
		c = RD_CNTRL(cntrl, RR1);	/* read possible error status */

		if (c &= (RR1_FRAMING_ERR | RR1_RX_ORUN_ERR | RR1_PARITY_ERR)) {

			WR_WR0(cntrl, WR0_RST_ERR)
#if DEBUG && !MFG
			if ( c & RR1_FRAMING_ERR && !ignerr) {
				DU_LOG_ACT(dp, DU_ACT_MIPS_GETCHAR_FRAME_ERR);
				UNLOCK_PORT(dp, s);
				cmn_err(CE_WARN,
			...
			if ( c & RR1_RX_ORUN_ERR && !ignerr) {
				DU_LOG_ACT(dp, DU_ACT_MIPS_GETCHAR_OVERRN_ERR);
				UNLOCK_PORT(dp, s);
				cmn_err(CE_WARN,
			...
			if ( c & RR1_PARITY_ERR && !ignerr) {
				DU_LOG_ACT(dp, DU_ACT_MIPS_GETCHAR_PARITY_ERR);
				UNLOCK_PORT(dp, s);
				cmn_err(CE_WARN,
		...
		rtn = ((c << 8) | (RD_DATA(dp->dp_data) & dp->dp_bitmask));
		dp->dp_rxchars++;
		UNLOCK_PORT(dp, s);
		DU_LOG_ACT_INFO(dp, DU_ACT_MIPS_GETCHAR_RET0, rtn & 0xf);
		return(rtn);
	}
	else {
		UNLOCK_PORT(dp, s);
		DU_LOG_ACT(dp, DU_ACT_MIPS_GETCHAR_RET1);
		return(-1);
	}
}
...
mips_du_stop_rx(dport_t *dp, int hup)
{
	...
	if (hup) {
		...
		if (!IS_GFXKEYPORT(dp->dp_index) && !IS_MOUSEPORT(dp->dp_index))
		{
			DU_DTR_DEASSERT(dp->dp_wr5);
			DU_RTS_DEASSERT(dp->dp_wr5);
...
mips_du_start_rx(dport_t *dp)
{
	...
	if (!(dp->dp_cflag & CLOCAL)
	    && !IS_GFXKEYPORT(dp->dp_index) && !IS_MOUSEPORT(dp->dp_index)
	    && dp->dp_state & (DP_ISOPEN | DP_WOPEN)) 
...
void
everest_dump_dport_info(__psint_t p1)
{
	struct dport *dp;
	int i;
	...
	qprintf("  tx chars %d rx chars %d\n", dp->dp_txchars,
		dp->dp_rxchars);
	qprintf("  errors: framing %d parity %d overrun %d\n",
		dp->dp_fe, dp->dp_parity, dp->dp_over);
	qprintf("          rb_over %d %d rb_hiwat %d tx_full %d\n",
		dp->dp_rb_over, dp->dp_rb_over1, dp->dp_rb_hiwat, dp->dp_full_tx_buf);
	qprintf("          rx_full %d rx_hiwat %d allocb %d\n",
		dp->dp_full_rx_buf, dp->dp_hiwat_rx_buf, dp->dp_allocb_fail);
...
void
everest_reset_dport_info(__psint_t p1)
{
	struct dport *dp;
...
void
everest_dump_dport_actlog(__psint_t p1)
{
	struct dport *dp;
	int i, j, act;
	...
	if (longest_string == 0) {
	    padding_len = (int)strlen(padding);
	    for(i = 0; i < DU_ACT_MAX; i++) {
		if ((j = (int)strlen(duact_strings[i])) > longest_string)
		    longest_string = j;
