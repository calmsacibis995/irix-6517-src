...
#pragma mips_frequency_hint NEVER
	...
	if (btp->stale == 0) {
		 ...
		 */
		enddup = (xfs_dir2_data_unused_t *)((char *)block + *tagp);
		/*
		 * If it's not free then can't do this add without cleaning up:
		 * the space before the first leaf entry needs to be free so it
		 * can be expanded to hold the pointer to the new entry.
		 */
		if (enddup->freetag != XFS_DIR2_DATA_FREE_TAG)
			dup = enddup = NULL;
		/*
		 ...
		 */
			...
			dup = (xfs_dir2_data_unused_t *)
			      ((char *)block + bf[0].offset);
			if (dup == enddup) {
				/*
				 * It is the biggest freespace, is it too small
				 * to hold the new leaf too?
				 */
				if (dup->length < len + (uint)sizeof(*blp)) {
#pragma mips_frequency_hint NEVER
					/*
...
#pragma mips_frequency_hint NEVER
		 ...
		 * Data object just before the first leaf entry.
		 */
		dup = (xfs_dir2_data_unused_t *)((char *)block + *tagp);
		/*
		 * If it's not free then the data will go where the
		 * leaf data starts now, if it works at all.
		 */
		if (dup->freetag == XFS_DIR2_DATA_FREE_TAG) {
			if (dup->length + (btp->stale - 1) *
			    (uint)sizeof(*blp) < len)
				dup = NULL;
		} else if ((btp->stale - 1) * (uint)sizeof(*blp) < len)
			dup = NULL;
		else
...
#pragma mips_frequency_hint NEVER
		...
		for (fromidx = toidx = btp->count - 1,
			highstale = lfloghigh = -1;
		     fromidx >= 0;
		     fromidx--) {
			if (blp[fromidx].address == XFS_DIR2_NULL_DATAPTR) {
				if (highstale == -1)
					highstale = toidx;
				else {
					if (lfloghigh == -1)
		...
		lfloglow = toidx + 1 - (btp->stale - 1);
		lfloghigh -= btp->stale - 1;
		btp->count -= btp->stale - 1;
		xfs_dir2_data_make_free(tp, bp,
			(xfs_dir2_data_aoff_t)((char *)blp - (char *)block),
			(xfs_dir2_data_aoff_t)((btp->stale - 1) * sizeof(*blp)),
			&needlog, &needscan);
		blp += btp->stale - 1;
		btp->stale = 1;
		/*
	 ...
	 */
	for (low = 0, high = btp->count - 1; low <= high; ) {
		mid = (low + high) >> 1;
		if ((hash = blp[mid].hashval) == args->hashval)
			break;
	...
	while (mid >= 0 && blp[mid].hashval >= args->hashval) {
#pragma mips_frequency_hint NEVER
		mid--;
	}
	/*
	...
	if (btp->stale == 0) {
		...
		xfs_dir2_data_use_free(tp, bp, enddup,
			...
			((char *)enddup - (char *)block + enddup->length -
			 sizeof(*blp)),
	 ...
	 */
		...
		for (lowstale = mid;
		     lowstale >= 0 &&
			blp[lowstale].address != XFS_DIR2_NULL_DATAPTR;
		     lowstale--)
			continue;
		for (highstale = mid + 1;
		     highstale < btp->count &&
			blp[highstale].address != XFS_DIR2_NULL_DATAPTR &&
	 ...
	 */
	blp[mid].hashval = args->hashval;
	blp[mid].address =
		XFS_DIR2_BYTE_TO_DATAPTR(mp, (char *)dep - (char *)block);
	...
	*tagp = (xfs_dir2_data_off_t)((char *)dep - (char *)block);
	/*
	...
	xfs_da_args_t		*args)		/* dir lookup arguments */
{
	...
	dep = (xfs_dir2_data_entry_t *)
	      ((char *)block + XFS_DIR2_DATAPTR_TO_OFF(mp, blp[ent].address));
	/*
...
#pragma mips_frequency_hint NEVER
	 ...
	 */
	while (mid > 0 && blp[mid - 1].hashval == args->hashval) {
#pragma mips_frequency_hint NEVER
		mid--;
	}
	/*
	 ...
	 */
	do {
		if ((addr = blp[mid].address) == XFS_DIR2_NULL_DATAPTR)
			continue;
		/*
	...
	} while (++mid < btp->count && blp[mid].hashval == hash);
	/*
...
#pragma mips_frequency_hint NEVER
	 ...
	 */
	blp[ent].address = XFS_DIR2_NULL_DATAPTR;
...
#pragma mips_frequency_hint NEVER
	...
	dep = (xfs_dir2_data_entry_t *)
	      ((char *)block + XFS_DIR2_DATAPTR_TO_OFF(mp, blp[ent].address));
	ASSERT(dep->inumber != args->inumber);
	/*
	 * Change the inode number to the new value.
	 */
	dep->inumber = args->inumber;
	...
	const void			*b)	/* second leaf entry */
{
	...
	la = a;
	lb = b;
	return la->hashval < lb->hashval ? -1 :
		(la->hashval > lb->hashval ? 1 : 0);
}
	...
	xfs_dabuf_t		*dbp)		/* data buffer */
{
	...
	while (dp->i_d.di_size > mp->m_dirblksize) {
		...
		if (bestsp[ltp->bestcount - 1] ==
		    mp->m_dirblksize - (uint)sizeof(block->hdr)) {
#pragma mips_frequency_hint NEVER
			if (error =
			    xfs_dir2_leaf_trim_data(args, lbp,
				    (xfs_dir2_db_t)(ltp->bestcount - 1)))
				goto out;
		} else {
			error = 0;
			goto out;
...
#pragma mips_frequency_hint NEVER
	...
	block = dbp->data;
	ASSERT(block->hdr.magic == XFS_DIR2_DATA_MAGIC);
	/*
	 * Size of the "leaf" area in the block.
	 */
	size = (uint)sizeof(block->tail) +
	       (uint)sizeof(*lep) * (leaf->hdr.count - leaf->hdr.stale);
	/*
	 ...
	 */
	if (dup->freetag != XFS_DIR2_DATA_FREE_TAG || dup->length < size) {
		error = 0;
		goto out;
	}
	/*
	 ...
	 */
	block->hdr.magic = XFS_DIR2_BLOCK_MAGIC;
	needlog = 1;
	needscan = 0;
	/*
	 ...
	 */
	btp = XFS_DIR2_BLOCK_TAIL_P(mp, block);
	btp->count = leaf->hdr.count - leaf->hdr.stale;
	btp->stale = 0;
	...
	ASSERT(to == btp->count);
	xfs_dir2_block_log_leaf(tp, dbp, 0, btp->count - 1);
	/*
...
#pragma mips_frequency_hint NEVER
	...
	xfs_dir2_data_use_free(tp, bp, dup,
		(xfs_dir2_data_aoff_t)((char *)dup - (char *)block),
		dup->length, &needlog, &needscan);
	/*
	 * Create entry for .
	 */
	dep = (xfs_dir2_data_entry_t *)
	      ((char *)block + XFS_DIR2_DATA_DOT_OFFSET);
	dep->inumber = dp->i_ino;
	dep->namelen = 1;
	dep->name[0] = '.';
	tagp = XFS_DIR2_DATA_ENTRY_TAG_P(dep);
	*tagp = (xfs_dir2_data_off_t)((char *)dep - (char *)block);
	xfs_dir2_data_log_entry(tp, bp, dep);
	blp[0].hashval = xfs_dir_hash_dot;
	blp[0].address =
		XFS_DIR2_BYTE_TO_DATAPTR(mp, (char *)dep - (char *)block);
	/*
	 * Create entry for ..
	...
	*tagp = (xfs_dir2_data_off_t)((char *)dep - (char *)block);
	xfs_dir2_data_log_entry(tp, bp, dep);
	blp[1].hashval = xfs_dir_hash_dotdot;
	blp[1].address =
		XFS_DIR2_BYTE_TO_DATAPTR(mp, (char *)dep - (char *)block);
	 ...
	 */
	qsort(blp, btp->count, sizeof(*blp), xfs_dir2_block_sort);
	/* 
	 * Log the leaf entry area and tail.
	 * Already logged the header in data_init, ignore needlog.
	 */
	ASSERT(needscan == 0);
	xfs_dir2_block_log_leaf(tp, bp, 0, btp->count - 1);
	xfs_dir2_block_log_tail(tp, bp);
	xfs_dir2_data_check(dp, bp);
	xfs_da_buf_done(bp);
	return 0;
