/*
 * Copyright 1988,1991 Silicon Graphics, Inc.  All rights reserved.
 *
...
#define vnew(len, type)         ((type *) Malloc((len) * sizeof(type)))
...
unsigned int
sc_hash(Scope *sc, unsigned char *name, int namlen)
{
	hash_t hash;
...
sc_search(Scope *sc, char *name, int *namlenp, int adding)
{
	...
	sym = &sc->sc_table[sc_hash(sc, (u_char *)name, namlen)];
...
Symbol *
sc_lookupsym(Scope *sc, void *name, int namlen)
{
	Symbol *sym;
	
	sym = sc_search(sc, (char *)name, &namlen, 0);
 ...
 */
Symbol *
sc_addsym(Scope *sc, void *name, int namlen, enum symtype type)
{
	Symbol *sym;

	sym = sc_search(sc, (char *)name, &namlen, 1);
	if (sym->sym_type != SYM_FREE)
		return sym;
	if (sc->sc_numfree == 0) {
		int count, length;
 ...
 */
void
sc_deletesym(Scope *sc, void *name, int namlen)
{
	Symbol *sym;
	...
	assert(sc->sc_numfree < SCALPHA(sc->sc_length));
	sc->sc_numfree++;
	...
	if (sc->sc_length > 1 << MINLOG2
		...
		for (;;) {
			...
			do {
				...
				hash = sc_hash(sc, (u_char *)sym2->sym_name,
					       sym2->sym_namlen);
