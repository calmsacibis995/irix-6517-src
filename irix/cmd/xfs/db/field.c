...
const ftattr_t	ftattrtab[] = {
...
#else
	{ FLDT_AEXTNUM, "aextnum", fp_num, "%d", SI(bitsz(xfs_attrextnm_t)),
	  FTARG_SIGNED, NULL, NULL },
#endif
	...
	{ FLDT_DEV, "dev", fp_num, "%#x", SI(bitsz(dev_t)), 0, NULL, NULL },
#if VERS >= V_62
...
#else
	{ FLDT_DIR_BLKINFO, "dir_blkinfo", NULL, (char *)dir_blkinfo_flds,
	  SI(bitsz(struct xfs_dir_blkinfo)), 0, NULL, dir_blkinfo_flds },
#endif
...
#else
	{ FLDT_DIR_NODE_ENTRY, "dir_node_entry", fp_sarray,
	  (char *)dir_node_entry_flds, SI(bitsz(struct xfs_dir_node_entry)), 0,
	  NULL, dir_node_entry_flds },
	{ FLDT_DIR_NODE_HDR, "dir_node_hdr", NULL, (char *)dir_node_hdr_flds,
	  SI(bitsz(struct xfs_dir_node_hdr)), 0, NULL, dir_node_hdr_flds },
#endif
