#ident "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash/RCS/report.c,v 1.1 1999/05/25 19:21:38 tjm Exp $"
...
get_putbuf()
{
	...
	if (K_DUMP_HDR->dmp_version > 2) {
		np = (char *)kl_alloc_block(K_DUMP_HDR->dmp_putbuf_size+500, K_TEMP);
		pbuf = (char *)kl_alloc_block(K_DUMP_HDR->dmp_putbuf_size+1, K_TEMP);
		if (LSEEK(LIBMEM_DATA, K_CORE_FD, 
				K_DUMP_HDR->dmp_hdr_sz, SEEK_SET) < 0) {
			return((char *)NULL);
		}
		read(K_CORE_FD, (void *)pbuf, K_DUMP_HDR->dmp_putbuf_size);
		for (i = 0, j = 0; i < K_DUMP_HDR->dmp_putbuf_size; i++) {
			if (pbuf[i] == '\0') {
				/* skip all the NULLs */
			} 
			else {
		...
		np = (char *)kl_alloc_block(1500, K_TEMP);
		pbuf = (char *)kl_alloc_block(1001, K_TEMP);
		kl_get_block(K_PUTBUF, 1000, pbuf, "putbuf");
...
print_repkthread_info(kaddr_t curkthread, k_ptr_t ktp, int i, FILE *ofp)
{
	...
	if (!curkthread) {
		...
		fprintf(ofp, " The command '%s' was running.", kl_kthread_name(ktp));
		kl_get_struct(kl_kaddr(ktp, "proc", "p_cred"), CRED_SIZE, cp, "cred");
		if (!KL_ERROR) {
			fprintf(ofp, " under UID %lld\n", KL_INT(cp, "cred", "cr_ruid"));
		} 
		else {
			fprintf(ofp, "\n");
		}
...
get_report_pda(int i, int *stkflg, kaddr_t *curkthread)
{
	...
	*curkthread = 0;
	...
	pdaval = kl_get_pda_s((kaddr_t)i, pdap);
	if (KL_ERROR) {
		kl_free_block(pdap);
		return((k_ptr_t)NULL);
	}
...
get_report_kthread(kaddr_t curkthread)
{
	...
	return(kl_get_kthread(curkthread, 0));
}
...
nmi_report(command_t cb, FILE *ofp)
{
	...
	for (i = 0; i < K_MAXCPUS; i++) {
		...
		if (IS_NMI) {
			...
			kl_NMI_saveregs(i, &errorepc, &epc, &sp, &ra);
...
availmon_report()
{
	...
	fprintf(afp, "    CRASH TIME: %d %s",
		K_DUMP_HDR->dmp_crash_time, ctime(&(K_DUMP_HDR->dmp_crash_time)));
	fprintf(afp, "    PANIC STRING: %s\n", K_DUMP_HDR->dmp_panic_str);
...
dump_header_report(FILE *ofp)
{
	...
	expand_header(K_DUMP_HDR, cb.ofp);
...
base_report(command_t cb, char *p, FILE *ofp)
{
	...
	if (K_ERROR_DUMPBUF) {
		...
		fprintf(cb.ofp, "0x%llx = \"", K_ERROR_DUMPBUF);
		dump_string(K_ERROR_DUMPBUF, cb.flags, cb.ofp);
		fprintf(cb.ofp, "\"");
	}
	fprintf(cb.ofp, "\n\n");
	...
	if ((K_IP == 19) || (K_IP == 21) || (K_IP == 25)) {
		SBE = kl_get_sym("sbe_log_errors", K_TEMP); 
		if (!KL_ERROR) {
			if (SBE->n_value != 0) {
