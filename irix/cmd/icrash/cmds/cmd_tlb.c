#ident "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash/cmds/RCS/cmd_tlb.c,v 1.17 1999/05/25 19:21:38 tjm Exp $"
...
print_tlb(int slot, k_ptr_t tlbp, FILE *ofp)
{
	...
	if (K_IP == 21 || K_IP == 26) {
		...
		for (i = 0; i < 3; i++) {
			 ...
			 */	
			pte = (k_ptr_t)((uint)t + K_REGSZ + 4);

			if (DEBUG(DC_PAGE, 1)) {
				fprintf(ofp, "print_tlb: pte=0x%x, pgi=0x%llx\n", pte, pgi);
			}
			...
			xormask = ((tlbhi & K_K2BASE) != K_K2BASE) ? (pid & 0x7f) : 0;
			vaddr |= ((slot ^ xormask) << K_PNUMSHIFT);
			...
			if (i == 0) {
				fprintf(ofp, "%3d  %16llx      %2x    %16llx  ", 
					slot, vaddr, pid, pgi);
			}
			else {
				fprintf(ofp, "     %16llx      %2x    %16llx  ",
					vaddr, pid, pgi);
			}
			fprintf(ofp, "%8d (%lld %lld %lld %lld)\n", 
				((PDE_SIZE == 8) ? (int)kl_pte_to_pfn((k_ptr_t)&pgi) :
				(int)kl_pte_to_pfn((k_ptr_t)((__psunsigned_t)&pgi+4))),
				(pgi & K_PDE_PG_CC) >> K_PG_CC_SHIFT,
				(pgi & K_PDE_PG_D)  >> K_PG_D_SHIFT,
				(pgi & K_PDE_PG_VR) >> K_PG_VR_SHIFT,
				(pgi & K_PDE_PG_G)  >> K_PG_G_SHIFT);
			
			t = (k_ptr_t)((uint)t + (2 * K_REGSZ));
		}
	}
	else {
		 ...
		 * do a sd.
		 */
		pte = (k_ptr_t)((__psunsigned_t)tlbp + K_REGSZ);
		pgi = kl_get_pgi(pte);
		...
		if (PDE_SIZE == 8) {
			pgi >>= 32;
			fprintf(ofp, "%3d  %16llx  EntryLo0  %16llx  ", slot, vaddr, pgi);
			fprintf(ofp, "%8ld ", (int)kl_pte_to_pfn((k_ptr_t)&pgi));
		}
		else {
			fprintf(ofp, "%3d  %16llx  EntryLo0  %16llx  ", slot, vaddr, pgi);
			fprintf(ofp, "%8ld ", (int)kl_pte_to_pfn((k_ptr_t)pte));
		}
		if (DEBUG(DC_PAGE, 1)) {
			fprintf(ofp, "print_tlb: pgi=0x%llx (0x%x)\n", 
				pgi, kl_pte_to_pfn(pte));
		}
		 ...
		 */
		pte = (k_ptr_t)((__psunsigned_t)tlbp + K_REGSZ + PDE_SIZE);
		pgi = kl_get_pgi(pte);
		...
		if (PDE_SIZE == 8) {
			pgi >>= 32;
			fprintf(ofp, "                       EntryLo1  %16llx  ", pgi);
			fprintf(ofp, "%8ld ", (int)kl_pte_to_pfn((k_ptr_t)&pgi));
		}
		else {
			fprintf(ofp, "                       EntryLo1  %16llx  ", pgi);
			fprintf(ofp, "%8ld ", (int)kl_pte_to_pfn(pte));
		}
		fprintf(ofp, "(%lld %lld %lld %lld)\n", 
			(pgi & K_PDE_PG_CC) >> K_PG_CC_SHIFT,
			(pgi & K_PDE_PG_D) >> K_PG_D_SHIFT,
...
tlb_cmd(command_t cmd)
{
	...
	for (i = 0; i < cmd.nargs; i++) {
		...
		tlbp = (uint*) ((uint)tlbdumptbl + ((cpuid * K_TLBDUMPSIZE) + 8));
		for (j = 0, k = 0; j < K_NTLBENTRIES; j++, k += K_TLBENTRYSZ) {
	...
	if (!cmd.nargs) {
		for (i = 0; i < K_MAXCPUS; i++) {
			...
			fprintf (cmd.ofp, "\nTLB DUMP FOR CPU %d:\n\n", i);
			tlb_banner(cmd.ofp, BANNER|SMAJOR);
			tlbp = (uint*) ((uint)tlbdumptbl + ((i * K_TLBDUMPSIZE) + 8));
			for (j = 0, k = 0; j < K_NTLBENTRIES; j++, k += K_TLBENTRYSZ) {
				print_tlb(j, (k_ptr_t)((uint)tlbp + k), cmd.ofp);
			}
			tlb_banner(cmd.ofp, SMAJOR);
