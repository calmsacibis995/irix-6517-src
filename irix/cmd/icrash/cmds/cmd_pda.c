#ident "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash/cmds/RCS/cmd_pda.c,v 1.35 1999/05/25 19:21:38 tjm Exp $"
...
pda_cmd(command_t cmd)
{
	...
	if (!cmd.nargs) {
		...
		for (i = 0; i < cmd.nargs; i++) {
			kl_get_value(cmd.args[i], NULL, K_MAXCPUS, &value);
			if (KL_ERROR) {
				KL_ERROR |= KLE_BAD_PDA;
				kl_print_error();
			}
			else {
				pdaval = kl_get_pda_s(value, pdap);
				if (KL_ERROR) {
					kl_print_error();
				}
				else {
