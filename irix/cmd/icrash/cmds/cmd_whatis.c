#ident  "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash/cmds/RCS/cmd_whatis.c,v 1.17 1999/05/25 19:21:38 tjm Exp $"
...
whatis_cmd(command_t cmd)
{
	...
	if (cmd.flags & C_LIST) {
		...
		for (i = 0; i < cmd.nargs; i++) {
			sdp = kl_sym_lkup(STP, cmd.args[i]);
			if (KL_ERROR) {
				kl_print_error();
			}
			else {
			...
			if (np->flags & DWARF_TYPE_FLAG) {
				...
				if (t->t_link) {
					if  ((((t->t_tag == DW_TAG_structure_type) ||
						...
						t1 = (dw_type_info_t*)
							kl_alloc_block(sizeof(*t1), K_TEMP);
