#ident "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash/cmds/RCS/cmd_mbuf.c,v 1.21 1999/05/25 19:21:38 tjm Exp $"
...
#define GET_UINT(SYMBOL_STR, KPOINTER, VAR, DEFAULT_VALUE) { \
	KPOINTER = kl_sym_addr(SYMBOL_STR);\
	if(!KL_ERROR) { \
		kl_get_block(KPOINTER, 4, &VAR, SYMBOL_STR); \
	} \
	if(KL_ERROR) { \
		VAR = DEFAULT_VALUE; \
	} \
...
extern kaddr_t **find_all_vsockets(int ,FILE *, 
	...
	FILE *,list_of_ptrs_t *,int *), k_ptr_t **);

extern int test_vfile_mbufsocket(kaddr_t,kaddr_t caddr,char *fbuf, 
	int flags,int j, FILE *ofp,list_of_ptrs_t *list, int *first_time);
...
{
	int n = 0;
	k_ptr_t Pmbuf = kl_alloc_block(msize,K_TEMP);

	while (m) {
		kl_get_struct(m, msize, Pmbuf, "mbuf");
		if (KL_ERROR) {
			break;
		}
		n++;
		m = kl_kaddr(Pmbuf, "mbuf", "m_next");
	}
	kl_free_block(Pmbuf);
	return(n);
}
 ...
 */
int 
print_mbufs_ifq(kaddr_t KPifq, k_ptr_t Pifq, int flags, FILE *ofp,
	char *Pchar,int *firsttime) 
{
	...
	Pmbuf  = kl_alloc_block(mbufconst.m_msize, K_TEMP);
	strcpy(str, Pchar);
	len = strlen(Pchar);
#define LENGTH_TO_PRETTY_PRINT 9
	...
	KPmbuf = kl_kaddr(Pifq, "ifqueue", "ifq_head");
	i = 0;
	while(KPmbuf) {
		kl_get_struct(KPmbuf, mbufconst.m_msize, Pmbuf, "mbuf");
		if (KL_ERROR) {
			break;
		}
		i++;
		...
		KPmbuf = kl_kaddr(Pmbuf, "mbuf", "m_act");
	}
	kl_free_block(Pmbuf);
	return(i);
}
...
mbuf_cmd(command_t cmd)
{
	 ...
	 */
	if(cmd.nargs == 0 && kl_struct_len("mb")) {
		...
		Pmbcache = kl_alloc_block((mbnum * kl_struct_len("mb")),K_TEMP);
		KPvoid = kl_sym_addr("mbcache");
		if (!KL_ERROR) {
			kl_get_block(KPvoid, K_NBPW * mbnum, Pmbcache, "mbcache");
			if (KL_ERROR) {
				kl_free_block(Pmbcache);
				return(1);
			}
		}
		...
		Pmbstat = 
			(struct mbstat *)kl_alloc_block(sizeof (struct mbstat), K_TEMP);

		Pmb = (struct mbstat *)kl_alloc_block(kl_struct_len("mb"), K_TEMP);
		 ...
		 */
		for(i = 0; i < mbnum; i++) {
			int mb_size = kl_struct_len("mb");

			if(!((kaddr_t *)Pmbcache)[i]) {
				continue;
			}
			if (PTRSZ32) {
				kl_get_struct((kaddr_t)((__uint32_t *)Pmbcache)[i],
					mb_size, Pmb, "mb");
			}
			else {
				kl_get_struct((kaddr_t)((kaddr_t *)Pmbcache)[i],
					mb_size, Pmb,"mb");
			}
			if (KL_ERROR) {
				continue;
			...
			/* small mbufs.
			 */
			if(KPvoid = kl_kaddr(Pmb,"mb","mb_sml")) {
				smlfree = icrash_mbuf_count(KPvoid, mbufconst.m_msize);
				bytesfree += smlfree * mbufconst.m_msize;

				if(PLATFORM_SNXX) {
					if(!firsttime) {
						firsttime++;
						fprintf(cmd.ofp, "FREE MBUF LIST:\n");
						fprintf(cmd.ofp,
							"             MBUF  TYPE   NODE  COUNT\n");
						fprintf(cmd.ofp,
							"=========================================\n");
					}
					fprintf(cmd.ofp," %16llx  SMALL  %4d   %4d\n", 
						KPvoid,i,smlfree);
					sprintf(string, "Free small mbuf list for node %d", i);
				} 
				else if (mbnum == 1) {
					if(!firsttime) {
						firsttime++;
					...
					fprintf(cmd.ofp," %16llx  SMALL  %4d\n", KPvoid,smlfree);
					sprintf(string, "Free small mbuf list number");
				}
				else {
					if(!firsttime) {
						firsttime++;
						fprintf(cmd.ofp, "FREE MBUF LIST:\n");
						fprintf(cmd.ofp,
							"             MBUF  TYPE  MBNUM  COUNT\n");
						fprintf(cmd.ofp,
							"=========================================\n");
					}
					fprintf(cmd.ofp," %16llx  SMALL  %4d   %4d\n", 
						KPvoid,i,smlfree);
					sprintf(string, "Free small mbuf list number %d", i);
				}
					
				mbuf_queue = (mbuf_queue_t *)
					kl_alloc_block(sizeof(struct mbuf_queue), K_TEMP);
				
				mbuf_queue->title = (char *)
					kl_alloc_block((strlen(string)+1) * sizeof(char),K_TEMP);
			...
			/* medium mbufs.
			 */
			if(KPvoid = kl_kaddr(Pmb,"mb","mb_med")) {
				medfree = icrash_mbuf_count(KPvoid, mbufconst.m_msize);
				bytesfree += 
					medfree * (mbufconst.m_msize + mbufconst.m_mbufclsize);
				...
				if(PLATFORM_SNXX) {
					...
					fprintf(cmd.ofp," %16llx  MEDIUM %4d   %4d\n",
						KPvoid,i,medfree);
					sprintf(string, "Free medium mbuf list for node %d", i);
				} 
				else if (mbnum == 1) {
					if(!firsttime) {
						firsttime++;
					...
					fprintf(cmd.ofp," %16llx  MEDIUM %4d\n", KPvoid,medfree);
					sprintf(string, "Free medium mbuf list number");
				}
				else {
					if(!firsttime) {
						firsttime++;
					...
					fprintf(cmd.ofp," %16llx  MEDIUM %4d   %4d\n", 
						KPvoid,i,medfree);
					sprintf(string, "Free medium mbuf list number %d", i);
				}
				mbuf_queue = (mbuf_queue_t *)
					kl_alloc_block(sizeof(struct mbuf_queue), K_TEMP);
			...
			/* large mbufs.
			 */
			if(KPvoid = kl_kaddr(Pmb,"mb","mb_big")) {
				bigfree = icrash_mbuf_count(KPvoid, mbufconst.m_msize);
				bytesfree += 
					(bigfree * (mbufconst.m_msize + mbufconst.m_mclbytes));
				...
				if(PLATFORM_SNXX) {
					...
					fprintf(cmd.ofp," %16llx  LARGE  %4d   %4d\n",
						KPvoid, i, bigfree);
					sprintf(string, "Free large mbuf list for node %d", i);
				} 
				else if(mbnum == 1) {
					if(!firsttime) {
						firsttime++;
					...
					fprintf(cmd.ofp," %16llx  LARGE  %4d\n", KPvoid,bigfree);
					sprintf(string, "Free large mbuf list number");
				}
				else {
					if(!firsttime) {
						firsttime++;
				...
				mbuf_queue = (mbuf_queue_t *)
					kl_alloc_block(sizeof(struct mbuf_queue),
							K_TEMP);
			
				mbuf_queue->title = (char *)
					kl_alloc_block((strlen(string)+1)*
							sizeof(char),K_TEMP);
				strcpy(mbuf_queue->title,string);
				mbuf_queue->mbuf = KPvoid;
				ENQUEUE(&head_mbuf_queue,mbuf_queue);
			...
			Pmbstat->m_mbufs += 
				KL_UINT(Pmb,"mb","mb_smltot") + 
				KL_UINT(Pmb,"mb","mb_medtot") + 
				KL_UINT(Pmb,"mb","mb_bigtot");
			
			Pmbstat->m_pcbtot += KL_UINT(Pmb,"mb","mb_pcbtot");
			Pmbstat->m_pcbbytes += KL_UINT(Pmb,"mb","mb_pcbbytes");
			Pmbstat->m_mcbtot += KL_UINT(Pmb,"mb","mb_mcbtot");
			Pmbstat->m_mcbfail += KL_UINT(Pmb,"mb","mb_mcbfail");
			Pmbstat->m_mcbbytes += KL_UINT(Pmb,"mb","mb_mcbbytes");

			Pmbstat->m_drops += KL_UINT(Pmb,"mb","mb_drops");
			Pmbstat->m_wait  += KL_UINT(Pmb,"mb","mb_wait"); 

			for (j = MT_DATA; j < MT_MAX; j++) {
				Pmbstat->m_mtypes[j] += *(int *)
					((__psunsigned_t)
					 ADDR(Pmb,"mb","mb_types") + 
					 (__psunsigned_t)
					 (j * sizeof(int)));
			}
			
			Pmbstat->m_mtypes[MT_FREE] += smlfree + medfree + bigfree;
		}
		if(firsttime) {
			fprintf(cmd.ofp,
				"======="
		...
		for(i=0;i<VFILE_HASHSIZE;i++)
		{
			while(Pvsockets_hashbase[i])
			{
				...
				if(KPvoid=kl_kaddr(rcvp,"sockbuf","sb_mb"))
				{
					if(!firsttime)
					{
						firsttime++;
				...
				if(KPvoid = kl_kaddr(sndp,"sockbuf","sb_mb")) {
					if(!firsttime) {
						firsttime++;
						fprintf(cmd.ofp,
							"SOCKET MBUF LIST:\n");
		...
		kl_free_block(sp);
		kl_free_block(Pvsockets_hashbase);
		while(mbuf_queue = (mbuf_queue_t *) DEQUEUE(&head_mbuf_queue)) {

			/* We will find some use for these queued mbuf's later.
			 */
			kl_free_block(mbuf_queue->title);
			kl_free_block(mbuf_queue);
		}
		 ...
		 * from icrash too.
		 */
		if((ifqueue_size=kl_struct_len("ifqueue")) &&
		   ((KPvoid = kl_sym_addr("ipintrq")) > 0))
		{
			Pvoid  = kl_alloc_block(ifqueue_size,K_TEMP);
			
			if(kl_get_struct(KPvoid,ifqueue_size,
					  Pvoid,"ifqueue"))
			{
		 ...
		 */
		if((size=kl_struct_len("per_netproc")) &&
			   ((KPvoid = kl_sym_pointer("netproc_data")) > 0)) {
			Pvoid = kl_alloc_block(size,K_TEMP);

			next = kl_sym_addr("max_netprocs");
			if (!KL_ERROR) {
				kl_get_block(next, 4, &j, "max_netprocs");
			}
			else {
				j = K_NUMNODES;
			}
			for(i=0;i<j;i++) {
				 ...
				 */
				kl_get_kaddr(KPvoid, &KPipq, "per_netproc");
				if(!KL_ERROR && 
						kl_get_struct(KPipq, size, Pvoid, "per_netproc")) {
					...
					print_mbufs_ifq(KPipq, Pvoid, (SMAJOR|BANNER),
		 ...
		 */
		if((size = kl_struct_len("pernfsq")) &&
				   ((KPvoid = kl_sym_pointer("nfsq_table")) > 0)) {
			Pvoid = kl_alloc_block(size,K_TEMP);
			Pmbuf  = kl_alloc_block(mbufconst.m_msize, K_TEMP);
			...
			for(i = 0; i < j; i++) {
				 ...
				 */
				kl_get_kaddr(KPvoid,&KPipq,"pernfsq");
				if(!KL_ERROR && kl_get_struct(KPipq,size,Pvoid, "pernfsq")) {
		 ...
		 */
		if((size=kl_struct_len("ipfrag_bucket")) &&
				   ((KPvoid = kl_sym_addr("ipfrag_qs")) > 0)) {
			Pvoid  = kl_alloc_block(size,K_TEMP);
			Pipq   = kl_alloc_block(kl_struct_len("ipq"),K_TEMP);
			Pmbuf  = kl_alloc_block(mbufconst.m_msize,K_TEMP);
			if(PLATFORM_SNXX || PLATFORM_EVEREST) {
				frag_buckets = 16;
			}
			...
			for(i = 0; i < frag_buckets; i++) {
				if(!kl_get_struct(KPvoid,size,Pvoid, "ipfrag_bucket")) {
					continue;
				}
				KPipq = KPbucketipq = KPvoid + 
					kl_member_offset("ipfrag_bucket","ipq");
				Pipq_next = (k_ptr_t)((__psunsigned_t)Pvoid +
							  kl_member_offset("ipfrag_bucket", "ipq"));
				while((KPipq=kl_kaddr(Pipq_next,"ipq",
							  "next")) !=  KPbucketipq) {
					/*
					 ...
					 * the m_next ...
					 */
					if(KPmbuf = kl_kaddr(Pipq_next, "ipq", "ipq_mbuf")) {
						while(KPmbuf && kl_get_struct(KPmbuf,
							mbufconst.m_msize, Pmbuf,"mbuf")) {
							if(!firsttime) {
								firsttime++;
								fprintf(cmd.ofp, "IP FRAGMENTATION QUEUE\n");
								fprintf(cmd.ofp, "             MBUF COUNT\n");
								fprintf(cmd.ofp, "=======================\n");
							}
							fprintf(cmd.ofp,
								"0x%16llx  %4d"
					...
					if(!kl_get_struct(KPipq,kl_struct_len("ipq"), 
								Pipq,"ipq")) {
						break;
					}
					Pipq_next = Pipq;
		...
		Pmbstat->m_clusters += (i + j) / K_PAGESZ;
		Pmbstat->m_clfree = bytesfree/K_NBPC;
		...
		for (mp = mbtypes; mp->mt_name; mp++) {
			if (mp->mt_flag == MBUF) {
				if (Pmbstat->m_mtypes[mp->mt_type]) {
					...
					fprintf(cmd.ofp, "\t%u %s%s allocated\n",
						(int)Pmbstat->m_mtypes[mp->mt_type], mp->mt_name,
						mp->mt_func(Pmbstat->m_mtypes[mp->mt_type]));
					totmbufs += Pmbstat->m_mtypes[mp->mt_type];
				}
			}
		}
		...
		for (i = MT_FREE; i < nmbtypes; i++) {
			if (!seen[i] && Pmbstat->m_mtypes[i]) {
				fprintf(cmd.ofp, "\t%u <type %d> allocated\n",
		...
		fprintf(cmd.ofp, "%u total page%s allocated to networking data\n",
		...
		fprintf(cmd.ofp, "\t%u page%s of other networking data in use\n",
		...
		fprintf(cmd.ofp, "%u request%s for memory denied\n", 
			(unsigned)Pmbstat->m_drops,
			plural(Pmbstat->m_drops));
		fprintf(cmd.ofp, "%u request%s for memory delayed\n", 
			(unsigned)Pmbstat->m_wait,
			plural(Pmbstat->m_wait));
		fprintf(cmd.ofp, "%u call%s to protocol drain routines\n", 
		...
		*/
		kl_free_block(Pmbcache);
		kl_free_block(Pmbstat);
		kl_free_block(Pmb);

	}
	else if (cmd.nargs > 0) {
		m = kl_alloc_block(MBUF_SIZE, K_TEMP);
		...
		for (i = 0; i < cmd.nargs; i++) {
			...
			kl_get_struct(mbuf, MBUF_SIZE, m, "mbuf");
			if (KL_ERROR) {
				KL_ERROR |= KLE_BAD_MBUF;
				kl_print_error();
			}
				...
				mbuf_cnt++;
				if (cmd.flags & C_NEXT) {
					next = kl_kaddr(m, "mbuf", "m_next");
					while (next) {
						kl_get_struct(next, MBUF_SIZE, m, "mbuf");
						if (KL_ERROR) {
							if (DEBUG(DC_GLOBAL, 1)) {
								kl_print_debug("mbuf_cmd");
							}
							break;
						}
						if (cmd.flags & C_FULL) {
							fprintf(cmd.ofp, "\n");
							mbuf_banner(cmd.ofp, BANNER|SMINOR);
						}
						print_mbuf(next, m, cmd.flags, cmd.ofp);
						mbuf_cnt++;
						next = kl_kaddr(m, "mbuf", "m_next");
					}
				}
