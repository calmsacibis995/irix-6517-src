#ident "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash/cmds/RCS/cmds.c,v 1.108 1999/11/30 21:11:41 lucc Exp $"
#include <sys/types.h>
#include <signal.h>
#include <setjmp.h>
#include <stdio.h>
...
struct _commands cmdset[] = {
	...
	{ "gfxinfo", 0, gfxinfo_cmd, gfxinfo_parse, gfxinfo_help, gfxinfo_usage },
	{ "h", "history" },
	...
	while (cmd->args[i]) {
		...
		if ((cmd->args[i][0] == '-') && (strlen(cmd->args[i]) > 1)) {
			switch (cmd->args[i][1]) {
				...
				case 'o' :
					if (flags & C_OCTAL) { cmd->flags |= C_OCTAL;
					    break;
					}
					return (0);
				...
				case 'u' :
					...
					if (i == (cmd->nargs - 1)) {
						 ...
						 *       do a kl_free_block() here.
						 */
						kl_free_block((k_ptr_t)cmd->args[i+1]);
						extra = 1;
						break;
					}
					return (0);
			...
			kl_free_block((k_ptr_t)cmd->args[i]);
...
get_cmdline()
{
	...
	cmdline = (char *)kl_alloc_block(256, K_PERM);
...
get_cmd(command_t *c, char *s)
{
	...
	*cp++ = 0;
	...
	while (*cp != NULL) {
		...
		c->args[i] = kl_alloc_block(len + 1, K_TEMP);
...
cat_cmd_args(command_t cmd)
{
	...
	cmdstr = kl_alloc_block(1024, K_TEMP);
...
run_var_cmd(command_t *cmd, variable_t *vp, FILE *ofp)
{
	...
	cmdline = (char *)kl_alloc_block(strlen(vp->v_exp) + 1, K_TEMP);
