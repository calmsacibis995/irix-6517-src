#ident  "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash/lib/libeval/RCS/eval.c,v 1.36 1999/05/25 19:21:38 tjm Exp $"
...
base_typestr(type_t *tp, int flags)
{
	...
	tsp = (char *)kl_alloc_block(len, flags);
	c = tsp;
	*c++ = '(';
	if (btp->actual_name) {
...
get_type(char *s, int flags)
{
	...
	if (!strncmp(s, "struct", 6)) {
		...
		if (symtable_type == DWARF_TYPE_FLAG) {
			if (!(dtp = dw_lkup(typename, DW_TAG_structure_type))) {
				return ((type_t *)NULL);
			}
		...
		typename[len] = 0;
		if (symtable_type == DWARF_TYPE_FLAG) {
			if (!(dtp = dw_lkup(typename, DW_TAG_union_type))) {
				return ((type_t *)NULL);
			}
			type = DWARF_TYPE_FLAG;
		} 
		...
		typename[len] = 0;
		if (symtable_type == DWARF_TYPE_FLAG) {
			if (!(dtp = dw_lkup(typename, DW_TAG_typedef))) {
				return ((type_t *)NULL);
			}
			type = DWARF_TYPE_FLAG;
		} 
	...
	switch (t->flag) {
		...
		case DWARF_TYPE_FLAG :
			dti = (dw_type_info_t *)kl_alloc_block(sizeof(*dti), flags);
		...
		if (t0->flag == POINTER_FLAG) {
			...
			if (is_temp_block(t0->t_ptr)) {
				if (t0->flag == DWARF_TYPE_FLAG) {
					...
					kl_free_block((k_ptr_t)t0->t_ptr);
				}
			}
			kl_free_block((k_ptr_t)t0);
			t0 = (type_t *)NULL;
		...
		switch (dp->d_tag) {
			...
			case DW_TAG_structure_type :
				...
				if (!dp->d_sibling_off) {
					...
					if (!(dtp = dw_lkup(dp->d_name, DW_TAG_structure_type))) {
			...
			case DW_TAG_union_type :
				...
				if (!dp->d_sibling_off) {
					...
					if (!(dtp = dw_lkup(dp->d_name, DW_TAG_union_type))) {
...
is_keyword(char *c, int len)
{
	...
	if (!strncmp(c, "struct", len)) {
		...
		c1 = (char*)kl_alloc_block(len+1, K_TEMP);
		strncat(c1, c, len);
		c1[len] = 0;
		if (dw_lkup(c1, DW_TAG_typedef)) {
			kl_free_block((k_ptr_t)c1);
			return(1);
		}
		kl_free_block((k_ptr_t)c1);
		...
		switch(*cp) {
			...
			case '(' : {
				...
				if (s = strpbrk(cp+1, " *)")) {
					...
					if (is_keyword((char*)((uint)(cp+1)), len)) {
						...
						len = (uint)s - (uint)(cp+1);
						tok->string = (char *)kl_alloc_block(len + 1, K_TEMP);
			...
			case '$' : {
				...
				if (vp->v_flags & V_TYPEDEF) {
					...
					tok->string = 
						(char *)kl_alloc_block(strlen(vp->v_exp) + 1, K_TEMP);
			...
			default: {
				...
				if (!strcmp(tok->string, "sizeof")) {
					tok->operator = SIZEOF;
					kl_free_block((k_ptr_t)tok->string);
					tok->string = 0;
				}
...
make_node(token_t *t, int flags)
{
	...
	if (t->type == OPERATOR) {
		...
		np->name = (char *)dup_block((k_ptr_t)t->string, K_TEMP);
		np->node_type = MEMBER;
	}
	else if (t->type == STRING) {
		if ((sp = kl_get_sym(t->string, K_TEMP)) && !(flags & C_NOVARS)) {
			 ...
			 */
			np->type = kl_alloc_block(sizeof(type_t), flags);
			np->type->flag = POINTER_FLAG;
			np->type->t_ptr = get_type("long", flags);
			np->node_type = TYPE_DEF;
			np->flags |= POINTER_FLAG;
			kl_get_block(np->address, K_NBPW, &np->value, "pointer value");
			if (!PTRSZ64) {
				np->value >>= 32;
			}
		}
			 ...
			 */
			dtp = dw_lkup(t->string, DW_TAG_structure_type);
			if (!dtp) { 
				/* Check to see if this is a union name
				 */
				dtp = dw_lkup(t->string, DW_TAG_union_type);
			}
			if (!dtp) {
				/* Check to see if this is a typedef name
				 */
				dtp = dw_lkup(t->string, DW_TAG_typedef);
			}
			if (dtp) {
				...
				dti = 
					(dw_type_info_t*)kl_alloc_block(sizeof(*dti), flags);

				dw_type_info(dtp->dt_off, dti, flags);

				np->node_type = TYPE_DEF;
				np->flags = DWARF_TYPE_FLAG;
				np->type = (type_t*)kl_alloc_block(sizeof(type_t), K_TEMP);
 ...
 *  allocated as K_TEMP or K_PERM depending on the contents of flags.
 *
 */
char *
node_to_typestr(node_t *np, int flags)
{
	...
	if (np->flags & DWARF_TYPE_FLAG) {
		 ...
		 */
		tsp = (char *)kl_alloc_block(6, flags);
		strcpy(tsp, "(int)");
	}
	return(tsp);
}
...
free_tokens(token_t *tp)
{
	...
	while (t) {
		tnext = t->next;
		if (t->string) {
			kl_free_block((k_ptr_t)t->string);
		}
		...
		if (n0->node_type == OPERATOR) {
			if (is_unary(n0->operator)) {
				...
				if (replace_unary(n0, error, flags) == -1) {
					if (n0->right) {
						kl_free_block((k_ptr_t)n0->right);
			...
			if (n0->type->flag != DWARF_TYPE_FLAG) {
				*error = E_BAD_INDEX;
				...
				if (dp->t_tag == DW_TAG_base_type) {
					size = dp->t_byte_size;
					kl_get_block(n0->address, size, &index, "index");
					if (shft = (8 - size)) {
						index = index >> (shft * 8);
					}
					n0->value = index;
	...
	if (n->operator == CAST) {
		...
		if (tp->flag == POINTER_FLAG) {
			...
			if (n->type->flag == POINTER_FLAG) {
				...
				kl_get_block(addr, K_NBPW, &n->value, "pointer value");
				if (!PTRSZ64) {
					n->value >>= 32;
				}
	...
	if (np->node_type == OPERATOR) {
		...
		if ((np->operator == RIGHT_ARROW) || (np->operator == DOT)) {
			...
			if (np->left->type->flag == POINTER_FLAG) {
				...
				kl_free_block((k_ptr_t)np->left->type);
			}
			else {
				tp = np->left->type;
			}
			...
			switch (tp->flag) {
				...
				case DWARF_TYPE_FLAG: {
					 ...
					 */
					kl_free_block((k_ptr_t)mp);
					
					if (np->type->flag == POINTER_FLAG) {
						np->flags |= POINTER_FLAG;
						kl_get_block(addr, K_NBPW, 
		...
		np->operator = 0;
		np->value = value;
		kl_free_block((k_ptr_t)np->left);
		kl_free_block((k_ptr_t)np->right);
		np->left = np->right = (node_t*)NULL;
	}
	return(np);
}
...
replace_variable(node_t *n, int *error, int flags)
{
	 ...
	 */
	if (!STP) { /* XXX - what is this for?!? */

		/* Lookup the variable type by name
		 */
		if (!(dtp = dw_lkup(n->name, DW_TAG_variable))) {
		...
		if (n->type->flag == POINTER_FLAG) {
			n->flags = POINTER_FLAG;
			kl_get_block(n->address, 8, &n->value, "pointer value (DWARF)");
		}
		else {
			n->value = n->address;
		}
...
type_to_number(node_t *np, int *error) 
{
	...
	kl_get_block(np->address, byte_size, &value1, "value");
...
eval(char **exp, int flags, int *error)
{
	...
	*error = 0;
	...
	if (*error) {
		n = (node_t *)kl_alloc_block(sizeof(node_t), flags);
		n->tok_ptr = tok->ptr;
	}
	else {
		t = tok;
	...
	switch (np->node_type) {
		...
		case TYPE_DEF: {
			...
			if (np->flags & DWARF_TYPE_FLAG) {
				...
				addr = np->value;
				kl_get_block(addr, size, ptr, "type");
				if (KL_ERROR) {
					kl_print_error();
					kl_free_block(ptr);
					return(1);
