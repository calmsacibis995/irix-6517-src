     ...
     int ignore, invoking_key;
{
  ...
  else if (rl_last_func != rl_complete)
...
maybe_unsave_line ()
{
  ...
  } else if (!saved_line_for_history) {
    ding ();
  }
  return 1;
}
...
void rl_variable_bind(char *, char *);
void rl_parse_and_bind(char *);
void rl_generic_bind (int, char *, char *, Keymap);
     ...
     char *statement;
{
  ...
  for (i = 0; parser_directives[i].name; i++)
    if (stricmp (directive, parser_directives[i].name) == 0)
      {
	  (void)(*parser_directives[i].function) (args);
	return (0);
      }
