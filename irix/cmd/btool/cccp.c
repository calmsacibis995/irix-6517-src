...
/* BRANCH */
/* no_btool_directives controls the output of #start_macro and #end_macro
   of btool. Lines of code between these directives won't be instrumented.
   Nonzero of no_btool_directives means the directives are not printed
   ( macros are instrumented ) */


int no_btool_directives;
/* BRANCH END */
  ...
  for (i = 1; i < argc; i++) {
	...
	if (!strcmp (argv[i], "-traditional")) {
	  ...
	  no_trigraphs = 0;
          ...
          /* BRANCH 
             option -test-macro */
          else if (!strcmp (argv[i], "-test-macro")) {
          no_btool_directives = 1;
	}
          /* BRANCH END */
	break;
...
/* BRANCH */
int Start_count = 0; /* Number of opportunities to issue #btool_start directive
                        Prevents inserting of directive for nested macros, 
                        since only for the first it is actually issued. */

char Macroline_correction[500]; /* string to buffer the #line directive */
...
do { ip->macro->type = T_MACRO;		\
     if (ip->free_ptr) free (ip->free_ptr);	\
      --indepth; } while (0);\
  /* BRANCH */ \
  /* After expansion, put out an end marker. */	\
  if (!no_btool_directives) { \
     if (Start_count == 1) \
     { \
       op->bufp = obp;\
       check_expand (op, strlen("\n#end_btool\n")); \
       obp = op->bufp; \
       strcpy(obp, "\n#end_btool\n"); \
       obp+=strlen("\n#end_btool\n"); \
       sprintf (Macroline_correction, "#line %d \n", op->lineno); \
       op->bufp = obp; \
       check_expand (op, strlen(Macroline_correction));\
       obp = op->bufp; \
       bcopy ( Macroline_correction, obp, strlen(Macroline_correction)); \
       obp += strlen(Macroline_correction); \
     } \
     Start_count--; }
  /* BRANCH END */
     ...
     int length;
{
  ...
  /* BRANCH - don't place instrumentation inhibition markers
     within the evaluation of an #if or #elseif expression. */
  int old_no_btool_directives = no_btool_directives;
  no_btool_directives = 1;
  ...
  no_btool_directives = old_no_btool_directives;	/* BRANCH */
  ...
  while (1) {
	...
	*eofp = 1;
	...
	++*count_newlines;
      ...
      if (match == '\'') {
#ifdef sgi
	/* not strictly correct but makes things like
	#if NEVER
		WON'T WORK
	#endif
	 * work correctly
...
/* BRANCH:  We make sure OUTPUT_LINE_COMMANDS is defined, because that's
   what K&R2 use. */
#define OUTPUT_LINE_COMMANDS
...
#if 0
  /* BRANCH:  We suppress this token because older versions of GCC choke on
     it. */
     ...
     FILE_BUF *op;
{
  ...
  /* BRANCH */
  /* Before expansion, put out a start marker. */
  /* end marker is set in macro POPMACRO. */
  /*
   * BEM:  Add #line, now that macros containing complete functions have
   * embedded branches instrumented.
   */
  ...
  if (!no_btool_directives) {
	...
	sprintf (Macroline_correction, "#line %d \n", op->lineno);
	check_expand (op, strlen(Macroline_correction));
	strcpy(op->bufp, Macroline_correction);
	op->bufp +=strlen(Macroline_correction);
      }
  }
  /* BRANCH END */
