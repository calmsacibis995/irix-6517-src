...
extern int Name_in_control;
extern int Function_in_macro;
extern int Inside_macro;
extern char *input_filename;
struct procedure_list
  {
    char *procedure_name;
    struct procedure_list *next;
  };

struct ctrlfile_list
  {
    int ino;
    char *filename;
    struct ctrlfile_list *next;
    struct procedure_list *proc;
  } ;

static struct ctrlfile_list *ctrlfile_first;
...
/*
  ...
  PURPOSE:
  initialize the list of files and procedures in the controlfile
  
  PRECONDITIONS:
  1. controlfile readable
  
  POSTCONDITIONS:
  1. linked list is stored in static global var *ctrlfile_first
  
  ERRORS:
  1. can't open controlfile
  2. file not existant (no inode for file)
  
  INTERNAL ERRORS:
  1. filename or procedurename longer than 1024 char.
  ...
  init_list ()
{
  ...
  int stat(), i;
  char *full_ctrlfile_name, *ctrl_string, *ctrl_name, c, *full_filename;
  FILE *ctrlfile_p;
  struct ctrlfile_list *ctrlfile_check, *ctrlfile_temp;
  struct procedure_list *proc_temp, *proc_hold;
  struct stat statbuf;
  extern char *Controlfile_name, *Dir_name;
  
  if (!init)
    {
      /* initialize list */
      
      full_ctrlfile_name = (char *) xmalloc 
	(strlen(Dir_name)+1+strlen(Controlfile_name) + 1);
	...
	{
#ifdef sgi
	  /* permit missing ctrl file to mean everything */
	  return;
#else
	  fatal ("Can't open controlfile %s \n",full_ctrlfile_name);
#endif
	}
      ...
      while ( ( c= getc(ctrlfile_p) )!= EOF )
	{
	  ...
	  if ( (c != '#') && (c != '\n') && (c!= EOF))
		...
		{
		  /* let's prepare space for the file entry*/
		  
		  ctrlfile_temp = (struct ctrlfile_list *) 
		    xmalloc (sizeof (struct ctrlfile_list));
		  ctrlfile_temp->filename = (char *) xmalloc 
		    (strlen(ctrl_name)+1);
		  strcpy(ctrlfile_temp->filename, ctrl_name);
		  ctrlfile_temp->ino = statbuf.st_ino;
		  ctrlfile_temp->next = NULL;
		  ctrlfile_temp->proc = NULL;
		      ...
		      while ( (c=getc(ctrlfile_p)) != EOF)
			{
			  ...
			  *ctrl_string++=c;
			  ...
			  if (i==(PATH_BUF_LEN-1))
			    {
			      *ctrl_string = '\0';
			      fatal("function name %s... too long \n", 
				    ctrl_name);
			    }
			}
		      *ctrl_string = '\0';
		      
		      proc_temp = (struct procedure_list *) 
			xmalloc (sizeof (struct procedure_list));
		      proc_temp->procedure_name = (char *) 
			xmalloc (strlen(ctrl_name)+1);
		      strcpy(proc_temp->procedure_name, ctrl_name);
		      proc_temp->next = NULL;
		      
		      /* add it to the main member */
		      if (ctrlfile_temp->proc == NULL)
			{
			  ctrlfile_temp->proc = proc_temp;
			  proc_hold = proc_temp;
			}
		      else
			{
			  proc_hold->next=proc_temp;
			  proc_hold = proc_temp;
			}
		    }
...
/*
  NAME:
  char *ctrlfile_name (filename)
  char *filename;
  
  PURPOSE:
  gives back the full filename (how the file was named in the controlfile)
  
  PRECONDITIONS:
  1. filename is existant
  2. controlfile_list is either existant or it can be created with
  init_list
  
  POSTCONDITIONS:
  The full filename will be returned. The list is unchanged.
  
  ERRORS:
  1. the filename has no inode
  2. if the list was not initialized, then the same errors of init_list
  */
char 
  *ctrlfile_name (filename)
char *filename;

{
  struct ctrlfile_list *ctrlfile_check;
  struct stat statbuf;
  ...
  while (ctrlfile_check != NULL)
      ...
      if (statbuf.st_ino == ctrlfile_check->ino)
	{
	  ...
	  return (ctrlfile_check->filename);
	  break;
	}
      else
	/* we keep on testing */
  ...
  /* not found */
  return NULL;
} /* end of ctrlfile_name */


/*
  NAME:
  char *ctrlfile_proc (filename,proc_name)
  char *filename, *proc_name;
  
  PURPOSE:
  returns 0 if the procedure was named after the filename in the 
  controlfile, 0 if the filename was not followed by procedure
  names at all, 1 otherwise.  (Inverted return values for historical 
  reasons.)
...
int 
  ctrlfile_proc (filename, proc_name)
char *filename, *proc_name;

{
  struct ctrlfile_list *ctrlfile_check;
  struct procedure_list *proc_check;
  struct stat statbuf;
  ...
  while (ctrlfile_check != NULL)
      ...
      if (statbuf.st_ino == ctrlfile_check->ino)
	{
	  ...
	  if (ctrlfile_check->proc == NULL)
	    ...
	    {
	      /* test here all functionnames */
	      proc_check = ctrlfile_check->proc;
	      
	      while (proc_check != NULL)
		{
		  if (!strcmp(proc_name, proc_check->procedure_name))
		    {
		      return 0;
		      break;
		    }
		  else
		    proc_check = proc_check->next;
		}
	      /* we checked all names but proc_name was not there */
	      return 1;
	      break;
	    }
	}
      else
	/* the file is not yet found */
	ctrlfile_check = ctrlfile_check->next;
    } 
  
  /* not found */
  return 1;
} /* end of ctrlfile_proc */
     ...
     tree declarator, declspecs;
{
  ...
  /*BRANCH */
  Name_in_control = ! ctrlfile_proc(input_filename,
				   DECL_PRINT_NAME(current_function_decl));
  Function_in_macro = Inside_macro;
