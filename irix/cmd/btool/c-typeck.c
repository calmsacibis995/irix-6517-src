     ...
     tree type1, type2;
{
  ...
  switch (TREE_CODE (t1))
	...
	if ((flag_signed_char == 0) &&
		((t1 == char_type_node && t2 == unsigned_char_type_node) ||
		 (t2 == char_type_node && t1 == unsigned_char_type_node)))
		 return 1;
	break;
#endif
    }
     ...
     tree exp;
{
  ...
  extern int should_instrument();	/* BRANCH */
...
/*BRANCH */
      if ((type == long_long_integer_type_node) && should_instrument()) {
	fatal ("btool: long long type not allowed in switch statement");
      }
