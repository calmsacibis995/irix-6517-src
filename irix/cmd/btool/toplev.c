...
char *Countfile_name = "btool_map.count";
char *Controlfile_name = "btool_ctrl";
char *Dir_name = ".";
int Test_dir = 0;
int Test_map = 0;
int Test_count = 0;
int Test_control =0;
int Btool_instr_quest=1;
...
/* Branch:  When the following code is included, system("grep...") fails with
   crt0: /usr/lib/ld.so mapping failure
   SunOS bug? */ 

#ifdef RLIMIT_STACK_BRANCH_REMOVED
  ...
  signal (SIGINT, interrupt_signal);	/* BRANCH */
  ...
  for (i = 1; i < argc; i++)
	...
	if (str[0] == 'Y')
	  str++;
/*BRANCH */
        if (!strcmp (str, "test-count"))
	    {
               Test_count =1;
               Countfile_name = argv[++i];
	     }
        else if (!strcmp (str, "test-map"))
	    {
               Test_map =1;
               Mapfile_name = argv[++i];
	     }
        else if (!strcmp (str, "test-control"))
	    {
               Test_control =1;
               Controlfile_name = argv[++i];
	     }
        else if (!strcmp (str, "test-dir"))
	  {
               Test_dir =1;
               Dir_name = argv[++i];
	     }
	else if (!strcmp (str, "test-quest-bug"))
	  {
	      Btool_instr_quest=0;
	    }
/*END BRANCH */
