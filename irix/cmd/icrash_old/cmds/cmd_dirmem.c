...
has_dirmem()
{
    ...
    if (!kl_get_block(K, addr_lo,   8, dirmem_value_lo,   "dirmem"))
	return(0);			/* directory memory not found */
    if (!kl_get_block(K, addr_hi,   8, dirmem_value_hi,   "dirmem"))
	return(0);			/* directory memory not found */
...
int dirmem_cmd(command_t cmd)
{
	...
	dirmem_disabled = 0;
        ...
        else if (K_DUMP_HDR(K)->dmp_version < 4) {
    ...
    if (!KL_ERROR) {
        ...
        get_value(opt_addr, &mode, 0, &start_addr);
        if (KL_ERROR) {
            kl_print_error(K);
            return(1);
        }
	...
	mem  = 1;
	prot = 1;
	ecc  = 0;	/* change to 1 if/when ecc dumping is implemented */
    }
     if (region == 1)
	 prot = 1;
	...
	for (i=reg_low; i<=reg_hi; ++i) {
	    ...
	    P1("--- Region %d prot: ", i);
            ...
            if (!kl_get_block(K, addr_prot, 8, dirmem_value_prot, "dirmem")) {
		...
		if (value_prot == DUMP_HUB_INIT_PATTERN) {
			 ...
			 */
    if (ecc) {
        if (!kl_get_block(K, addr_ecc, 8, dirmem_value_ecc, "dirmem")) {
	    P0("\n");
