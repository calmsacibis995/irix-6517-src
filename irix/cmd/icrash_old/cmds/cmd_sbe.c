...
*                        since reboot.
*/
#ident "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash_old/cmds/RCS/cmd_sbe.c,v 1.1 1999/05/25 19:19:20 tjm Exp $"
#ifndef _KERNEL
#define _KERNEL 1
#define _K64U64 1
...
challenge_sbe_cmd(int flags, FILE *ofp)
{
	...
	if ((tsym = get_sym("sbe_log_errors", B_TEMP)) == (struct syment *)NULL) {
		fprintf(ofp,
	...
	/* Get evconfig 
	 */
	ecbuf = (evcfginfo_t *)alloc_block(sizeof(evcfginfo_t), B_TEMP);
	if (!kl_get_block(K, (kaddr_t)EVCFGINFO, 
							sizeof(evcfginfo_t), ecbuf, "evcfg")) {
		fprintf(ofp, "sbe: could not read evconfiginfo structure\n");
		free_block((k_ptr_t)ecbuf);
		free_sym(tsym);
		return 1;
	}
	free_sym(tsym);

	if (!(tsym = get_sym("mc3_errcount", B_TEMP))) {
		fprintf(ofp,
			"sbe: could not retrieve mc3 bank/simm error count symbol\n");
		free_block((k_ptr_t)ecbuf);
		return (-1);
	}
	if (!(kl_get_block(K, tsym->n_value, sizeof(mc3_errcount),
...
origin_sbe_cmd(int flags, FILE *ofp)
{
	...
	for (i = 0; i < K_NUMNODES(K); i++) {
		...
		sbeinfoval = kl_kaddr(K, npdap, "nodepda_s", "sbe_info");
		if (sbeinfoval) {
			sbebuf = (sbe_info_t *)alloc_block(sizeof(sbe_info_t), B_TEMP);
			if (!kl_get_block(K, sbeinfoval, sizeof(sbe_info_t),
				sbebuf, "sbe_info_t")) {
					continue;
			}
