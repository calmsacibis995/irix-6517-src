#ident "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash_old/cmds/RCS/cmd_set.c,v 1.1 1999/05/25 19:19:20 tjm Exp $"
#include <stdio.h>
#include <sys/types.h>
#include "icrash.h"
#include "eval.h"
...
set_cmd(command_t cmd)
{
	...
	if (!cmd.nargs) {
		...
		cmdstr = (char *)alloc_block(len + 5, B_TEMP);
		strcat(cmdstr, "set ");
		...
		*eqp++ = 0;
		 ...
		 * get expanded.
		 */
		exp = (char *)alloc_block(strlen(eqp) + 1, B_TEMP);
		strcpy(exp, eqp);
		...
		if (*c == '\"') {
			 ...
			 * was allocated by alloc_block(). Otherwise the attempt 
