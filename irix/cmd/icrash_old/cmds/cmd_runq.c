#ident "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash_old/cmds/RCS/cmd_runq.c,v 1.1 1999/05/25 19:19:20 tjm Exp $"
...
{                                                                        \
									 \
                                                                         \
	if(IS_FIELD("pda_s",FIELDNAME))                                  \
	{                                                                \
		KPkthread = kl_kaddr(K,Ppda,"pda_s",FIELDNAME);          \
		Pkthread = kl_get_kthread(K,KPkthread,flags);            \
		fprintf(ofp," " ## PRINTFORMAT ## "  %4lld",            \
			    KPkthread,Pkthread ? KL_INT(K,Pkthread,	 \
	                               "kthread", "k_pri") : 0);         \
		if(Pkthread)                                             \
		{   					                 \
			K_BLOCK_FREE(K)(NULL,Pkthread); 		 \
		}						         \
	}                                                                \
}
...
{                                                                        \
	...
	{                                                                \
		...
		{                                                        \
			...
			indent_it(flags,ofp);                            \
		        fprintf(ofp,                                     \
				"%s=" ## PRINTFORMAT ## ", K_PRI=%4lld",\
				PRINTFNAME,KPkthread,                    \
				Pkthread ? KL_INT(K,Pkthread,            \
					     "kthread", "k_pri") : 0);   \
			fprintf(ofp,"\n");                               \
...
print_runq_pda_full(kaddr_t KPpda, k_ptr_t Ppda, int flags, FILE *ofp)
{
	...
	Pcpu_s = (k_ptr_t)((__psunsigned_t)Ppda + 
			   (__psunsigned_t)FIELD("pda_s","p_cpu"));
	...
	indent_it(flags,ofp);
	if(IS_FIELD("cpu_s","c_onq"))
	{
		fprintf(ofp,"C_ONQ=%lld, ",
			KL_UINT(K,Pcpu_s,"cpu_s","c_onq"));
	}
		
	if(IS_FIELD("cpu_s","c_restricted"))
	{
		fprintf(ofp,"C_RESTRICTED=%lld, ",
			KL_UINT(K,Pcpu_s,"cpu_s","c_restricted"));
	}

	if(IS_FIELD("cpu_s","c_peekrotor"))
	{
		fprintf(ofp,"C_PEEKROTOR=0x%llx, ",
			KL_INT(K,Pcpu_s,"cpu_s","c_peekrotor"));
	}
	 ...
	 */
	PRINT_FIELD_KTHREAD_PDA("CURKTHREAD","p_curkthread",
				"%llx");
			
	PRINT_FIELD_KTHREAD_PDA("NEXTTHREAD","p_nextthread",
				"%llx");
			 
	if(!IS_FIELD("cpu_s","c_threads"))
		goto error;

	KPc_threads = KPkthread = kl_kaddr(K,Pcpu_s,"cpu_s",
					   "c_threads");
	...
	if(KPkthread) 
	{
		...
		if(Pkthread)
		{
			...
			KPrflink = kl_kaddr(K,Pkthread,"kthread",
					    "k_rflink");
			K_BLOCK_FREE(K)(NULL,Pkthread);
		}
		 ...
		 */
		i=0;
		while (KPrflink && KPrflink != KPkthread && 
		       KPrflink != KPc_threads)
		{
			...
			if(Pkthread)
			{
				...
				{                                 
					...
					kthread_banner(ofp,
						       SMINOR|BANNER|
						       C_INDENT);
					print_kthread(KPkthread,
						      Pkthread,
						      (flags|C_INDENT)&
						      ~C_NEXT,
						      ofp);
					kthread_banner(ofp,SMINOR|
						       C_INDENT);    
				}
				KPrflink = kl_kaddr(K,Pkthread,
						    "kthread",
						    "k_rflink");
				K_BLOCK_FREE(K)(NULL,Pkthread);
			}
...
print_runq_pda_brief(kaddr_t KPpda, k_ptr_t Ppda, int flags, FILE *ofp)
{
	   ...
	   (Pkthread = kl_get_kthread(K,KPkthread,flags))) 
	{
		       ...
		       KPrflink != KPc_threads)
		{
			KPkthread = KPrflink;
			if(Pkthread = 
			   kl_get_kthread(K,KPkthread,flags))
			{
				KPrflink = kl_kaddr(K,Pkthread,
						    "kthread",
						    "k_rflink");
...
print_runq_rt_gq(int flags,FILE *ofp)
{
	...
	KPrt_gq = KPkthread = kl_kaddr_to_ptr(K,kl_sym_addr(K,"rt_gq"));
	...
	while(KPkthread && (!num_threads || KPkthread != KPrt_gq))
	{
		...
		if(Pkthread) 
		{
			...
			KPkthread = kl_kaddr(K,Pkthread,"kthread", "k_rflink");
			K_BLOCK_FREE(K)(NULL,Pkthread);
			num_threads ++;
		}
		else
...
runq_cmd(command_t cmd)
{
	...
	if (cmd.nargs == 0) 
	{
		...
		Ppda = alloc_block(PDA_S_SIZE(K), B_TEMP);

		fprintf(cmd.ofp,"\nCPU SCHEDULER INFORMATION:");
		for(cpu=0;cpu<K_NUMCPUS(K);cpu++) 
		{
			KPpda = kl_get_pda_s(K, (kaddr_t)cpu, Ppda);
			if (KL_ERROR) 
			{
				continue;
			}
