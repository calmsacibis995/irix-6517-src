...
walk_cmd(command_t cmd)
{
	...
	if (STRUCT(cmd.args[0]) == 0) {
		 ...
		 */
		byte_size = value;
		if (byte_size % K_NBPW(K)) {
			byte_size += (K_NBPW(K) - (byte_size % K_NBPW(K)));
		}
		size = (byte_size / K_NBPW(K));
		...
		while (next_addr) {
			 ...
			 */
			kl_is_valid_kaddr(K, next_addr, (k_ptr_t)NULL, WORD_ALIGN_FLAG);
			if (KL_ERROR) {
				break;
			}
			 ...
			 */
			last_addr = next_addr;
			if (!kl_get_kaddr(K, (next_addr + offset), 
						(k_ptr_t)&next_addr, "addr") || KL_ERROR) {
				break;
			}
