#ident  "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash_old/cmds/RCS/cmd_stat.c,v 1.1 1999/05/25 19:19:20 tjm Exp $"
#define _KERNEL  1
#include <sys/types.h>
#include <sys/param.h>
#include <stdio.h>
...
stat_cmd(command_t cmd)
{
	...
	kl_get_block(K, K_LBOLT(K), 4, &lbolt, "lbolt");
	...
	if (K_ERROR_DUMPBUF(K)) {
		fprintf(cmd.ofp, "\n\nERROR DUMPBUF:\n\n");
		fprintf(cmd.ofp, "0x%llx = \"", K_ERROR_DUMPBUF(K));
		dump_string(K_ERROR_DUMPBUF(K), cmd.flags, cmd.ofp);
		fprintf(cmd.ofp, "\"\n");
	}
	fprintf(cmd.ofp, "\n");
	return(0);
