#ident "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash_old/cmds/RCS/cmd_utrace.c,v 1.1 1999/05/25 19:19:20 tjm Exp $"
...
utrace_init(tracelist_t *elt, k_ptr_t pdap)
{
	...
	entrylen = kl_struct_len(K, "tstamp_event_entry");
	elt->entries = kl_kaddr(K, pdap, "pda_s", "p_tstamp_entries");
	elt->nentries = (kl_kaddr(K, pdap, "pda_s", "p_tstamp_last") -elt->entries)
		/ entrylen + 1;
	elt->entry = alloc_block(entrylen, B_TEMP);

	/* The index is in one of two places, depending on the eobmode */
	eobmode = KL_UINT(K, pdap, "pda_s", "p_tstamp_eobmode");
	if (eobmode == RT_TSTAMP_EOB_WRAP) {
		elt->index = (kl_kaddr(K, pdap, "pda_s", "p_tstamp_ptr") -elt->entries)
			/ entrylen;
	...
	p = kl_kaddr(K, pdap, "pda_s", "p_tstamp_objp");
	p = kl_kaddr_to_ptr(K, p + 
						kl_member_offset(K, "tstamp_obj_t", "shared_state"));
	b = kl_get_struct(K, p, 0, NULL, "tstamp_shared_state");
	if (KL_ERROR) {
		kl_print_error(K);
		elt->evtype = (char *)0;
		return 1;
	}
	elt->index = KL_UINT(K, b, "tstamp_shared_state", "curr_index");
	free_block(b);

	evtype = (char *)alloc_block(elt->nentries, B_TEMP);
	entries = kl_get_struct(K, elt->entries, entrylen * elt->nentries,
							NULL, "tstamp_event_entry_t");
	if (KL_ERROR) {
		kl_print_error(K);
		free_block((caddr_t *)evtype);
		elt->evtype = (char *)0;
		return 1;
	}
	...
	for(sz=0, left=elt->nentries, i=elt->index;
		...
		if (KL_UINT(K, entry, "tstamp_event_entry", "evt") == JUMBO_EVENT) {
			evtype[i] = EVTYPE_JUMBO;
			continue;
		}

		/* Is the ths start of a stop-mode vevent? */
		sz = KL_UINT(K, entry, "tstamp_event_entry", "reserved");
		if (sz > elt->nentries) {
...
get_qual(k_ptr_t event, int num)
{
	...
	source = (k_ptr_t)(ADDR(K, event, "tstamp_event_entry", "qual") + num*8);
...
utrace_xlate(k_uint_t num, char *out, int slen)
{
	...
	if ((ptr = kl_sym_addr(K, "utrace_trtbl")) == 0 ||
		(len = kl_struct_len(K, "utr_trtbl_s")) == 0) {
		return 0;				/* translation failed */
	}

	entry = alloc_block(len, B_TEMP); /* buffer for one rec. at a time */

	/* This should probably be a hash table */
	do {
		kl_get_struct(K, ptr, len, entry, "utr_trtbl_s");
		if (KL_ERROR) {
			kl_print_error(K);
			free_block(entry);
			return 0;
		...
		/* Did we find it? */
		s = kl_kaddr(K, entry, "utr_trtbl_s", "name");
		if (s && (v = kl_uint(K, entry, "utr_trtbl_s", "evt", 0)) == num) {
			free_block(entry);
			kl_get_block(K, s, slen, out, "name");
			if (KL_ERROR) {
				kl_print_error(K);
				return 0;
			}
...
utrace_print(FILE *ofp, tracelist_t *elt, int flags)
{
	...
	len = kl_struct_len(K, "tstamp_event_entry");
	event = elt->entry;
	index = elt->index;
	num = kl_uint(K, event, "tstamp_event_entry", "evt", 0);
	...
	fprintf(ofp, "%-23s%4d %016llx",
			string,
			elt->cpu,
			KL_UINT(K, event, "tstamp_event_entry", "tstamp"));
	...
	if (flags & C_FULL) {
		fprintf(ofp, "\n    ADDR %llx: EVT=%lld, CPU=%lld, RESERVED=%lld",
				...
				num,
				KL_UINT(K, event, "tstamp_event_entry", "cpu"),
				KL_UINT(K, event, "tstamp_event_entry", "reserved"));
		if (num == UTRACE_EVENT) 
	...
	c = KL_UINT(K, event, "tstamp_event_entry", "reserved");
	event = alloc_block(len, B_TEMP);
	c2 = 0;
    for (; c; c--) {
		int c3;
		k_uint_t v;
        index = (index + 1) % elt->nentries;
		kl_get_struct(K, index * len + elt->entries, len,
	...
	num = KL_UINT(K, event, "tstamp_event_entry", "evt");
    while (num == JUMBO_EVENT && index != elt->prev) {
		k_uint_t v;
        for (c=kl_member_offset(K, "tstamp_event_entry", "tstamp");
			 c<len; c += sizeof(v), c2++) {
            if (c2 % 3 == 0)
                fprintf(ofp, "\n%27s", "");
...
{ \
	...
	do { \
		...
		kl_get_struct(K, (E).index * (entrylen) + (E).entries, \
		...
		(KL_UINT(K, (E).entry, "tstamp_event_entry", "evt") == JUMBO_EVENT)); \
}
			 ...
			 int flags)
{
	...
	ut_entrylen = kl_struct_len(K, "tstamp_event_entry");
	...
	for (i=0; i<cpu_cnt; i++) {
		(void)kl_get_pda_s(K, (kaddr_t)list[i].cpu, pdap);
		if (KL_ERROR) {
			kl_print_error(K);
			goto done;
		}
	...
	while(rec_cnt) {
		...
		hiidx = 0;
		hiclk = kl_uint(K, list[hiidx].entry, "tstamp_event_entry", "tstamp", 0);
		for (i=1; i<cpu_cnt; i++) {
			clk = kl_uint(K, list[i].entry, "tstamp_event_entry", "tstamp", 0);
			if (clk > hiclk) {
				hiclk = clk;
				hiidx = i;
			}
	...
	for (i=0; i<cpu_cnt; i++) {
		if (list[i].entry)
			free_block(list[i].entry);
		if (list[i].evtype)
			free_block((caddr_t *)(list[i].evtype));
	}
}
...
utrace_cmd(command_t cmd)
{
	...
	if (kl_sym_addr(K, "utrace_bufsize") == 0) {
	...
	if (cmd.flags & C_ALL) {	/* do all CPUs */
		if (cmd.nargs == 0) {
			...
			get_value(cmd.args[arg++], NULL, 0, &num_recs);
			if (KL_ERROR) {
		...
		list = alloc_block(K_MAXCPUS(K) * sizeof(tracelist_t), B_TEMP);
		for(p=list, i=0; i < K_MAXCPUS(K); i++, p++)
			p->cpu = i;
	}
		...
		list = alloc_block((cmd.nargs - arg) * sizeof(tracelist_t), B_TEMP);
		for(p=list; arg < cmd.nargs; arg++) {
			get_value(cmd.args[arg], NULL, K_MAXCPUS(K), &value);
			if (KL_ERROR) {
	...
	for (s=d=list; s<p; s++) {
		*d = *s;
		(void)kl_get_pda_s(K, (kaddr_t)s->cpu, pdap);
		if (KL_ERROR) {
			kl_print_error(K);
			continue;
		}
		if (!kl_kaddr(K, pdap, "pda_s", "p_tstamp_objp")) {
