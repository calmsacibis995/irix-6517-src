...
has_hubregs()
{
    ...
    if (bwin != 0 || swin != 1) 
		return(-1);
    for (node=0; node < K_NUMNODES(K); node++) {
	REM_NODE(addr);
	ADD_NODE(addr, node);
	if (kl_get_block(K, addr, 8, hubreg_value, "hubreg")) {
	    return(1);
	}
    }
