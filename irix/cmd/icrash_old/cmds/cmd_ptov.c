#ident  "$Header: /proj/irix6.5.7m/isms/irix/cmd/icrash_old/cmds/RCS/cmd_ptov.c,v 1.1 1999/05/25 19:19:20 tjm Exp $"
...
ptov_cmd(command_t cmd)
{
	...
	for(i = 0; i < cmd.nargs; i++) {
		 ...
		 */
		if (mode == 1) { 
			ret = kl_addr_convert(K, value, PFN_FLAG, &paddr, &pfn, 
				&K0vaddr, &K1vaddr, &K2vaddr);
		} 
		else {
			ret = kl_addr_convert(K, value, PADDR_FLAG, &paddr, &pfn, 
				&K0vaddr, &K1vaddr, &K2vaddr);
		}
