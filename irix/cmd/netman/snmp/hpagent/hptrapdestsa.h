...
class hpTrapDestSubAgent : public subAgent {
	friend void updateTrapDestTable(void);

private:
	asnObjectIdentifier	subtree;

	unsigned int		trapdestnumber;
	mibTable *		trapdesttable;

public:
	hpTrapDestSubAgent(void);
	~hpTrapDestSubAgent(void);
