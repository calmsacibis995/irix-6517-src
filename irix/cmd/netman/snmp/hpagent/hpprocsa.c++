...
updateProcTable(void)
{
	...
	for ( numVal = 0; numVal < vsp->numval; numVal++) {
		...
		pmExtractValue(vsp->valfmt, vp, desc.type, &atomval, desc.type);
		tmpVal = (unsigned int) atomval.ul;
		...
		pmExtractValue(vsp->valfmt, vp, desc.type, &atomval, desc.type);
		// Metric is in Kbytes. MIB object is in pages. 1 page = 4 Kbytes
		g = (unsigned int) 1024 * atomval.ul / getpagesize();
		a = &g;
		hpprocsa->proctable->set(&oi, &a);
		continue;
		...
		ps_starttime = (unsigned long) atomval.ul;
		// Metric is in seconds while Timetick = 1/100th sec
		itmpVal = (int) 100 * 
		    ((time_t) atomval.ul - (hpprocsa->sysboottime_secs));
		...
		pmExtractValue(vsp->valfmt, vp, desc.type, &atomval, desc.type);
		proc_timestamp = (unsigned int) atomval.ul;
		c = (unsigned int) atomval.ul;
		a = &c;
		hpprocsa->proctable->set(&oi, &a);
		continue;
		...
		pmExtractValue(vsp->valfmt, vp, desc.type, &atomval, desc.type);
		unsigned int ps_state = (unsigned int) atomval.ul;
		...
		proc_ticks_total = (unsigned int) (hpprocsa->tickspersec * atomval.ul) / 1000;
