...
ipSubAgent::get(asnObjectIdentifier *o, asnObject **a, int *)
{
...
#ifdef sgi
                if (is_elf64)
		  ipf = nl64[N_IPFORWARDING].n_value;
                else
#endif /* sgi */
		ipf = nl[N_IPFORWARDING].n_value;

		if (ipf == 0)
		    return SNMP_ERR_noSuchName;

		klseek(ipf);
		if (kread(&i, sizeof i) < 0)
		    return SNMP_ERR_noSuchName;
		if (i != 0)
	...
	case ipForwDatagrams :
	    {
		// Check if ipForwarding is on
                // int i;
                // off_t ipf = nl[N_IPFORWARDING].n_value;
                // if (ipf == 0)
                    // return SNMP_ERR_noSuchName;
                ...
                // if (i != 0)
                    *a = new snmpCounter
			((unsigned int) (ipstat_p->ips_forward
...
updateIpAddrTable(void)
{
    ...
    oi.getValue(&subid, &len);
    oid aoi = "1.3.6.1.2.1.2.2.1.25.0";		// if_addrlist
    aoi.getValue(&sub, &l);
    ...
    for ( ; ; ) {
	// Get next if
	// This could probably be simpler since ifs currently never go away
	rc = ifsa->iftable->formNextOID(&aoi);
        if (rc != SNMP_ERR_noError)
	    break;
	rc = ifsa->iftable->get(&aoi, &a);
	if (rc != SNMP_ERR_noError)
	    break;

	// Find the if_addrlist
	ifs = ((asnOctetString *) a)->getValue();
	ifaddr = strtoul(ifs, (char **)0, 16);
	delete (asnInteger *) a;

	// Read all the addresses for this interface from the kernel
	for ( ; ifaddr != 0; ifaddr = (off_t) ia.ia_ifa.ifa_next) {
	    // Read from kernel
	    klseek(ifaddr);
	    if (kread(&ia, sizeof ia) < 0)
		break;

	    // Form the oid
	    addr = IA_SIN(&ia)->sin_addr.s_addr;
	    ...
	    i = (int) ia.ia_netbroadcast.s_addr & 1;
	    ipsa->addrtable->set(&oi, &a);
...
updateIpRouteTable(void)
{
...
#ifndef sgi
    struct fullrtentry *frt;
    struct mbuf **routehash, *m, mb;
    unsigned int n, hashsize, size;
#endif
    subID *subid, *sub;
    unsigned int len, x, l;
#if _MIPS_SZLONG == 64
    unsigned int addr;
#else
    unsigned long addr;
    ...
    if (is_elf64)
       if (nl64[N_RTHASHSIZE].n_value == 0
      	   || nl64[N_RTHOST].n_value == 0
   	   || nl64[N_RTNET].n_value == 0)
    else
    if (nl[N_RTHASHSIZE].n_value == 0
	|| nl[N_RTHOST].n_value == 0
	|| nl[N_RTNET].n_value == 0)
	return;
#endif
    ...
    // Cache the values of the if pointers
    ift = new iftbl[ifsa->ifnumber];
    for (x = 0; x < ifsa->ifnumber ; x++) {
	// Get next if
	// This could probably be simpler since ifs currently never go away
	rc = ifsa->iftable->formNextOID(&aoi);
        if (rc != SNMP_ERR_noError)
	...
	ift[x].index = sub[l - 1];
	// ift[x].address = (int)((asnInteger *) a)->getValue();
	ifs = ((asnOctetString *) a)->getValue();
	ift[x].address = strtoul(ifs, 0, 16);
	delete (asnInteger *) a;
    }
	...
	exc_errlog(LOG_ERR, 0, "out of space =%d", errno);
	return;
    }
	...
	if ((rtmsg->rtm_addrs & (RTA_DST|RTA_GATEWAY)) == (RTA_DST|RTA_GATEWAY)) {
...
#endif
	    rt->k_rt_flags = rtmsg->rtm_flags &(RTF_UP | RTF_GATEWAY | RTF_HOST);
	...
	for (x = 0; x < ifsa->ifnumber; x++) {
	    /* Check this one */
	    unsigned long tmp = (unsigned long) rtmsg->rtm_addrs;
	    if (ift[x].address == (int) tmp) {
		i = ift[x].index;
		break;
	    }
	}
...
#ifdef sgi
    if (is_elf64)
       klseek(nl64[N_RTHASHSIZE].n_value);
    else
#endif /* sgi */
    klseek(nl[N_RTHASHSIZE].n_value);
    if (kread(&hashsize, sizeof hashsize) < 0) {
	delete ift;
	return;
    }

    // Allocate the hash table
    size = hashsize * sizeof(struct mbuf *);
    if (sizeof(long) == 8)
        routehash = (struct mbuf **) new unsigned long[hashsize];
    else
        routehash = (struct mbuf **) new unsigned int[hashsize];

    for (unsigned int q = N_RTHOST; ; q = N_RTNET) {
	// Read routes
#ifdef sgi
    if (is_elf64)
	klseek(nl64[q].n_value);
    else
#endif /* sgi */
	klseek(nl[q].n_value);
	if (kread(routehash, size) < 0) {
	    delete ift;
	    delete routehash;
	    return;
	}

	// Go through route table
	for (n = 0; n < hashsize; n++) {
		...
		// Read in mbuf
		klseek((off_t) m);
		if (kread(&mb, sizeof mb) < 0)
		    break;
		m = mb.m_next;

		// Get fullrtentry
		frt = mtod(&mb, struct fullrtentry *);

		// Only do AF_INET routes
		if (frt->r.rt_dst.sa_family != AF_INET)
		    continue;

		// Form the oid
		addr = SIN(frt->r.rt_dst)->sin_addr.s_addr;
		subid[len - 4] = (subID) ((addr & 0xFF000000) >> 24);
		subid[len - 3] = (subID) ((addr & 0x00FF0000) >> 16);
		subid[len - 2] = (subID) ((addr & 0x0000FF00) >> 8);
		subid[len - 1] = (subID) (addr & 0x000000FF);
		...
		subid[len - 5] = ipRouteIfIndex;
		for (x = 0; x < ifsa->ifnumber; x++) {
		    unsigned long tmp = (unsigned long) frt->r.rt_ifp;
		    if (ift[x].address == tmp) {
			i = ift[x].index;
			break;
		    }
		}
		...
		subid[len - 5] = ipRouteProto;
		if (frt->r.rt_flags & RTF_DYNAMIC
		    || frt->r.rt_flags & RTF_MODIFIED) {
		    i = routeProtoIcmp;
		    ipsa->routetable->set(&oi, &a);
		} else {
		    rc = ipsa->routetable->get(&oi, &a);
		...
		subid[len - 5] = ipRouteNextHop;
		p = SIN(frt->r.rt_gateway)->sin_addr.s_addr;
		a = &p;
		ipsa->routetable->set(&oi, &a);

		subid[len - 5] = ipRouteMask;
		addr = SIN(frt->r.rt_gateway)->sin_addr.s_addr;
		if (addr == 0)
#if _MIPS_SZLONG == 64
                    p = (unsigned int) 0;
#else
