...
remoteSubAgentHandler::readCfg(void) {
	...
	exc_errlog(LOG_WARNING, errno,  "Cannot open remote sub agent config file %s: ",
		REMOTE_SUB_AGENT_FILE);
	return 0;
    }
    exc_errlog(LOG_DEBUG, 0, "Opened remote sub agent file '%s'",
	    REMOTE_SUB_AGENT_FILE);
    int linecount = 0;
    int rsacount = 0;
	...
	if (tok) {
		...
		if (tok) {
			...
			exc_errlog(LOG_WARNING, 0, 
				    "Bad remote sub agent config file format at %d: %s; no timeout", 
				    linecount, buf);
			delete line;
			delete tree;
			delete addr;
		    ...
		    exc_errlog(LOG_WARNING, 0,
				"Bad remote sub agent config file format at %d: %s: no port", 
				linecount, buf);
		    delete line;
		    delete tree;
		    delete addr;
		...
		exc_errlog(LOG_WARNING, 0,
			    "Bad remote sub agent config file format at %d: %s: no address", 
			    linecount, buf);
		delete line;
		delete tree;
		continue;
	    ...
	    exc_errlog(LOG_WARNING, 0, 
			"Bad remote sub agent config file format at %d: %s: No mib tree", 
			linecount, buf);
	    delete line;
	    continue; 
	}
	...
	exc_errlog(LOG_DEBUG, 0,
		"Found remote sub agent %d: %s	%s  %s	%s  %s", 
...
#ifdef DEBUG
    char *s, *s2;
    exc_errlog(LOG_DEBUG, 0, "appended varbind %s to rsa %s", 
		s = vb->getString(),
		s2 = reqrsa->getString());
    delete s; delete s2;
#endif
...
remoteSubAgentHandler::doRequests(snmpPacket &req, getResponsePDU &respdu,
	...
	switch (type) {
		...
		sprintf(reqtype, "Get");
		exc_errlog(LOG_DEBUG, 0, "making Get req with vbl %s to %s", 
	    ...
	    default:
		exc_errlog(LOG_WARNING, 0,
			    "remoteSubAgentHandler: unsupported PDU");
		continue;
		//XXX do I need to clean up?
	}
	exc_errlog(LOG_DEBUG, 0, "remote sub agent  request(%s) status %d", 
		    reqtype, errstat);
	...
	if (errstat <= SNMP_ERR_genErr) {
	    ...
	    exc_errlog(LOG_DEBUG, 0, "remote sub agent remote response: %s", 
			s = vbl->getString());
	    delete s;
