...
init_nlist(char *sysname)
{
    ...
    if (sizeof(long) == 8) {
        if (!is_elf64 ||
            ...
            || (0 > lseek(kmem, (long)nl64[N_KERNEL_MAGIC].n_value  & 0x7fffffffffffffff, 0))
            || sizeof(kern_end) != read(kmem, &kern_end, sizeof(kern_end))            || (kern_end != nl64[N_END].n_value)) {
            exc_errlog(LOG_ERR, errno, "wrong system namelist(1): %s", sysname);
            exit(1);
        }
      }
      else
	...
	if (nl[N_KERNEL_MAGIC].n_type == 0 || nl[N_END].n_type == 0
	    ...
	    exc_errlog(LOG_ERR, errno, "wrong system namelist(2): %s", sysname);
	    exit(1);
	}
    }
    else {
	if (nl[N_KERNEL_MAGIC].n_type == 0 || nl[N_END].n_type == 0
	    ...
	    || sizeof(kern_end) != read(kmem, &kern_end, sizeof(kern_end))
	    || kern_end != nl[N_END].n_value) {
	    exc_errlog(LOG_ERR, errno, "wrong system namelist(3): %s", sysname);
	    exit(1);
	}
   }
}
