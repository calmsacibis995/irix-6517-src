...
extern int hpGetIntegerValue(pmAtomValue *atomvalP, int type);
...
hpFsSubAgent::hpFsSubAgent(void)
{
    ...
    // Define table
    fstable = table(fsTable, 1, updateFsTable);
    fsnumber = 0;
}
...
hpFsSubAgent::get(asnObjectIdentifier *o, asnObject **a, int *)
{
	...
	if (len != sublen + 4		   // subtree.table.entry.column.id1.id2
...
printf("\nhpfssa: LOOP: numEntries=%d\tnumVal=%d\n", numEntries, numVal);
		...
		i = (unsigned int) 2 * hpGetIntegerValue(&atomval, desc.type);
		a = &i;
		hpfssa->fstable->set(&oi, &a);
		continue;
...
int
hpGetIntegerValue(pmAtomValue *atomvalP, int type)
{
    switch(type) {
    case PM_TYPE_32:
	return atomvalP->l;
	...
	return atomvalP->ul;
    case PM_TYPE_64:
	return (int) atomvalP->ll;
    case PM_TYPE_U64:
	return (int) atomvalP->ull;
    default:
	exc_errlog(LOG_ERR, 0,
		   "hpfssa: error this integer type unknown");
	return 0;
    }
}
