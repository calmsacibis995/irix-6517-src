 ...
 *********************************************************************
 *
 * if format == FMT_LONG then we print the structure, otherwise we
 * print only the name and type.
 *
 *********************************************************************
 */
int
xlv_prnt_list(int format)
{
	...
	while (hash_entry != NULL) {
		...
		do {
			...
			switch (oref->obj_type) {
				case XLV_OBJ_TYPE_VOL:
				    if (format == FMT_LONG) {
				 	xlv_oref_print (oref, XLV_PRINT_BASIC);
					printf("\n");
				    } else {
					pfmt(stdout, MM_NOSTD, 
						":52:Volume:\t\t '%s'", 
					...
					{
					pfmt(stdout, MM_NOSTD, 
	":: Incomplete- missing unique piece.");
					}
					if (oref->vol->state == 
						E_VOL_STATE_MISS_SUBVOL)
					{
					pfmt(stdout, MM_NOSTD, 
	":: Incomplete- missing required subvolume.");
					}
					pfmt(stdout, MM_NOSTD,"::\n"); 
				    }
					break;
...
#ifdef DEBUG
		if (oref != NULL) {
			printf("DBG: xlv_prnt_list() next is not null \n");
		}
#endif
		} while (oref != NULL);
		hash_entry = Tcl_NextHashEntry (&search_ptr);
...
xlv_prnt_name(char *name)
{
	...
	if (target != NULL) {
		...
		pfmt(stdout, MM_NOSTD, 
			":62: Requested entity '%s' was not found \n", name);
		return (ENO_SUCH_ENT);
	}
