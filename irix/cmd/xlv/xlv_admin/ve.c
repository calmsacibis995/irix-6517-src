	    ...
	    int			vnum)
{
	...
	if (mode == ADD) {
		...
		if (I_ve == NULL) {
#ifdef DEBUG
			pfmt(stderr, MM_NOSTD, ":62: I_ve is null exit() \n");
#endif
	...
	if (mode == ADD) 
	{
		...
		if (vnum == (int)owning_plex->num_vol_elmnts) {
			...
			ASSERT(size > 0);
			ASSERT(owning_plex->vol_elmnts[indx].end_block_no > 0);
			last = owning_plex->vol_elmnts[indx].end_block_no;
...
printf("DBG: call to xlv_add_ve_to_plex() failed with %d \n", rval);
		...
		if (subvol != NULL) {
			...
			/*
			 * XXXjleong Should check that the new ve does
			 * not overlap with any other ve in the other
			 * plexes.
			 *
