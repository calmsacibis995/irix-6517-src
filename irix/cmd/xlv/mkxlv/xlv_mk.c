...
/*
 * List of volume headers that we read in.
 */
...
#if 0
/* XXXjleong XLV device nodes are created by xlv_assemble */
	...
	xlv_lab2_write (&xlv_vh_list, vol, &(vol->uuid), &status);
	if (status != 0) {
    ...
    /* Print the result */
    xlv_tab_print (xlv_tab_vol, xlv_tab, XLV_PRINT_BASIC);
}
		    ...
		    char *component[], int num_components)
{
    ...
    for (i = 0; i < num_components; i++) {
	strncpy (plex->vol_elmnts[i].veu_name, 
		    ...
		    char *component[], int num_components)
{
    ...
    strncpy (ve->veu_name, obj_name, sizeof(xlv_name_t));
    ve->type = XLV_VE_TYPE_STRIPE;	/* XXX stripe, just like before */
    ve->grp_size = num_components;
    ve->stripe_unit_size = 0;
    ve->state = XLV_VE_STATE_EMPTY;

    for (i = 0; i < num_components; i++) {
        xlv_fill_dp(&ve->disk_parts[i], name_to_dev(component[i]));
	if (DP_DEV(ve->disk_parts[i]) == 0)
	...
	else
	{
	    ...
	    if (blocks != min_blocks) {
	        fprintf (stderr,
		    "Partition sizes for %.*s are not the same, truncating.\n",
    ...
    ve->stripe_unit_size = 128; /* as with mkfs_efs.  track size on scsi
		was basicly always a myth.  128 is a reasonable default... */
    if ((ve->stripe_unit_size == (unsigned)-1) && (ve->disk_parts[0].dev != 0))
	xlv_error ("Cannot determine tracksize of %.*s.",
                sizeof(xlv_name_t), component[0]);

    /* If we are striping, make sure that the number of blocks is
       an integral multiple of the stripe_unit_size. */
    if ((num_components > 1) && (min_blocks > 0) && 
	(ve->stripe_unit_size != (unsigned)-1)) {
	min_blocks = (min_blocks / ve->stripe_unit_size) *
			ve->stripe_unit_size;
    }
    ...
    for (ve_node = vol_elmnt_nodes; ve_node != NULL;
        ...
        if (ve_node->part_of_a_plex == B_FALSE)
            xlv_error ("Volume element %.*s is not part of a plex",
                    sizeof(xlv_name_t), ve_node->vol_elmnt.veu_name);
