 ...
 * Modified Summer 1994 at Silicon Graphics, Inc.
 *      Added -V option to verify data.
 *
 * Distribution Status -
...
int doverify = 0;               /* !0=perform data verification on receive */
...
Options specific to -r:\n\
	...
	-T	\"touch\": access each byte as it's read\n\
        -V      Verify received data in sink mode\n\
";	
...
char **argv;
{
	...
	while ((c = getopt(argc, argv, "drstuvBDTVb:f:l:n:p:A:O:")) != -1) {
		switch (c) {
...
"ttcp: -b option ignored: SO_SNDBUF/SO_RCVBUF socket options not supported\n");
	...
	CurrTime = time(0);
        fprintf(stdout,"TTCP-%s: %s", (trans?"T":"R"), ctime(&CurrTime));
	...
	if (!udp)  {
		...
		if(connect(fd, &sinhim, sizeof(sinhim) ) < 0)
			err("connect");
		fprintf(stdout,"ttcp-t: connect to %s\n", 
...
char *s;
{
        fflush(stdout);   /* In case stderr redirected to stdout */
	perror(s);
	fprintf(stderr,"ttcp%s: ERROR - errno=%d\n", trans?"-t":"-r",errno);
	exit(errno);
}
...
int offset;
{
	...
	if (*cp != (c & 0x7F)) {
	    fflush(stdout);   /* In case stderr redirected to stdout */
	    fprintf(stderr,
		    "ttcp-r: ERROR - Bad data after %d bytes (offset %d) expected %x, got %x\n",
		    offset+i, i, c, *cp);
	    exit(1);
	}
	++cp;
