...
static u_int *ht = 0;			/* hash locks */
static hashentry *he = 0;		/* hash entries */
static u_int stride;
...
void inittrans()
{
  ...
  if (ht == NULL || he == NULL) {
    printf("Out of memory; failed to allocate %d bytes.\n",
	   (int) (HTSIZE * (sizeof(u_int) + sizeof(hashentry))));
    exit(0);
  }
...
void cleantrans()
{
  bzero(ht, HTSIZE * sizeof(u_int));
  bzero(he, HTSIZE * sizeof(hashentry));
}
