  ...
  
{
    ...
    int doprt;
#ifdef COMPILER_BUG    
    volatile int save_i, save_nprint;
#endif
...
#ifdef COMPILER_BUG        
        save_i = i; save_nprint = NPRINT;
        doprt = (save_i % save_nprint);
        printf("doprt:%d, ID: %d, --- si:%d, sNPRINT:%d\n",
               doprt, ProcID, save_i, save_nprint);
#else
        doprt = i % NPRINT;
#endif        
        if ( (doprt == 0) || ((NSAVE > 0) && ((i % NSAVE) == 0))) {
