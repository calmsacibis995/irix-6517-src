...
#ifdef NUMA
   /*
    * We need Global initialized (ANLinit), and
    * NPROC set (startrun).
    */
   numa_init();
#endif
...
#ifdef NUMA
   /*
    * If we're going to use RR allocation over the complete
    * mldset, we have to create a PM here, and associate the
    * complete data set to this PM.
    * Otherwise, if we're going to be using fixed allocation,
    * each slave creates its own PM, with its own MLD, and
    * associates it to its section of the data set.
    * In all cases, the association between processes and MLDs
    * is done at the beginning of each slave.
    */
#endif   
...
#ifdef NUMA
/*
 * numa_init: Create and place Memory Locality Domains
 */
void
numa_init()
{
        ...
        int i;
        pmo_handle_t mld;
        pmo_handle_t mldlist[(MAX_PROC + 1)/2];
        pmo_handle_t mldset;
        ...
        /*
         * The actual allocation policy is decided later when
         * we create the slave processes.
         * We have several options:
         * 1) Create just one PM and do RR allocation over the MLDSET.
         * 2) Create as many PM's as MLDS, doing fixed allocation
         */

        /*
         * The actual binding between processes and mlds is also done
...
*/
...
#ifdef NUMA
   /*
    * First we link this process to the corresponding MLD so that
    * the scheduler knows it should activate us on a cpu close to
    * the MLD where our data will be placed.
    */
   if (process_mldlink(0, Local[ProcessId].mld, RQMODE_ADVISORY) < 0) {
           perror("process_mldlink");
           exit(1);
   }

   /*
    * If we're doing fixed allocation, we have to create our own PM
    * with our corresponding PM, and associate our data set.
    */

   if ((pm = pm_create_simple("PlacementFixed",
                              (void*)Local[ProcessId].mld,
                              "ReplicationDefault",
                              NULL,
                              PM_PAGESZ_DEFAULT)) < 0) {
           perror("pm_create");
   ...
   /*
    * Now we ask for our data to be allocated using the PM
    * we created above.
    */

   /*
    * We should tell the compiler to place each element of local_memory
    * in separate pages.
    */
   if (pm_attach(pm,
                 &(Local[ProcessId]),
                 sizeof(struct local_memory)) < 0) {
           perror("pm_attach");
           exit(1);
   }

   /*
    * The pm_attach syscall only defines the policy to allocate the page if
    * it hasn't been allocated already.
    * We have two options:
    * 1) We can do the pm_attach before we touch any of the data.
    * 2) We can migrate the data to its corresponding MLD.
    *    (if data is correctly placed, no migration occurs, otherwise,
    *     data is moved to a correctly placed page).
    */

   if (migr_range_migrate(&(Local[ProcessId]),
                          sizeof(struct local_memory),
                          Local[ProcessId].mld) < 0) {
           perror("migr_range_migrate");
           exit(1);
   }
   
   if (pm_attach(pm,
                 &(Local[ProcessId].mybodytab),
                 maxmybody * sizeof(bodyptr) - 1) < 0) {
           perror("pm_attach2");
           exit(1);
   }

   if (migr_range_migrate(&(Local[ProcessId].mybodytab),
                          maxmybody * sizeof(bodyptr) - 1,
                          Local[ProcessId].mld) < 0) {
           perror("migr_range_migrate2");
           exit(1);
   }

   if (pm_attach(pm,
                 &(Local[ProcessId].mycelltab),
                 maxmycell * sizeof(cellptr) - 1) < 0) {
           perror("pm_attach3");
           exit(1);
   }

   if (migr_range_migrate(&(Local[ProcessId].mycelltab),
                          maxmycell * sizeof(cellptr) - 1,
                          Local[ProcessId].mld) < 0) {
           perror("migr_range_migrate3");
           exit(1);
   }

   if (pm_attach(pm,
                 &(Local[ProcessId].myleaftab),
                 maxmyleaf * sizeof(leafptr) - 1) < 0) {
           perror("pm_attach4");
           exit(1);
   }

   if (migr_range_migrate(&(Local[ProcessId].myleaftab),
                          maxmyleaf * sizeof(leafptr) - 1,
                          Local[ProcessId].mld) < 0) {
           perror("migr_range_migrate4");
           exit(1);
   }
