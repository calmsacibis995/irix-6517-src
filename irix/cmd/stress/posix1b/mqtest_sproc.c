 ...
 * mqtest_sproc.c
 *
 * 	test program for Posix.1b message queues using sprocs
 *
...
static long num_sproc_pairs = NUM_SPROC_PAIRS;
...
pid_t *pids;
	...
	if ((int) mqd < 0) {
		sprintf(errbuf,
		"mqtest_sproc: ERROR - mq_open(%.20s) failed",mq_name);
		perror(errbuf);
		EXIT(2);
	}
	if (verbose_mode) {
	...
	printf("\n%-15s %-s \n\n"," ","posix mq functions executed",
								"mq_open");
	printf("%-8s %-15s "," ","mq_open");
	printf("%-15s %-15s\n", "mq_getattr", "mq_setattr");
	printf("%-8s %-15s "," ","mq_send");
...
unsigned int msgprio;
	...
	if (mq_getattr(mqd, &mqattr) < 0) {
		perror("mqtest_sproc: ERROR - mq_getattr failed");
		EXIT(2);
	}
	if (verbose_mode) {
	...
	if ((buf = (char *) malloc(mqattr.mq_msgsize)) == NULL){
		fprintf(stderr,"mqtest_sproc: ERROR - malloc( %d bytes)\n",
					mqattr.mq_msgsize);
		EXIT(2);
	}
	...
	for(index = 1; index <= send_recv_msgs; index++) {
		...
		if (verify_chksum(buf,msg_size)) {
			...
			*/
			 ...
			 */
			fprintf(stderr,
			"mqtest_sproc: ERROR - mq_recv - msg corruption\n");
			EXIT(2);
		}
		if (verbose_mode)
			printf("%-12d %-10s %-8d %-8d %-30s\n", getpid(),
	...
	if (mq_close(mqd)) {
		perror("mqtest_sproc: ERROR - mq_close failed");
		EXIT(2);
	}
	EXIT(0);
}
...
struct mq_attr mqattr, old_mqattr;
	...
	if (mq_setattr(mqd, &mqattr, &old_mqattr) < 0) {
		perror("mqtest_sproc: ERROR - mq_setattr failed");
		EXIT(2);
	}
	mqattr.mq_flags = 0;
	if (mq_setattr(mqd, &mqattr, &old_mqattr) < 0) {
	...
	if (((mqattr.mq_flags & O_NONBLOCK) == 0) &&
		...
		fprintf(stderr,"mqtest_sproc: ERROR - %-9s %-9d %-s\n",
...
sp_unlink(void *mq_name)
{
	...
	if (mq_unlink(mq_name)) {
		perror("mqtest_sproc: ERROR - mq_unlink failed");
		EXIT(2);
	} else if ((mq_open(mq_name, MSG_PERM) >= 0) || (errno != ENOENT)) {
		fprintf(stderr,"mqtest_sproc: ERROR - %-9s %-5d %-s\n","Sproc",
				getpid(),
...
sigset_t sigset;
	...
	if (sigaction(SIGUSR1, &sa, NULL) < 0) {
		perror("mqtest_sproc: ERROR - sigaction failed");
		EXIT(2);
	}
	sigemptyset(&sigset);
	sigaddset(&sigset, SIGUSR1);
	if (sigprocmask(SIG_UNBLOCK, &sigset, NULL)) {
		perror( "mqtest_sproc: ERROR - pthread_sigmask failed");
		EXIT(2);
	}
	...
	if (mq_notify(mqd, &notify_sigev)) {
		perror("mqtest_sproc: ERROR - mq_notify failed");
		EXIT(2);
	}
	notify_on = 1;
	pause();
