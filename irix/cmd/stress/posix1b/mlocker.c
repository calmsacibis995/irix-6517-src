...
stack_test(void)
{
	 ...
	 * since some pages might have already been locked and some
	 * additional pages might be locked due to pre-growth of stack
	 * Here we try to make sure the basic stuff is locked down
	 */
	if (nlocked < (curlocked+n)) {  
		errprintf(ERR_EXIT, "%d pages locked should be greater than %d\n",
			  nlocked, curlocked+n);
	}
