/*
 * MMSC OOB protocol xmit/rcv routines.
 * 
 * $Id: mmsc.c,v 1.1 1997/12/11 03:03:12 jfd Exp $
 */
...
mmsc_t *mmsc_open(char *name, int baud, int console)
{
    ...
    m->baud_old = (int) cfgetispeed(&m->ti_old);
	...
	if (cfsetispeed(&m->ti_new, (speed_t) m->baud) < 0 ||
	    cfsetospeed(&m->ti_new, (speed_t) m->baud) < 0) {
...
mmsc_t *mmsc_open_plus(char *name, int baud)
{
  ...
  m->baud_old = (int) cfgetispeed(&m->ti_old);
  if (m->baud_old == 0)
    m->baud_old = 9600; /* a "0" speed is returned, set it correctly */
  ...
  ocap = cap_acquire(1, &cap_device_mgt);
  /*  if (tcsetattr(m->tty_fd, TCSAFLUSH, &m->ti_new) < 0) { */
  if (tcsetattr(m->tty_fd, TCSADRAIN, &m->ti_new) < 0) {
    cap_surrender(ocap);
    warning("Could not set TTY attributes: %s\n", STRERR);
    goto fail;
  }
...
int mmsc_close_plus(mmsc_t *m)
{
  ...
  if( cfsetispeed(&m->ti_old, m->baud_old) < 0 || 
      cfsetospeed(&m->ti_old, m->baud_old) < 0){
    r = -1;
  }
  if (tcsetattr(m->tty_fd, TCSADRAIN, &m->ti_old) < 0) {
    warning("Could not set TTY attributes: %s\n", STRERR);
