 ...
 *	such as those sent by sysctlrd, the IRIX daemon that talks to the FFSC.
 */
...
/*
 * The FFSC can communicate with IRIX over the IO6 using out-of-band
 * messages.  Messages can originate from either IRIX or the FFSC, and
...
#define FFSC_COMMAND		61	/* Send command to FFSC */
#define FFSC_INIT		62	/* Initialize connection */
...
/* Public functions */
unsigned char	oobChecksumMsg(const unsigned char *);
void		oobProcessMsg(struct console *, unsigned char *);
STATUS		oobSendMsg(int, unsigned char *);
