...
others()
{
	...
	finput = fopen( fname_parser, "r" );
	if (finput == NULL ) error(":5", "Cannot find parser %s", fname_parser) ;
	...
	if ((fdebug = fopen(fname_debug, "r")) == NULL)
	...
	while ((c=getc(finput)) != EOF) {
		if (c == '$') {
			...
			switch (c) {
			case 'A':	/* copy actions */
				faction = fopen(fname_acts, "r");
				if (faction == NULL) 
					error(badopenid, badopen, fname_acts,
						strerror(errno));
				...
				c = getc(finput);
				break;
			case 'M':	/* copy module prefix */
				fputs( modpref, ftable );
				c = getc(finput);
				break;
			default:
				(void) putc('$', ftable);
