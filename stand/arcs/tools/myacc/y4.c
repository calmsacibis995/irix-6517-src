...
/* #ident	"@(#)yacc:y4.c	1.4" */
#ident	"$Header: /proj/irix6.5.7m/isms/stand/arcs/tools/myacc/RCS/y4.c,v 1.1 1996/03/17 02:05:40 csm Exp $"
# include "dextern"
...
void callopt(){
	...
	register i, *p, j, k, *q;
	static const char badtemp[] = "Bad tempfile";
	static const char badtempid[] = ":103";
	...
	if( (finput=fopen(fname_temp,"r")) == NULL )
...
more:
	for ( n= nn; n<new_actsize; ++n) {
		flag = 0;
		for( r = q1; r < q2; r += 2 ){
 			while (*r + n >= new_actsize) exp_act((int **)NULL);
 			s = *r + n + amem;
 			if( *r + n < 0 ) goto nextn;
			if( *s == 0 ) ++flag;
	...
	/* check that the position equals another only if the states are identical */
		...
		for( r = q1; r < q2; r += 2 ){
			...
			if( s > maxa ) maxa = s;
			if( *s != 0 && *s != r[1] ) error(":104", "Clobber of amem array, pos'n %d, by %d", s-amem, r[1] );
			*s = r[1];
			}
		indgo[i] = n;
...
static void osummary(){
	...
	(void) pfmt( foutput, MM_NOSTD, ":105:Optimizer space used: input %d/%d, output %d/%d\n",
		optimmem-tracemem+1, new_memsize, maxa-amem+1, new_actsize );
	(void) pfmt( foutput, MM_NOSTD, ":106:%d table entries, %d zero\n", (maxa-amem)+1, i );
	(void) pfmt( foutput, MM_NOSTD, ":107:Maximum spread: %d, maximum offset: %d\n", maxspr, maxoff );
...
static void aoutput(){ /* this version is for C */
	...
	(void) fprintf( ftable, "# define %sYYLAST %d\n", modpref, maxa-amem+1 );
...
static void arout( s, v, n ) char *s; int *v, n; {
	...
	(void) fprintf( ftable, "%syytabelem %s%s[]={\n", modpref, modpref, s );
	for( i=0; i<n; ){
...
int **ptr;
{
	...
	if (amem == NULL) error(":108", "Not enough memory to expand action table");
