/*
 * Implement server process that acts as ethernet packet source/sink
 * for mace ethernet simulation.
 *
 * Created by a fork/exec from the sim server.
 *
...
serviceTX(int s){
  ...
  case 0:
    ...
    if (tmp!=t.len){
      fprintf(stderr,
	      " SIM EtherXR- TX: read %d bytes, expected %d. Exiting.\n",
	      tmp,
	      t.len);
      exit(1);
    }

    /* If no socket, just dump the buffer */
    if (s==0)
      dumpBuf("TX",&t,32); 
    else
      /* Otherwise, send it */
      if (write(s,t.buf,t.len) != t.len){
	fprintf(stderr, " SIM EtherXR- TX write error: %s\n", strerror(errno));
      }
    break;
  default:
...
serviceSkt(int s){
  ...
  /* Handle no socket case */
  if (s==0)
    return;

  len = read(s,&epkt,sizeof epkt);
  if (len==0){
    fprintf(stderr, " SIM EtherXR- RX zero length read\n");
    return;
  }
  if (len<0){
    fprintf(stderr, "  SIM EtherXR- RX read error %s\n",strerror(errno));
    exit(1);
  }
...
setupSkt(void){
  ...
  if (bind(s,&sr,sizeof sr)<0){
    fprintf(stderr,
	    " SIM EtherXR- bind error: %s, exiting.\n",
	    strerror(errno));
    exit(1);
  }
  ...
  if (ioctl(s,SIOCADDSNOOP,&sf)<0){
    fprintf(stderr,
	    "  SIM EtherXR- ioctl error 1: %s, exiting.\n",
	    strerror(errno));
    exit(1);
  }
  ...
  if (ioctl(s,SIOCADDSNOOP,&sf)<0){
    fprintf(stderr,
	    "  SIM EtherXR- ioctl error 2: %s, exiting.\n",
	    strerror(errno));
    exit(1);
  }
  ...
  if (ioctl(s,SIOCSNOOPING,&on)<0){
    fprintf(stderr,
	    "  SIM EtherXR- ioctl error 3: %s, exiting.\n",
	    strerror(errno));
    exit(1);
  }  
  return s;
...
main(int argc, char** argv){
  ...
  while(run==0){
	...
	serviceSkt(skt);
      ...
      if (errno!=EINTR){
	fprintf(stderr,"SIM EtherXR- select error: %s\n",
		sts, strerror(errno));
	break;
      }
  }
  
  fprintf(stderr,"\n  SIM EtherXR- server exiting\n");
  fflush(stderr);
}
