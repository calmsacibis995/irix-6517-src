/*
 * ===================================================
 * $Date: 1997/08/18 20:40:26 $
 * $Revision: 1.1 $
 * $Author: philw $
 * ===================================================
 *  test name:      mem_ECCtst21.c
 ...
 */
static void	ecc_interrupt21     (void)              ;
...
mem_ECCtst21 (void)
{
    ...
    uint16_t	hdata    ; 
    uint32_t	wdata    ;
    uint64_t	ddata, ecc_replacement, tmp, expected_data ; 
    int32_t     *interrupt_flag, i ; 
     ...
     */
    * (interrupt_flag = (int32_t *) regist_interrupt (ecc_interrupt21))
       = 0xdeadbeef ;
     ...
     */
    ecc_replacement = (uint64_t)((uint32_t)(CHECKBIT1 ^ 0x9));
    ...
    *(uint8_t *)((kseg1|TESTADDR)+0x7) = 0xca ; /* a byte write	    	*/
    ...
    if (*interrupt_flag != ((int32_t)0xacaccaca)) {
      printf ("\n"
              "Error: [071] Interrupt does not happen\n");
      return 0;
    }      

    ecc_replacement = CHECKBIT1 ^ 0x9;/* Setup double bit errors.*/
    memptr->ecc_repl = ecc_replacement ;    /* make it so in crime.     */
    memptr->status = ECCREPL ;	    /* and get ready to use replacement.*/
    BlockInit(TESTADDR) ; 	    /* write to memory (block write). 	*/
    memptr->status = ECCENABLE ;    /* Now enable ecc checking.         */
    ...
    *(uint16_t *)((kseg1|TESTADDR)+0x6) = 0xacac;/* a byte write    	*/
    ...
    ecc_replacement = CHECKBIT1 ^ 0x9;/* Setup double bit error. */
    memptr->ecc_repl = ecc_replacement ;    /* make it so in crime.     */
    memptr->status = ECCREPL ;	    /* and get ready to use replacement.*/
    BlockInit(TESTADDR) ; 	    /* write to memory (block write). 	*/
    memptr->status = ECCENABLE ;    /* Now enable ecc checking.         */
 ...
 * =================================================
 *
 * $Log: mem_ECCtst21.c,v $
 * Revision 1.1  1997/08/18 20:40:26  philw
 * updated file from bonsai/patch2039 tree
 *
 * Revision 1.2  1996/10/31  21:50:31  kuang
 * Bring Bonsai IP32 debugcard up to the level of IP32 debugcard v2.4 on Pulpwood
 *
 * Revision 1.2  1996/10/04  20:32:51  kuang
 * Ported to run with crime ECC
 *
 * Revision 1.1  1996/04/04  23:49:59  kuang
 * Ported from petty_crime design verification test suite
 *
 * Revision 1.3  1995/11/21  19:41:14  pries
 * Modified to support tests that take cache errors where they used to take
 * bus errors (due to correction of cpu model).
 *
 * Revision 1.2  1995/07/13  01:12:33  kuang
 * Avoid the incompleted read cause by a bus error exception.
 *
 * Revision 1.2  1995/07/12  23:26:38  kuang
 * Avoid the Triton C model incomplete bus error read problem.
 *
 * Revision 1.1  1995/07/06  22:39:38  kuang
 * Initial revision
 *
