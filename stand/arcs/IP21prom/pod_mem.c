...
int pod_walk(__psunsigned_t lo, __psunsigned_t hi, int cont, int safe)
{
    __psunsigned_t addr;
    uint data, read_val;
    char key;
	...
	while (data != 0) {
	    ...
	    if (safe) {
		read_val = wr_restore((uint *)addr, data);
	    } else {
		store_word((uint *)addr, data);
		read_val = load_word((uint *)addr);
	   } 
	...
	for (b = 0; b < num_banks; b += 8) {
		if (banks[b]->bnk_slot == slot) {
			if (banks[b + bank]->bnk_slot != slot) {
				loprintf("*** IP21 prom logic error: disable_slot_bank()!\n");
				return -1;
			}
