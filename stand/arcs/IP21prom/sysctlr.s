...
LEAF(sysctlr_message)
	...
	move	ta2, a0			# Save message to be written
	...
	move	a0, ta2			# (BD) pointer to message
...
1:	
	...
	nop
	sub	ta2, 1			# (BD)
	beqz	ta2, 9f			# If timeout expires goto end
	nop				# (BD)
	...
	bne	v0, ta3, 9f		# Got the response character
...
3:	#
	...
	bne	v0, ta2, 9f		# Return if values don't match 
	move	ta2, zero		# (BD)

	jal	ccuart_getc		# Get response type
	li	ta2, SC_DEBUG		# (BD) 
	bne	ta2, v0, 9f		#  Return if values don't match 
	move	ta2, zero		# (BD) Return zero on error
...
4:					# DO
	...
	bnez	ta3, 4b			#   Loop back up to read
	sub	ta3, 1			#   (BD) decrement counter
					# UNTIL ta3 is zero
...
1:	
	...
	nop
	sub	ta3, 1			# (BD)
	beqz	ta3, 9f			# If timeout expires goto end
	nop				# (BD)
	beqz	v0, 1b			# If no character loop
	nop				# (BD) 
...
3:	#
	...
	bne	v0, ta3, 9f		# Return if values don't match 
	move	ta3, zero		# (BD)

	jal	ccuart_getc		# Get response type
	li	ta3, SC_PANEL		# (BD) 
	bne	ta3, v0, 9f		#  Return if values don't match 
	move	ta3, zero		# (BD) Return zero on error
