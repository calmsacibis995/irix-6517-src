#ident	"$Id: parser.s,v 1.1 1994/07/21 01:15:12 davidl Exp $"
/**************************************************************************
 *									  *
...
/*
 * comamnd_parser: void cmd_parser(int menu_level)
 *
 * Function Description:
 * At the monitor prompt, the user enters either a number to run
 * a test in the test table or a command name.
 *
 * The passed parameter "menu_level" is zero if the current menu
 * is the main menu, otherwise it is a test module menu.
 *
 ...
 * 	s1 - test number
 *	s2 - menu_level (0 - main menu, otherwise test module menu)
 *
 * registers used:
 *	a0, v0, t0, s0-s2
 */
...
LEAF(cmd_parser)
	move	s2, a0			# save the menu level param.
_getinput:
	#
	# print command prompt
	#
	PRINT("\nDiag")
	beq	s2, zero, 1f		# branch if main menu
	PRINT("/")			#   else print module name
...
1:
	PRINT(" >>")
	jal	_get_cmdstr		# get a cmd string into nvram buffer
	beq	v0,zero,_getinput	# wait until something entered

	li	a0,_NVBUF		# get addr ptr to nvram cmd buf
	jal	addr_nvram
	move	s0,v0			# s0 is ptr to nvram cmd string buf 
...
_checkinput:
	...
	beq	t0,char_0,_cmd_invalid	# illegal if test number = 0
	 ...
	 */
	bne	s2, zero, _realtest	# branch if test module menu
...
2:
	...
	beq	v0,zero,_parser_exit	# Command Return Status:
					#   0-Ok, 1-Invalid cmd syntax
	PRINT("\nSyntax error\n")
	j	_parser_exit
...
LEAF(_get_cmdstr)
	...
	li	a0,_NVBUF		# get addr ptr to nvram buf
	jal	addr_nvram
...
_getcmd_done:
	...
	beq	t1, zero, 1f		# keep previous cmd if no new command
