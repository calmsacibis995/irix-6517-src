#ident	"$Id: dtag_mats.s,v 1.1 1994/07/21 01:25:06 davidl Exp $"
/**************************************************************************
 *									  *
...
#include "cache_err.h"
 ...
 #									      #
 #	Tests data cache tag for stuck-at faults.			      #
 #									      #
 #	User Information:						      #
 #									      #
...
LEAF(Dtag_mats)
	 ...
	 */
	li	t0,0x55555540		# alternating one's pattern
	li	t1,0xaaaaaa80		# another alternating one's pattern
	...
	nop
1:
	cache	CACH_PD | C_IST,(a0)	# store 5's to tag
	nop
	...
	nop
	cache	CACH_PD | C_IST,(a0)	# store a's
	nop
	addu	a0,a0,t5		# next tag
...
1:
	...
	nop
	and	a2,a2,0xfffffffe	# ignore the parity bit
	bne	a2,a1,2f		# is it 5's?
	nop

	mtc0	t1,C0_TAGLO		# load TAGLO with a's
	nop
	cache	CACH_PD | C_IST,(a0)	# store a's to tag
	nop
	addu	a0,a0,t5		# next tag
	blt	a0,t6,1b		# reach last tag yet?
	nop
...
1:
	cache	CACH_PD | C_ILT,(a0)	# load TAGLO for cache tag
	nop
	 ...
	 */
	cache	CACH_PD | C_ILT,(a0)	# load TAGLO from tag
	nop
	mfc0	a2,C0_TAGLO		# load a2 from TAGLO
	nop
	and	a2,a2,0xfffffffe	# ignore the parity bit
