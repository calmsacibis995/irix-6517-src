...
static char bad_epc[]=		"EPC CHIP FAILED!";
...
static char nomc3_long[]=
	"The IP19 PROM did not recognize any memory boards in the system.";
static char noio4_long[]=
	"The IP19 PROM did not recognize any IO4 boards in the system.";
...
pod_scmsg_t scmsg_map[] = {
    ...
    {EVDIAG_EPCREG_FAILED,	bad_epc,		badepc_long},
    {EVDIAG_LOOPBACK_FAILED,	"EPC UART FAILED!",	(char *)NULL},
    {EVDIAG_EPCREG_BUSERR,	bad_epc,		badepc_long},
...
void scroll_message(char *message, int reps)
{
	int display_size;
	int length;
	int i;
	...
	while (!pod_poll() && (reps != 0)) {
		if (i < display_size) {
			...
			sysctlr_bootstat(buffer, display_size - i);
		} else {
...
char *get_long_scmsg(unsigned char diagcode) {
    int num_entries;
    int i;
...
char *get_diag_string(unsigned int diagcode) {
    int num_entries;
    int i;
...
main()
{
    ...
    printf("Everest IP19 PROM Diagnostic Codes and Their Meanings\n\n");
