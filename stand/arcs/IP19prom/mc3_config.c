...
check_banks(int num_banks, evbnkcfg_t *bank[], uint action)
{
	...
	if (setfault(fault_buf, &prev_fault)) {
	    ...
	    loprintf("\n*** Bus error occurred while checking mem board %d\n",
							bank[b]->bnk_slot);
	    if (addr_valid) {
		loprintf("    Error in ");
...
{
    int 	mem_slots;	/* The results of the SYSCONFIG op */
    ...
    int		retval;		/* Return value: nonzero == error */
    int		intdest;	/* Interrupt destination */
     ...
     * we go through the banks array and switch on banks which are present
