...
struct obj_inf {
	...
	unsigned int scn_addr[NUM_SECTS];
	unsigned int scn_end[NUM_SECTS];
	unsigned int *scn_ptr[NUM_SECTS];
} obj;
...
static void	elf32_get_entry(Elf *, char *, unsigned int *);
static void	elf64_get_entry(Elf *, char *, unsigned int *);
...
static void
elf32_get_entry(Elf *elf, char *name, unsigned int *value)
{
	Elf_Scn *scn;
...
static void
elf64_get_entry(Elf *elf, char *name, unsigned int *value)
{
	Elf_Scn *scn;
	int ndx, i, symtabsize;
	...
	for(i = 0; i < symtabsize; i++)
	if (strcmp(name,
		   elf_strptr(elf, symtabshdr64->sh_link, symtab[i].st_name))
	    == 0)
		break;
		
	if (i == symtabshdr64->sh_info) {
		fprintf(stderr, "nextract: Fatal error - couln't find symbol: %s\n", name);
		exit(1);
	}
	else
...
{
	int i, j, k;
	int high_addr, low_addr;
	int real_num = NUM_REAL_SECTS;
	unsigned int cur_addr, cur_scn;
	unsigned line_buf[LINE_SIZE];
	unsigned cur_index, cur_scn_ptr, next_scn_start, cur_scn_end;
	int cur_line;
	int scn_swizzle;
