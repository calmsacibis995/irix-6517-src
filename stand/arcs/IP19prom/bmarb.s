...
#define SYSCTLR_TIMEOUT 2000000		/* Number of cycles to wait */
...
LEAF(arb_bootmaster)
	 ...
	 * others.
	 */
	li	t0, EV_RTC		# Load address of Realtime clock
...
rearb_bootmaster:
	...
	li	t3, EV_IP0		# Load the interrupt pending addr
78:					# DO
	ld	t4, 0(t3)		#   Get Interrupt Pending mask
	nop				#
	and	t4, (1 << BOOT_LEVEL)	#   Check to see if intr arrived 
	beq	t4, zero, 78b		# LOOP UNTIL int or timer expired
	nop				# (BD)
...
1:					# DO
	...
	nop				#
	and	t4, (1 << BOOT_LEVEL)	#   Check to see if intr arrived 
	beq	t4, zero, 1b		# LOOP UNTIL int or timer expired
...
2:
	 ...
	 */
	li	t0, EV_IGRMASK		# Load the Int Group Mask addr
