#ident	"IP22prom/lmem_conf.s:  $Revision: 1.26 $"
 ...
 * this requires setting up the tlb before any memory access
 */
#define	HIGH_MEM_BASE		0x20000000	# base of high local memory,
						#	512M
#define	LOW_MEM_TOP		0x18000000	# top of low local memory, 384M
...
LEAF(init_memconfig)
	...
	.set	noreorder
	mfc0	v0,C0_SR	# enable the fpu such that it's registers set
	nop			#	can be used
	li	v1,SR_CU1
	or	v0,v1
	mtc0	v0,C0_SR
...
init_mem_restart:
	...
	jal	map_high_memory		# map high local memory for probing
...
1:
	...
	nop
	.set	reorder
	li	a0,HIGH_MEM_BASE	# starting addr
...
1:
	...
	lw	a1, 0(a0)		# first make sure DMA starts
	and	a2, a1, 0x40
	beqz	a2, vdma_err1

1:	lw	a1, 0(a0)		# now wait for DMA to finish
	and	a2, a1, 0x40
	bnez	a2, 1b
	...
	beq	v0,zero,memory_cleared	# done if there's noting in high 
					#	local memory
	LI	a0,PHYS_TO_K1(DMA_MEMADR)
	li	a1,HIGH_MEM_BASE	# start of high local memory
	sw	a1,0(a0)		# starting high local memory location
...
1:	cache	CACH_SD|C_IST,0(v0)
	...
	mfc0	v0,C0_SR	# disable the fpu now we don't need it
	li	v1,~SR_CU1	# any more
	and	v0,v1
	mtc0	v0,C0_SR
	mtc0	zero,C0_PGMASK  # restore 4K page mask
	.set	reorder
...
#define	VDMA_PRINTREG(str,reg)	\
	...
	VDMA_PRINTREG(vmda_er_gea, GIO_ERR_ADDR)
 ...
 */
#if IP24		/* define Simm Code for translation */
#define	_SC0	1
#define	_SC1	2
#define	_SC2	3
#define	_SC3	4
...
LEAF(memflash)
	# enable power off switch interrupt
#ifdef IP24
	LI	T00,PHYS_TO_K1(HPC3_INT3_ADDR)	# use INT3
#else
	LI	T00,PHYS_TO_K1(HPC3_INT3_ADDR)	# assume IOC1/INT3
	IS_IOC1(T01)
	bnez	T01, 1f				# branch if IOC1/INT3
...
#if _MIPS_SIM == _ABI64
	li	T00,(SR_KX|SR_IE|SR_BEV|SR_IBIT4|SR_CU0)
#else
	li	T00,(SR_IE|SR_BEV|SR_IBIT4|SR_CU0)
#endif
	.set	noreorder
	mtc0	T00,C0_SR
	.set	reorder
...
#ifdef IP24
	li	T01,KBD_MS_RESET|PAR_RESET|VIDEO_RESET|ISDN_RESET|LED_GREEN_OFF
#else
...
#define	DATA	0x11111111
	...
	li	a0,HIGH_MEM_BASE	# starting addr to probe
	li	a1,DATA
...
probe_subbank:
	...
	li	a0,HIGH_MEM_BASE
	srl	a1,T20,1
	addu	a0,a1			# first addr in the 2nd subbank
	li	a1,DATA
...
/*
 * map 512M into high local memory for sizing, starting from HIGH_MEM_BASE.
 * virtual address is equivalent to physical address in this case.
 * cannot use low local memory because 512M bank doesn't fit there and
 * 256M bank doesn't align correctly
 *
 * this is a level 0 routine
 */
LEAF(map_high_memory)
	.set	noreorder
	li	a0,0x01ffe000		# set up page mask to map 32M per
	mtc0	a0,C0_PGMASK		# 	tlb entry, 2 16M pages

	mfc1	v0,MAXSIZE_REG
	li	a1,0x02000000		# 32M increment
	li	a3,0x40000		# 16M bit in entrylo's pfn field
	li	a0,HIGH_MEM_BASE	# base addr of high local memory
	srl	v0,25			# number of tlb entries needed
	move	v1,zero			# tlb table index

1:
	mtc0	v1,C0_INX		# tlb index
	mtc0	a0,C0_TLBHI		# tlb entry high, VPN
	srl	a2,a0,TLBLO_PFNSHIFT	# PFN field in tlblo
	or	a2,TLBLO_UNCACHED|TLBLO_D|TLBLO_V|TLBLO_G
	TLBLO_FIX_250MHz(C0_TLBLO_0)	# 250MHz R4K workaround
	TLBLO_FIX_250MHz(C0_TLBLO_1)
	mtc0	a2,C0_TLBLO_0		# tlb entry lo0, pfn
	addu	a2,a3			# next 16M page
	mtc0	a2,C0_TLBLO_1		# tlb entry lo1, pfn
	nop
	c0	C0_WRITEI		# write entry

	add	v1,1			# point to the next tlb entry
	bne	v1,v0,1b		# branch id not done with all entries
	addu	a0,a1			# branch delay slot, next 32M
...
#endif
	move	a0,ra
	jal	setup_regs		# setup registers depending
	nop				# on revision of MC
	move	ra,a0
...
vdma_er2m:
	.asciiz "\r\nVDMA Clear aborted due to erro, cause: "
