/*
 * File: mc3_config.c
 * Purpose: Contains various routines which are used to configure and
...
extern void	print_slot_bank(__uint64_t, __uint64_t);
 ...
 *		   fastest memory configuration, but the response time 
...
check_banks(int num_banks, evbnkcfg_t *bank[], uint action)
{
    ...
    ulong	end_o_blk;	/* Last bloc of an interleave */
    evreg_t	ertoip;		/* contents of ertoip register on buserr */
    ulong	addr_bloc;	/* Bloc of address being tested */
    ulong	addr_off;	/* Offset of address being tested */
    ulong	addr_valid;	/* The addr_* numbers are valid */
	...
	if (setfault(fault_buf, &prev_fault)) {
		...
		xlate_ertoip(ertoip);
	    loprintf("    EPC %x CAUSE %x BADVADDR %x SCCADDR %x\n", 
		     _epc(0, 0),_cause(0, 0), _badvaddr(0, 0), LD(EV_ERADDR));
	    if (addr_valid) {
		loprintf("    Error in ");
...
mc3_config(evcfginfo_t *cfginfo)
{
	...
	if (cfginfo->ecfg_board[slot].eb_type == EVTYPE_MC3) {
	    ...
	    type = (int)EV_GET_CONFIG(slot, MC3_TYPE);
		...
		for (bank = 0; bank < MC3_BANKS_PER_LEAF; bank++) {
		    DPRINTF(("  Configuring leaf %d, bank %d...", 
			     leaf, bank));
		    
		    banks[bpos] = 
			&(cfginfo->ecfg_board[slot].eb_banks[4*leaf + bank]);
		    banks[bpos]->bnk_size = (int)
			EV_GET_CONFIG(slot, MC3_BANK(leaf, bank,BANK_SIZE));
