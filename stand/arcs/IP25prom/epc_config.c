...
io4_initmaster(evcfginfo_t *evcfg)
{
	...
	if (evcfg->ecfg_board[slot].eb_type == EVTYPE_IO4) {
		...
		(u_char)(EV_GET_CONFIG(slot, IO4_CONF_REVTYPE) &
...
{
    uint iodev[2];		/* The I/O device configuration registers */ 
    int i;			/* IOA index */
    ...
    iodev[0] = (uint)EV_GET_CONFIG(slot, IO4_CONF_IODEV0);
    iodev[1] = (uint)EV_GET_CONFIG(slot, IO4_CONF_IODEV1);
...
find_epc(evbrdinfo_t *io4info)
{
	...
	if (io4info->eb_ioarr[i].ioa_type == IO4_ADAP_EPC) {
	    ...
	    set_BSR((i << BSR_EPCIOA_SHFT) | get_BSR());
...
{
    return ((get_BSR() & BSR_EPCIOA_MASK) >> BSR_EPCIOA_SHFT);
}
