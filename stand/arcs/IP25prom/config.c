...
init_cfginfo(evcfginfo_t *cfginfo)
{
    ...
    DPRINTF(("init_cfginfo: init_cpuSpeed()\n"));
    cfginfo->ecfg_clkfreq = initCPUSpeed();
	 ...
	 */ 
		...
		type = (uint)EV_GET_CONFIG(i, EV_A_BOARD_TYPE);
		DPRINTF(("CPU: Slot %x type %x\n", i, type));
		if (type == EV_IP19_BOARD) 
		    cfginfo->ecfg_board[i].eb_type = EVTYPE_IP25;
		else  
		    cfginfo->ecfg_board[i].eb_type = EVTYPE_WEIRDCPU;
		 cfginfo->ecfg_board[i].eb_rev = 
		     (unsigned char)EV_GET_CONFIG(i, EV_A_LEVEL);

	    } else if (config_hi & (1 << i)) {

		/* Slot contains a memory board */
		type = (uint)EV_GET_CONFIG(i, MC3_TYPE);
		...
		/* Check for an IO4 */
		type = (uint)EV_GET_CONFIG(i, IO4_CONF_REVTYPE);
    ...
    cfginfo->ecfg_debugsw = (uint)sysctlr_getdebug();
}
...
set_timeouts(evcfginfo_t *cfginfo)
{
	...
	switch (brd->eb_type) {
	  ...
	  default:
	    ccloprintf("Unknown board type (%d) encountered (slot %d).\n", 
		       brd->eb_type, slot);
	    break;
	}
    }
...
init_inventory(evcfginfo_t *cfginfo, uint nv_valid)
{
	...
	switch (brd->eb_type) {
	  ...
	  default:
	    loprintf("*** Warning: unexpected board type %d\n", brd->eb_type);
	    /* Bad stuff */
	    break;
	}
    } 
...
configure_cpus(evcfginfo_t *info)
{
    ...
    slot =  (uint)((LD(EV_SPNUM) & EV_SLOTNUM_MASK) >> EV_SLOTNUM_SHFT);
    slice = (uint)((LD(EV_SPNUM) & EV_PROCNUM_MASK) >> EV_PROCNUM_SHFT);

    for (i = 1; i < EV_MAX_SLOTS; i++) {
	brd = &(info->ecfg_board[i]);

	/* Skip non-IP25 boards */
        if (brd->eb_type != EVTYPE_IP25)
	    continue;
...
init_mpconf(void)
{
    extern void *get_epcuart_base(void);
    ...
    evcfginfo = EVCFGINFO;
    ppid = (uint)(LD(EV_SPNUM) & EV_SPNUM_MASK);
    ...
    evcfginfo->ecfg_board[slot].eb_cpuarr[proc].cpu_speed = initCPUSpeed();
 
    /* Set this processor's cache size information */

    scachesz = (__uint32_t)EV_GETCONFIG_REG(slot, proc,EV_CACHE_SZ);
    if (scachesz > (sizeof(log2vals)/sizeof(log2vals[0]))
		    || log2vals[scachesz] == 0) {
	loprintf("*** slot %d proc %d Invalid cache size (%x), "
		 "assuming 1 MB.\n",
		 slot,proc, scachesz);
	scachesz = 3;
    }
    ...
    mpconf->pr_id = (int)readCP0(C0_PRID);
