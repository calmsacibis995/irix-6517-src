...
int pod_check_rawmem(int num_banks, evbnkcfg_t *bank_arr[])
{
	...
	for (i = 0; i < num_banks; i += 8) {
	    DPRINTF(("Starting BIST on slot %b\n", bank_arr[i]->bnk_slot));
	...
	/*
	 * The MC3 will set both bits to 0 for a short period between
	 * BIST0 and BIST1, thus, wait a long time for them to complete.
	 * Then try polling.
	 *
	 * This fixes a bug where seeing both bits 0 allows the prom 
	 * to continue, and then run into errors.
	 */
	...
	for (i = 0; i < num_banks; i++) {
		...
		if ((bank_arr[i]->bnk_size != MC3_NOBANK) &&
			...
			DPRINTF(("Turning on slot %d, leaf %d bank %d\n", 
				 bank_arr[i]->bnk_slot, leaf, bank));
			EV_SET_CONFIG(bank_arr[i]->bnk_slot, MC3_BANKENB,
								status);
		} else {
		    DPRINTF(("Turning OFF slot %d, leaf %d bank %d\n", 
			     bank_arr[i]->bnk_slot, leaf, bank));
			status = EV_GET_CONFIG(bank_arr[i]->bnk_slot,
								MC3_BANKENB);
			status &= ~(MC3_BENB(leaf, bank));
					...
					uint i_position, uint simm_type)
{
	...
	DPRINTF(("base = %x, if = %d, ip = %d, type = %d",
...
static char bank_letter(uint leaf, uint bank)
{
	if ((leaf > 1) || (bank > 3))
		return '?';
...
int 
disable_slot_bank(__uint64_t bloc, __uint64_t off, evbnkcfg_t *banks[], 
		  int num_banks, uint diagval)
{
	uint slot;
	uint bank;
	uint simm;
	...
	for (b = 0; b < num_banks; b += 8) {
		if (banks[b]->bnk_slot == slot) {
			if (banks[b + bank]->bnk_slot != slot) {
				loprintf("*** IP25 prom logic error: "
					 "disable_slot_bank()!\n");
				return -1;
			}

			if (banks[b + bank]->bnk_enable == 0) {
				loprintf("*** Slot %a, leaf %d, bank %d "
					 "ALREADY DISABLED!\n",
				slot, bank >> 2, bank & 3);
				return -1;
			}
...
static int addr2sbs(uint bloc, uint offset, uint *slot, uint *bank, uint *simm)
{
	...
	if (mem_slots & (1 << s)) {
		...
		/* Nonzero means enabled */
			...
			*slot = s;
			...
			DPRINTF(("Winner is slot %a, leaf %d, bank %d\n",
						s, l, b));
