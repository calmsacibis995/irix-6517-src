/*
 * File: pod_io.c
 * Purpose:    High level printf/getc routines for pod - use either CC uart 
 *		or EPC usrt.
 *
...
void loprf(void (*putc)(char), char *fmt, va_list adx)
{
	...
	for (;;) {
		...
		case 'b':	/* Print a hex byte. */
			...
			loputs(putc, prbuf, 0);
			break;
		case 'l':	/* Print a variable-length hex number */
			/* Note: this supports %llx. */
			c = EVCFLIP(dosw, fmt++);
			c = EVCFLIP(dosw, fmt++);
...
void lo_sprintn(char *tgt, __scunsigned_t n, int b, int digits)
{
	...
	do {
		if (getendian())	/* 1 = little */
			*cp++ = get_char((u_char *)"32107654ba98fedc" + n%b);
		else
			*cp++ = get_char((u_char *)"0123456789abcdef" + n%b);
		n /= b;
		if (digits)
			digits--;
	} while (n || digits);
...
logets(int *buf, int max_length)
{
	...
	/*NOTREACHED*/
	return (int *)0;	/* to keep the compiler happy */
}
...
char *
logetstring(char *buf, int max_length)
{
	char c;
	char *bufp;
