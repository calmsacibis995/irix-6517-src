 ...
 * the following flow:
 *
 * Write 0 to all addresses. Read 0, write 1, read 1 in ascending order. 
 * Read1, write 0, read 0 in descending order. Read 0.
 * This will detect address decoder faults, stuck-at-faults, transition faults,
 * coupling faulst, and linked transition faults.
 */
...
#if _MIPS_SIM == _ABI64
extern int dblmarchY(struct memstr *, int);
#else
extern int dblmarchY(struct memstr *, int, unsigned long long);
#endif
...
dblmarch_y()
{
    ...
    msg_printf(SUM, "Double Word March Y test\n");
    if (quick_mode()) {
	...
	/* memsize = 0x2000000; */
#undef PHYS_CHECK_LO
#define PHYS_CHECK_LO 0x2000000
#endif
...
SLOTFAILED:
	...
	msg_printf(ERR, "Double Word March Y test detected non-zero MC3 error register contents\n");
