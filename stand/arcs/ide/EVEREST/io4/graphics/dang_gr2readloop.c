...
/************************************************************************
 *									*
 *	dang_gr2readloop.c - dang chip gio bus read scope loop		*
 *									*
 ************************************************************************/
...
int
dang_gr2readloop(int argc, char** argv)
{
    unsigned int gio_address, count;
    ...
    msg_printf(INFO,
	"\ndang_gr2readloop - DANG/GIO Gr2 data read scope loop\n");
    ...
    if (argc != 5) 
    {
	msg_printf(ERR,"dang_gr2readloop: bad command line format\n");
	msg_printf(VRB,"usage:\n");
	msg_printf(VRB,
	    "\tdang_gr2readloop slot# adap# gio_address loopcount\n");
	return (1);
    }
    ...
    /*
     * read gr2 at location gio_address "count" times
     */
    while (count--)
	pattern = ((unsigned int *)gio_va)[gio_address];
