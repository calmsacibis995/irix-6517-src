...
/************************************************************************
 *									*
 *	dang_gr2writeloop.c - dang chip gio bus write scope loop	*
 *									*
 ************************************************************************/
...
int
dang_gr2writeloop(int argc, char** argv)
{
    ...
    msg_printf(INFO,
	"\ndang_gr2writeloop - DANG/GIO Gr2 data write scope loop\n");
    ...
    if (argc != 6) 
    {
	msg_printf(ERR,"dang_gr2writeloop: bad command line format\n");
	msg_printf(VRB,"usage:\n");
	msg_printf(VRB,
	    "\tdang_gr2writeloop slot# adap# gio_address pattern count\n");
	return (1);
    }
    ...
    /*
     * write "pattern" to address "gio_address" "count" times
     */
    while (count--)
