...
/*    SetSR((osr | SR_CU1) & ~SR_FR & ~SR_KX); */
    ...
    if (i1 != TESTBAK){
	err_msg(FMULDIVD_DIV, cpu_loc, TESTBAK, i1);
	retval = 1;
    }
    ...
    f6 = f5 * f1;
    if (f6 != f2){
	err_msg(FMULDIVD_MUL, cpu_loc, f2, f6);
	retval = 1;
    }
    SetSR(osr);
    msg_printf(INFO, "Completed double precision FP mul/div\n");
