...
fmulsubd()
{
    ...
    f3 = f2 - f1;
    if (f3 != f1) {
	err_msg(FMULSUBD_RES, cpu_loc, f1, f3);
	retval = 1;
    }

    i1 = f3;		/* convert back to fixed point number */
    if (i1 != TESTPAT){
	err_msg(FMULSUBD_FIX, cpu_loc, TESTPAT, i1);
	retval = 1;
    }

    if (GetFPSR() & ~FPSR_COND)
    {
	err_msg(FMULSUBD_STS, cpu_loc, GetFPSR());
	retval = 1;
    }
    SetSR(osr);
    msg_printf(INFO, "Completed double precision FP mul/sub\n");
