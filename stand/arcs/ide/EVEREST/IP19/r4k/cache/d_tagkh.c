 ...
 *	Diag name: PdTagKh
 *
 *	Description: This diag tests the data integrity of the primary data
 *		     cache tag ram with the Knaizuk Hartmann algorithm. It
 *		     treats the tag ram array as a ordinary memory array.
...
void d_tagkh_error(uint, uint, uint);
...
PdTagKh()
{
	 ...
	 */
	oneloc = config_info.dline_size;
	twolocs = config_info.dline_size * 2;
	threelocs = config_info.dline_size * 3;

	first_addr = 0x80000000;
	last_addr = first_addr + config_info.dcache_size - oneloc;
	...
	for (ptr = first_addr + oneloc ; ptr <= last_addr; ptr += twolocs) {
		...
		IndxStorTag_D(ptr, local_expected_data);
	...
	for (ptr = first_addr + oneloc; ptr <= last_addr; ptr += threelocs) {
		...
		last_value_read = ( IndxLoadTag_D(ptr) & dmask);
		if (last_value_read != local_expected_data) {
			/*Eprintf("Partition 1 error after partition 0 set to 0xaaaaaaaa.\n");*/
			err_msg( DTAGKH_ERR1, cpu_loc);
			d_tagkh_error(ptr, local_expected_data, last_value_read);
			retval = FAIL;
		}
	}
	...
	for (ptr = first_addr; ptr <= last_addr; ptr += twolocs) {
		...
		if ((ptr <= last_addr) && ((last_value_read = (IndxLoadTag_D(ptr) & dmask) ) != local_expected_data)) {
			/*Eprintf("Partition 1 error after partition 1 set to 0xaaaaaaaa.\n");*/
			err_msg( DTAGKH_ERR4, cpu_loc);
			d_tagkh_error(ptr, local_expected_data, last_value_read);
			retval = FAIL;
		}
	}
	...
	msg_printf(INFO, "Completed primary data TAG Knaizuk Hartmann test\n");
	return(retval);
}



void d_tagkh_error(uint address, uint expected, uint actual)
{
