/*
 *
 * Copyright 1991,1992 Silicon Graphics, Inc.
 * All Rights Reserved.
 *
 * This is UNPUBLISHED PROPRIETARY SOURCE CODE of Silicon Graphics, Inc.;
 * the contents of this file may not be disclosed to third parties, copied or
 * duplicated in any form, in whole or in part, without the prior written
 * permission of Silicon Graphics, Inc.
 *
 * RESTRICTED RIGHTS LEGEND:
 * Use, duplication or disclosure by the Government is subject to restrictions
 * as set forth in subdivision (c)(1)(ii) of the Rights in Technical Data
 * and Computer Software clause at DFARS 252.227-7013, and/or in similar or
 * successor clauses in the FAR, DOD or NASA FAR Supplement. Unpublished -
 * rights reserved under the Copyright Laws of the United States.
 */

#ident "$Revision: 1.5 $"
/*
 *	This test verifies the block (4 words) write mode in data cache.
 *	It writes to K0 (0x80020000) cached space, causing the cache dirty.
 *	Then it replace the cache line by reading 0x80022000, different cache
 *	line with same offset.  This causes the data in 0x80020000 wrtie back 
 *	to memory which now has the same data as in 0x80020000.  Multiple
 *	cache lines are tested back to back.
 *
 *	5/17/90: Gus Wang wrote the original version.
 *	7/16/91: Richard Frias modified to run with secondary cache
 */
#include "menu.h"
#include "sys/types.h"
#include <ide_msg.h>
#include <everr_hints.h>
#include <ip19.h>
#include "prototypes.h"

static __psunsigned_t cpu_loc[2];

/* Set address to beyond 8 M */
#define TSTADR 0x800000
#define ALIASADR (TSTADR + config_info.sidcache_size)


/* 'end' defined by the compiler as the end of your code */
extern u_int end;
extern int scacherrx;


d_dirtywbw()
{
	register volatile u_int data;
	register volatile u_int *addr;
	__psunsigned_t osr;
	uint retval;

	retval = PASS;
	getcpu_loc(cpu_loc);
	msg_printf(INFO, " (d_dirtywbw) \n");
	msg_printf(DBG, "cpu_loc[0] %d  cpu_loc[1] %d\n", cpu_loc[0], cpu_loc[1]);

	osr = GetSR();
	/* Enable Cache error exceptions? */
	if( scacherrx == NO)
		SetSR(osr | SR_DE);
	else
		SetSR(osr & ~SR_DE);

	/* get cache sizes and invalidate caches */
	setconfig_info();

	/* init K1 uncached space */
	for (addr = (u_int *)PHYS_TO_K1(TSTADR);
		addr < (u_int *)PHYS_TO_K1(TSTADR + 0x20); addr++) {

		*addr = 0x5aa50f0f;
	}

	/* write to K0 cached space with unique data */
	/* Implied read of cache block before write */
	for (addr = (u_int *)PHYS_TO_K0(TSTADR),data=0;
	        addr < (u_int *)PHYS_TO_K0(TSTADR + 0x20);addr++,data += 0x11111111) {

		*addr = data;
	}

	/* read back data through uncached space, it should not write through */
	for (addr = (u_int *)PHYS_TO_K1(TSTADR);
		addr < (u_int *)PHYS_TO_K1(TSTADR + 0x20);addr++) {

		if (*addr != 0x5aa50f0f) {
			/*Eprintf("Unexpected Cache write through to memory\n");*/
			err_msg( DIRTYWBW_ERR1, cpu_loc);
			msg_printf( ERR, "addr %x, expected %x, actual %x, XOR %x\n",
				addr, 0x5aa50f0f, *addr, *addr ^ 0x5aa50f0f );
			msg_printf( ERR, "Seconday TAG %x\n", IndxLoadTag_SD((__psunsigned_t)addr) );
			retval = FAIL;
		}
	}

	/* init memory before reading it */
	for (addr = (u_int *)PHYS_TO_K1(ALIASADR);
		addr < (u_int *)PHYS_TO_K1(ALIASADR + 0x20);addr++) {

		*addr = 0xdeadbeef;
	}
	/* replace cache line, 4 words each */
	data = *(unsigned *)PHYS_TO_K0(ALIASADR);
	data = *(unsigned *)PHYS_TO_K0(ALIASADR + 0x10);

	/* read back data through uncached space, it should write back to memory */
	for (addr = (u_int *)PHYS_TO_K1(TSTADR),data=0;
		addr < (u_int *)PHYS_TO_K1(TSTADR + 0x20);addr++,data += 0x11111111) {

		if (*addr != data)  {
			/*Eprintf("Cache writeback did not occur on a dirty line\n");*/
			err_msg( DIRTYWBW_ERR2, cpu_loc);
			msg_printf( ERR, "addr %x\n", addr);
			msg_printf( ERR, "expected %x, actual %x, XOR %x\n", 
				data, *addr, data ^ *addr );
			msg_printf( ERR, "Seconday TAG %x\n", IndxLoadTag_SD((__psunsigned_t)addr) );
			retval = FAIL;
		}
	}
	SetSR(osr);
	ide_invalidate_caches();
	msg_printf(INFO, "Completed primary data dirty writeback word test\n");
	return(retval);

}
