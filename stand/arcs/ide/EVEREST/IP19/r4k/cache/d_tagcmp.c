...
/*=============================================================================
 *
 *	File Name: d_tagcmp.c
 *
 *	Diag name: d_tagcmp
 *
 *	Description: This diag tests the comparator at the D-cache tag for
...
void d_tagcmp_error(__psunsigned_t, uint, __psunsigned_t);
...
d_tagcmp()
{
	 ...
	 * "j" starts with the value of dcache_size which should be 8k for
	...
	for(i = 0; i < config_info.dcache_size; i += config_info.dline_size) {
		addr = K0BASE + i;
		for(j = config_info.dcache_size; j < 0x20000000; j = j << 1) {
			...
			IndxStorTag_D(addr, taglo | PState_Dirty_Exclusive);
			...
			if(ChkDTag_CS(addr, 1, PState_Dirty_Exclusive, config_info.dline_size)) {
				/*Eprintf("D-cache tag comparator did not detect a miss.\n");*/
				err_msg( DTAGCMP_ERR1, cpu_loc);
				d_tagcmp_error(addr, taglo >> TAG_PTAG_SHIFT,
						K0_TO_PHYS(addr) >> 12);
				retval = FAIL;
			}

			HitInv_D(addr | j, 1, config_info.dline_size);
			if(ChkDTag_CS(addr, 1, PState_Invalid, config_info.dline_size)) {
				/*Eprintf("D-cache tag comparator did not detect a hit.\n");*/
				err_msg( DTAGCMP_ERR2, cpu_loc);
				d_tagcmp_error(addr, taglo >> TAG_PTAG_SHIFT,
	...
	msg_printf(INFO, "Completed primary data TAG comparitor test\n");
	return(retval);
}


void d_tagcmp_error(__psunsigned_t tag_index_addr, uint ptag, __psunsigned_t pfn)
{
