...
int RadSetup(void)
{
    ...
    mask = (GIO64_ARB_EXP0_RT | GIO64_ARB_EXP0_MST);
    flags=  GIO64_ARB_EXP0_RT | GIO64_ARB_EXP0_MST ; 
    ...
    /* reset RAD by writing all 1's to the status register */
    ...
    /* Initialialize the latency register */

    RAD_CFG_WRITE(RAD_CFG_LATENCY,~0);


    /* Setup PCI Holdoff to one shot */
    
    RAD_WRITE(RAD_PCI_HOLDOFF,0);
    ...
    valueptr = (uint_t *)Map_to_K1(BYTE_COUNT);
    *valueptr = 256;
    ...
    RAD_WRITE(RAD_RESET,RAD_RESET_VALUE);
    RAD_WRITE(RAD_MISC_CONTROL,RAD_MISC_VALUE);
...
{ 
  ...
  msg_printf(DBG,"RadSetupDMA: Setup of base adress %x\n",
	     (uint_t)K1_TO_PHYS(baseAddr));
  RAD_WRITE(RadDMALoAdr[dmaType],(uint_t)K1_TO_PHYS(baseAddr));
  ...
  msg_printf(DBG,"RadSetupDMA: High address %x %x\n",RadDMAHiAdr[dmaType],
    ...
    */
	...
	RAD_WRITE(descriptorAddr,
		  (uint_t)K1_TO_PHYS((uint_t)baseAddr));  
	/* Hi Address  */
