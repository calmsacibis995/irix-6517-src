...
char **argv;
{
	...
	int patn1, patn2;
	int vdma_modes = DMA_WRITE | VDMA_LBURST | VDMA_FILL | VDMA_INCA;


   	msg_printf (VRB, "Virtual DMA memory fill test\n");
	...
	if (cache_mode) { /* Disable Snooping */
		...
		msg_printf (DBG, "lg1_memfill: disabling snoop\n");
	}
	else {
		msg_printf (DBG, "lg1_memfill: enabling snoop\n");
