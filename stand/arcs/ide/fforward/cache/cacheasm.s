...
getout:	.set	noreorder
	...
	nop
	nop
	CACHE_BARRIER		# probably not needed due to mfc0...
 ...
 * addr should always be in k0seg 
 */
NESTED(dpline_to_secondary, 16, ra)
	subu	sp, 16			# set up the stack frame so can save
	PTR_S	ra, 0(sp)		#   ra if calling flush2ndline
	.set	noreorder
	cache	CACH_PD|C_HWB, 0(a0)	# writeback the line
	NOP_0_1
	.set	reorder

	PTR_L	ra, 0(sp)		# restore the stack frame and return
	addu	sp, 16
	j	ra
	END(dpline_to_secondary)

#endif /* R4k cache tests only */
