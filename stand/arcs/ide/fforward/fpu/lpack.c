...
static int matgen(double *a, long lda, long n, double *b, double *norma);
static int dgefa(double *a, long lda, long n, long *ipvt, long *info);
static int dgesl(double *a, long lda, long n, long *ipvt, double *b);
static int daxpy(long n, double da, double *dx, double *dy);
static int dscal(long n, double da, double *dx);
static long idamax(long n, double *dx);
static double epslon(void);
static int dmxpy(long n1, double *y, long n2, long ldm, double *x,double *m);
...
int lpackd(void)
{
	...
	for(i=0;i<75;i++) {
		failed = lpackd_();
		if (failed) {
			msg_printf(VRB, "lpackd test failed\n");
			SetSR(oldSR);
			return(1);
		}
		/* if(Chkpe() == 1) return(1); */
...
*/
	...
	msg_printf(VRB, "lpackd test passed\n");
...
static int lpackd_(void)
{
    ...
    if ((a = (double*)malloc(WORKSIZE*WORKSIZE*sizeof(double))) != NULL &&
        (b = (double*)malloc(WORKSIZE*sizeof(double))) != NULL &&
        (x = (double*)malloc(WORKSIZE*sizeof(double))) != NULL &&
        (ipvt = (long*)malloc(WORKSIZE*sizeof(long))) != NULL) {
		...
		if ((1.0e0-x[0]) < 1.0e-13 && (1.0e0-x[n-1]) < 1.0e-13 && 
	  	x[0] < 1.0e0 && x[n-1] < 1.0e0) 
			...
			msg_printf(DBG, "lpackd failed\n");
			msg_printf(DBG, "  x[0] = %lf, x[n-1] = %lf\n", x[0], x[n-1]);
			msg_printf(DBG, "  Residn = %lf, Resid = %lf\n", residn, resid);
		}
	}
...
{
    /* System generated locals */
    long m_dim1, m_offset, i__1, i__2;
