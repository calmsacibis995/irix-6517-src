...
enable_scache(void)
{
	...
	tlbwired_sc(index, 0, vaddr, tlblo0, tlblo1); /* map cache control addr */
	index++;
...
u_int
sc_map_addr(int index, u_int *ptr)
{
	u_int vaddr, paddr, tlblo0, tlblo1, tlblo_attribute;
		
	tlblo_attribute = (TLBLO_D | TLBLO_V | TLBLO_G | TLBLO_NONCOHRNT);

	paddr = (u_int)ptr;
	vaddr = paddr; /* scache test addrs */
	...
	if(index>tlb_index_save)
	   tlb_index_save = index;	/* keep track of entries used */
...
disable_scache(void)
{
	...
	*scache_disable = 0; /* halfword write disables caceh */
	flush_pd_cache(); 
	msg_printf(DBG,"tlb_index_save = %x\n, memsize = %x\n, sa[1ff] = %x\n",
			tlb_index_save, memsize, sc_save_area[0x1ff]);

	for(i=0;i<=tlb_index_save;i++) /* invlaidate tlb entries used */
	{
	  msg_printf(DBG,"invalidate tlb index %x\n",i);
	  invaltlb(i);
	}

	set_pgmask(TLBPGMASK_MASK); /* restore page mask register */

	msg_printf(DBG,"enable K0 space cacheing\n");
...
write_block(int argc, char *argv[])
{
	...
	fill_ipline(addr);	/*fill line to insure an I-cache hit */
	write_ipline(addr);
	return(0);
}		
...
sc_mem_test(int argc, char *argv[])
{
	...
	msg_printf(DBG,"Start addr = %x, end addr = %x\n",p1,p2);
...
sc_hit_test(int argc, char *argv[])
{
	...
	p1 = (u_int *) (SC_BASE);
	p2 = (u_int *) (SC_BASE + SC_SIZE);
	msg_printf(DBG,"test from %x to %x\n",p1,p2);
...
sc_hit_n_miss(int argc, char *argv[])
{
	...
	msg_printf(VRB,"SC hit/miss test\n");
...
/************************************************************************/
/* This routine depends on the SCache logic cacheing a block write to 
 * non-existent memory, when it writes to the space an exception is generated
 * but when the same address is read back it hits the secondary cache. 
 */
u_int sc_probe_tag(volatile u_int *base_addr)
{
        jmp_buf faultbuf;
	...
	{
		msg_printf(DBG,"write exception occured\n");

		if (setjmp(faultbuf))
	   	{
		   msg_printf(DBG,"read exception occured\n");
               	   nofault = 0;
               	   return(FALSE);
	   	}
        	nofault = faultbuf;
...
sc_tag_ram(int argc, char *argv[])
{
	...
	register u_int	*p1, *p2, test_addr;
	u_int	mem_limit, offset1, offset2, pattern, temp, status=0, done=FALSE;

	msg_printf(VRB,"SC tag test\n");

	sc_save_vectors();

	pattern = 0x87654321;	/* a unique (hopefully) test pattern */

	msg_printf(DBG,"memsize = %x\n",memsize);
	mem_limit = memsize + PHYS_RAMBASE;

	for(offset1 = SC_OFFSET; offset1<mem_limit; offset1 = offset1 << 1)
	{
		p1 = (u_int *)(SC_BASE | offset1);
		p2 = (u_int *) PHYS_TO_K1(SC_BASE | offset1);

		if ((u_int)p1<mem_limit)
		{
		   sc_map_addr(3,p1);	/* map test address */
		   ...
		   *p1 = pattern;  /* special test pattern */
		   ...
		   for(offset2 = SC_OFFSET; offset2<mem_limit; offset2 = offset2<<1)
		   {
		
		   /* read from an address with one other bit flipped in the tag */
		      p1 = (u_int *)(test_addr ^ offset2);
		      if(((u_int)p1 < mem_limit && ((u_int)p1 >= SC_BASE)))
		      {
		   	   sc_map_addr(4,p1);	/* map alias address */
...
/************************************************************************/
	...
	{
		p1 = (u_int *)(test_addr | offset1);

		sc_map_addr(3,p1);	/* map test address */
		temp = sc_probe_tag(p1);
		if(!temp)
		{
		  msg_printf(ERR,"Tag error at addrss %x\n",
				p1);
		  status = -1;
		}
	}
...
u_int
sc_test_all(int argc, char *argv[])
{
	u_int status;

	if(0 == _r4600sc_sidcache_size)
	{
		msg_printf(ERR,"R4600SC secondary cache not detected\n");
		return(-1);
	}
	status = enable_scache();

	if (status == 0)
		status = sc_mem_test(argc,argv);

	if (status == 0)
		status = sc_miss_test(argc,argv);

	if (status == 0)
		status = sc_hit_test(argc,argv);

	if (status == 0)
		status = sc_hit_n_miss(argc,argv);

	if (status == 0)
		status = sc_tag_ram(argc,argv);

	if (status == 0)
		status = disable_scache();
	else
		disable_scache();
