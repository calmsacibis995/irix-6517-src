...
Kh_l(int first_address, int last_address)
{
...
busy(1);
	...
	for (ptr = first_addr; ptr <= last_addr; ptr += 3) {
		if ((last_value_read = *ptr) != local_expected_data) {
			error = 1;
memerr(ptr,local_expected_data, last_value_read, sizeof(u_int));
/*			ONERRCHK(4, err_mark, err_mode, loopflag, loop, done);*/
		}
	}
/*	LOOPCHK(4, err_mark, loopflag, loop);*/
...
memerr(ptr,local_expected_data, last_value_read, sizeof(u_int));
/*			ONERRCHK(5, err_mark, err_mode, loopflag, loop, done);*/
		}
	}
/*	LOOPCHK(5, err_mark, loopflag, loop);*/
