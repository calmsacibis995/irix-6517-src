 ...
 */
#ident "ide/godzilla/mem/threebit.c: $Revision 1.1$ " 
...
u_int *WriteLoop14(u_int *, u_int *, u_int, u_int, u_int);
...
bit3_err1:
	if ((received ) != (pattern1 ) ) {
		msg_printf(DBG,"Test Error 1\n");
		msg_printf(DBG,"Failing address %x\n", wrkptr-1);
		msg_printf(DBG,"expected %x, actual %x, Xor %x\n", 
...
static int
dwd_3bit(__psunsigned_t begadr, __psunsigned_t endadr, u_int pat1, u_int pat2,
	 u_int pat3, u_int mask, u_int errlimit, u_int ecc_on)
{
	u_int	r_code;
	u_int	rtn_errors= 0;
	u_int	max_errors;
