 ...
 *
 * for IP30: this is not used for bring-up diags or ide, but by mfg
 *		I started modifying it for IP30 specifics, commented out
 *		what I did not upgrade with NOTYET_XXX	XXX
 */
#ident "ide/godzilla/mem/checker.c:   $Revision: 1.3 $ "
...
int
checkerbd(u_int first, u_int last, struct tstvar *tstparms)
{
	__uint32_t	localMEMCFG0, localMEMCFG1, localMEMCFG2, localMEMCFG3;
	heartreg_t	localHEART_MEMCFG0, localHEART_MEMCFG1;
	...
	/* only test upper banks */ /* XXX IP30 */
	/* for IP30, there are 4 banks, 2 in MEMCFG0, 2 in MEMCFG1 */
	PIO_RD_REG_64(HEART_MEMCFG0, ~0x0, localHEART_MEMCFG0); /* XXX useless*/
	PIO_RD_REG_64(HEART_MEMCFG1, ~0x0, localHEART_MEMCFG1); /* XXX useless*/
	/* get memory configuration word for a bank */
	localMEMCFG0 = get_mem_conf(0);
	localMEMCFG1 = get_mem_conf(1);
	localMEMCFG2 = get_mem_conf(2);
	localMEMCFG3 = get_mem_conf(3);
#ifndef IP30	/* old XXX */
	...
	msg_printf(VRB, "MEMCFG0: 0x%x\n", localMEMCFG0);
	msg_printf(VRB, "MEMCFG1: 0x%x\n", localMEMCFG1);
	msg_printf(VRB, "MEMCFG2: 0x%x\n", localMEMCFG2);
	msg_printf(VRB, "MEMCFG3: 0x%x\n", localMEMCFG3);

#ifndef IP30    /* old XXX : same for bank1 */
	...
	msg_printf(DBG, "vld:0x%x & memcfg0:0x%x = %x\n", HEART_MEMCFG_VLD, 
		localMEMCFG0, (HEART_MEMCFG_VLD & localMEMCFG0));
	if ((HEART_MEMCFG_VLD & localMEMCFG0) == 0) { /* bank 0 is invalid */
		msg_printf(INFO,"Bank 0 is invalid or empty.\n");
	}
        else bank0 = 1;

	msg_printf(DBG, "vld:0x%x & memcfg0:0x%x = %x\n", HEART_MEMCFG_VLD, 
		localMEMCFG1, (HEART_MEMCFG_VLD & localMEMCFG1));
	if ((HEART_MEMCFG_VLD & localMEMCFG1) == 0) { /* bank 1 is invalid */
	...
	if (MEMCFG_VLD & localMEMCFG1)
		bank3 = 1;
#endif
	if (HEART_MEMCFG_VLD & localMEMCFG2) bank2 = 1;/* bank 2 is valid! */
	if (HEART_MEMCFG_VLD & localMEMCFG3) bank3 = 1;
   ...
   for (x = 1; x <= num_pats; x++) { 
	...
	for (i = start_bank; i < num_banks; i++) { 
	   ...
	   if (i == 1 || i == 3) 
		addr_mask = MEMCFG_ADDR_MASK; /* 0x00ff */	/* XXX */
	   else if (i == 2)
		addr_mask = ADDR_MASK_02;   /* XXX */
	   ...
	   bank_base_addr = (bank_base_addr << 22);
 	   ...
 	   for (j = 0; j < 4; j++) { /* write single-sided,1st side of double */
		   ...
		   chkrbd1simm_wr(bank_base_addr, j, x); 
	   }
	   ...
	   if (doublehigh) {
		...
		msg_printf(DBG,"DOUBLE HIGH WRITE, offset: 0x%x\n", offset);
			...
			msg_printf(INFO,"Writing bank %d, simm %d hi, pat:%d\n",
				i, j, x);
			chkrbd1simm_wr(bank_base_addr + offset, j, x);
	    	} 
	   } /* double */
	} /* foreach bank */	
	...
	for (i = start_bank; i < num_banks; i++) {
	   ...
	   bank_base_addr = (bank_base_addr << 22);
 	   ...
 	   for (j = 0; j < 4; j++) { /* rd single-sided, 1st side of double */
		   ...
		   fail1 |= chkrbd1simm_rd(bank_base_addr, j, x); 
	   }
	   ...
	   if (doublehigh) {
		...
		for (j = 0; j < 4; j++) { /* foreach simm double-sided */
			msg_printf(INFO,"Reading bank %d, simm %d hi, pat:%d\n",
				i, j, x);
			fail2 |= chkrbd1simm_rd(bank_base_addr + offset, j,x);
	        } 
	   }
	} /* foreach bank */	
   } /* foreach pattern */
...
int 
chkrbd1simm_wr(u_int first, int simm_num, int patnum)
{
	int i, j, k, tmp;
	...
	for (i = 0; i < num_rows; i++)
	   for (j = 0; j < num_cols; j++)
		unit_cell_array[i][j] = pat_array[2*patnum - 2 + i][j];
...
int 
chkrbd1simm_rd(u_int first, int simm_num, u_int patnum)
{
	int i, j, k, tmp;
	register int num_rows, num_cols;
	register int row_a, col_a;
