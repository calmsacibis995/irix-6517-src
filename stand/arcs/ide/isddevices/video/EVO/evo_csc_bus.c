...
evo_csc_BusTest(__uint32_t argc, char **argv)
{
	...
	return -1;
     ...
     if ( bus == 0 ){
     	errCode = CSC_ADDR_BUS_TEST ;
	msg_printf(SUM, "-->Evo_csc%d_addr_bus started\n",num);
     } else {
	errCode = CSC_DATA_BUS_TEST ;
	msg_printf(SUM, "-->Evo_csc%d_data_bus started\n",num);
     }
	...
	if ( errCode == CSC_ADDR_BUS_TEST){ 
		msg_printf(SUM, "-->Evo_csc%d_addr_bus passed\n",num);
	} else { 
		msg_printf(SUM, "-->Evo_csc%d_data_bus passed\n",num);
	}
     } else { 			/*errors*/
	if ( errCode == CSC_ADDR_BUS_TEST){ 
		msg_printf(SUM, "-->Evo_csc%d_addr_bus failed\n",num);
	} else { 
		msg_printf(SUM, "-->Evo_csc%d_data_bus failed\n",num);
	}
     }
