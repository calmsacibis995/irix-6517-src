...
__uint32_t  evo_VGI1(__uint32_t argc, char **argv);
	...
	__uint32_t evo_VGI1DMATest (__uint32_t vgi_num, __uint32_t chan, __uint32_t op,
				    ...
				    __uint32_t freeze, __uint32_t  yval, ulonglong_t inv, 
		 		    __uint32_t pgsiz, __uint32_t blank, __uint32_t dma);


		__uint32_t  _evo_VGI1DMAGetBufSize(void);
		__uint32_t  _evo_dmaPixelDepth (__uint32_t giofmt);

		__uint32_t  _evo_VGI1DMA(__uint32_t dma);
...
evo_VGI1(__uint32_t argc, char **argv)
{
    ...
    __uint32_t	m_buf = 1; /*flag to indicate whether buffer is to be allocated within this program*/
			   /* default is yes - buffer will be allocated within program*/
    __uint32_t	m_dma = 1; /*flag to indicate whether dma is to occur/otherwise only setup will be done*/
			   /*default is yes -  dma will occur*/
    ...
    evobase->nFields = 2;
    evobase->vgi1[VGI1_CHANNEL_1].stride = 0; /*for both channels*/
    evobase->vgi1[VGI1_CHANNEL_2].stride = 0;
    ...
    while (argc && argv[0][0]=='-' && argv[0][1]!='\0') {
        switch (argv[0][1]) {
			...
			if (argv[0][2]=='\0') {
                ...
                case 'n':
			if (argv[0][2]=='\0') {
			    atobu(&argv[1][0], &m_buf); 
			    argc--; argv++;
			} else {
			    atobu(&argv[0][2], &m_buf);
			}
                        break;
                case 'd':
			if (argv[0][2]=='\0') {
			    atobu(&argv[1][0], &m_dma); 
			    argc--; argv++;
			} else {
			    atobu(&argv[0][2], &m_dma);
			}
                        break;
	...
	-n --> allocate buffers 0-No 1-Yes\n\
	-d --> do dma 0-No 1-Yes\n\
	");
	return(-1);
    } else {
	 if (m_vgi_num == 1){
	 	msg_printf(SUM, "--> Evo_vgi1_A_dma started\n");
	 } else {
	 	msg_printf(SUM, "--> Evo_vgi1_B_dma started\n");
	 }
	 if(m_buf == 1){
	    ...
	    msg_printf(DBG, "leaving evo_VGI1..\n");
	    m_errors = evo_VGI1DMATest(m_vgi_num, m_chan, m_op, evobase->vidFmt, evobase->tvStd, 
	    evobase->nFields, evobase->vgi1[m_chan].stride, evobase->vin_round, evobase->vout_expd,
	    evobase->vout_freeze, m_yval, m_inv, evobase->pageSize, m_blank,m_dma);
    }
    if(m_errors == 0){
	 if (m_vgi_num == 1){
	 	msg_printf(SUM, "--> Evo_vgi1_A_dma passed\n");
	 } else {
	 	msg_printf(SUM, "--> Evo_vgi1_B_dma passed\n");
	 }
    } else {
	 if (m_vgi_num == 1){
	 	msg_printf(SUM, "--> Evo_vgi1_A_dma failed\n");
	 } else {
	 	msg_printf(SUM, "--> Evo_vgi1_B_dma failed\n");
	 }
    }
    return(m_errors);
...
evo_VGI1DMATest (__uint32_t vgi_num, __uint32_t chan, __uint32_t op, __uint32_t vidfmt, 
		 ...
		 __uint32_t pgsiz, __uint32_t blank, __uint32_t _dma)

{
    ...
    msg_printf(DBG, "entering evo_VGI!DMATest\n");
    msg_printf(DBG, "vginum %d chan %d op %d vidfmt %d tvStd %d nFields %d yval 0x%x\n",vgi_num,chan,op,vidfmt,tvStd,nFields,yval);
    ...
    if (vgi_num == 2) {
	EVO_SET_VGI1_2_BASE;
	evobase->vgi1_bridge_int_num = 0x20;
    } else { 
	EVO_SET_VGI1_1_BASE;
	evobase->vgi1_bridge_int_num = 0x10;
    }
    msg_printf(DBG, "evobase->vgi1_bridge_int_num 0x%x\n", evobase->vgi1_bridge_int_num);
 ...
 /*   EVO_W1 (evobase, MISC_REF_OFFSET_LO, LOBYTE(BOFFSET_CENTER));  XXXD Not required for EVO set default timing */
 /*   EVO_W1 (evobase, MISC_REF_OFFSET_HI, HIBYTE(BOFFSET_CENTER));  XXXD Not required for EVO set default timing */

    if (op & 0x1) {
	if (vgi_num == 1) {
    		/* Send Black Reference to both channels of VBAR1 for DMA READ */
		/* The path is BlackRef-> VBAR2 Crossfeed1 Output -> VGI1A (Input from VBAR2 Crossfeed 1 )*/
		EVO_SET_VBAR2_BASE;
		...
		EVO_SET_VBAR1_BASE;
    		...
    		EVO_VBAR_W1(evobase, EVOA_VGI1_CH1_OUT, xp); /*XXX Check VBAR1/2 Address*/
    		EVO_VBAR_W1(evobase, EVOA_VGI1_CH2_OUT, xp); /*XXX Check VBAR1/2 Address*/
		/* Load VBARA Video Output with VGI1 output*/ 
		xp = MUX_VGI1_CH1_IN;
		EVO_VBAR_W1(evobase, EVOA_VIDEO_CH1_OUT, xp);
		xp = MUX_VGI1_CH2_IN;
		EVO_VBAR_W1(evobase, EVOA_VIDEO_CH2_OUT, xp);
		...
		EVO_SET_VBAR2_BASE;
    		xp =  MUXB_BLACK_REF_IN;
    		EVO_VBAR_W1(evobase, EVOB_VGI1B_CH1_OUT, xp); /*XXX Check VBAR1/2 Address*/
    		EVO_VBAR_W1(evobase, EVOB_VGI1B_CH2_OUT, xp); /*XXX Check VBAR1/2 Address*/
	} 
	...
	if (tmp == VGI1_NO_VIDEO_INPUT) {
    	...
    	msg_printf(DBG, "Buffer: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
	...
	if (vgi_num == 1) {
    		/* Send Black Reference to both channels of VGI1-1 for DMA READ */
		/* The path is BlackRef-> VBAR2 Crossfeed1 Output -> VGI1A (Input from VBAR2 Crossfeed 1 )*/
		EVO_SET_VBAR2_BASE;
    		xp =  MUXB_BLACK_REF_IN;
    		EVO_VBAR_W1(evobase, EVOB_TO_VBARA_CH1, xp); /*XXX Check VBAR1/2 Address*/
		...
		EVO_VBAR_W1(evobase, EVOA_VIDEO_CH2_OUT, xp);
    		...
    		/* Send Black Reference to both channels of VGI1-2 for DMA READ */
		/* The path is BlackRef -> VGI1B CH1/2 In*/
		EVO_SET_VBAR2_BASE;
    		xp =  MUXB_BLACK_REF_IN;
    		EVO_VBAR_W1(evobase, EVOB_VGI1B_CH1_OUT, xp); /*XXX Check VBAR1/2 Address*/
...
__end_of_dma:
    	/* Ensure that the chip is in good shape to accept DMA requests before exiting*/
...
_evo_VGI1DMA(__uint32_t _dma)
{
    ...
    /* set up TV standard */
    /* EVO_W1(evobase, MISC_TV_STD, evobase->tvStd);  XXXD Not required for EVO */
    /* delay(300);  XXXD Not required for EVO */
    ...
    msg_printf(DBG, "Freezing video out chanOutCtrl 0x%x\n",
		chanOutCtrl);
 /*   EVO_W1 (evobase, chanOutCtrl, 0x1);  XXXD Not required for EVO */
...
_evo_VGI1BuildDMADescTable (uchar_t *pDesc)
{
    ...
    pvgi1->pDMADataPhys = (uchar_t *)kv_to_bridge32_dirmap(evobase->brbase,
    ...
    pvgi1->pFieldDescPhys = (uchar_t *)kv_to_bridge32_dirmap(evobase->brbase,
...
__uint32_t i, int_val, junk, _tmp;
ulonglong_t	vgi1Status;
...
PIO_REG_RD_64(BRIDGE_INT_STATUS,0xffffffff,int_val);
msg_printf(SUM, "Before dma programming bridge status 0x%x\n", int_val);
...
/* VGI1_W64(evobase, pvgi1->ch_int_enab_reg_offset, 0x2d0e, mask); */
...
do {
	...
	PIO_REG_RD_64(BRIDGE_INT_STATUS,0xffffffff,int_val);
	junk = int_val;
	int_val = int_val & (evobase->vgi1_bridge_int_num);
} while ( (!int_val) && (i < VGI1_POLL_LARGE_TIME_OUT));
us_delay(1000000);
VGI1_R64(evobase, pvgi1->ch_status_reg_offset, vgi1Status, ~0x0);
msg_printf(SUM, "vgi1 status reg 0x%x\n", vgi1Status);
msg_printf(DBG, "junk 0x%x\n", junk);
...
{
    __uint32_t		i, j, k, tmpError, errors = 0;
    evo_vgi1_info_t	*pvgi1 = &(evobase->vgi1[(evobase->chan)]);
    ulonglong_t		mask = ~0x0;
    ulonglong_t		temp1x  ;
...
#if 0
PIO_REG_RD_64(BRIDGE_INT_STATUS,0xffffffff,int_val);
msg_printf(SUM, "Before  S&P DMA bridge status 0x%x\n", int_val);
#endif
...
_start_dma:
    ...
    VGI1_R64(evobase, pvgi1->ch_status_reg_offset, evobase->VGI1DMAOpErrStatus, mask);
...
__end_of_process_dma:
    ...
    PIO_REG_RD_64(BRIDGE_INT_STATUS,0xffffffff,int_val);
    VGI1_R64(evobase, pvgi1->ch_status_reg_offset, evobase->VGI1DMAOpErrStatus, mask);
    msg_printf(SUM, "Bridge Int. Status 0x%x\n", int_val);
    msg_printf(SUM, "VGI1 Ch1 Status 0x%x\n", evobase->VGI1DMAOpErrStatus);
