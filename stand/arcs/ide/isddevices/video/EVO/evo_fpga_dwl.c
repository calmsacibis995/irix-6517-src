...
#define MAX_FILE_SIZE 70000 /* check the file */
...
evo_download_file(int argc, char **argv)
{
    ...
    unsigned short conf_word, stat_word, done_word, read_word, temp_word;
    ...
    for(byte_count=0; byte_count<MAX_FILE_SIZE; byte_count++){
	buf[byte_count] = -999;
    }
    byte_count=0;
...
*/
    ...
    *bp = '\0' ;
    ...
    i=0;
    byte_count=0;
    read_word = _evo_inhw(evobase->evo_misc+(__paddr_t)MISC_STATUS_CONF); /*read register first */
    read_word = read_word | 0x1ff;
    _evo_outhw(evobase->evo_misc+(__paddr_t)MISC_STATUS_CONF, read_word); /*write 1ff first */
    ...
    temp_word = (read_word & (~(conf_word))); /*ensures no other bits are touched*/ 
    _evo_outhw(evobase->evo_misc+(__paddr_t)MISC_STATUS_CONF, temp_word); /*drives pin low*/ 

    us_delay(50); /*hold pins low for 10 microseconds*/
    _evo_outhw(evobase->evo_misc+(__paddr_t)MISC_STATUS_CONF, read_word); /*return pin to high*/
#if 0
	...
	msg_printf(DBG, "Done word is 0x%x\n", done_word);
		...
		msg_printf(SUM, "WARNING: Altera Down Load Completed Before End of Buffer\n");
	...
	msg_printf(SUM, "Altera Down Load Successful \n");
    } 
    else {
		msg_printf(SUM, "Altera Down Load Incomplete \n");
		msg_printf(SUM, "\n %d Bytes downloaded\n", byte_count);
		errors++;
		Close(fd);
		return (0);
     ...
     Close(fd);
     msg_printf(DBG, "errors = %d, errcode is %d\n", errors, errCode);
