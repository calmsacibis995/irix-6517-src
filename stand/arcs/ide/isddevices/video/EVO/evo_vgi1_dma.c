...
__uint32_t  evo_VGI1DMATest (__uint32_t argc, char **argv);
 ...
 */
__uint32_t
evo_VGI1DMATest (__uint32_t argc, char **argv)
{
    __uint32_t	errors = 0;
    __uint32_t	badarg = 0;
    ...
    while (argc && argv[0][0]=='-' && argv[0][1]!='\0') {
        switch (argv[0][1]) {
			...
			if (argv[0][2]=='\0') {
                ...
                case 'f':
			if (argv[0][2]=='\0') {
			    atobu(&argv[1][0], &(evobase->nFields));
			    argc--; argv++;
			} else {
			...
			if (argv[0][2]=='\0') {
                ...
                case 'z':
			if (argv[0][2]=='\0') {
			    atobu(&argv[1][0], &(evobase->vout_freeze));
			    argc--; argv++;
			} else {
    ...
    if (badarg || argc || _evo_VGI1DMAValidateArgs(chan) ) {
	msg_printf(SUM, "\
	Usage: evo_vgidma -c [1|2] -o [1|2|3] -t [1|2|3|4] -v [0|1|2|3]\n\
	       -f [1|2] -z [0|1]\n\
	-c --> channel 1[2] \n\
...
/* Set giobase address to VGI_1 or VGI_2 */
	...
	if (vgi_num == 1) {
    		...
    		xp = EVO_VGI1_CH1_IN | BLACK_REF_OUT_SHIFT;
    		EVO_W1(evobase, MISC_MUX_ADDR, xp); /*XXX Check VBAR1/2 Address*/
    		xp = EVO_VGI1_CH2_IN | BLACK_REF_OUT_SHIFT;
    		EVO_W1(evobase, MISC_MUX_ADDR, xp); /*XXX Check VBAR1/2 Address*/
	...
	if (vgi_num == 1) {
    		/* Send Black Reference to both channels for DMA READ */
    		xp = EVO_VGI1_CH1_IN | VIDEO_IN_SHIFT; /*XXX Moosecam ?*/
    		EVO_W1(evobase, MISC_MUX_ADDR, xp);
    		xp = EVO_VGI1_CH2_IN | VIDEO_IN_SHIFT; /*XXX Moosecam ?*/
    		EVO_W1(evobase, MISC_MUX_ADDR, xp);
	} else {
		msg_printf(SUM, "How will the VGI1-2 get the Black ref ?\n");
	}
