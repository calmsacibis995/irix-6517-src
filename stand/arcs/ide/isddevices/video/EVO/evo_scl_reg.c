...
evo_scl_RegTest(__uint32_t argc, char **argv)
{
	...
	errCode = SCL_ADDR_TEST ;
	msg_printf(SUM, "--> Evo_scl%d_addr_bus started\n",num);
	...
	/* So to test only these addresses write 0-0x11 */
		...
		EVO_SCL_R1(evobase, i, rcv );
     	...
     	errCode = SCL_DATA_TEST ;
	msg_printf(SUM, "--> Evo_scl%d_data_bus started\n",num);
	...
	if ( test < 0 || test > 3 ) test = 3;
     ...
     if(errors == 0){				/*no errors*/
     	if ( errCode == SCL_ADDR_TEST) { 	
		msg_printf(SUM, "--> Evo_scl%d_addr_bus passed\n",num);
     	} else {
		msg_printf(SUM, "--> Evo_scl%d_data_bus passed\n",num);
	}
     } else {					/*errors*/
     	if ( errCode == SCL_ADDR_TEST) { 	
		msg_printf(SUM, "--> Evo_scl%d_addr_bus failed\n",num);
     	} else {
		msg_printf(SUM, "--> Evo_scl%d_data_bus failed\n",num);
	}
     }
