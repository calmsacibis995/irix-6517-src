...
int _mgv_cc1tovgi1loopx( unsigned char ch_in, unsigned char ch_out, unsigned char tv_std, unsigned format )
{
int i;
...
vout_patterns_t vid_gonogo[] = { WALK1_PTRN, WALK0_PTRN, ALT01_PTRN, ALT10_PTRN, 0xFF };
	...
	/* set Genlock reference to be freerun */
	GAL_IND_W1(mgvbase, GAL_REF_CTRL, 0);
	...
	/*Set active channel to pass-through,pass chroma,non-freeze,non-blank*/
        tmp = (ch_out == ONE) ? GAL_CH1_CTRL : GAL_CH2_CTRL;
	GAL_IND_W1(mgvbase, tmp, 0x01 );

	/* Set inactive channel to blank */
        tmp = (ch_out == ONE) ? GAL_CH2_CTRL : GAL_CH1_CTRL;
	GAL_IND_W1(mgvbase, tmp, 0x03 );

	/* set xp from CC1 output (pixels/alpha) to vid out channel */
        tmp = (ch_out == ONE) ? 0x98 : 0x97;
...
msg_printf(DBG,"\nPATTERN BUFFER SIZE: %d\nvid_data_p:%x\
	...
	msg_printf(JUST_DOIT,"\n\nWriting pattern to CC1 and video output channel %d", ch_out);
		...
		msg_printf(JUST_DOIT,"\nTV Std: %s\tCC1 Field: %s\tPattern: %s",
			asc_tv_std_p[ tv_std ],
			(i%2) ? "ODD":"EVEN",
			asc_bit_pat_p[ vid_gonogo[ image_blk ] ] );
		...
		/* check video input status (ie: confirm cable is connected) */
        	if( _mgv_check_vin_status( ch_in, tv_std ) )
                	return( 0 );
		...
		msg_printf(DBG,"\nsetting up parameters for DMA");
        	tmp = (ch_in == ONE) ? VGI1_CHANNEL_1 : VGI1_CHANNEL_2;
        	_mgv_setup_VGI1DMA(tmp, tv_std, format, 1,
                		read_field_in_p, VGI1_DMA_WRITE, 0);

		/* do the vidtomem DMA */
		msg_printf(JUST_DOIT,"\nReading the loopback video output at video input channel %d", ch_in);
		msg_printf(DBG,"\ndoing vidtomem DMA");
		if( _mgv_VGI1DMA() )
		{
			_mgv_VGI1DMADispErr();
...
for(i=0;i < 100;i++)
	msg_printf(SUM," %d:%2x/%2x ", i, *(send_field_out_p+i),  *((unsigned char*)(read_field_in_p+i)) );
		...
		_mgv_compare_byte_buffers(ysize[tv_std]/2, 
					ysize[tv_std]/2,
					send_field_out_p,
					read_field_in_p,
					0, 0);
...
/***************************************************************************
*
*	Function: mgv_cc1tovgi1x.c
*
*	Description: see top header description
*
****************************************************************************/

int mgv_cc1tovgi1x( int argc, char *argv[] )
{
unsigned char chan	= 0;
unsigned char  ch_in	= CHANNEL_DEFAULT;
unsigned char  ch_out	= CHANNEL_DEFAULT;
...
msg_printf(DBG,"Entering argc\n");
		...
		/* if valid input channel, change default */
		chan = atoi( argv[CHAN_IN_ARG] );
		if( (chan == 1) || (chan == 2) )
			ch_in = chan;

		/* if valid output channel, change default */
		chan = atoi( argv[CHAN_OUT_ARG] );
		if( (chan == 1) || (chan == 2) )
			ch_out = chan;
	...
	msg_printf(JUST_DOIT,"\n\tCC1 TO VGI1 8-BIT LOOPBACK TEST FOR OUTPUT CHANNEL %d TO INPUT CHANNEL %d\n",ch_out, ch_in);
	...
	if( std == ALL_STD )
	{
		for( std = 0; std < 4; std++)
		{
			status = _mgv_cc1tovgi1loopx(ch_in,ch_out,std, format);
			if(status)
				return(-1);
			delay( mseconds );
			status = _mgv_cc1tovgi1loopx(ch_in,ch_out,std, format);
	...
	msg_printf(SUM,"\n\nEND OF CC1 8-BIT LOOPBACK TEST FOR OUTPUT CHANNEL %d TO INPUT CHANNEL %d.\
			\n\n",ch_out, ch_in);
