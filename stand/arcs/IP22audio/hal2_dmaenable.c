 ...
 *  hal2_dmaenable.c
 *
 *     test DMA_ENABLE register in the hal2.
 *
 ************************************************************************/
...
hal2_dma_enable_test()
{
    ...
    printf("Entering HAL2_DMA_ENABLE test\n");
    ...
    for (j = 0; j < 1000; j++) {
	    ...
	    value = i;
	    *idr0 = value;
	    *iar  = HAL2_DMA_ENABLE_W;
	    while (*isr & HAL2_ISR_TSTATUS);

	    *idr0 = 0xbadbad;
	    *iar  = HAL2_DMA_ENABLE_R;
	    while (*isr & HAL2_ISR_TSTATUS);
	    readback[0] = *idr0;
	    if (readback[0] != value) {
		errors[value]++;
    ...
    printf("Exiting HAL2_DMA_ENABLE test\n");
}
