 ...
 *   hal2_line2aes.c -
 * 
 *      Loop the line input to the AES output. 
 * 
...
#define AES_RX_DMA_ENABLE     0x2
#define AES_TX_DMA_ENABLE     0x4
#define CODEC_A_DMA_ENABLE    0x8
#define CODEC_B_DMA_ENABLE    0x10 

#define DRIVE_DMA(channel)    (1<<channel)
...
scdescr_t  adc_input_descriptors[MAX_DESCRIPTORS];
...
hal2_loopaes()
{
    ...
    BuildDescriptors(aes_output_descriptors, inbuf,NSAMPS*sizeof(long)*2);
    ...
    BuildDescriptors(adc_input_descriptors, inbuf,NSAMPS*sizeof(long)*2);

    printf("Set Up HAL2\n");
    SetupHAL2();

    printf("Initiate ADC input DMA\n");
    InitiateDMA(adc_input_descriptors, 1, 2);

    printf("Initiate AES output DMA\n");
    InitiateDMA(aes_output_descriptors, 0, 4);
    ...
    *idr0p = CODEC_B_DMA_ENABLE + AES_TX_DMA_ENABLE;
...
SetupHAL2(void)
{
    ...
    *idr0p = (0<<10)+(0<<8)+(0x0<<4)+(0x0);
    /* unmute, left & right from line-in, A/D gains == ff */
