/*
 * hal2_endiantimestamp.c - test endianness and timestamping
 */
...
SetupHAL2(void)
{
    ...
    *idr0p = 2;   /* make dma channel 1 little endian */
...
ExamineTimestamps(int *buf, int nframes, int mode)
{
	...
	timestampLSW = buf[3];      
/*
	timestampMSW = ((0xffff & buf[2]) << 16) | (0xffff & (buf[2] >> 16));
	timestampLSW = ((0xffff & buf[3]) << 16) | (0xffff & (buf[3] >> 16));
*/
#ifdef NOPE
        timestampMSW = ((0xff & buf[2]) << 8) | 
                        (0xff & (buf[2] >> 8)) |
                        ((0xff & (buf[2] >> 16)) << 24) |
                        ((0xff & (buf[2] >> 24)) << 16);
        timestampLSW = ((0xff & buf[3]) << 8) | 
                        (0xff & (buf[3] >> 8)) |
                        ((0xff & (buf[3] >> 16)) << 24) |
                        ((0xff & (buf[3] >> 24)) << 16);
#endif
