/*
 * change the original (written by Bruce K.) to do output of samples
 * to the digital transmitter.
 */
void hal2_1ktone();
...
/*
 * this should be the only routine that is changed substantially
 * from the original to allow for output to the digital TX.
 */
void
SetupHAL2(void)
{
    ...
    int inc, mod, modctrl;
    volatile unsigned long  *aestx_cr1 = (unsigned long *)PHYS_TO_K1(AESTX_CR1);
    volatile unsigned long  *aestx_cr2 = (unsigned long *)PHYS_TO_K1(AESTX_CR2);
    volatile unsigned long  *aestx_cr3 = (unsigned long *)PHYS_TO_K1(AESTX_CR3);
