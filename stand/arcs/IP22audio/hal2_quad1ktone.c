...
int sintab[48000];
int tunetab[NSAMPS*4];	/* the *4 is because it's quad */
...
MakeTune(int *tunetab, int *sintab, int step, int nsamps)
{
	...
	*tunetab++=sintab[48000-j]<<8;	/* back L is pi out of phase */
	*tunetab++=sintab[48000-j]<<8;  /* back R is pi out of phase */
        j+=step;
	if(j>=48000)j-=48000;
    }
}
...
SetupHAL2(void)
{
    ...
    *isrp = HAL2_ISR_CODEC_RESET_N|HAL2_ISR_GLOBAL_RESET_N;
    /* silly chip has to be brought out of reset before CODEC_MODE is toggled */
    us_delay(50);
    *isrp = HAL2_ISR_CODEC_RESET_N|HAL2_ISR_GLOBAL_RESET_N|HAL2_ISR_CODEC_MODE;
    if (*isrp != (HAL2_ISR_CODEC_RESET_N|HAL2_ISR_GLOBAL_RESET_N|HAL2_ISR_CODEC_MODE)) {
	printf("BAD ISR 0x%x!\n",*isrp);
    }
...
*ser1ptr=0x55555555; *ser1ptr=0x55555555; *ser1ptr=0x55555555;
	...
	if (ntimes == 2000) {
	    ...
	    *idr1ptr = (0x00<<7)	/* Left D/A output atten */
