 ...
 *     
 *    >>>>>>>>> THIS FORMULA (FROM PMOD) IS INCORRECT <<<<<<<<
 *    MODCTRL = 0x00FFFF & (MOD - INC - 1)
 *
 *    Corrected formula for modctrl:
 *    modctrl = 0x00ffff & (inc - mod - 1)   ->  modctrl < 0
 *
 ************************************************************************/
...
hal2_bres2_ctrl2()
{
    ...
    printf("Entering HAL2_BRES2_CTRL2 test\n");
	...
	for (i = 0; i < mod; i+=32) {             /* loop inc up to mod */
	    ...
	    /*
	     * based on new information from tm, the formula
	     * for modctrl is now: modctrl = inc - mod - 1
	     */
	    ...
	    *iar  = HAL2_BRES2_CTRL2_W;
	    
	    /* spin */
	    while (*isr & HAL2_ISR_TSTATUS);
	    
	    *idr0 = 0xbadbad;
	    *iar  = HAL2_BRES2_CTRL2_R0;
	    while (*isr & HAL2_ISR_TSTATUS);
	    inc_rb = *idr0;
	    if (inc_rb != inc) {
		inc_errors++;
    ...
    printf("Exiting HAL2_BRES2_CTRL2 test\n");
}
