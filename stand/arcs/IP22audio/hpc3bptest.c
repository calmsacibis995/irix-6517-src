...
#define NSAMPS 301 	/* 2 seconds of tune */
...
*/
    ...
    printf("Wait for input DMA (input desc = 0x%x)\n",K1_TO_PHYS(input_descriptors));
    WaitForOneDMA(1);
    for(i=0;i<1000;i++) {
	printf("%x ",soundbuf[i]);
	if(i%8==7)printf("\n");
...
BuildDescriptors(scdescr_t *list,void *startaddr,int nbytes)
{
    ...
    desc--;
    desc->nbp = (unsigned long) K1_TO_PHYS(list);
    desc->eox = 0;
}
...
WaitForOneDMA(int which)
{
	...
	bp = *bpptr;
	if (bp < (unsigned long) K1_TO_PHYS(soundbuf) || bp > (unsigned long) K1_TO_PHYS(soundbuf) + 2 * sizeof(long) * NSAMPS) {
		printf("bp=0x%x\n",bp);
	}
    }
    if(ntimes==maxtimes) {
