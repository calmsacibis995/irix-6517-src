...
int sintab[48000];
int tunetab[NSAMPS*4];	/* the *1 is because it's quad */
...
MakeTune(int *tunetab, int *sintab, int step, int nsamps)
{
	...
	*tunetab++=sintab[j]<<8;	/* front R */
        j+=step;
	*tunetab++=sintab[j]<<8;	/* back L */
        j+=step;
	*tunetab++=sintab[j]<<8;	/* back R */
        j+=step;
	if(j>=48000)j-=48000;
    }
}
...
WaitForDMA(void)
{
    ...
    volatile unsigned  long *idr1ptr=(unsigned long *)PHYS_TO_K1(HAL2_IDR0);
    volatile unsigned long *iarp=(unsigned long *)PHYS_TO_K1(HAL2_IAR);
    volatile unsigned long *isrp=(unsigned long *)PHYS_TO_K1(HAL2_ISR);
