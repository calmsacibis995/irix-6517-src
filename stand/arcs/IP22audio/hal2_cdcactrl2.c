...
hal2_codecactrl2()
{
    ...
    printf("begin test of HAL2 CODECA CTRL2 register\n");
    *isr = 0x8;
    /*
	...
	for (j = 0; j < 4096; j++) {
	    ...
	    *idr1 = 0xffff;
	     ...
	     * words.
	     */
	    *iar = HAL2_CODECA_CTRL2_R0;
	    while (*isr & HAL2_ISR_TSTATUS);
	    rb0 = *idr0;
	    *iar = HAL2_CODECA_CTRL2_R1;
	    while (*isr & HAL2_ISR_TSTATUS);
	    rb1 = *idr0;
	    if (rb0 != val0) {
		badval0++;
		printf("ERROR: wrote (word 0) 0x%x,  got 0x%x\n", val0, rb0);
	    }
	    if (rb1 != val1) {
		badval1++;
		printf("ERROR: wrote (word 1) 0x%x,  got 0x%x\n", val1, rb1);
	    }
	}
    }
    if (badval0 || badval1) {
    ...
    printf("end test of HAL2 CODECA CTRL2 register\n");
}
