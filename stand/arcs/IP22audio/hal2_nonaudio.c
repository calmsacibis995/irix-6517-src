...
#define NSAMPS 196000 	/* 2 seconds of tune */
...
{
    volatile int i;
    printf("HAL2 nonaudio bit test entered\n");
    hal2_configure_pbus_pio();
    hal2_configure_pbus_dma();    

    printf("soundbuf = 0x%x\n",soundbuf);
...
*/
    ...
    InitiateDMA(input_descriptors, 1, 0);
    ...
    printf("HAL2 nonaudio bit test done\n");
}
...
SetupHAL2(void)
{
    ...
    *idr0p = 1;  /* Activate only ports 0 and 1 */
    ...
    *idr0p = 0x10;  /* Enable only DAC A and ADC B*/
    *iarp = HAL2_DMA_ENABLE_W;
    SPIN;
}
...
WaitForOneDMA(int which)
{
    ...
    int ntimes;
    int maxtimes=3000;
    long *prevbp = (long *) soundbuf;
    long *newbp;
    volatile long *ptr=(long *)soundbuf+1;
