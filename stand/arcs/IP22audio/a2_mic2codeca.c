...
static int right_gain, left_gain;
...
WaitForTwoDMAs(void)
{
    ...
    for(ntimes=0;ntimes<maxtimes;) {
	ctrl0val = *ctrl0ptr;
	ctrl1val = *ctrl1ptr;
	if(!ctrl0val.ch_act && !ctrl1val.ch_act)break;
	us_delay(10000);
        printf("enter left gain setting (0-15):\n");
        gets(str1);
        printf("enter right gain setting (0-15):\n");
        gets(str2);
        left_gain = atoi(str1);
        right_gain = atoi(str2);
        SetGain(left_gain, right_gain);
    }
    if(ntimes==maxtimes) {
...
SetGain(int left, int right)
{
    ...
    *idr0p = (0<<10)+(3<<8)+((0xf & left)<<4)+(0xf & right);
    /* unmute, left & right from mic, A/D gains == ff */
