...
rad1_install(COMPONENT *comptp)    /* standalone */
{
	...
	ssvid = pcdp->sub_sys_vendor_id;
	ssid = pcdp->sub_sys_id;
#if 0 /* the offset in the struct are not right. endian problem. so */
	/* if either endian works were ok. this protects us when it gets */
	/* fixed. turn the prints on to see the problem  check pci spec) */
printf("ssvid = 0x%x, ssid = 0x%x\n",ssvid,ssid);
printf("ssvid_p = 0x%x, ssid_p = 0x%x\n",&(pcdp->sub_sys_vendor_id),&(pcdp->sub_sys_id));
printf("vid = 0x%x, id = 0x%x\n",pcdp->vendor_id,pcdp->device_id);
#endif
	if (!(((pcdp->sub_sys_vendor_id == PSITECH_ID) &&
	(pcdp->sub_sys_id == PSITECH_SYS_ID)) ||
	((pcdp->sub_sys_id == PSITECH_ID) &&
	(pcdp->sub_sys_vendor_id == PSITECH_SYS_ID)))) return FALSE;

        nasid = GET_HSTNASID_FROM_KEY(comptp->Key);
        mask = 0x1ffff; /* space 0 mask (size - 1) */
        space0 = (char *)NODE_BWIN_BASE(nasid, 0) +
            (pcdp->base_addr_regs[0] & ~mask);
        mask = 0x0ffff; /* expansion rom mask (size - 1) */
        space2 = (char *)NODE_BWIN_BASE(nasid, 0) +
            (pcdp->expansion_rom & ~mask);
