...
{
	uchar_t	xdata[NVRAM_SIZE], rdata[NVRAM_SIZE];
	int		xlen, rlen, i, addr;

	addr = NVRAM_MAGIC_AD;
	for (i = 0; i < NVRAM_SIZE; i++, addr++) {

		/* set the xdata location to read from */
		xdata[0] = (uchar_t) (addr & 0xff);

		/* lock out hub interrupts -- reset ints should be locked as well */
		hub_lock_timeout(HUB_LOCK_I2C, LOCK_TIMEOUT);

		/* perform a single read on NVRAM at the right offset */
		(void) i2c_master_xmit_recv(nasid, I2C_ADDR_RAM | addr >> 8 & 7,
			xdata, 1, &xlen, rdata, 1, &rlen, -1, I2C_TIMEOUT, 0);

		/* unlock hub interrupts as they relate to the I2C bus */
		hub_unlock(HUB_LOCK_I2C);

		/* update the character in the read buffer */
		elsc_nvram_data[i] = (char) rdata[0];
	}
}
...
int elsc_nvram_write(elsc_t *e, int addr, char *buf, int len)
{
	...
	/* this never happens, but leave it in */
	if (len < 0) {
		i = strlen(buf) + 1;
		len = (i > -len) ? -len : i;
	}

	/* get the offset to the first address */
	if ((addr >= NVRAM_MAGIC_AD) && (addr <= NVRAM_MAGIC_AD +0xf)) {
	    j = addr - NVRAM_MAGIC_AD;
	}

	/* increment through each element we are writing out */
	for (i = 0; i < len; i++, addr++, j++) {

		/* set up the address and data to write out across I2C */
		data[0] = (uchar_t) (addr & 0xff);
		data[1] = (uchar_t) buf[i];

		/* lock out hub interrupts */
		hub_lock_timeout(HUB_LOCK_I2C, LOCK_TIMEOUT);
		...
		/* NVRAM cache only exists if running from the IP27prom */
#if _STANDALONE
		/* 
		 * save the value written to NVRAM to the local cached copy 
		 * if the cache exists (see comment in elsc_nvram_read)
		 */
		if ((elsc_nvram_buffer[0] == NVRAM_MAGIC_NO) && 
		    (addr >= NVRAM_MAGIC_AD) && (addr <= NVRAM_MAGIC_AD +0xf)) {
		    elsc_nvram_buffer[j] = buf[i];
		}
#endif
	}
...
int elsc_nvram_read(elsc_t *e, int addr, char *buf, int len)
{
	...
	/*
	 * The old method of doing doing NVRAM reads was to probe into
	 * NVRAM each time we want to do a read.  Now we return the
	 * results from our internal I2C NVRAM buffer.  This prevents
	 * us from trying to do a read each time, which in some cases
	 * can cause problems on the I2C bus with newer system
	 * controllers.
	 *
	 * Only read from the buffer if a) we're in the prom, and b) the
	 * buffer contains the correct magic number.  The IO6prom will
	 * also attempt to read from this address, but the cache is no
	 * longer valid when the IO6prom is running.
	 */
	...
	if ((elsc_nvram_buffer[0] == NVRAM_MAGIC_NO) &&
	    ...
	    /* back up the address to the right buffer location */
	    addr -= NVRAM_MAGIC_AD;

	    /* iterate for all bytes */
	    for (i = 0; i < len; i++) {
		buf[i] = elsc_nvram_buffer[addr + i]; 
	    }
	    r = len;
	} else {
#endif
