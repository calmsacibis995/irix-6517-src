#ident	"IP26prom/lmem_conf.s:  $Revision: 1.10 $"

/* NOTE: This is different than IP20/22/26 as it maps all memory high so
 *       we avoid having a hole in memory and can support 3*256MB.
 */
...
#include <pon.h>
	...
	/* all code here runs uncched, mostly w/o mem configed so we are ok */
...
#define	MAXBANKS	3		/* max # of memory banks */
#define	MAXUNITS	32		/* max # of memory units in a bank */
#define UNITSIZE	0x01000000	/* 16MB per memory unit */
#define UNITSHIFT	24		/* 1 << 24 == 16MB */
#define PROBE_MEMCFG	0x3f20		/* default memcfg value when probing */
#define PROBE_BASE	SEG0_BASE
#define MAXSIZE		0x20000000	/* max of 512MB per bank */
#define LOWCLEAR	(16*1024*1024)	/* 16MB: clear and wait size */

/* use fpu register to store the following variables */
#define	MEM_SIZE_REG		$f0	/* stow memory size */
...
init_mem_restart:
	...
	CLI	v1,PHYS_TO_COMPATK1(MEMCFG0)	# Controls Banks 0&1
...
bank_loop: 	
	...
	/* find the size in 16 MB, of the current bank */ 
	move	a0,T40			# bank number
	jal	size_bank
	move	T41,v0			# size in 16 MB
	beq	v1,zero,1f
	...
	mfhi	a3
	or	a3,v1
	mthi	a3			# save bad SIMMs code in mult-hi
...
1:
	...
	move	a0,T40				# bank number
	move	a1,T41				# size in 16 MB
	jal	set_bank_size

	/* validate the bank and get the correct base addr */
	move	a0,T40				# bank number
	li	a1,PROBE_MEMCFG & (MEMCFG_VLD|MEMCFG_ADDR_MASK)
	or	a1,v0				# set base addr
	jal	set_bank_conf

	/* run the walking bit test */
	LI	a0,PHYS_TO_K1(PROBE_BASE)	# starting addr
...
1:
	...
	LA	a0,mem_size_done
	jal	pon_puts
	CLI	T40,PHYS_TO_COMPATK1(MEMCFG0)
	lw	a0,0(T40)
	jal	pon_puthex
	li	v1,0x20
	jal	pon_putc
...
addr_test:
	mfc1	a1,MEM_SIZE_REG		# memory size
...
mem_ok:
	 ...
	 * 	- First clear 16MB and wait.
	 *	- Then fire off remainder (checked in wait_clear_mem)
	 */
	CLI	a0,PHYS_TO_COMPATK1(CPUCTRL0)	# base mc address
	...
	li	v0,LOWCLEAR			# amount of RAM to clear + wait
...
1:	lw	a1,mcreg(DMA_RUN,a0)		# now wait for DMA to finish
	...
	sub	v0,T41,LOWCLEAR			# memsize - LOWCLEAR

	sw	zero,mcreg(DMA_CAUSE,a0)	# clear interrupts

	beq	v0,zero,2f			# any more memory?
	li	a1,SEG0_BASE+LOWCLEAR		# rest of memory addr
	sw	a1,mcreg(DMA_MEMADR,a0)		# starting memory location
 ...
 */
LEAF(wait_clear_mem)
	CLI	a0,PHYS_TO_COMPATK1(CPUCTRL0)	# MC base
...
LEAF(memflash)
	CLI	T00,PHYS_TO_COMPATK1(HPC3_INT2_ADDR)
	...
	CLI	T00,PHYS_TO_COMPATK1(HPC3_WRITE1)
 ...
 *
 * return:
 *	v0 - memory bank size in unit of 16 MB
 *	v1 - bad SIMMs code
 *
...
#define	DATA	0x11111111
	...
	LI	a0,PHYS_TO_K1(PROBE_BASE)	# starting addr to probe
	li	a1,DATA
	 ...
	 * in a bank are there
	 *
	 * NOTE: IP28 probes 2 locations since it tends to tristate a bit
	 * different than IP22/IP26 baseboards.  So we probe 2 addrs here
	 * per SIMM here.
	 */
	.set	noreorder
...
memory_available:
	...
	sw	zero,8(a0)		# a0 still equal to PROBE_BASE
	sw	zero,12(a0)
...
probe_subbank:
	...
	li	a1,PROBE_MEMCFG|MEMCFG_BNK	# set 2-subbanks
	jal	set_bank_conf

	LI	v0,PHYS_TO_K1(PROBE_BASE)
	li	a1,MAXSIZE>>1
	PTR_ADDU	a0,v0,a1	# first addr in the 2nd subbank
	li	a1,DATA
	...
	/* this is a workaround for the IP28 bb noise problem */
	.set	noreorder
	sw	zero,0(v0)		# use different data for each SIMM
	sw	zero,4(v0)		# we write when there's no SIMM
	sw	zero,8(v0)
	sw	zero,12(v0)
	lw	zero,0(v0)
	lw	zero,4(v0)
	...
	sw	zero,16(a0)		# clear out bank 1
	lw	a2,0(a0)		# read back the first word
	beq	a1,a2,1f
	or	v1,BAD_SIMM0		# data mismatch, SIMM 0 has 1 subbank

1:
	sw	zero,20(a0)		# clear out bank 1
	lw	a2,4(a0)		# read back the second word
	sll	a1,1
	beq	a1,a2,2f
...
2:
	sw	zero,24(a0)		# clear otu bank 1
	lw	a2,8(a0)		# read back the third word
	sll	a1,1
	beq	a1,a2,3f
...
3:
	sw	zero,28(a0)		# clear out bank 1
	lw	a2,12(a0)		# read back the fourth word
	sll	a1,1
	beq	a1,a2,4f
...
4:
	...
	srl	v0,T11,24-1		# memory size in 16 MB units
...
5:
	...
	srl	v0,T11,24		# memory size in 16 MB units
 ...
 * set_bank_addrs
 *	a0 - memory bank number
 *	a1 - memory bank size in units of 16 MB
 *
 * return
...
LEAF(set_bank_addrs)
	...
	srl	T11,UNITSHIFT		# convert to base address
...
next_bank:
	...
	beq	T10,zero,addr_done	# all banks done?
...
addr_done:
	sll	v0,T11,UNITSHIFT	# shift to MB
	sub	v0,PHYS_RAMBASE		# remove offset
 ...
 */
LEAF(set_bank_conf)
	CLI	T00,PHYS_TO_COMPATK1(MEMCFG0)	# addr of MEMCFG0
	blt	a0,2,1f
...
3:
	...
	lw	zero,0(T00)	# WBFLUSHM (read from MC)
	j	ra
	END(set_bank_conf)
 ...
 */
LEAF(szmem)
	CLI	v1,PHYS_TO_COMPATK1(MEMCFG0)	# addr of MEMCFG0
	PTR_ADDU T00,v1,MEMCFG1-MEMCFG0		# addr of MEMCFG1
	move	v0,zero			# init memory counter
...
3:
	.set	noreorder
	bne	v1,T00,1b
	move	v1,T00			# BDSLOT: now MEMCFG1
	.set	reorder

	sll	v0,UNITSHIFT-8		# shift to bytes (rev D MC shift)
