...
#define starttune1_buflen      123055
#define starttune1_playlen     85000
#define starttune1_maxperiod   188
...
static pluckparams_t starttune2_plist[] = {
...
static pluckparams_t starttune3_plist[] = {
...
#define stoptune1_buflen      130979
#define stoptune1_playlen     80000
#define stoptune1_maxperiod   99
...
static pluckparams_t stoptune2_plist[] = {
...
static pluckparams_t stoptune3_plist[] = {
...
#define MAXTUNES	3L		/* number of tunes supported */
struct tunelist {
	pluckparams_t *plist;
	int buflen;
	int maxperiod;
...
static struct tunelist boottunes[MAXTUNES] = {
...
static struct tunelist stoptunes[MAXTUNES] = {
 ...
 */
{
    ...
    /*  On some systems the fast IU power-on diagnostic fails.  It's believed
     * in mfg that resetting the machine helps this.  To avoid this looking
     * totally bad, we do this in software.  SEMAPHORE_13 will be set if
     * we have done this, so do not re-play the tune if this bit is set.
     */
    if (tune_type == 0) {
	...
	*sema = 0;		/* clear the semaphore */
    }

    grab_loc = (char *)PHYS_TO_K0(0x08c00000);		/* 12MB, cached */
    ...
    if (tune == -1) {
	/* pick a random tune based on the TCC_COUNTER */
	tune = (*(volatile long *)PHYS_TO_K1(TCC_COUNT)) % MAXTUNES;
    }
